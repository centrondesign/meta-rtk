...
static struct attribute *rtk_13xxd_ddrc_tfc_event_attrs[] = {
	TFC_DRV_EVENT_REF(tfc_refresh),
	TFC_DRV_EVENT_REF(tfc_overflow),
...
static struct attribute_group rtk_13xxd_ddrc_tfc_event_attr_group = {
	.name = "events",
	.attrs = rtk_13xxd_ddrc_tfc_event_attrs,
};
...
static struct attribute *rtk_13xxd_tfc_sysfs_attrs[] = {
	TFC_SYSFS_ATTR_GROUP(rt, 0),
	TFC_SYSFS_ATTR_GROUP(nrt, 1),
	TFC_SYSFS_ATTR_GROUP(vcpu, 3),
	TFC_SYSFS_ATTR_GROUP(gpu, 4),
...
static struct attribute_group rtk_13xxd_tfc_sysfs_attr_group = {
	.name = "settings",
	.attrs = rtk_13xxd_tfc_sysfs_attrs,
};
...
static const struct attribute_group *rtk_13xxd_ddrc_tfc_attr_groups[] = {
	/* must be Null-terminated */
	[RTK_PMU_ATTR_GROUP__COMMON] = &rtk_pmu_common_attr_group,
	[RTK_PMU_ATTR_GROUP__FORMAT] = &rtk_tfc_format_attr_group,
	[RTK_PMU_ATTR_GROUP__EVENT] = &rtk_13xxd_ddrc_tfc_event_attr_group,
	[RTK_PMU_ATTR_GROUP__NUM] = &rtk_13xxd_tfc_sysfs_attr_group,
	[RTK_PMU_ATTR_GROUP__NUM + 1] = NULL
};
...
static const struct rtk_pmc_set_meta rtk_16xxb_ddrc_tfc_ps_meta[] = {
	{
...
static const struct rtk_pmc_set_meta rtk_13xxd_ddrc_tfc_ps_meta[] = {
	{
		.name = "13xxd TFC for DDRC RT",
		.type = PMC_SET__TFC,
		.init = rtk_tfc_ps_init,
		.group_size = _TFC_GR_SIZE,
		.config_size = _TFC_CFG_SIZE,
...
int rtk_13xxd_ddrc_tfc_init(struct rtk_pmu *pmu, struct device_node *dt)
{
	int ret = rtk_tfc_pmu_init(pmu, dt,
				   "rtk_13xxd_ddrc_tfc_pmu",
				   _TFC_CTRL_OFFSET_,
				   rtk_13xxd_ddrc_tfc_attr_groups,
				   rtk_13xxd_ddrc_tfc_ps_meta,
				   RTK_PMU_META_NR(rtk_13xxd_ddrc_tfc_ps_meta));

	__reset_tfc_channel(pmu);

	return ret;
}
EXPORT_SYMBOL(rtk_13xxd_ddrc_tfc_init);
MODULE_LICENSE("GPL");
