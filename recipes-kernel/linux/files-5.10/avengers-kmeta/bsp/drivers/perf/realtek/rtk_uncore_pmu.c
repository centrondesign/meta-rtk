		     ...
		     const struct rtk_pmc_set_meta *psm)
{
	...
	for_each_child_of_node(dt, child) {
		...
		if (ret == -EINVAL || ps->nr_defects == 0) {
			...
			pr_err("Parsing nr_defects of %s failed\n", psm->name);
			return ret;
		}
		...
		ret = of_property_read_u32_array(child, "defects", ps->defects,
						 ps->nr_defects);
		if (ret < 0) {
			pr_err("Parsing defect descriptions of %s failed\n",
			       psm->name);
			return ret;
		}
...
defect_complete:
		...
		if (ret == -EINVAL || ps->has_ext == 0) {
			...
			pr_err("Parsing has_ext of %s failed\n", psm->name);
			return ret;
		}
		ret = of_property_read_u32(child, "ext_info", &ps->ext_info);
		if (ret == -EINVAL) {
			pr_err("Missing ext_info of %s\n", psm->name);
			return ret;
		} else if (ret < 0) {
			pr_err("Parsing ext_info of %s failed\n", psm->name);
			return ret;
		}
