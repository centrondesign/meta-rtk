...
struct rtk_ahci_port {
	...
	struct reset_control *rst[RTK_SATA_MAX_RST];
};
...
static int rtk_ahci_probe(struct platform_device *pdev)
{
	...
	for_each_available_child_of_node(node, child) {
		...
		port->powerio = gpiod_get_from_of_node(child, "sata-gpios", 0, GPIOD_OUT_LOW, child->name);
		if (IS_ERR(port->powerio)) {
			if (PTR_ERR(port->powerio) == -EPROBE_DEFER)
				return -EPROBE_DEFER;
			dev_warn(dev, "failed to set sata-gpios: %ld\n", PTR_ERR(powerio));
		}
		for (i = 0; i < RTK_SATA_MAX_RST; i++) {
			port->rst[i] = of_reset_control_get_by_index(child, i);
			if (!IS_ERR(port->rst[i]))
				reset_control_deassert(port->rst[i]);
		}
	...
	for (i = 0; i < RTK_SATA_MAX_PORT; i++) {
		port = priv->ports[i];
		if (port) {
			if (!IS_ERR(port->powerio)) {
				gpiod_direction_output(port->powerio, GPIOD_OUT_HIGH);
				gpiod_put(port->powerio);
			}
		}
	}
...
static int rtk_ahci_resume(struct device *dev)
{
	...
	for (i=0; i<hpriv->nports; i++) {
		if (priv->ports[i] == NULL)
			continue;
		for (j = 0; j < RTK_SATA_MAX_RST; j++)
			if(!IS_ERR(priv->ports[i]->rst[j]))
				reset_control_deassert(priv->ports[i]->rst[j]);
