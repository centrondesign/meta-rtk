...
{
	struct rtk_gen_heap *gh = m->private;
...
static int gheap_attr_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "flags : 0x%llx \n", (long long unsigned int)gh->flag);
...
static int gheap_secure_show(struct seq_file *m, void *v)
{
	...
	list_for_each_entry_safe(rtk_protect_info, tmp, protect_list, list) {
		...
		seq_printf(m, "base = 0x%x , size = 0x%x \n", (unsigned int)base, (unsigned int)size);
	}
...
{
	struct rtk_cma_heap *ch = m->private;
	struct rtk_heap_task *rtk_heap_task, *tmp;
...
static int cheap_attr_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "flags : 0x%llx \n", (long long unsigned int)ch->flag);
...
static void rheap_gh_debugfs_add(struct rtk_gen_heap *gh, struct dentry
					 *root_dentry)
{
	struct dentry *tmp;
	struct gen_pool *pool;
...
static void rtk_cma_debug_show_areas(struct seq_file *m,
		struct rtk_cma_heap *ch)
{
	...
	seq_printf(m, "\t\tRTK CMA number of available pages: ");
	for (;;) {
		next_zero_bit = find_next_zero_bit(ch->use_bitmap, nbits, start);
		if (next_zero_bit >= nbits)
			break;
		next_set_bit = find_next_bit(ch->use_bitmap, nbits, next_zero_bit);
		nr_zero = next_set_bit - next_zero_bit;
		nr_part = nr_zero << cma->order_per_bit;
		seq_printf(m, "%s%lu@%lu", nr_total ? "+" : "", nr_part,
				next_zero_bit);
		nr_total += nr_part;
		start = next_zero_bit + nr_zero;
	}
	seq_printf(m, "=> %lu free of %lu total pages\n", nr_total, cma->count);

	start = 0;
	nr_total = 0;
	seq_printf(m, "\t\tCMA number of available pages: ");
	for (;;) {
...
static int rtk_heap_summay_fops_show(struct seq_file *m, void *v)
{
	...
	list_for_each_entry(ch, &cheap_list, hlist) {
		...
		seq_printf(m,
			   "\n\tHeap: %-30s(%-12s):[0x%x ~ 0x%x], "
			   "size = 0x%-8x pages, flag = 0x%-8x\n",
			   dma_heap_get_name(ch->heap), ch->cma->name,
			   (unsigned int)heap_base_addr, (unsigned int)heap_end_addr, (unsigned int)ch->cma->count,
		...
		seq_printf(m, "\t\tUsage: 0x%-8x pages, free: 0x%-8x pages"
				", Max_usage: 0x%-8x pages\n",
			   (unsigned int)used_pages, (unsigned int)((ch->cma->count) - used_pages), (unsigned int)ch->max_used_pages);
		rtk_cma_debug_show_areas(m, ch);
		list_for_each_entry_safe(rtk_heap_task, tmp, &ch->task_list,
					  list) {
			...
			list_for_each_entry(rtk_alloc,
				...
				seq_printf(m,
					   ...
					   rtk_alloc->start,
					   (rtk_alloc->end - 1),
					   (int)(rtk_alloc->end - rtk_alloc->start));
			}
		}
	...
	list_for_each_entry(gh, &gheap_list, hlist) {
		...
		seq_printf(
			m,
			"\n\tHeap: %-30s: [0x%x ~ 0x%x], size = 0x%-8x pages, "
			"flag = 0x%-8x\n",
			dma_heap_get_name(gh->heap), (unsigned int)heap_base_addr,
			(unsigned int)(heap_base_addr + heap_size - 1), (unsigned int)(heap_size >> PAGE_SHIFT),
			(unsigned int)gh->flag);
		seq_printf(m, "\tflag:%s \n",
			   name = ka_dispflag(gh->flag, GFP_KERNEL));
		kfree(name);
		...
		list_for_each_entry_safe(rtk_heap_task, tmp, &gh->task_list,
			...
			list_for_each_entry(rtk_alloc,
				...
				seq_printf(m,
					   "\t\t    range[%-8lx ~ %-8lx]:  "
					   "%d bytes\n",
					   (long unsigned int)rtk_alloc->start,
					   (long unsigned int)(rtk_alloc->end - 1),
					   (int)(rtk_alloc->end - rtk_alloc->start));
			}
		}
	...
	list_for_each_entry(gh, &pheap_list, plist) {
		...
		gh_avail_get(gh, &heap_avail);
		seq_printf(m,
			   "\n\t%-30s: [0x%x ~ 0x%x], size = 0x%8x pages, "
			   "flag = 0x%-8x\n",
			   dma_heap_get_name(gh->heap), (unsigned int)heap_base_addr,
			   (unsigned int)(heap_base_addr + heap_size - 1),
			   (unsigned int)(heap_size >> PAGE_SHIFT), (unsigned int)gh->flag);
		seq_printf(m, "\tflag:%s \n",
			   name = ka_dispflag(gh->flag, GFP_KERNEL));
		kfree(name);
		used_pages = (heap_size - heap_avail) >> PAGE_SHIFT;
		seq_printf(m, "\t\tUsage: 0x%8lx pages, free: 0x%8lx pages\n",
			   (long unsigned int)used_pages, (long unsigned int)(heap_avail >> PAGE_SHIFT));
		mutex_lock(&gh->mutex);
		list_for_each_entry_safe(rtk_heap_task, tmp, &gh->task_list,
					  list) {
			seq_printf(m, "\t\t%-18s: alloc %-8d bytes\n",
				   rtk_heap_task->comm, (int)rtk_heap_task->size);
			list_for_each_entry(rtk_alloc,
					     &rtk_heap_task->alloc_list, list) {
				seq_printf(m,
					   "\t\t    range [%-8lx ~ %-8lx]: "
...
static void rheap_ch_debugfs_add(struct rtk_cma_heap *ch, struct dentry
					 *root_dentry)
{
	struct dentry *tmp;
	struct cma *cma;
	__attribute__((unused)) char name[100];
