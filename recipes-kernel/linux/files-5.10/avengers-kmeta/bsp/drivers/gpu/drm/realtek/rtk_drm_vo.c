...
static void rtk_vo_swap_post_cleanup(struct rtk_vo_swap_post *cfg)
{
	if (cfg == NULL)
		return;
	if (cfg->sync_file)
		filp_close(cfg->sync_file->file, NULL);
	kfree(cfg);
}
...
static void rtk_vo_swap_post_release(struct rtk_vo_swap_post *cfg)
{
	...
	if (cfg->sync_file) {
		err = dma_fence_wait_timeout(cfg->sync_file->fence, 1, RTK_CRTC_SWAP_SHORT_FENCE_TIMEOUT_MS);
		if (err == -ETIME) {
			DRM_ERROR("DRM %s:%d fence %p\n", __FUNCTION__, __LINE__, cfg->sync_file->fence);
			err = dma_fence_wait_timeout(cfg->sync_file->fence, 1, RTK_CRTC_SWAP_SHORT_FENCE_TIMEOUT_MS);
		}
		if (err < 0)
			DRM_ERROR("DRM %s:%d fence %p\n", __FUNCTION__, __LINE__, cfg->sync_file->fence);
...
/*	if (fb->modifier != 0) {
	...
	sync_file = DC_QueueBuffer(&buf);
	if (sync_file == NULL)
		return -1;

	cfg = rtk_vo_swap_post_create(fb, sync_file, rtk_vo, event);
	if (cfg == NULL) {
		filp_close(sync_file->file, NULL);
		return -1;
	}
/*
	if (fb) {
