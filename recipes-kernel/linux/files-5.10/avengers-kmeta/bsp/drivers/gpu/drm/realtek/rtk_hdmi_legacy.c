...
static int rtk_hdmitx_hotplug_callback(struct device *dev, void *data)
{
	int state;

	state = show_hpd_status(dev, false);
...
rtk_hdmitx_conn_detect(struct drm_connector *connector, bool force)
{
	...
	err = driver_for_each_device(drv, NULL, &state, rtk_hdmitx_hotplug_callback);
	if (!err) {
		if (state == 0)
...
static struct {
       ...
       {1280, 720,  DRM_MODE_TYPE_PREFERRED},
       {1920, 1080, DRM_MODE_TYPE_DEFAULT},
       {720, 480,  DRM_MODE_TYPE_DRIVER},
       {720, 576,  DRM_MODE_TYPE_DRIVER},
       {  -1,  -1, -1}
};
