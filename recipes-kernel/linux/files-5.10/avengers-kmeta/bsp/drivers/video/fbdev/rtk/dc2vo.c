...
#define dprintk(fmt, arg...) \
	do { if (debug) pr_debug("D/DC:" fmt, ##arg); } while(0)
#define eprintk(fmt, arg...) \
	do { pr_err("E/DC:" fmt, ##arg); } while(0)
#define wprintk(fmt, arg...) \
	do { if (warning) pr_warn("W/DC:" fmt, ##arg); } while(0)
#define iprintk(fmt, arg...) \
	do { if (info) pr_info("I/DC:" fmt, ##arg); } while(0)
...
void dc2vo_send_interrupt(DC_INFO *pdc_info)
{
	...
	if (pdc_info->vo_vsync_flag != NULL &&
	    DC_HAS_BIT(pdc_info->vo_vsync_flag, VO_DC_SET_NOTIFY)) {
		if (pdc_info->hwlock)
			hwspin_lock_timeout_irqsave(pdc_info->hwlock, UINT_MAX, &flags);
		DC_SET_BIT(pdc_info->vo_vsync_flag, VO_DC_FEEDBACK_NOTIFY);
		if (pdc_info->hwlock)
			hwspin_unlock_irqrestore(pdc_info->hwlock, &flags);
		regmap_write(pdc_info->acpu_int_base, 0xa80, (ACPU_INT_SA | ACPU_INT_WRITE));
	}
}

void dc2vo_send_interrupt_to_vo(void)
{
	if (gpdc_info)
		dc2vo_send_interrupt(gpdc_info);
}
EXPORT_SYMBOL(dc2vo_send_interrupt_to_vo);
...
int DC_Get_System_Time_Info(struct fb_info *fb, struct rtk_dc_info *video_info, DC_SYSTEM_TIME_INFO * param)
{
	...
	pdc_info->DC_TIME_INFO.RTK90KClock	 = refclk_get_val_raw();
...
static void *dc_kernel_map(int handle)
{
	...
	/* Todo: add new data member store 'dma_buf' structure.*/
...
long dc_wait_vsync_timeout(DC_INFO *pdc_info)
{
	...
	if (timeout == -ETIME) {
		flag = pli_IPCReadULONG((BYTE*)pdc_info->vo_vsync_flag);
...
typedef void (*kernel_vsync_update_t)   (const ktime_t time);
kernel_vsync_update_t kernel_vsync_update_cb = NULL;
DEFINE_RWLOCK(kernel_vsync_update_lock);

void kernel_vsync_update(const ktime_t time)
{
	{
		unsigned long flags;
		read_lock_irqsave(&kernel_vsync_update_lock, flags);
		if (kernel_vsync_update_cb)
			kernel_vsync_update_cb(time);
		read_unlock_irqrestore(&kernel_vsync_update_lock, flags);
	}
}

EXPORT_SYMBOL(kernel_vsync_update_cb);
EXPORT_SYMBOL(kernel_vsync_update_lock);
EXPORT_SYMBOL(kernel_vsync_update);
...
void dc_update_vsync_timestamp(DC_INFO *pdc_info)
{
	...
	kernel_vsync_update(ktime_get());
}

static void dc_fence_wait(DC_INFO * pdc_info, struct dma_fence *fence)
{
	...
	int err = dma_fence_wait_timeout(fence, 1, DC_SHORT_FENCE_TIMEOUT);

	if (err >= 0)
		return;

	if (err == -ETIME)
		err = dma_fence_wait_timeout(fence, 1, DC_LONG_FENCE_TIMEOUT);

	if (err < 0)
		pr_warn("error waiting on fence: %d\n", err);
}

static void dc_fence_put(union _fence_data *fdata)
{
	struct dma_fence *fence = fdata->fence;
...
{
	if (buf->fb_dmabuf.k_ptr != NULL) {
		dma_buf_put(buf->fb_dmabuf.k_ptr);
		buf->fb_dmabuf.k_ptr = NULL;
	}
	dc_fence_put(&buf->acquire);
	kfree(buf);
}
		...
		struct dc_buffer __user *buf_from_user, struct dc_buffer *buf)
{
	...
	buf->acquire.fence = NULL;
	if (user_buf.acquire.fence_fd > 0) {
		buf->acquire.fence = sync_file_get_fence(user_buf.acquire.fence_fd);
	...
	if (buf->share_fd > 0) {
		buf->fb_dmabuf.k_ptr = dma_buf_get(buf->share_fd);
		if (IS_ERR(buf->fb_dmabuf.k_ptr))
			return PTR_ERR(buf->fb_dmabuf.k_ptr);
	} else
		buf->fb_dmabuf.k_ptr = NULL;
...
int dc_do_static_post_config(struct rtk_dc_info * video_info)
{
	...
	dc_fence_wait(pdc_info, complete_fence->fence);
...
#else
	if (buffer->flags & eBuffer_AFBC_Enable) {
		obj.mode = MODE_AFBC;
		obj.ext1 = (buffer->flags & eBuffer_AFBC_Split)?1:0;
		obj.ext2 = (buffer->flags & eBuffer_AFBC_YUV_Transform)?1:0;
	} else if (buffer->flags & eBuffer_AFRC_Enable) {
		//obj.format = INBAND_CMD_GRAPHIC_FORMAT_RGBA8888;
		obj.mode = MODE_AFRC;
		obj.ext1 = buffer->coding_unit;
		obj.ext2 = STRIDE_HW_PITCH; //stride_en

		if (obj.ext2 == STRIDE_USER_PITCH) {
			unsigned int cu_size = 0;
			switch (buffer->coding_unit) {
				case CODING_UNIT_BYTES_16:
					cu_size = 16;
					break;
				case CODING_UNIT_BYTES_24:
					cu_size = 24;
					break;
				case CODING_UNIT_BYTES_32:
					cu_size = 32;
					break;
				default:
					cu_size = 0;
			...
			obj.pitch = _ALIGN(obj.width, 64) * cu_size;
		}
		//pr_info("[%s %d] AFRC w %d h %d cu %d stride_en %d pitch %d\n", __func__, __LINE__, obj.width, obj.height, obj.ext1, obj.ext2, obj.pitch);
	} else {
...
static void dc_post_work_func(struct kthread_work *work)
{
	...
	list_for_each_entry_safe(post, next, &saved_list, head) {
		...
		for (i = 0; i < post->config.n_bufs; i++) {
			struct dma_fence *fence = post->config.bufs[i].acquire.fence;
			if (fence)
...
int dc_init(struct rtk_dc_info *video_info, struct fb_info *fbi, int irq, struct hwspinlock *hwlock)
{
	int retval = 0;
	...
	if (video_info->dc_info == NULL) {
		...
		pr_info("[%s] rpc_common_base = 0x%lx, ipc = 0x%lx\n", __func__, (unsigned long)rpc_common_base, (unsigned long)ipc);
