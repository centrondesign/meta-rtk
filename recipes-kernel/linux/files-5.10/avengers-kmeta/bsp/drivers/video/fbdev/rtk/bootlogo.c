...
{
	if (vaddr) {
		dev_info(dev, "\033[1;32m" "memunmap 0x%08lx" "\033[m\n", (long)vaddr);
		memunmap(vaddr);
	}
...
void bootlogo_release(void)
{
	...
	case TAKE_UP:
		...
		dev_info(&pdev->dev,
			...
			(unsigned int)paddr, size, size, (long)pdata->vaddr,
			(long)high_memory);
...
static int bootlogo_probe(struct platform_device *pdev)
{
	...
	dt_node = of_parse_phandle(dev->of_node, "memory-region", 0);
	...
	dev_info(dev, "base=0x%08x size=0x%lx(%lu) v=0x%08lx\n",
		(unsigned int)paddr, (long)size, (long)size, (long)pdata->vaddr);
