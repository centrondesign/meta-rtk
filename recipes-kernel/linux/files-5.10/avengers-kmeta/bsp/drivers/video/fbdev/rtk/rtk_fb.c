...
static int rtk_fb_mmap(struct fb_info *info, struct vm_area_struct *vma)
{
	...
	return fb->fb_dmabuf->ops->mmap(fb->fb_dmabuf, vma);
...
phys_addr_t rtk_fb_mem_pa(struct ion_buffer *buffer)
{
...
void *rtk_fb_mem_va(struct ion_buffer *buffer)
{
...
void rtk_fb_mem_free(struct rtk_fb *rtk_fb)
{
	struct dma_buf *dmabuf = rtk_fb->fb_dmabuf;
	struct dma_buf_attachment *attach = rtk_fb->fb_attach;
	struct sg_table *table = attach->sgt;
	struct device *dev = rtk_fb->dev;

	dev_err(dev, "%s free dmabuf\n", __func__);
	BUG_ON(!dmabuf);
	BUG_ON(!rtk_fb->mem->va_addr);
	BUG_ON(!dmabuf->ops);
	BUG_ON(!dmabuf->ops->end_cpu_access);

	dma_buf_vunmap(dmabuf, rtk_fb->mem->va_addr);
...
static struct dma_buf *rtk_fb_mem_alloc(struct device *dev, size_t size,
			 ...
			 void **ion_phy, void **ion_virt)
{
	...
	alloc_flags = RTK_FLAG_SCPUACC | RTK_FLAG_HWIPACC | RTK_FLAG_NONCACHED;
	...
	*table = dma_buf_map_attachment(*attach, DMA_BIDIRECTIONAL);
	...
	ret_acc = dma_buf_begin_cpu_access(dmabuf, DMA_BIDIRECTIONAL);
	if (ret_acc) {
		dev_err(dev, "dma_buf_begin_cpu_access failed\n");
		ret = ERR_PTR(ret_acc);
		goto access_err;
	}
	...
	*ion_phy = (void *)sg_phys((*table)->sgl);
	...
	dev_err(dev, "%s *ion_phy = 0x%lx , *ion_virt = 0x%lx", __func__,
...
int rtk_fb_mem_alloc_ion(struct rtk_fb *rtk_fb, int pixel, int flags)
{
	...
	dmabuf = rtk_fb_mem_alloc(rtk_fb->dev, size, &attach, &table,
			 (void **)&mem->pa_addr, (void **)&mem->va_addr);
	if (IS_ERR_OR_NULL(dmabuf)) {
		ret = -ENOMEM;
		goto err;
	}
	...
	mem->pixel = pixel;
	mem->flags = flags;
	rtk_fb->fb_dmabuf = dmabuf;
	rtk_fb->fb_attach = attach;
...
int rtk_fb_ion_fd(struct rtk_fb *fb)
{
	...
	if (fb->mem->handle < 0) {
		dev_err(fb->dev, "dma buffer failed to export (%d)\n",
			 fb->mem->handle);
		dma_buf_vunmap(fb->fb_dmabuf, fb->mem->va_addr);
		dma_buf_put(fb->fb_dmabuf);
		return fb->mem->handle;
	}

	get_dma_buf(fb->fb_dmabuf);
...
static int rtk_fb_probe(struct platform_device *pdev)
{
	...
	rtk_fb->irq = irq_of_parse_and_map(pdev->dev.of_node, 1);
#else
	...
	lock_id = of_hwspin_lock_get_id(rtk_fb->dev->of_node, 0);
	if (lock_id > 0 || (IS_ENABLED(CONFIG_HWSPINLOCK) && lock_id == 0)) {
		lock = devm_hwspin_lock_request_specific(rtk_fb->dev, lock_id);

		if (lock) {
			dev_info(rtk_fb->dev, "use hwlock%d\n", lock_id);
		}
       } else {
	       if (lock_id != -ENOENT)
		       dev_err(rtk_fb->dev, "failed to get hwlock: %pe\n", ERR_PTR(lock_id));
	}

	dc_init(&rtk_fb->video_info, &rtk_fb->fb, rtk_fb->irq, lock);
