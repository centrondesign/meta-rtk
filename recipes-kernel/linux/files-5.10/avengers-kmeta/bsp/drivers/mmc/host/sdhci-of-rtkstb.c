...
static inline void rtkstb_sdhci_set_bit(void __iomem *base, u32 offset, u32 bit)
{
	u32 reg;
...
static inline void rtkstb_sdhci_clear_bit(void __iomem *base, u32 offset, u32 bit)
{
	u32 reg;
...
static void rtkstb_sdhci_ssc_enable(struct sdhci_rtkstb *rtk_host)
{
	struct regmap *crt_base = rtk_host->crt_base;
	void __iomem *wrap_base = rtk_host->wrap_base;

	if (soc_device_match(rtk_soc_parker))
		regmap_update_bits(crt_base, SYS_PLL_SD2, SSC_DEPTH,
				   SSC_DEPTH_1_N);
	else
		rtkstb_sdhci_set_bit(wrap_base, SDHCI_RTK_SSC1, EN_SSC);
}

static void rtkstb_sdhci_ssc_disable(struct sdhci_rtkstb *rtk_host)
{
	struct regmap *crt_base = rtk_host->crt_base;
	void __iomem *wrap_base = rtk_host->wrap_base;

	if (soc_device_match(rtk_soc_parker))
		regmap_update_bits(crt_base, SYS_PLL_SD2, SSC_DEPTH, 0);
	else
		rtkstb_sdhci_clear_bit(wrap_base, SDHCI_RTK_SSC1, EN_SSC);
}
...
static void rtkstb_sdhci_set_ssc(struct sdhci_host *host)
{
	...
	if (soc_device_match(rtk_soc_parker)) {
		...
		regmap_update_bits(crt_base, SYS_PLL_SD2, RTD13XXE_SSC_OC_EN, 0);

		clk_enable(rtk_host->pll);
...
static void sdhci_rtkstb_hw_init(struct sdhci_host *host)
{
	...
	clk_prepare_enable(rtk_host->pll);

	clk_set_rate(rtk_host->pll, 200000000);
...
static void rtkstb_sdhci_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (host->timing > MMC_TIMING_LEGACY)
		rtkstb_sdhci_set_bit(dhc_base, SDHCI_RTK_DUTY_CTRL, SDIO_STOP_CLK_EN);
}
...
static int rtkstb_sdhci_change_phase(struct sdhci_host *host, u8 sample_point, int tx)
{
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_rtkstb *rtk_host = sdhci_pltfm_priv(pltfm_host);
	struct clk *phase_clk = tx ? rtk_host->phase_tx : rtk_host->phase_rx;
	u32 degree;
	u16 clk = 0;
	...
	clk_disable(rtk_host->pll);
	degree = DIV_ROUND_CLOSEST(sample_point * 360, 32);
	clk_set_phase(phase_clk, degree);
	clk_enable(rtk_host->pll);
...
static void rtkstb_sdhci_down_clock(struct sdhci_host *host)
{
	...
	switch (clk_get_rate(rtk_host->pll)) {
	case 208000000:
		freq = 200000000;
		break;
	case 200000000:
	...
	clk_set_rate(rtk_host->pll, freq);

	dev_err(mmc_dev(host->mmc), "clock down to %ld Hz, and retune rx\n",
		clk_get_rate(rtk_host->pll));
}
...
static int rtkstb_sdhci_execute_tuning(struct sdhci_host *host, u32 opcode)
{
	...
	do {
		...
		if (ret) {
			retune = 1;
			if (clk_get_rate(rtk_host->pll) == 50000000) {
	...
	if (!retune && !(rtkquirks & RTKQUIRK_SSC_CLK_JITTER))
		rtkstb_sdhci_ssc_enable(rtk_host);

	dev_info(mmc_dev(host->mmc), "after tuning, current pll %ld Hz\n",
		 clk_get_rate(rtk_host->pll));
...
static void rtkstb_sdhci_voltage_switch(struct sdhci_host *host)
{
	...
	rtkstb_sdhci_clear_bit(dhc_base, SDHCI_RTK_DUTY_CTRL, SDIO_STOP_CLK_EN);
...
static int sdhci_rtkstb_probe(struct platform_device *pdev)
{
	...
	rtk_host->pll = devm_clk_get(&pdev->dev, "pll");
	if (IS_ERR(rtk_host->pll)) {
		dev_err(&pdev->dev, "error %pE: failed to get clock PLL\n", rtk_host->pll);
		ret = PTR_ERR(rtk_host->pll);
		goto err_free_pltfm;
	}

	rtk_host->phase_tx = devm_clk_get(&pdev->dev, "tx");
	if (IS_ERR(rtk_host->phase_tx)) {
		dev_err(&pdev->dev, "error %pE: failed to get TX phase clock\n", rtk_host->phase_tx);
		ret = PTR_ERR(rtk_host->phase_tx);
		goto err_free_pltfm;
	}

	rtk_host->phase_rx = devm_clk_get(&pdev->dev, "rx");
	if (IS_ERR(rtk_host->phase_rx)) {
		dev_err(&pdev->dev, "error %pE: failed to get RX phase clock\n", rtk_host->phase_rx);
		ret = PTR_ERR(rtk_host->phase_rx);
		goto err_free_pltfm;
	}
...
static void sdhci_rtkstb_disable_pll_clk(struct sdhci_host *host)
{
	...
	if (!IS_ERR(rtk_host->pll))
		clk_disable_unprepare(rtk_host->pll);
...
static int __maybe_unused sdhci_rtkstb_resume(struct device *dev)
{
	...
	mask = SSC_RSTB | SSC_PLL_RSTB | SSC_PLL_POW;

	regmap_update_bits(crt_base, SYS_PLL_SD4, mask, mask);
