...
static int dw_mci_rtk_execute_tuning(struct dw_mci_slot *slot, u32 opcode)
{
	...
	do {
		...
		pr_err("rx_window = 0x%x, rx_best = 0x%x\n", rx_window, rx_best);
		...
		if (mmc->ios.timing == MMC_TIMING_MMC_HS400) {
			...
			if (max == 0) {
				if (fail == false) {
					...
					ret = -EFAULT;
					pr_err("Cannot find a proper dqs window, dqs tap bitmap=0x%x !\n", bitmap);
					goto out;
				}
			} else {
				...
				dqs_retry = false;
				pr_err("max sample point = %d, bitmap = 0x%x, DQS = 0x%x\n",
	...
	do {
		...
		if (tx_window == 0) {
			pr_err("tx_window = 0, cannot find a proper tx phase\n");
			ret = -EFAULT;
			goto out;
	...
	if (tx_best == 0xff) {
		pr_err("no 6 continuous points for tx phase\n");
		ret = -EFAULT;
		goto out;
	}
	dw_mci_rtk_phase_tuning(host, tx_best, 0xff);
	pr_err("tx_window = 0x%x, tx_best = 0x%x\n", tx_window, tx_best);
...
static int dw_mci_rtk_parse_dt(struct dw_mci *host)
{
	...
	if (prop) {
		...
		pr_err("[%s] emmc mode : %d\n", __func__, priv->emmc_mode);
	} else {
		pr_err("[%s] use default emmc sdr50 mode !\n", __func__);
	}
...
static int dw_mci_rtk_init(struct dw_mci *host)
{
	...
	/*Set the eMMC wrapper little Endian before Kent*/
	if (!host->dma_64bit_address) {
...
static int dw_mci_rtk_suspend(struct device *dev)
{
	...
	pr_err("[%s] AHB=0x%x, dw_mci_cqe_suspend ret=%d\n",
...
static int dw_mci_rtk_resume(struct device *dev)
{
	...
	pr_err("[%s] AHB=0x%x, dw_mci_cqe_resume ret=%d\n",
		__func__, mci_readl(host, AHB), ret);
...
static const struct of_device_id dw_mci_rtk_match[] = {
	...
	{ .compatible = "rtd13xxd-dw-cqe-emmc",
		.data = &rtk_drv_data },
	{ .compatible = "rtd13xxe-dw-cqe-emmc",
                .data = &rtk_drv_data },
