...
static int dw_mci_rtk13xx_execute_tuning(struct dw_mci_slot *slot, u32 opcode)
{
	...
	if (mmc->ios.timing == MMC_TIMING_MMC_HS400) {
		...
		if(fail == true)
			...
			pinctrl_select_state(priv->pinctrl, priv->pins_hs400);
		if ((mmc->card->cid.manfid == 0x2f) && (strcmp(mmc->card->cid.prod_name, "05S000") == 0)) {
			pr_err("!!! Do Not need to tune TX1_WINDOW!!!\n");
			ret = 0;
			goto out;
		}
	}
...
static int dw_mci_rtk13xx_prepare_hs400_tuning(struct dw_mci *host, struct mmc_ios *ios)
{
	...
	if ((mmc->card->cid.manfid == 0x2f) && (strcmp(mmc->card->cid.prod_name, "05S000") == 0))
		cmd_delay_tap_setting(host, 0xb8);
	else
		cmd_delay_tap_setting(host, host->cmd_dly_tap);

	if (host->cmd_dly_tap != 0)
		pr_info("host->cmd_dly_tap = 0x%x\n", host->cmd_dly_tap);
