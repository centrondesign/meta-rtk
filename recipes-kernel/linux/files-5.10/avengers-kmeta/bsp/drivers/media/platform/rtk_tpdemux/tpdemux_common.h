...
#else /* IS_ENABLED(CONFIG_DMABUF_HEAPS_REALTEK) */

#if IS_ENABLED(CONFIG_ION_REALTEK) || IS_ENABLED(CONFIG_ION_RTK_DHC_HEAP)
#include <linux/ion.h>
#include <uapi/linux/ion.h>
#include <soc/realtek/uapi/ion_rtk.h>
#include <ion_rtk_alloc.h>
#define USE_ION_BUFFER (1)
#define ion_alloc ext_rtk_ion_alloc
#else /* IS_ENABLED(CONFIG_ION_REALTEK) || IS_ENABLED(CONFIG_ION_RTK_DHC_HEAP) */
#define ion_alloc(...) (NULL)
#endif /* IS_ENABLED(CONFIG_ION_REALTEK) || IS_ENABLED(CONFIG_ION_RTK_DHC_HEAP) */

#define rheap_alloc(...) (NULL)
#endif /* IS_ENABLED(CONFIG_DMABUF_HEAPS_REALTEK) */
...
#define TPD_WARN(format, ...) printk(KERN_WARNING "[tpd-w] " format "\n", ## __VA_ARGS__)
#define TPD_NOTI(format, ...) printk(KERN_NOTICE "[tpd-n] " format "\n", ## __VA_ARGS__)
#define TPD_INFO(format, ...) printk(KERN_INFO "[tpd-i] " format "\n", ## __VA_ARGS__)
#if 1
#define  TPD_DBG(format, ...) printk(KERN_DEBUG "[tpd-d] " format "\n", ## __VA_ARGS__)
#else
#define  TPD_DBG(...)
#endif

#define USE_KTHREAD_WORK 1
...
#else
	struct sg_table *ddr_q_sgt;
	struct dma_buf_attachment *ddr_q_attach;
#endif
...
struct demux_info {
	unsigned int tp_id; /* DMX_TP_ID A0,A1... */
	int tp_mapping; /* TP_ID: A, B, C */
	int active;
...
#else
	struct kthread_work ktwork;
#endif

#ifdef USE_ION_BUFFER
	struct dma_buf *mmbuf_dmabuf;
#else
	struct sg_table *mmbuf_sgt;
	struct dma_buf_attachment *mmbuf_attach;
#endif
...
struct filter_info *rtk_filter_init(struct demux_info *dmx, int index, int rq_id);
