	...
	struct VIDEO_RPC_VOUT_CONFIG_TV_SYSTEM *tv_system)
{
	...
	ret = RPC_ToAgent_QueryConfigTvSystem(tx_dev, &cur_tv_system);
	...
	struct hdmi_format_setting *format)
{
	...
	case VO_HDMI_OFF:
		format->mode = DISPLAY_MODE_OFF;
		break;
	case VO_DVI_ON:
		format->mode = DISPLAY_MODE_DVI;
		break;
	case VO_HDMI_ON:
		hdmi2px_feature = tv_system->hdmiInfo.hdmi2px_feature;
		if ((hdmi2px_feature & HDMI2PX_FRLRATE_MASK) != 0)
			format->mode = DISPLAY_MODE_FRL;
		else
			format->mode = DISPLAY_MODE_HDMI;
		break;
	default:
		ret = -1;
...
int set_hdmitx_format(struct device *dev, struct hdmi_format_setting *format)
{
	...
	/* HDMI OFF */
	if ((format->vic == 0) || (format->mode == DISPLAY_MODE_OFF)) {
		/* cvbs format */
		if (format->vic != 0) {
	...
	if ((format->mode == DISPLAY_MODE_FRL) &&
		...
		RPC_TOAGENT_HDMI_Config_TV_System(tx_dev,
						hdmitx_edid_info, &tv_system);
		memset(&tv_system, 0, sizeof(tv_system));
	}

	tv_system.videoInfo.standard = standard;

	if (format->mode == DISPLAY_MODE_DVI)
...
config:
	...
	/* Send RPC */
	RPC_TOAGENT_HDMI_Config_TV_System(tx_dev, hdmitx_edid_info, &tv_system);
exit:
	return ret;
}


int get_hdmitx_format(struct device *dev, struct hdmi_format_setting *format)
{
	int ret;
	hdmitx_device_t *tx_dev = dev_get_drvdata(dev);
...
int set_hdmitx_format_new(struct device *dev,
		struct hdmi_format_setting *format)
{
	int ret;
	...
	dev_info(dev, "set format new: mode=%u vic=%u freq_shift=%u "
		"color=%u depth=%u 3D=%u HDR=%u misc=0x%x",
		format->mode, format->vic, format->freq_shift,
		format->color, format->color_depth,
		format->_3d_format, format->hdr, format->misc);
	...
	if ((tx_dev->frl->is_support_frl == 0) ||
		(hdmitx_edid_info->max_frl_rate == 0))
		goto skip_lt;
	...
	} else if ((format->mode == DISPLAY_MODE_OFF) &&
		(tx_dev->frl->current_frl_rate != 0)) {
		/* FRL_Rate=0 */
		dev_info(dev, "set frl_rate=0, current_frl_rate=%u",
				tx_dev->frl->current_frl_rate);
		hdmitx_write_scdc_port(SCDCS_Config_1, 0);
	}

skip_lt:
	if (frl_rate) {
		output_fmt.frl_rate = frl_rate;
	} else {
		tx_dev->frl->current_frl_rate = 0;
	...
	output_fmt.cmd_version = 0;
	output_fmt.display_mode = format->mode;
	output_fmt.vic = format->vic;
	output_fmt.clock = 1485000;
	output_fmt.is_fractional_fps = format->freq_shift;
	output_fmt.colorspace = format->color;
	output_fmt.color_depth = format->color_depth;
	output_fmt.hdr_mode = format->hdr;

	if (format->color_depth == 8 &&
		(format->hdr == HDR_CTRL_HDR10 || format->hdr == HDR_CTRL_INPUT))
		output_fmt.en_dithering = 1;
	else
		output_fmt.en_dithering = 0;

	switch (format->_3d_format) {
	case FORMAT_RTK_3D_FP:
		output_fmt.src_3d_fmt = HDMI_3D_FRAME_PACKING;
		output_fmt.dst_3d_fmt = HDMI_3D_FRAME_PACKING;
		break;
	case FORMAT_RTK_3D_SS:
		output_fmt.src_3d_fmt = HDMI_3D_SIDE_BY_SIDE_HALF;
		output_fmt.dst_3d_fmt = HDMI_3D_FRAME_PACKING;
		break;
	case FORMAT_RTK_3D_TB:
		output_fmt.src_3d_fmt = HDMI_3D_TOP_AND_BOTTOM;
		output_fmt.dst_3d_fmt = HDMI_3D_FRAME_PACKING;
		break;
	case FORMAT_3D_FP:
		output_fmt.src_3d_fmt = HDMI_2D;
		output_fmt.dst_3d_fmt = HDMI_3D_FRAME_PACKING;
		break;
	case FORMAT_3D_SS:
		output_fmt.src_3d_fmt = HDMI_2D;
		output_fmt.dst_3d_fmt = HDMI_3D_SIDE_BY_SIDE_HALF;
		break;
	case FORMAT_3D_TB:
		output_fmt.src_3d_fmt = HDMI_2D;
		output_fmt.dst_3d_fmt = HDMI_3D_TOP_AND_BOTTOM;
		break;
	default:
		output_fmt.src_3d_fmt = HDMI_2D;
		output_fmt.dst_3d_fmt = HDMI_2D;
	}

	ret = rpc_toagent_set_display_format(dev, &output_fmt);

	return ret;
}

int get_hdmitx_format_new(struct device *dev, struct hdmi_format_setting *format)
{
	int ret;
	struct VIDEO_RPC_VOUT_DISPLAY_OUTPUT_FORMAT cur_fmt;
	unsigned char itc;
	unsigned char content_type;

	ret = rpc_toagent_get_display_format(dev, &cur_fmt);
	if (ret)
		return ret;

	format->mode = cur_fmt.display_mode;
	format->vic = cur_fmt.vic;
	format->freq_shift = cur_fmt.is_fractional_fps;
	format->color = cur_fmt.colorspace;
	format->color_depth = cur_fmt.color_depth;

	switch (cur_fmt.src_3d_fmt) {
	case HDMI_2D:
		switch (cur_fmt.dst_3d_fmt) {
		case HDMI_2D:
			format->_3d_format = FORMAT_3D_OFF;
			break;
		case HDMI_3D_FRAME_PACKING:
			format->_3d_format = FORMAT_3D_FP;
			break;
		case HDMI_3D_SIDE_BY_SIDE_HALF:
			format->_3d_format = FORMAT_3D_SS;
			break;
		case HDMI_3D_TOP_AND_BOTTOM:
			format->_3d_format = FORMAT_3D_TB;
			break;
		default:
			format->_3d_format = FORMAT_3D_OFF;
	...
	format->hdr = cur_fmt.hdr_mode;

	itc = cur_fmt.avi_infoframe.date_byte[2] & 0x80 ? 1 : 0;
	content_type = (cur_fmt.avi_infoframe.date_byte[4] >> 4) & 0x3;
	format->misc = (itc << 2) | content_type;
