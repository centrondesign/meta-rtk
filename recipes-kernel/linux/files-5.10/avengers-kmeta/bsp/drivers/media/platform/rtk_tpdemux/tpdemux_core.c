...
#ifndef USE_KTHREAD_WORK
static void output_feedwork(struct work_struct *work)
#define DI_WORK_MEMBER work
#else
static void output_feedwork(struct kthread_work *work)
#define DI_WORK_MEMBER ktwork
#endif
{
	struct demux_info *dmx = (struct demux_info *)container_of(work,
					struct demux_info, DI_WORK_MEMBER);
	struct rtktpfei *fei = dmx->fei;
	struct dvb_demux_feed *dvbdmxfeed = fei->dvbdmxfeed[dmx->tp_id];
...
extern void rtk_tp_reset_filters(struct demux_info *dmx, struct rtktpfei *fei);

#ifndef USE_KTHREAD_WORK
#define TPD_QUEUE_WORK(f, d) queue_work(f->wq, &d->work)
#define TPD_INIT_WORK(d, fnt) INIT_WORK(&d->work, fnt)
#define TPD_CANCEL_WORK_SYNC(d) cancel_work_sync(&d->work);
#else
#define TPD_QUEUE_WORK(f, d) kthread_queue_work(f->ktworker, &d->ktwork)
#define TPD_INIT_WORK(d, fnt) kthread_init_work(&d->ktwork, fnt)
#define TPD_CANCEL_WORK_SYNC(d) kthread_cancel_work_sync(&d->ktwork);
#endif

#define RST_TH (1)
#if (RST_TH > 1)
#define TPD_CHECK_RESET(c) ((c % RST_TH == (RST_TH-1)) ? 1 : 0)
#else
#define TPD_CHECK_RESET(c) (1)
#endif
...
static void rtk_tp_timer_interrupt(struct timer_list *t)
{
	...
	for (dmx_num = 0; dmx_num < fei->num_dmx; dmx_num++) {
		...
		if (dmx->active && dmx->regs.base) {
			int need_reset = 0;
			if (rtk_is_tp_enable(dmx)) {
				dmx->reset_cnt = 0;
			} else {
				need_reset = TPD_CHECK_RESET(dmx->reset_cnt);
				dmx->reset_cnt++;
			}

			if (need_reset) {
				rtk_tp_reset_filters(dmx, fei);
			}

			TPD_QUEUE_WORK(fei, dmx);
		}
	}

	if (fei->global_feed_count > 0)
		mod_timer(&fei->timer, jiffies + usecs_to_jiffies(POLL_USECS));
}
...
static int rtk_tp_start_feed(struct dvb_demux_feed *dvbdmxfeed)
{
	...
	fei->dvbdmxfeed[stdemux->tsin_index] = dvbdmxfeed;
	...
	if (stdemux->running_feed_count == 0) {
		...
		TPD_INIT_WORK(dmx, output_feedwork);
		dmx->active = 1;
	}
	...
	if (fei->model_id != TP_HANK) {
		if (index == 0 && fei->fi[0].ddr_q_id == 1)
			rq_offs = 1;
	}

	/* search for real used entry of ddr queues(in fi[]) */
	if (dvbdmxfeed->pid == 8192) { /* fi[0] or fi[1] */
		rq_idx = rq_offs;
	} else {
		int start = 1 + rq_offs; /* fi[1] or fi[2] */

		// pids for lower bit rate, including PAT/CAT/SDT/EIT etc for DVB
		// these pids start at 32, the beginning of the lbr memory pool
		// i.e. use the minimum size of buffer in the pre-allocated pool
		if (dvbdmxfeed->pid < 0x0020) {
			start = (TP_PID_FILTER_COUNT/2)+1;
		}

		for (i = start; i < TP_PID_FILTER_COUNT; i++) {
			if (fei->fi[i].ddr_q_id == 0) {
				rq_idx = i;
				break;
			}
		}
		...
		if (rq_idx == -1)
			rq_idx = rq_offs; /* set to the largest size */
	}

	filter = rtk_filter_init(dmx, index, rq_idx);
	...
	dev_dbg(fei->dev, "[TPD%d] set pf %d %p ind %d rqi %d\n",
		stdemux->tsin_index, dvbdmxfeed->pid, dmx->regs.base,
		index, rq_idx);
...
static int rtk_tp_stop_feed(struct dvb_demux_feed *dvbdmxfeed)
{
	...
	dev_dbg(fei->dev, "[TPD%d] stop pf %d\n",
		stdemux->tsin_index, dvbdmxfeed->pid);
...
#ifdef USE_KTHREAD_WORK
#define TP_RT_PRIO (MAX_RT_PRIO-1)
static int rtk_tp_init_kthread(struct rtktpfei *fei)
{
	struct task_struct *task;
	struct sched_param param = { .sched_priority = TP_RT_PRIO };

	fei->ktworker = kthread_create_worker(0, "%s", "fei_kt");
	if (fei->ktworker) {
		task = fei->ktworker->task;
	} else {
		pr_err("initialize fei kthread failed\n");
		return -ENOMEM;
	}
...
static int rtk_tp_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_string(np, "compatible", &compati)) {
		dev_info(&pdev->dev, "[TPD] No compatible found, use default\n");
		model_id = TP_HANK;
	}

	if (compati) {
		if (strcmp(compati, rtk_tp_ids[TP_HANK].compatible) == 0)
			model_id = TP_HANK;
		else if (strcmp(compati, rtk_tp_ids[TP_PARKER].compatible) == 0)
			model_id = TP_PARKER;
		else
			model_id = TP_HANK;

		dev_info(&pdev->dev, "[TPD] %s model %d\n", compati, model_id);
	}

	fei->model_id = model_id;
	...
	ret = of_property_read_u8(np, "m2m-idx", &fei->m2m_idx);
	if (ret) {
		dev_err(&pdev->dev, "[TPD] No m2m-idx found\n");
		fei->m2m_idx = 0xff;
	}
	...
	for_each_child_of_node(np, child) {
		...
		tp_reg_mapping(dmx->tp_id, &dmx->regs, model_id);
		index++;
	}
	fei->num_dmx = index;
	mutex_init(&fei->lock);
...
#else
	ret = rtk_tp_init_kthread(fei);
	if (ret) {
		dev_err(dev, "rtk_tp_init_kthread failed (%d)\n", ret);
		goto error;
	}
#endif
	...
	rtk_tp_dev_init(fei); /* support dev_tp */
...
static int rtk_tp_remove(struct platform_device *pdev)
{
...
#else
	if (fei->ktworker) {
		kthread_flush_worker(fei->ktworker);
		kthread_destroy_worker(fei->ktworker);
	}
#endif
	...
	misc_deregister(&fei->mdev); /* support dev_tp */
...
static const struct of_device_id rtk_tp_ids[] = {
	{ .compatible = "realtek,rtd1319-tp" }, /* TP_HANK */
#if 0
	{ .compatible = "realtek,rtd1619b-tp" }, /* TP_STARK */
#else
	{ .compatible = "null" }, /* not supported for STARK */
#endif
	{ .compatible = "realtek,rtd1319d-tp" }, /* TP_PARKER */
