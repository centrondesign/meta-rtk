...
void hdcp_lib_auto_ri_check(bool state)
{
	...
	if (state == true) {
		...
		if (request_irq(hdcp.hdcp_irq_num, HDCP_interrupt_handler, IRQF_SHARED, "rtk_hdcp", irq_dev_id)) {
			...
			/*
			 * Clear riupdated before enable ri interrupt,
			 * otherwise interrupt might be triggered immediately
			 */
			if (hdcp.use_new_mac) {
				regmap_read(hdcp.hdcp_base, HDMI_NEW_INTST_SCPU, &intst);
				riupdated = HDMI_NEW_INTST_SCPU_get_ri_upd_int_status(intst);
				if (riupdated)
					regmap_write(hdcp.hdcp_base, HDMI_NEW_INTST_SCPU,
						HDMI_NEW_INTST_SCPU_ri_upd_int_status(1) |
						HDMI_NEW_INTST_SCPU_write_data(0));
			} else {
