 ...
 * @payload_len: Dolby VSADB payload length
 * @payload_data: Dolby VSADB payload data
 */
struct dolby_vsadb {
	u_char payload_len;
	u_char payload_data[27];
} __attribute__ ((packed));
...
#define HDMI_GET_FULL_EDID				_IOWR(HDMI_IOCTL_MAGIC, 13, struct full_edid)
...
#define HDMI_SKIP_EDID_CHECK            _IOWR(HDMI_IOCTL_MAGIC, 27, int)
...
/* frl_phy_setting */
extern int frl_set_phy(struct device *dev,
...
extern int get_pll_mode(struct device *dev,
...
int set_hdmitx_format_new(struct device *dev,
		struct hdmi_format_setting *format);
int get_hdmitx_format_new(struct device *dev,
		struct hdmi_format_setting *format);
...
extern void ta_hdcp_set_keepout_win(unsigned int start, unsigned int end);
extern void ta_hdcp_set_rekey_win(unsigned char rekey_win);
#else
...
static inline void ta_hdcp_set_keepout_win(unsigned int start, unsigned int end)
{
}

static inline void ta_hdcp_set_rekey_win(unsigned char rekey_win)
{
}
#endif
