...
unsigned int hdmitx_read32(struct device *dev, unsigned int offset)
{
	...
	if (tx_dev->use_new_mac)
		max_offset = HDMI_NEW_FRL_PKT6;
	else
		max_offset = HDMI_FRL_TRAINING;
...
static long rtk_hdmitx_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case HDMI_CONFIG_AVI_INFO:
		return ops_config_avi_info((void __user *)arg, tx_dev);

	case HDMI_SET_FREQUNCY:
		return ops_set_frequency((void __user *)arg, tx_dev);

	case HDMI_SEND_AUDIO_MUTE:
		return ops_set_audio_mute((void __user *)arg, tx_dev);

	case HDMI_SEND_AUDIO_VSDB_DATA:
		return ops_send_audio_vsdb_data((void __user *)arg, tx_dev);

	case HDMI_SEND_AUDIO_EDID2:
		return ops_send_audio_edid2((void __user *)arg, tx_dev);
	...
	case HDMI_GET_FULL_EDID:
		return ops_get_full_edid((void __user *)arg, tx_dev);

	case HDMI_SEND_VOUT_EDID_DATA:
		return ops_send_vout_edid_data((void __user *)arg, tx_dev);
	...
	case HDMI_SKIP_EDID_CHECK:
		return ops_skip_edid_check((void __user *)arg, tx_dev);
...
static int rtk_hdmitx_suspend(struct device *dev)
{
	...
	ret_val = rtk_hdmitx_extcon_suspend(tx_dev);
...
static int rtk_hdmitx_resume(struct device *dev)
{
	...
	ret_val = rtk_hdmitx_extcon_resume(tx_dev);
...
static void rtk_hdmitx_shutdown(struct platform_device *pdev)
{
	...
	rtk_hdmitx_extcon_suspend(tx_dev);
...
static int rtk_hdmi_probe(struct platform_device *pdev)
{
	...
	p_adap = i2c_get_adapter(1);
	if (p_adap == NULL) {
		dev_info(&pdev->dev, "i2c1 is not ready");
		goto deferred;
	}
	...
	/* new mac */
	ret_value = of_property_read_u32(pdev->dev.of_node, "use-new-mac",
		&tx_dev->use_new_mac);
	if (ret_value < 0)
		tx_dev->use_new_mac = 0;

	dev_info(&pdev->dev, "use_new_mac=%u", tx_dev->use_new_mac);
	if (tx_dev->use_new_mac)
		syscon_np = of_parse_phandle(pdev->dev.of_node, "syscon", 4);
	else
		syscon_np = of_parse_phandle(pdev->dev.of_node, "syscon", 0);

	if (IS_ERR_OR_NULL(syscon_np)) {
		dev_err(&pdev->dev, "Parse hdmi syscon phandle fail");
		goto end;
	}

	tx_dev->reg_base = syscon_node_to_regmap(syscon_np);
	if (IS_ERR_OR_NULL(tx_dev->reg_base)) {
		dev_err(&pdev->dev, "Remap hdmi syscon to reg_base fail");
	...
	if (!ret_value) {
		tx_dev->frl->is_support_frl = frl_support;
		dev_info(&pdev->dev, "support-frl=%d", frl_support);
	} else {
		tx_dev->frl->is_support_frl = 0;
	...
	mutex_init(&tx_dev->hdmi_data->info_lock);

	ret_value = init_hdmitx_rpc_info(&pdev->dev, &tx_dev->rpc_info);
	if (ret_value)
		goto err_clk_hdmi;
	...
	if (register_hdmitx_extcon_dev(tx_dev)) {
		dev_err(&pdev->dev, "Could not register_hdmitx_extcon_dev");
...
static int rtk_hdmi_exit_callback(struct device *dev, void *data)
{
	...
	deregister_hdmitx_extcon_dev(tx_dev);
	misc_deregister(&tx_dev->miscdev);
