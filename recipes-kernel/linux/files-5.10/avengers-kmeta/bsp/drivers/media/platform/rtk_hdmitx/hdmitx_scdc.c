...
/**
 * set_scdc_tmds_configuration - SCDC TMDS Configuration
 *
 * The maximum time period between write SCDC Scrambling_Enable
 * and output scrambled video is 100ms in spec.
 *
 * Return: SCDC TMDS_Config value
 */
unsigned char set_scdc_tmds_configuration(
	struct device *dev,
	struct VIDEO_RPC_VOUT_DISPLAY_OUTPUT_FORMAT *output_fmt)
{
	unsigned char config_data;
	unsigned char en_deep_color;
	unsigned char en_frame_packing;
	...
	/* Skip TMDS_Config in FRL mode */
	if (output_fmt->display_mode == DISPLAY_MODE_FRL)
		return config_data;

	if (((int)output_fmt->colorspace != (int)COLOR_YUV422)
		&& (output_fmt->color_depth > 8))
		en_deep_color = 1;
	else
		en_deep_color = 0;

	if (output_fmt->dst_3d_fmt == HDMI_3D_FRAME_PACKING)
		en_frame_packing = 1;
	else
		en_frame_packing = 0;

	switch (output_fmt->vic) {
	...
	case VIC_4096X2160P30:
		if ((en_deep_color == 1) || (en_frame_packing ==1))
			config_data = 0x3;
		else
			config_data = 0x0;
		break;
	...
	case VIC_4096X2160P60:
		if ((int)output_fmt->colorspace != (int)COLOR_YUV420) {
			config_data = 0x3;
		} else {
			/*	Ycbcr420 */
			if (en_deep_color)
	...
	if ((hdmitx_edid_info->scdc_capable&SCDC_PRESENT) ||
		(config_data != 0)){
		dev_info(dev, "Send SCDC TMDS_Config(0x%02x)", config_data);
		hdmitx_write_scdc_port(SCDCS_TMDS_Config, config_data);
	}
