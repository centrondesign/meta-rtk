...
EXPORT_SYMBOL(ta_hdcp_fix480p);

void ta_hdcp_set_keepout_win(unsigned int start, unsigned int end)
{
	int ret;
	struct tee_param *invoke_param = NULL;
	struct tee_ioctl_invoke_arg arg_I;
	...
	invoke_param[0].u.value.a = start; /* keepoutwinstart */
	invoke_param[0].u.value.b = end; /* keepoutwinend */
...
void ta_hdcp_set_rekey_win(unsigned char rekey_win)
{
	int ret;
	struct tee_param *invoke_param = NULL;
	struct tee_ioctl_invoke_arg arg_I;
	...
	arg_I.func = TA_TEE_HDCP14_SetRekeyWin;
	arg_I.session = arg.session;
	arg_I.num_params = ARG_NUM_ELEMENT;
...
EXPORT_SYMBOL(ta_hdcp_set_rekey_win);
