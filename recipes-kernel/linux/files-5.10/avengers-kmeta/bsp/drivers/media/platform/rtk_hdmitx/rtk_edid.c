...
/**
 * parse_VendorSpecificAudio_db - parse vendor specific audio data block
 * @sink_cap: sink_capabilities_t
 * @db: vendor specific audio data block
 *
 * Only store dolby audio information
 */
static void parse_VendorSpecificAudio_db(struct sink_capabilities_t *sink_cap, const u8 *db)
{
	...
	length = db[0]&0x1F;
	if (length < 4) {
		pr_info("Wrong length in VENDOR_SPECIFIC_AUDIO_DATA_BLOCK\n");
		goto exit;
	}

	if (length < 5) {
		pr_info("No additional payload in VENDOR_SPECIFIC_VIDEO_DATA_BLOCK\n");
		goto exit;
	}
	...
	if ((oui[0] == 0x46) && (oui[1] == 0xD0) && (oui[2] == 0x00)) {
		pr_info("Found dolby VSADB in EDID\n");

		dolby_audio = &sink_cap->dolby_audio;
		dolby_audio->payload_len = length-4;
		for (i = 5; i <= length; i++)
			dolby_audio->payload_data[i-5] = db[i];
	}
			...
			struct sink_capabilities_t *sink_cap, const u8 *db)
{
	...
	case VENDOR_SPECIFIC_AUDIO_DATA_BLOCK:
		pr_devel("[%s] VENDOR_SPECIFIC_AUDIO_DATA_BLOCK (%u bytes)",
			 __func__, dbl);
		parse_VendorSpecificAudio_db(sink_cap, db);
		break;
...
static struct edid *rtk_do_get_edid(hdmitx_device_t *tx_dev)
{
	...
	else if ((block[0x7e] > 8) && skip_edid_check)
		return (struct edid *)block;
...
void rtk_set_allow_invalid_edid(int skip_check)
{
	if (skip_check)
		skip_edid_check = 1;
	else
