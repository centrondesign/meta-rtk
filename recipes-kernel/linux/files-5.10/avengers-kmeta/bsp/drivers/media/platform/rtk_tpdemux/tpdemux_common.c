...
// size of the only specific buffer to process streams of all pids
#if IS_ENABLED(CONFIG_RTK_TPDEMUX_QSIZE_512KB)
#define DDR_BUF_ALL_SIZE (512 * 1024)
#elif IS_ENABLED(CONFIG_RTK_TPDEMUX_QSIZE_768KB)
#define DDR_BUF_ALL_SIZE (768 * 1024)
#elif IS_ENABLED(CONFIG_RTK_TPDEMUX_QSIZE_1024KB)
#define DDR_BUF_ALL_SIZE (1024 * 1024)
#elif IS_ENABLED(CONFIG_RTK_TPDEMUX_QSIZE_2048KB)
#define DDR_BUF_ALL_SIZE (2048 * 1024)
#else
#define DDR_BUF_ALL_SIZE (512 * 1024)
#endif

// size of buffer for high bitrate streams
#define DDR_BUF_HBR_SIZE (DDR_BUF_ALL_SIZE/2)
// size of buffer for medium bitrate streams
#define DDR_BUF_MBR_SIZE (DDR_BUF_ALL_SIZE/4)
// size of buffer for low bitrate streams
#define DDR_BUF_LBR_SIZE (DDR_BUF_ALL_SIZE/8)
...
struct tp_pid_param {
	...
	/* skip unnecessary */
	//unsigned char disable_nx;
	//unsigned char dec_then_enc;
	...
	/* skip */
	//unsigned char kte_index_clear_valid;
	//unsigned char kte_index_clear;
	//unsigned char kte_index_odd_valid;
	//unsigned char kte_index_odd;
	//unsigned char kte_index_even_valid;
	//unsigned char kte_index_even;

	//unsigned char ofb_ls;
	//unsigned char mode_ls;
	//unsigned char algo_ls;
	//unsigned char ofb_esa;
	//unsigned char mode_esa;
	//unsigned char algo_esa;
	//unsigned char ofb_ld;
	//unsigned char mode_ld;
	//unsigned char algo_ld;

	unsigned char cc_mode_index;
	unsigned char cc_mode;
	//unsigned char secondary_pid_en;
...
static void _tp_framer_enable(struct rtk_tp_reg *regs, unsigned char en);
...
static void _tp_framer_set_cntl2(struct rtk_tp_reg *regs,
					unsigned int cntl)
{
	cntl |= TP_TF_CNTL_WRITE_DATA(1);

	write_reg32(regs->reg_TF_CNTL2,  cntl);
	write_reg32(regs->reg_TF_CNTL2, ~cntl);
}

#if 0
static void _tp_framer_get_cntl2(struct rtk_tp_reg *regs,
					unsigned int *pCntl)
{
	*pCntl = read_reg32(regs->reg_TF_CNTL2) & (~0x01);
}
#endif
...
/* NeptuneTP::SetMode -> NeptuneTPFramer::SetMode */
...
/* SetTSMode -> NeptuneTP::SetTSIF -> NeptuneTPFramer::SetTSIF */
					...
					unsigned int enable)
{
	...
	} else if (regs->model_id == TP_PARKER) {
		enable &= ~TP_TF_INT_WRITE_DATA(1);
		write_reg32(regs->reg_TF_INT_EN, TP_TFX_INT_EN_MASK(enable)  |
							TP_TF_INT_WRITE_DATA(1));
		write_reg32(regs->reg_TF_INT_EN, TP_TFX_INT_EN_MASK(~enable) |
						TP_TF_INT_WRITE_DATA(0));
	}
					...
					unsigned int val)
{
	if (regs->model_id == TP_HANK) {
		...
		write_reg32(regs->reg_TF_INT, TP_TFX_INT_VALUE_MASK(val) |
						TP_TF_INT_WRITE_DATA(0));
	}
}
...
static void _tp_framer_reset(struct rtk_tp_reg *regs)
{
	...
	/* TPFramer::Reset() */
	if (regs->model_id == TP_PARKER) {
		cnt_init = cnt_init |
			TP_TF_CNTL_DYNC_CNTL_EN(1) |
			TP_TF_CNTL_RAW_MODE_VALID_SEL(0) |
			TP_TF_CNTL_BUF_RDY_CTL(1); // 0 in Hank

		cntl2_init =  TP_TF_CNTL2_OVERFLOW_DROP_DIS (0) |
			     TP_TF_CNTL2_ALGO_PID_MODE(1) |
			     TP_TF_CNTL2_TI_ALL_EN(0);
	}
	...
	if (regs->model_id == TP_HANK) {
		...
		//_tp_framer_set_int_enable(regs, TP_TFX_INT_EN);
		_tp_framer_clear_int_value(regs, TP_TFX_INT_VALUE);
		_tp_framer_set_cntl2(regs, cntl2_init);
	}
}
				...
				struct tp_pid_param param)
{
	...
	/* skip TP_PID_DATA2_NX_DIS_DESCRAMBLE */
...
#if 0
	write_reg32(regs->reg_TP_PID_DATA4,
		TP_PID_DATA4_KTE_IDX_CLEAR_VALID(Param.kte_index_clear_valid) |
		TP_PID_DATA4_KTE_IDX_CLEAR(Param.kte_index_clear)             |
		TP_PID_DATA4_KTE_IDX_ODD_VALID(Param.kte_index_odd_valid)     |
		TP_PID_DATA4_KTE_IDX_ODD(Param.kte_index_odd)                 |
		TP_PID_DATA4_KTE_IDX_EVEN_VALID(Param.kte_index_even_valid)   |
		TP_PID_DATA4_KTE_IDX_EVEN(Param.kte_index_even));

	write_reg32(regs->reg_TP_PID_DATA5, TP_PID_DATA5_OFB_LS(Param.ofb_ls) |
					TP_PID_DATA5_MODE_LS(Param.mode_ls)   |
					TP_PID_DATA5_ALGO_LS(Param.algo_ls)   |
					TP_PID_DATA5_OFB_ESA(Param.ofb_esa)   |
					TP_PID_DATA5_MODE_ESA(Param.mode_esa) |
					TP_PID_DATA5_ALGO_ESA(Param.algo_esa) |
					TP_PID_DATA5_OFB_LD(Param.ofb_ld)     |
					TP_PID_DATA5_MODE_LD(Param.mode_ld)   |
					TP_PID_DATA5_ALGO_LD(Param.algo_ld));

	write_reg32(regs->reg_TP_PID_DATA6,
			TP_PID_DATA6_CC_MODE_INDEX(Param.cc_mode_index) |
			TP_PID_DATA6_CC_MODE(Param.cc_mode)                   |
			TP_PID_DATA6_SECONDARY_PID_EN(Param.secondary_pid_en) |
			TP_PID_DATA6_PRIMARY_PID(Param.primary_pid));
#endif
...
#if 0
	/* PARKER */
	//pParam->enc_key_odd = (data >> 19) & 0x01;
	//pParam->enc = (data >> 17) & 0x03;
	pParam->enc_key_odd = (data >> 18) & 0x01;
	pParam->enc = (data >> 17) & 0x01;
	pParam->dec = (data >> 16) & 0x01;
	pParam->clear_head = (data >> 15) & 0x01;
	pParam->ddr_q2_v = (data >> 14) & 0x01;
	pParam->ddr_q2 = (data >> 8) & 0x3F;
#endif
	pParam->ext_pid = data & 0x3F;

        /* skip */
#if 0
	data = read_reg32(regs->reg_TP_PID_DATA4);
	pParam->kte_index_clear_valid = (data >> 23) & 0x01;
	pParam->kte_index_clear = (data >> 16) & 0x7F;
	pParam->kte_index_odd_valid = (data >> 15) & 0x01;
	pParam->kte_index_odd = (data >> 8) & 0x7F;
	pParam->kte_index_even_valid = (data >> 7) & 0x01;
	pParam->kte_index_even = data & 0x7F;

	data = read_reg32(regs->reg_TP_PID_DATA4);
	pParam->ofb_ls = (data >> 22) & 0x03;
	pParam->mode_ls = (data >> 20) & 0x03;
	pParam->algo_ls = (data >> 16) & 0x0F;
	pParam->ofb_esa = (data >> 14) & 0x03;
	pParam->mode_esa = (data >> 12) & 0x03;
	pParam->algo_esa = (data >> 8) & 0x0F;
	pParam->ofb_ld = (data >> 6) & 0x03;
	pParam->mode_ld = (data >> 4) & 0x03;
	pParam->algo_ld = data & 0x0F;

	data = read_reg32(regs->reg_TP_PID_DATA6);
	pParam->cc_mode_index = (data >> 16) & 0x0F;
	pParam->cc_mode = (data >> 14) & 0x03;
	pParam->secondary_pid_en = (data >> 13) & 0x01;
	pParam->primary_pid = data & 0x1FFF;
#endif
	mutex_unlock(&regs->pid_ctrl_mutex);
}
...
static void _tp_buffer_filter_check(struct filter_info *filter)
{
       struct rtk_tp_reg *regs = &filter->dmx->regs;
...
void rtk_tp_reset_filters(struct demux_info *dmx, struct rtktpfei *fei)
{

       struct dvb_demux_feed *feed = fei->dvbdmxfeed[dmx->tp_id];
       ...
       for (i = 0; i < TP_PID_FILTER_COUNT; i++) {
               ...
               if (dmx->pid_tbl_info[i].used == 1) {
                       ...
                       _tp_buffer_filter_check(filter);
               }
...
void rtk_set_ts_input_select(struct demux_info *dmx)
{
	...
	if (dmx->input_sel == TS_IN_TSIO) {
		...
		if (((read_reg32(regs->reg_TF_CNTL) /*& (~0x01)*/) &
			...
			} else if (regs->model_id == TP_PARKER) { /* also TP_STARK */
				/* raw mode */
				if (((read_reg32(regs->reg_TF_CNTL) >> 29) & 0x01) == 1) {
					write_reg32(regs->reg_TF_FRMCFG,
							(read_reg32(regs->reg_TF_FRMCFG) &
							~TP_TF_FRMCFG_LOCKNO(0xF)) |
							TP_TF_FRMCFG_LOCKNO(1));
...
struct filter_info *rtk_filter_init(struct demux_info *dmx, int i, int rq_id)
{
	...
	dev_dbg(fei->dev, "[TPD-%d] pt[%d] ptid[%d] bufid[%d] qid[%d] \n",
		dmx->tp_id, i, filter->pid_table_id, buf_idx, filter->ddr_q_id);
...
#ifdef USE_ION_BUFFER
	filter->ddr_q_dmabuf = fei->fi[rq_id].ddr_q_dmabuf;
#else
	filter->ddr_q_sgt= fei->fi[rq_id].ddr_q_sgt;
	filter->ddr_q_attach= fei->fi[rq_id].ddr_q_attach;
#endif
	filter->ddr_q_phy_addr = fei->fi[rq_id].ddr_q_phy_addr;
	filter->ddr_q_virt_addr = fei->fi[rq_id].ddr_q_virt_addr;
	filter->ddr_q_len = fei->fi[rq_id].ddr_q_len;
	...
	fei->fi[rq_id].ddr_q_id = 1; /* it means this ddr_q is assigned */
	filter->rq_id = rq_id; /* link filter to fi[rq_id] */

	_tp_framer_set_mode(regs, TP_FRAMER_NORMAL);

	/* FIXME: set M2MTP mode */
	if (fei->m2m_idx == DMX_TP_A_1 ||
	    (fei->model_id == TP_HANK && dmx->tp_id == DMX_TP_A_1))
...
void rtk_filter_uninit(struct filter_info *filter)
{
	...
	if (filter->rq_id > -1 && filter->rq_id < TP_PID_FILTER_COUNT)
		fei->fi[filter->rq_id].ddr_q_id = 0;
				...
				struct rtktpfei *fei)
{
	...
	result = dvb_net_init(adap, &demux->dvbnet, &demux->dvb_demux.dmx);
	if (result < 0) {
		dev_err(fei->dev, "dvb_net_init failed (%d)\n", result);
		goto err_net_init;
	}
...
err_net_init:
	demux->dvb_demux.dmx.disconnect_frontend(&demux->dvb_demux.dmx);
...
static void rtktpfe_alloc_ddr_q(struct rtktpfei *fei)
{
	struct filter_info *filter;
	int i;
	size_t ddr_queue_size;

	for (i = 0; i < TP_PID_FILTER_COUNT; i++) {

		filter = &fei->fi[i];

		// allocate the maximum size for the buffer of pid 8192
		if (i < 1)
			ddr_queue_size = DDR_BUF_ALL_SIZE;
		// e.g 1~16,17~32,33~63 for total 64 pfs
		else if (i <= (TP_PID_FILTER_COUNT/4))
			ddr_queue_size = DDR_BUF_HBR_SIZE;
		else if (i <= (TP_PID_FILTER_COUNT/2))
			ddr_queue_size = DDR_BUF_MBR_SIZE;
		else
			ddr_queue_size = DDR_BUF_LBR_SIZE;
...
static void rtktpfe_free_ddr_q(struct rtktpfei *fei)
{
	struct filter_info *filter;
	int i;
...
static void rtktpfe_alloc_TPA1_mmbuf(struct rtktpfei *fei)
{
...
static void rtktpfe_free_TPA1_mmbuf(struct rtktpfei *fei)
{
	struct demux_info *dmx = fei->demux_data[DMX_TP_A_1];
...
static void rtk_tp_free(struct dma_buf_attachment *att,
			struct sg_table *table)
{
	...
	dma_buf_unmap_attachment(att, table, DMA_BIDIRECTIONAL);
	dma_buf_detach(dmabuf, att);
	dma_buf_put(dmabuf);
}

static int rtk_tp_alloc(struct device *dev, size_t size,
			unsigned int flags,
			...
			phys_addr_t *phys, void **virt)
{
	...
	dmabuf = rheap_alloc("rtk_media_heap", size, flags);
	if (IS_ERR_OR_NULL(dmabuf)) {
		dev_err(dev, "dma_buf_get failed\n");
		ret = PTR_ERR(dmabuf);
		goto reset_args;
	}
	...
	*att = dma_buf_attach(dmabuf, dev);
	if (IS_ERR(*att)) {
		dev_err(dev, "attaching dmabuf failed\n");
		ret = PTR_ERR(*att);
		goto dma_put;
	}

	*tbl = dma_buf_map_attachment(*att, DMA_BIDIRECTIONAL);
	if (IS_ERR(*tbl)) {
		dev_err(dev, "mapping attachment failed\n");
		ret = PTR_ERR(*tbl);
		goto detach_this;
	}
	...
	*phys = (phys_addr_t)sg_phys((*tbl)->sgl);

	dev_dbg(dev, "%s *phys = 0x%llx *virt = %p \n", __func__, *phys, *virt);

	return 0;
end_access:
	dma_buf_end_cpu_access(dmabuf, DMA_BIDIRECTIONAL);
unmap_attach:
	dma_buf_unmap_attachment(*att, *tbl, DMA_BIDIRECTIONAL);
detach_this:
	dma_buf_detach(dmabuf, *att);
dma_put:
	dma_buf_put(dmabuf);
...
static void rtktpfe_alloc_ddr_q(struct rtktpfei *fei)
{
	...
	size_t ddr_queue_size;
	int ret = -1;
	struct device *dev = fei->dev;
	unsigned int flags = RTK_FLAG_NONCACHED | RTK_FLAG_SCPUACC
			    | RTK_FLAG_ACPUACC;

	if (fei->model_id == TP_HANK) {
		nextof_pidall = 1;
	} else {
		nextof_pidall = 2;
	}
	...
	for (i = 0; i < TP_PID_FILTER_COUNT; i++) {
		...
		// allocate the maximum size for the buffer of pid 8192
		if (i < nextof_pidall) /* 1 entry for Hank, 2 for Parker */
			ddr_queue_size = DDR_BUF_ALL_SIZE;
		// e.g ~16,17~32,33~63 for total 64 pfs
		else if (i <= (TP_PID_FILTER_COUNT/4))
			ddr_queue_size = DDR_BUF_HBR_SIZE;
		else if (i <= (TP_PID_FILTER_COUNT/2))
			ddr_queue_size = DDR_BUF_MBR_SIZE;
		...
		ret = rtk_tp_alloc(dev, ddr_queue_size, flags,
				&filter->ddr_q_attach,
				&filter->ddr_q_sgt,
				&filter->ddr_q_phy_addr,
				(void **)&filter->ddr_q_virt_addr);
		if (ret < 0) {
			pr_err("%s rtk_tp_alloc failed i:%d\n", __func__, i);
			goto free_all;
		}

		filter->ddr_q_len = (ddr_queue_size/TP_BUFFER_ALIGNMENT) *
					TP_BUFFER_ALIGNMENT;
		filter->ddr_q_id = 0; /* initial */
...
free_all:
	while (--i > 0) {
		filter = &fei->fi[i];
		if (filter->ddr_q_attach) {
			dma_buf_vunmap(filter->ddr_q_attach->dmabuf,
					filter->ddr_q_virt_addr);
			rtk_tp_free(filter->ddr_q_attach, filter->ddr_q_sgt);
		}
	};
}
...
static void rtktpfe_free_ddr_q(struct rtktpfei *fei)
{
	...
	for (i = 0; i < TP_PID_FILTER_COUNT; i++) {
		...
		if (!IS_ERR_OR_NULL(filter->ddr_q_attach)) {
			dma_buf_vunmap(filter->ddr_q_attach->dmabuf,
					filter->ddr_q_virt_addr);
			rtk_tp_free(filter->ddr_q_attach, filter->ddr_q_sgt);
		}

		filter->ddr_q_attach = NULL;
		filter->ddr_q_sgt = NULL;
		filter->ddr_q_virt_addr = NULL;
		filter->ddr_q_phy_addr = 0;
	}
}
...
static void rtktpfe_alloc_TPA1_mmbuf(struct rtktpfei *fei)
{
	int ret;
	struct demux_info *dmx = fei->demux_data[DMX_TP_A_1];
	unsigned int flags = RTK_FLAG_SCPUACC | RTK_FLAG_ACPUACC;

	ret = rtk_tp_alloc(fei->dev, MM_BUFFER_SIZE, flags,
			&dmx->mmbuf_attach,
			&dmx->mmbuf_sgt,
			&dmx->mmbuf_phy_addr,
			(void **)&dmx->mmbuf_virt_addr);

	if (ret < 0) {
		pr_err("%s rtk_tp_alloc failed\n", __func__);
		return;
	}
...
static void rtktpfe_free_TPA1_mmbuf(struct rtktpfei *fei)
{
	...
	if (!IS_ERR_OR_NULL(dmx->mmbuf_attach)) {
		dma_buf_vunmap(dmx->mmbuf_attach->dmabuf,
				dmx->mmbuf_virt_addr);
		rtk_tp_free(dmx->mmbuf_attach, dmx->mmbuf_sgt);
	}

	dmx->mmbuf_attach = NULL;
	dmx->mmbuf_sgt = NULL;
	dmx->mmbuf_virt_addr = NULL;
	dmx->mmbuf_phy_addr = 0;
	dmx->mmbuf_len = 0;
}
#endif /* USE_ION_BUFFER */
				...
				void *stop_feed)
{
	...
	if (fei->m2m_idx == DMX_TP_A_1 || fei->model_id == TP_HANK)
		rtktpfe->demux[DMX_TP_A_1].dvb_demux.dmx.write = rtk_tp_write;
...
void rtk_tp_deliver_data(struct dvb_demux_feed *feed)
{
	...
	for (i = 0; i < TP_PID_FILTER_COUNT; i++) {
		...
		if (dmx->pid_tbl_info[i].used == 1) {
			...
			filter = dmx->filter_data[i];
			if (filter == NULL ||
			    IS_ERR_OR_NULL(filter->ddr_q_virt_addr)) {
				mutex_unlock(&stdemux->dmxdev.mutex);
				continue;
			}
...
int rtk_tp_register(struct rtktpfe **rtktpfe,
			...
			void *stop_feed)
{
	rtktpfe_alloc_ddr_q(fei);
	if (fei->m2m_idx == DMX_TP_A_1 || fei->model_id == TP_HANK)
		rtktpfe_alloc_TPA1_mmbuf(fei);

	*rtktpfe = rtktpfe_create(fei, start_feed, stop_feed);
	if (!*rtktpfe) {
		rtktpfe_free_ddr_q(fei);
		if (fei->m2m_idx == DMX_TP_A_1 || fei->model_id == TP_HANK)
			rtktpfe_free_TPA1_mmbuf(fei);
