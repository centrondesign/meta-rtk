...
static unsigned int rtk_ir_find_wake_keycode(struct rtk_ir *ir, unsigned int code)
{
	struct pm_dev_param *pm_node = rtk_pm_get_param(PM);
	struct pm_private *dev_pm = dev_get_drvdata(pm_node->dev);
	struct irda_wakeup_key_v1 *tbl_v1;
	struct irda_wakeup_key_v2 *tbl_v2;
	int i, j;

	if (dev_pm->version == RTK_PCPU_VERSION_V2)
		tbl_v2 = ir->pcpu_data_v2.key_tbl;
	else
		tbl_v1 = ir->pcpu_data_v1.key_tbl;

	for (i = 0; i < MAX_KEY_TBL; i++) {
		for (j = 0; j < MAX_WAKEUP_CODE; j++) {
			if (dev_pm->version == RTK_PCPU_VERSION_V2) {
				if (tbl_v2[i].pkeys[j] == code)
					break;
			} else {
				if (htonl(tbl_v1[i].wakeup_scancode[j]) == code)
					break;
			}
		}
		if (j != MAX_WAKEUP_CODE)
			break;
	}

	if (i != MAX_KEY_TBL)
		return ir->wake_keycode[i][j];
	else
		return 0;
}
...
static int rtk_ir_wakeup_key_handle(struct rtk_ir *ir)
{
	...
	if (soc_device_match(rtk_soc_hank)) {
		keycode = rtk_ir_find_wake_keycode(ir, (val >> 3));
		if (keycode)
			regmap_write(ir->iso, 0x65c, keycode);
		return 0;
	}
			    ...
			    void *unused)
{
	...
	if (soc_device_match(rtk_soc_hank)) {
		keycode = rtk_ir_find_wake_keycode(ir, (val >> 3));
		if (keycode)
			regmap_write(ir->iso, 0x64c, keycode);

		pr_err("[%s] Keycode: 0x%x\n", __func__, keycode);
		input_report_key(rc->input_dev, keycode, 1);
		input_sync(rc->input_dev);
		input_report_key(rc->input_dev, keycode, 0);
		input_sync(rc->input_dev);
	} else {
		dec->scancode(&request, val);
		...
		if (i != map->len && wakeup_event == IR_EVENT){
			pr_err("[IRDA_RESUME][%s] Scan code: 0x%x\n", __func__, request.scancode);
			if (request.scancode != 0x8018) {
				pr_err("[IRDA_RESUME][%s] Give the power key: 0x%x\n", __func__, 0x8018);
			...
			pr_err("[ALARM_RESUME][%s] Check Wi-Fi connection.\n", __func__);
			rc_keydown(rc, map->rc_proto, 0x8077, 0);
...
static int rtk_ir_probe(struct platform_device *pdev)
{
	...
	if (__is_defined(CONFIG_IR_RTK_RAW)) {
