...
typedef void (*kernel_irda_update_t)   (const ktime_t time, unsigned int key_code);
kernel_irda_update_t kernel_irda_update_cb = NULL;
DEFINE_RWLOCK(kernel_irda_update_lock);

void kernel_irda_update(const ktime_t time, unsigned int key_code)
{
	{
		unsigned long flags;
		read_lock_irqsave(&kernel_irda_update_lock, flags);
		if (kernel_irda_update_cb)
			kernel_irda_update_cb(time, key_code);
		read_unlock_irqrestore(&kernel_irda_update_lock, flags);
	}
}

EXPORT_SYMBOL(kernel_irda_update_cb);
EXPORT_SYMBOL(kernel_irda_update_lock);
EXPORT_SYMBOL(kernel_irda_update);
...
void rtk_ir_isr_hw(struct rtk_ir *ir)
{
	...
	request.repeat = 0;
	request.repeat_time = dec->repeat_time;
	...
	kernel_irda_update(ktime_get(), request.scancode);
}
