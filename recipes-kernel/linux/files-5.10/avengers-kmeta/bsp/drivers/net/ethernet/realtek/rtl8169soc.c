			    ...
			    int cmd)
{
	...
	case SIOCDEVPRIVATE_RTLMDNS:
		...
		break;
	default:
		return phy_do_ioctl_running(dev, ifr, cmd);
	}
...
static int rtl_proc_file_register(struct rtl8169_private *tp)
{
	...
	rtk_proc = proc_mkdir_data("eth0", 0555, init_net.proc_net, NULL);

	if (!rtk_proc) {
		dev_err(tp_to_dev(tp), "procfs:create /proc/net/eth0 failed\n");
		return -EPERM;
	}
	...
	if (!tp->dir_dev) {
		...
		if (!dir_dev) {
			dev_err(tp_to_dev(tp), "procfs:create /proc/net/eth0/r8169 failed\n");
	...
	if (!entry) {
		dev_err(tp_to_dev(tp), "procfs:create /proc/net/eth0/r8169/wol_enable failed\n");
		ret |= EPERM;
	}
	...
	if (!entry) {
		dev_err(tp_to_dev(tp), "procfs:create /proc/net/eth0/r8169/pwr_saving failed\n");
		ret |= EPERM;
	}
	...
	if (!entry) {
		dev_err(tp_to_dev(tp), "procfs:create /proc/net/eth0/r8169/mac_reinit failed\n");
		ret |= EPERM;
	}
	...
	if (!entry) {
		dev_err(tp_to_dev(tp), "procfs:create /proc/net/eth0/r8169/phy_reinit failed\n");
		ret |= EPERM;
	}
	...
	if (!entry) {
		dev_err(tp_to_dev(tp), "procfs:create /proc/net/eth0/r8169/eee failed\n");
		ret |= EPERM;
	}
	...
	if (!entry) {
		dev_err(tp_to_dev(tp), "procfs:create /proc/net/eth0/r8169/driver_var failed\n");
		ret |= EPERM;
	}
	...
	if (!entry) {
		dev_err(tp_to_dev(tp), "procfs:create /proc/net/eth0/r8169/ocp failed\n");
		ret |= EPERM;
	}
	...
	if (!entry) {
		dev_err(tp_to_dev(tp), "procfs:create /proc/net/eth0/r8169/eth_phy failed\n");
		ret |= EPERM;
	}
	...
	if (!entry) {
		dev_err(tp_to_dev(tp), "procfs:create /proc/net/eth0/r8169/eth_led failed\n");
		ret |= EPERM;
	}
	...
	if (!entry) {
		dev_err(tp_to_dev(tp), "procfs:create /proc/net/eth0/r8169/ext_regs failed\n");
		ret |= EPERM;
	}
	...
	if (!entry) {
		dev_err(tp_to_dev(tp), "procfs:create /proc/net/eth0/r8169/registers failed\n");
		ret |= EPERM;
	}
	...
	if (!entry) {
		dev_err(tp_to_dev(tp), "procfs:create /proc/net/eth0/r8169/tx_desc failed\n");
		ret |= EPERM;
	}
	...
	if (!entry) {
		dev_err(tp_to_dev(tp), "procfs:create /proc/net/eth0/r8169/rx_desc failed\n");
		ret |= EPERM;
	}
	...
	if (!entry) {
		dev_err(tp_to_dev(tp), "procfs:create /proc/net/eth0/r8169/tally failed\n");
		ret |= EPERM;
	}
	...
	if (!entry) {
		dev_err(tp_to_dev(tp), "procfs:create /proc/net/eth0/r8169/wpd_event failed\n");
		ret |= EPERM;
	}
	...
	if (!entry) {
		dev_err(tp_to_dev(tp), "procfs:create /proc/net/eth0/r8169/wol_packet failed\n");
		ret |= EPERM;
	}
	...
	if (!entry) {
		dev_err(tp_to_dev(tp), "procfs:create /proc/net/eth0/r8169/wake_mask failed\n");
		ret |= EPERM;
	}
	...
	if (!entry) {
		dev_err(tp_to_dev(tp), "procfs:create /proc/net/eth0/r8169/wake_crc failed\n");
		ret |= EPERM;
	}
	...
	if (!entry) {
		dev_err(tp_to_dev(tp), "procfs:create /proc/net/eth0/r8169/wake_idx_en failed\n");
		ret |= EPERM;
	}
	...
	if (!entry) {
		dev_err(tp_to_dev(tp), "procfs:create /proc/net/eth0/r8169/wake_dump failed\n");
		ret |= EPERM;
	}
	...
	if (tp->chip->features & RTL_FEATURE_PAT_WAKE) {
		...
		if (!entry) {
			dev_err(tp_to_dev(tp), "procfs:create /proc/net/eth0/r8169/wake_offset failed\n");
			ret |= EPERM;
		}
		...
		if (!entry) {
			dev_err(tp_to_dev(tp), "procfs:create /proc/net/eth0/r8169/wake_pattern failed\n");
			ret |= EPERM;
		}
	}
	...
	if (tp->chip->features & RTL_FEATURE_STORM_CTRL) {
		...
		if (!entry) {
			dev_err(tp_to_dev(tp), "procfs:create /proc/net/eth0/r8169/storm_ctrl failed\n");
			ret |= EPERM;
		}
	}
...
static void rtd13xx_mdio_init(struct rtl8169_private *tp)
{
	...
	case OUTPUT_RGMII_TO_PHY:
		...
		break;
	default:
		dev_err(d, "%s:%d: unsupport output mode (%d)\n",
...
static int rtl_init_one(struct platform_device *pdev)
{
	...
	netif_napi_add(ndev, &tp->napi, rtl8169_poll, NAPI_POLL_WEIGHT);
