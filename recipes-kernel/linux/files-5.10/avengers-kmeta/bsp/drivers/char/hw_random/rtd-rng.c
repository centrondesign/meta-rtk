...
static int rtd_rng_read(struct hwrng *rng, void *buf, size_t max,
			       bool wait)
{
	struct rtd_rngc *rngc = container_of(rng, struct rtd_rngc, rng);
	void __iomem *rng_base = rngc->rng_base;
...
static int rtd16xxb_rng_init(struct hwrng *rng)
{
	...
	sclk_rate = clk_get_rate(rngc->clk);
...
static int rtd_rng_probe(struct platform_device *pdev)
{
	...
	rngc->rng_base = devm_platform_ioremap_resource(pdev, 0);
	rngc->dev = dev;

        rngc->rng.name = "rtd",
        rngc->rng.read = rtd_rng_read,
        rngc->rng.quality = 1000,
	rngc->rng.init = of_device_get_match_data(&pdev->dev);
	if (!rngc->rng.init) {
...
{
	struct rtd_rngc *rngc = platform_get_drvdata(pdev);
...
{
	struct rtd_rngc *rngc = dev_get_drvdata(dev);
	if (rngc->rng.init)
		rngc->rng.init(&rngc->rng);
	return 0;
}
