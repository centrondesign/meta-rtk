...
__attribute__((unused)) static void dpi_rx_dq_cal_clk_check(struct dpi_rx_dq_cal_priv *priv)
{
...
static void dpi_rx_dq_cal_clk_disable(struct dpi_rx_dq_cal_priv *priv)
{
	...
	dpi_reg_update_bits(dpi, DPI_DLL_DPI_CTRL_0, 0x30, 0x30);					// Disable rx 3 point K
	dpi_reg_update_bits(dpi, DPI_DLL_READ_CTRL_1, 0x40000000, 0x40000000);	 // rx_cal_dis = 1
	// dpi_reg_update_bits(dpi, DPI_DLL_CAL_VREF_CTRL, 0x3f00, 0x0);			// Disable RX Vref auto calibration
	dpi_reg_update_bits(dpi, DPI_RW_READ_CTRL_4(0), 0x3, 0x0);
	dpi_reg_update_bits(dpi, DPI_RW_READ_CTRL_4(1), 0x3, 0x0);
	dpi_reg_update_bits(dpi, DPI_RW_READ_CTRL_4(2), 0x3, 0x0);
	dpi_reg_update_bits(dpi, DPI_RW_READ_CTRL_4(3), 0x3, 0x0);
	// dpi_reg_update_bits(dpi, DPI_DLL_DPI_CTRL_0, 0x3, 0x1);			 // fw_set_mode: during refresh
	dpi_reg_update_bits(dpi, DPI_DLL_DPI_CTRL_1, 0xc, 0xc);			 // fw_set_rd_dly
	msleep(1);
}
...
static void rx_rd_dly_apply(struct dpi_rx_dq_cal_priv *priv)
{
	struct dpi_device *dpi = priv->dpi;
	uint32_t slice, sel, dpi_rw_cal_out_0, dpi_rw_cal_out_1;
	uint32_t TE_P[9], TE_N[9], DELTA_P[9], DELTA_N[9];

	for(slice = 0; slice < 4; ++slice)
	{
		for(sel = 0; sel < 4; ++sel) {
			dpi_reg_update_bits(dpi, DPI_RW_READ_DBG_CTRL(slice), 0xf000, (sel << 12));
			dpi_reg_read(dpi, DPI_RW_CAL_OUT_0(slice), &dpi_rw_cal_out_0);
			dpi_reg_read(dpi, DPI_RW_CAL_OUT_1(slice), &dpi_rw_cal_out_1);
			TE_P[2 * sel] = (dpi_rw_cal_out_0 >> 8) & 0xff;
			TE_P[2 * sel + 1] = (dpi_rw_cal_out_0 >> 24) & 0xff;
			TE_N[2 * sel] = (dpi_rw_cal_out_0 >> 0) & 0xff;
			TE_N[2 * sel + 1] = (dpi_rw_cal_out_0 >> 16) & 0xff;
			DELTA_P[2 * sel] = (dpi_rw_cal_out_1 >> 8) & 0xf;
			DELTA_P[2 * sel + 1] = (dpi_rw_cal_out_1 >> 24) & 0xf;
			DELTA_N[2 * sel] = (dpi_rw_cal_out_1 >> 0) & 0xf;
			DELTA_N[2 * sel + 1] = (dpi_rw_cal_out_1 >> 16) & 0xf;
		}

		dpi_reg_update_bits(dpi, DPI_RW_READ_DBG_CTRL(slice), 0xf000, (0x4 << 12));
		dpi_reg_read(dpi, DPI_RW_CAL_OUT_0(slice), &dpi_rw_cal_out_0);
		dpi_reg_read(dpi, DPI_RW_CAL_OUT_1(slice), &dpi_rw_cal_out_1);
		TE_P[8] = (dpi_rw_cal_out_0 >> 8) & 0xff;
		TE_N[8] = (dpi_rw_cal_out_0 >> 0) & 0xff;
		DELTA_P[8] = (dpi_rw_cal_out_1 >> 8) & 0xf;
		DELTA_N[8] = (dpi_rw_cal_out_1 >> 0) & 0xf;
		...
		dpi_reg_write(dpi, DPI_RW_DQS_IN_DLY_3(slice), (TE_N[7] << 24) | (TE_N[6] << 16) | (TE_N[5] << 8) | (TE_N[4] << 0));
		dpi_reg_write(dpi, DPI_RW_DQS_IN_DLY_1_DBI(slice), TE_P[8]);
		dpi_reg_write(dpi, DPI_RW_DQS_IN_DLY_3_DBI(slice), TE_N[8]);
		dpi_reg_write(dpi, DPI_RW_DQ_DEL_PSEL(slice), (DELTA_P[7] << 28) | (DELTA_P[6] << 24) | (DELTA_P[5] << 20) | (DELTA_P[4] << 16) | (DELTA_P[3] << 12) | (DELTA_P[2] << 8) | (DELTA_P[1] << 4) | (DELTA_P[0] << 0));
		dpi_reg_write(dpi, DPI_RW_DQ_DEL_NSEL(slice), (DELTA_N[7] << 28) | (DELTA_N[6] << 24) | (DELTA_N[5] << 20) | (DELTA_N[4] << 16) | (DELTA_N[3] << 12) | (DELTA_N[2] << 8) | (DELTA_N[1] << 4) | (DELTA_N[0] << 0));
		dpi_reg_write(dpi, DPI_RW_DM_DEL_PNSEL(slice), (DELTA_N[8] << 4) | (DELTA_P[8] << 0));
	}

	// fw_rd_delta_upd = fw_rd_te_upd = 1
	dpi_reg_update_bits(dpi, DPI_RW_READ_CTRL_4(0), 0x3, 0x3);
	dpi_reg_update_bits(dpi, DPI_RW_READ_CTRL_4(1), 0x3, 0x3);
	dpi_reg_update_bits(dpi, DPI_RW_READ_CTRL_4(2), 0x3, 0x3);
	dpi_reg_update_bits(dpi, DPI_RW_READ_CTRL_4(3), 0x3, 0x3);
	dpi_reg_update_bits(dpi, DPI_DLL_DPI_CTRL_1, 0xc, 0xc);			 // fw_set_rd_dly
}

static void dpi_rx_dq_delta_cal_run(struct dpi_rx_dq_cal_priv *priv)
{
	struct dpi_device *dpi = priv->dpi;

	dpi_reg_update_bits(dpi, DPI_DLL_DPI_CTRL_2, 0x0c000000, 0x0);	  // rst_3point_mode = disable reset 3 point

	dpi_reg_update_bits(dpi, DPI_DLL_CAL_MODE_CTRL, BIT(5), 0x0);		// fw_delta_force = 0
	msleep(1);

	dpi_reg_update_bits(dpi, DPI_DLL_CAL_MODE_CTRL, BIT(8), BIT(8));	// cal_offline = 1
	msleep(priv->cal_time);

	dpi_reg_update_bits(dpi, DPI_DLL_DPI_CTRL_0, 0x30, 0x30);	 		// disable 3 point K
	dpi_reg_update_bits(dpi, DPI_DLL_CAL_MODE_CTRL, BIT(5), BIT(5));	// fw_delta_force = 1
	dpi_reg_update_bits(dpi, DPI_DLL_CAL_MODE_CTRL, BIT(8), 0x0);		// cal_offline = 0
	rx_rd_dly_apply(priv);
}
...
static int dpi_rx_dq_cal_cb(struct notifier_block *nb, unsigned long event, void *data)
{
	...
	if(priv->cal_time) {
		...
		if (dpi->quirks->alter_rx_dq_cal_flow)
			dpi_rx_dq_cal_run(priv);
		else
			dpi_rx_dq_delta_cal_run(priv);
...
static int of_dpi_rx_dq_cal_parse_data(struct device_node *np, struct dpi_rx_dq_cal_priv *priv)
{
	...
	if (!of_find_property(np, "cal-time", &len))
		return -EINVAL;

	if (len > sizeof(priv->cal_time))
		return -EINVAL;

	if((ret = of_property_read_u32_array(np, "cal-time", &priv->cal_time, len/4)))
		return ret;
