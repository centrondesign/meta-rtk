...
static int consumer_device_is_cpu(struct device *dev)
{
...
static int regulator_supply_alias_setter_probe(struct platform_device *pdev)
{
	...
	if (consumer_device_is_cpu(dev)) {
		data->supply_name = "cpu";
		data->consumers[0].name = "cpu";
		data->consumers[0].dev = get_cpu_device(0);
	} else {
...
static const struct of_device_id regulator_supply_alias_setter_ids[] = {
	{ .compatible = "cpu-supply-alias-setter", .data = (void *)1, },
