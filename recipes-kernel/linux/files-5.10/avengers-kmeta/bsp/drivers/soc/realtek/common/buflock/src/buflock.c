...
static int rtk_buflock_alloc(struct device *dev, size_t size,
			 struct dma_buf_attachment **attach,
			 ...
			 void **ion_phy, void **ion_virt)
{
	...
	alloc_flags = RTK_FLAG_NONCACHED | RTK_FLAG_SCPUACC | RTK_FLAG_ACPUACC
					 | RTK_FLAG_VCPU_FWACC;
	...
	*ion_phy = (void *)sg_phys((*table)->sgl);
	...
	dev_err(dev, "%s *ion_phy = 0x%p , *ion_virt = 0x%p \n", __func__,
			 (void *)*ion_phy, (void *)*ion_virt);
...
static int __init buflock_module_init (void)
{
	...
	do {
		...
		struct dma_buf_attachment *attach;
		struct sg_table *table;
		size_t ion_size = sizeof(unsigned int) * module->max_handles;

		ret = rtk_buflock_alloc(module->device,
					 ion_size, &attach, &table,
					 (void **)&ion_phyAddr,
					 (void **)&ion_virAddr);
		if (ret)
			break;
		...
		module->dmabuf = attach->dmabuf;
		module->uStatusPhyAddr = (unsigned int) ion_phyAddr;
		module->pStatusMemory = (buflock_status_t *)ion_virAddr;
		break;
...
static void __exit buflock_module_exit(void)
{
    ...
    if (module->dmabuf) {
	dma_buf_end_cpu_access(module->dmabuf, DMA_BIDIRECTIONAL);
	dma_buf_unmap_attachment(module->attach, module->attach->sgt,
				 DMA_BIDIRECTIONAL);
	dma_buf_detach(module->dmabuf, module->attach);
...
static ssize_t buflock_fops_read (struct file *file, char *buf, size_t size, loff_t *f_pos)
{
    ...
    do  {
        ...
        s += sprintf(s, "#%s ion(%p) phy=%08zx max=%zu checkloop_ms=%d destroy_timeout_ms=%d\n", MODULE_NAME,
                module->dmabuf, (long unsigned int)module->uStatusPhyAddr, module->max_handles, module->release_loop_ms, module->destroy_timeout_ms);
