...
#if(1) // trace for SA5-3293, for multi-event in one ISR turn
	{
		int bitSet=0;
		for(i=0; i < 32; i++)
		{	if(event & (0x1<<i)) bitSet++; }
		if(bitSet > 1)
		{
			SC_WARNING("event=0x%lx\n",event); // always dump to uart, maybe this TX/RX is ok.
			SC_TX_INT_DBG(p_this->tx_int_log_fifo, "event=0x%lx\n",event);
		}
	}
#endif

/*  SA5-3293, event SC_RCV_INT handle after TX_xxx event if multi-events happened.
	...
	if (event & SC_RCV_INT && p_this->tx_status != SC_TX_DONE) {
		...
		SC_WARNING("RX Receive without TX done. Reduce dump first! event=0x%lx, no rst!(remain=%d)\n",event, tx_data_len);
		//SET_SCCR(p_this->base, GET_SCCR(p_this->base) | SC_FIFO_RST(1));
	}
#endif
*/
	...
	if (event & SC_TXDONE_INT)  // sending one byte in TXFIFO
	{
		int tx_data_len = kfifo_len(&p_this->tx_fifo);  // get number of data to xmit
		if (tx_data_len) {
			unsigned char tx_buff[TX_RX_DEPTH];
			...
			SC_TX_INT_DBG(p_this->tx_int_log_fifo, "Send %d Bytes! (0x%02x, 0x%02x)\n",
					tx_data_len, tx_buff[0], tx_buff[tx_data_len-1]);
	...
	if (event & SC_RCV_INT && p_this->tx_status == SC_TX_DONE) //RX received at least one character
	{
		if ((len = GET_SC_RXLENR(p_this->base)))
		{
			...
			if (kfifo_in_locked(&p_this->rx_fifo, buff, len, &p_this->rx_fifo_lock)<len)
			{
				SC_TX_INT_DBG(p_this->tx_int_log_fifo, "mars_scd_fsm_active : fifo over flow... flush data...\n");
				kfifo_reset(&p_this->rx_fifo);
			}
		...
		{
			SC_WARNING("Rx over flow!\n");
			SC_TX_INT_DBG(p_this->tx_int_log_fifo, "Rx over flow!\n");
			SET_SCCR(p_this->base, GET_SCCR(p_this->base) | SC_FIFO_RST(1));
		}
	}
	...
	{
		int tx_data_len = kfifo_len(&p_this->tx_fifo); // get number of data to xmit
		SC_WARNING("RX Receive without TX done. event=0x%lx, tx remain=%d, ignore!\n",event, tx_data_len);
		SC_TX_INT_DBG(p_this->tx_int_log_fifo, "RX Receive without TX done. event=0x%lx, tx remain=%d, ignore!\n",event, tx_data_len);
		event = event & ~SC_RCV_INT; // oo>> don't clear event, handle it in next turn......
		//SET_SCCR(p_this->base, GET_SCCR(p_this->base) | SC_FIFO_RST(1));
	}
#endif
...
static int mars_scd_xmit_int_txdone(mars_scd* p_this, unsigned char* p_data, unsigned int len)
{
	...
	SC_TX_INT_DUMP(p_this->tx_int_log_fifo, len, (SC_SUCCESS==ret) ? false : true);
...
static int mars_scd_read_rx_timeout(mars_scd* p_this, unsigned char* data, unsigned int len)
{
	...
	unsigned long cur_jiffies, busywait_jiffies;
...
if(g_oo_hack)	gpio_direction_output(47, 1); // oo hack
	...
	end_jiffies = start_jiffies + ((HZ*p_this->rx_timeout)/1000);
	busywait_jiffies = end_jiffies - SCHEDULE_OUT_JIFFIES;

	do
	{
		cur_jiffies = jiffies;
		//if(cur_jiffies >= end_jiffies)
		if(time_after(cur_jiffies, end_jiffies))
			break; // time's up
		...
		if(time_before(cur_jiffies, busywait_jiffies))
		{
...
if(g_oo_hack) gpio_direction_output(47, 0);
	...
	if(ret <= 0)
		SC_WARNING("jiff(%lu->%lu), ret=%d\n", start_jiffies, end_jiffies, ret);
	...
	**************************************/
	...
	if (p_this->vcc != p_this->pwr_sel_polarity) {
		SC_WARNING("SC%d = default pwr changed to %s\n",
			...
			p_this->vcc_temp = p_this->pwr_sel_polarity;
	}
#endif
