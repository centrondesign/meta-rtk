...
static void dpi_tx_phase_tune(struct dpi_tx_phase_priv *priv, uint32_t *pi_val_q, int state)
{
			   ...
			   void *data)
{
	...
	pr_debug("%s: event = %d, priv->state = %d, new_state = %d\n", __func__, (int)event, priv->state, new_state);
...
static int dpi_tx_phase_of_init(struct device *dev, struct dpi_tx_phase_priv *priv)
{
	...
	if ((ret = dpi_tx_phase_read_reg_values(dev, "state-0-value", &priv->pi_val_q_0, &len)))
		return ret;

	ret = dpi_tx_phase_read_reg_values(dev, "state-1-value", &priv->pi_val_q_1, &len);
