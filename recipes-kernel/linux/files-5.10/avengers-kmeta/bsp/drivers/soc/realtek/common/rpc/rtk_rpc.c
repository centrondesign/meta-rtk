...
static int rpc_debug_node_show(struct seq_file *s, void *unused)
{
	...
	list_for_each_entry(proc, &extra->tasks, list) {
		task = pid_task(find_get_pid(proc->pid), PIDTYPE_PID);
		seq_printf(s, "\nProcess:%s\n", task ? task->comm : "N/A");
...
void rpc_send_interrupt(int type)
{
	...
	case RPC_AUDIO:
		if (rpc_acpu_int_flag != NULL && RPC_HAS_BIT(rpc_acpu_int_flag, AUDIO_RPC_SET_NOTIFY)) {
			if (hwlock)
				hwspin_lock_timeout_irqsave(hwlock, UINT_MAX, &flags);
...
static int rpc_interrupt_init(struct device_node *np)
{
	...
	while ((rpc_get_flag(RPC_AUDIO) == 0xffffffff) && ((max_count--) > 0))
		mdelay(1);

	acpu_rpc_enable = (max_count > 0) ? 1 : 0;
	dev_warn(rpc_dev, "%s (RPC_AUDIO FLAG = 0x%08x)\n",
		(max_count > 0) ? "OK" : "timeout", rpc_get_flag(RPC_AUDIO));
	...
	vcpu_rpc_enable = (max_count > 0) ? 1 : 0;
	...
	if (ve3_clk_enable) {
		...
		ve3_rpc_enable = (max_count > 0) ? 1 : 0;
	...
	hifi_rpc_enable = (max_count > 0) ? 1 : 0;
...
#endif
	lock_id = of_hwspin_lock_get_id(rpc_dev->of_node, 0);
	if (lock_id > 0 || (IS_ENABLED(CONFIG_HWSPINLOCK) && lock_id == 0)) {
		struct hwspinlock *lock = devm_hwspin_lock_request_specific(rpc_dev, lock_id);

		if (lock) {
			dev_info(rpc_dev, "use hwlock%d\n", lock_id);
			hwlock = lock;
		}
	} else {
		if (lock_id != -ENOENT)
			dev_err(rpc_dev, "failed to get hwlock: %pe\n", ERR_PTR(lock_id));
	}
...
{
	writel_relaxed(RPC_INT_WRITE_1, rpc_int_base + RPC_SB2_INT_EN);
	return 0;
}
...
#ifdef CONFIG_REALTEK_RPC_HIFI
	intr_enable = RPC_INT_SA | RPC_INT_SV | RPC_INT_SH;
#else
	intr_enable = RPC_INT_SA | RPC_INT_SV;
#endif
	writel_relaxed(intr_enable | RPC_INT_WRITE_1, rpc_int_base + RPC_SB2_INT_EN);

	return 0;
}

static const struct dev_pm_ops rtk_rpc_pm_ops = {
        SET_SYSTEM_SLEEP_PM_OPS(rtk_rpc_pm_suspend, rtk_rpc_pm_resume)
};
#endif
