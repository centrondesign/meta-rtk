...
static struct rpc_debug_flag_memory * get_debug_flag_memory(struct device *dev)
{
...
static struct rpc_debug_print_memory * get_debug_print_memory(struct device *dev)
{
	struct sg_table *table;
	struct dma_buf_attachment *attach;
...
static struct rpc_debug_flag *get_debug_flag(struct device *dev)
{
...
static struct rpc_debug_print_memory *get_debug_print(struct device *dev)
{
...
int rpc_ctrl_open(struct inode *inode, struct file *filp)
{
	...
	if (minor == 100)
		filp->private_data = rpc_dev;
	else
		/* set dev as private_data would be used afterward */
		filp->private_data = (&rpc_poll_extra[minor/RPC_NR_PAIR])->sdev;
