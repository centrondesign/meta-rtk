                        ...
                        uint32_t flags)
{
    ...
    SC_WARNING("oo>>> in len(to:from)=(%d,%d), f=0x%x(d=%d), (%02x, %02x, %02x, %02x, %02x)\n",
   ...
   //if (direction == HD_ISO_SEND_TYPE) {
    if(flags & SCD_CHILI_SESSION_FLAG_to_card) {
        ...
        *from_card_len = (int)(to_card[4] + 2);
        ...
        // SA5-3395, SW1=0x9X as normally completed.
        //if( ME1_OK == from_card[(*from_card_len)-2] ) {
        if( ME1_OK == (from_card[(*from_card_len)-2] & 0xF0) )  {
