...
#define AUTOREAD_VAL_MASK		GENMASK(16, 0)
#define AUTOREAD_TIME_MASK		GENMASK(31, 0)
#define AUTOREAD_PMU_READ_MASK	GENMASK(16, 1)
#define AUTOREAD_PMU_ADD_MASK		BIT(17)
#define AUTOREAD_INA_SET_READ_TIME	0x492
...
struct autoread_pmu {
	...
	int		num_counters;
	u64		val_counter_mask;
	u64		time_counter_mask;
...
static struct attribute *autoread_pmu_events_attrs[] = {
	AUTOREAD_PMU_EVENT_ATTR(autoread_timestamp,	0),
	AUTOREAD_PMU_EVENT_ATTR(autoread_ina_0,		1),
	AUTOREAD_PMU_EVENT_ATTR(autoread_ina_1,		2),
	AUTOREAD_PMU_EVENT_ATTR(autoread_ina_2,		3),
	AUTOREAD_PMU_EVENT_ATTR(autoread_ina_3,		4),
	AUTOREAD_PMU_EVENT_ATTR(autoread_ina_4,		5),
	AUTOREAD_PMU_EVENT_ATTR(autoread_ina_5,		6),
	AUTOREAD_PMU_EVENT_ATTR(autoread_ina_6,		7),
	AUTOREAD_PMU_EVENT_ATTR(autoread_ina_7,		8),
	AUTOREAD_PMU_EVENT_ATTR(autoread_ina_8,		9),
	AUTOREAD_PMU_EVENT_ATTR(autoread_ina_9,		10),
	AUTOREAD_PMU_EVENT_ATTR(autoread_ina_10,	11),
	AUTOREAD_PMU_EVENT_ATTR(autoread_ina_11,	12),
	AUTOREAD_PMU_EVENT_ATTR(autoread_ina_12,	13),
	AUTOREAD_PMU_EVENT_ATTR(autoread_ina_13,	14),
	AUTOREAD_PMU_EVENT_ATTR(autoread_ina_14,	15),
	AUTOREAD_PMU_EVENT_ATTR(autoread_ina_15,	16),
	NULL,
};
...
static void rtk_autoread_ina_setting(struct rtk_autoread_data *autoread_data)
{
	...
	for (i = 0; i < 16; i++) {
		...
		calibration_value =
			(i != INA_CPU_ID) ? autoread_data->ina_config->calibration_value : 17067;
...
static int autoread_ina_target_setting(struct rtk_autoread_data *autoread_data)
{
	int i, ret;
	unsigned char val[2];
	unsigned int device_id;
...
static int autoread_setting(struct rtk_autoread_data *autoread_data)
{
	...
	ret = autoread_ina_target_setting(autoread_data);
	if (ret < 0)
		return ret;
			      ...
			      const char *buf, size_t count)
{
	...
	if (auto_en == 1) {
		pr_info("Start to run autoread\n");

		ret = autoread_setting(autoread_data);
		if (ret) {
			pr_err("autoread_setting failed with error code %d\n", ret);
			return ret;
		}

		AUTOREAD_WRITEL(0x3, DMA_CTRL);
	} else if (auto_en == 0) {
		pr_info("Stop to run autoread\n");
				   ...
				   struct device *dev)
{
	...
	if (autoread_data->sysdbg_addr)
		iounmap(autoread_data->sysdbg_addr);
...
static uint32_t __transform_raw_data(struct autoread_pmu *pmu, uint32_t val, int idx)
{
	struct rtk_autoread_data *autoread_data = pmu->autoread_data;
	const struct ina2xx_config *ina_config = autoread_data->ina_config;
	uint64_t tmp_val = (uint64_t)ina_config->bus_voltage_lsb * val;
	int power_lsb;
	int current_lsb;

	switch (ina_target) {
	case INA2XX_SHUNT_VOLTAGE:
		tmp_val = val * ina_config->shunt_voltage_lsb;
		val = (uint32_t)(tmp_val >> 10);
		break;
	case INA2XX_BUS_VOLTAGE:
		tmp_val = val * ina_config->bus_voltage_lsb;
		val = (uint32_t)(tmp_val >> 10);
		break;
	case INA2XX_POWER:
		current_lsb = ((idx - 1) != INA_CPU_ID) ? ina_config->current_lsb : 500;
		power_lsb = ina_config->power_lsb * current_lsb;
		tmp_val = val * power_lsb;
		val = (uint32_t)(tmp_val >> 10);
		break;
	case INA2XX_CURRENT:
		current_lsb = ((idx - 1) != INA_CPU_ID) ? ina_config->current_lsb : 300;
		tmp_val = val * current_lsb;
		val = (uint32_t)(tmp_val >> 10);
		break;
	}
...
static uint32_t __read_dma_data(struct autoread_pmu *pmu, int idx)
{
	struct rtk_autoread_data *autoread_data = pmu->autoread_data;
	void *current_read_ptr = pmu->current_read_ptr;
	void *tmp_read_ptr;
	void *next_set_ptr;
	uint32_t val, sysdbg_val;
	...
	if(pmu->read_bitmap & AUTOREAD_PMU_ADD_MASK ||
		((pmu->read_bitmap & AUTOREAD_PMU_READ_MASK) == AUTOREAD_PMU_READ_MASK)) {
		while (max_iterations--) {
			...
			if ((pmu->read_bitmap & AUTOREAD_PMU_READ_MASK) == AUTOREAD_PMU_READ_MASK)
				break;
		}

		sysdbg_val = READ_REG32(autoread_data->sysdbg_addr);
		val = *(uint32_t *)tmp_read_ptr;
		if (sysdbg_val - val > AUTOREAD_INA_SET_READ_TIME)
			current_read_ptr = tmp_read_ptr;
		else
			current_read_ptr = autoread_data->dma_buff_0
				+ ((tmp_read_ptr - autoread_data->dma_buff_0)
				- SET_SIZE + dma_total_size) % dma_total_size;
...
static uint32_t autoread_pmu_read_counter(struct autoread_pmu *pmu, int idx)
{
	uint32_t val = 0;
...
static void autoread_pmu_event_update(struct perf_event *event)
{
	...
	mask = (hwc->idx != 0) ? pmu->val_counter_mask : pmu->time_counter_mask;
	delta = now & mask;
...
static int __autoread_pmu_init(struct autoread_pmu *pmu)
{
	...
	pmu->val_counter_mask = AUTOREAD_VAL_MASK;
	pmu->time_counter_mask = AUTOREAD_TIME_MASK;

	/* set start performance trace bit */
	pmu->name = "autoread_pmu";
	pmu->cpuhp_name = "perf/autoread_pmu:online";
...
static int rtk_autoread_probe(struct platform_device *pdev)
{
	...
	autoread_data->sysdbg_addr = ioremap(SYSDBG_MAP_BASE, 0x4);
	if (!(autoread_data->sysdbg_addr)) {
		dev_err(dev,  "[Autoread] Fail to ioremap SYSDBG\n");
		goto err_cleanup;
	}
