...
ssize_t r_program_write(int opt, RPC_DEV_EXTRA *extra, RPC_DEV *dev, char *buf, size_t count)
{
	...
	pr_debug("%s: program:%u version:%u procedure:%u taskID:%u sysTID:%u sysPID:%u size:%u context:%x 90k:%u %s\n",
			...
			ntohl(rpc->parameterSize),ntohl(rpc->mycontext), (u32)refclk_get_val_raw(), in_atomic() ? "atomic" : "");
...
{
	struct dma_buf *dmabuf = release_entry->rpc_dmabuf;
	struct dma_buf_attachment *attach = release_entry->attachment;

	BUG_ON(!dmabuf);
	BUG_ON(!attach);

	dma_buf_vunmap(dmabuf, release_entry->vaddr);
...
retry:
		...
		pr_info("%s alloc_size=0x%x  ion_alloc_flags=0x%x \n", __func__,
				(unsigned int)alloc_size, (unsigned int)ion_alloc_flags);
...
void add_release_list(int pid, int serialno)
{
	...
	list_for_each(listptr, &release_proc_lists.list) {
		...
		if (tmp_entry->pid == pid && tmp_entry->type == type){
	...
	proc_entry->pid = pid;
	proc_entry->type = type;
	if (serialno == 2 || serialno == 3)
		proc_entry->cnt = 2;
	else
		proc_entry->cnt = 4;
...
void rm_release_list(int pid, int serialno)
{
	struct list_head *listptr;
...
int check_dead_process(int pid, int serialno){
...
void rpc_dispatch(unsigned long data)
{
	...
	if (curr != NULL || *ringOut == *ringIn || *ringOut != nextRpc) {
		...
		if (curr != NULL && check_dead_process(curr->pid, extra->serialno) == 0) {
	...
	case HIFI_AGENT:
		...
		if (proc == NULL || check_dead_process(taskID, extra->serialno) == 0) {
	...
	case REPLYID:
		if (rpc.versionID == REPLYID &&
			...
			proc = lookup_by_taskID(extra, taskID);
			if (proc == NULL || check_dead_process(proc->pid, extra->serialno) == 0) {
	...
	default:
		if (in_atomic() && __ratelimit(&ring_dump_state)) {
			pr_err("%s:%d invalid programID:%u!!! Out:%p next:%x In:%p\n",
					__func__, __LINE__, rpc.programID, (void *)ringOut, nextRpc,
					(void *)ringIn);
...
int update_rpc_release_lists(struct device *dev, int type)
{
	...
	release_entry->rpc_dmabuf = dmabuf;
	attach = dma_buf_attach(release_entry->rpc_dmabuf, dev);
	if (IS_ERR(attach)) {
		dev_err(dev, "Failed to attach dmabuf\n");
		ret = PTR_ERR(attach);
		goto attach_err;
	}
	release_entry->attachment = attach;
	table = dma_buf_map_attachment(attach, DMA_BIDIRECTIONAL);
	if (IS_ERR(table)) {
		dev_err(dev, "Failed to map attachment \n");
		ret = PTR_ERR(table);
	...
	release_entry->paddr = sg_phys(table->sgl);
	ret = dma_buf_begin_cpu_access(release_entry->rpc_dmabuf,
				 DMA_BIDIRECTIONAL);
	if (ret)
		goto access_err;
...
kmap_err:
	dma_buf_end_cpu_access(release_entry->rpc_dmabuf, DMA_BIDIRECTIONAL);
access_err:
	dma_buf_unmap_attachment(attach, table, DMA_BIDIRECTIONAL);
map_err:
	dma_buf_detach(release_entry->rpc_dmabuf, attach);
attach_err:
...
int rpc_intr_open(struct inode *inode, struct file *filp)
{
	...
	if (!filp->private_data) {
		...
		if (rpc_type == 1)
			ret = update_rpc_release_lists(proc->extra->sdev,
							RPC_AUDIO);
		else if (rpc_type == 3)
			ret = update_rpc_release_lists(proc->extra->sdev,
							RPC_VIDEO);
...
#ifdef CONFIG_REALTEK_RPC_HIFI
int RPC_DESTROY_HIFI_FLOW(int pid, phys_addr_t paddr, void *vaddr)
{
	struct AUDIO_RPC_PRIVATEINFO_PARAMETERS *cmd = NULL;
	struct AUDIO_RPC_PRIVATEINFO_RETURNVAL *res;
	uint32_t RPC_ret;
...
static int rpc_intr_flush(struct file *filp, fl_owner_t id)
{
	...
	add_release_list(proc->pid, extra->serialno);
	...
	if (extra->currProc == proc) {
		...
		if (minor == 3 || minor == 7 || minor == 9 || minor == 11) {
			if (!rpc_done(extra)) {
	...
	if (rpc_intr_is_paused) {
		...
		task = pid_task(find_pid_ns(proc->pid, &init_pid_ns), PIDTYPE_PID);
		if (task != NULL && (task->flags & PF_SIGNALED))
			task = NULL;

		if (extra->serialno & BIT(0)) {
			spin_lock_bh(&rpc_release_lock);
			if (rpc_type == 1)
			...
			if (rpc_release_entry != NULL && task == NULL) {
				...
				if(!strcmp(extra->name, "AudioIntrRead")) {
					...
					RPC_DESTROY_HIFI_FLOW(proc->pid, rpc_release_entry->paddr, rpc_release_entry->vaddr);
				}
#endif
			}
	...
	rm_release_list(proc->pid, extra->serialno);
		...
		loff_t *f_pos)
{
	...
	if (access_ok(VERIFY_READ, buf, sizeof(RPC_STRUCT))) {
		...
		if (programID == AUDIO_SYSTEM || programID == REPLYID || programID == 97) {
			rpc->versionID = htonl((u32)refclk_get_val_raw());
		} else {
