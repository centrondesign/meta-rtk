...
static int s_common_memory_mmap_kernel_buffer = 0;
...
static int vpu_alloc_dma_buffer(vpudrv_buffer_t *vb)
{
	...
	ret = pu_alloc_dma_buffer(s_vpu_dev.this_device, vb->size, &vb->phys_addr, &vb->base, vb->mem_type);
	if (ret == -1) {
		pr_err("%s [%d]%s.Physical memory allocation error size=%d\n",DEV_NAME,__LINE__,__func__,vb->size);
		return -1;
	}
#else
	UNUSED_PARAMETER(ret);
...
static irqreturn_t ve1_irq_handler(int irq, void *dev_id)
{
	...
	if (vpu_int_sts_ve1) {
		...
		//if ((interrupt_reason_ve1 != 0x8) && (interrupt_reason_ve1 != 0x4000))
		//{
		//	len = kfifo_len(&s_interrupt_pending_q_ve1);
		//	pr_info("%s %d.reason:0x%lx.kfifo_len:%d\n",DEV_NAME,__LINE__,interrupt_reason_ve1,len);
		//}
	}
	...
	if (vpu_int_sts_ve1) {
		if (core == 0) {
			...
			if (!kfifo_is_full(&s_interrupt_pending_q_ve1)) {
				kfifo_in(&s_interrupt_pending_q_ve1, &interrupt_reason_ve1, sizeof(unsigned long));
			}
			else {
				pr_err("%s %d.kfifo_is_full kfifo_count=%d.reason:0x%lx\n",DEV_NAME,__LINE__,kfifo_len(&s_interrupt_pending_q_ve1),interrupt_reason_ve1);
			}
...
static long vpu_ioctl(struct file *filp, u_int cmd, u_long arg)
{
	...
	case VDI_IOCTL_WAIT_INTERRUPT:
	{
		...
		if (info.core_idx == 0) {
			...
			intr_reason_in_q = 0;
			interrupt_flag_in_q = kfifo_out(&s_interrupt_pending_q_ve1, &intr_reason_in_q, sizeof(unsigned long));
			...
			smp_rmb();
#ifdef USE_HRTIMEOUT_INSTEAD_OF_TIMEOUT
			ret = wait_event_interruptible_hrtimeout(s_interrupt_wait_q_ve1, atomic_read(&s_interrupt_flag_ve1) != 0, ktime);
			if (ret) {
				ret = -ETIME;
				//pr_info("%s %d.timeout\n",DEV_NAME,__LINE__);
				break;
...
INTERRUPT_REMAIN_IN_QUEUE:
			//if ((dev->interrupt_reason_ve1 != 0x8) && (dev->interrupt_reason_ve1 != 0x4000))
			//{
			//	pr_info("%s %d.dev->interrupt_reason_ve1:0x%lx\n",DEV_NAME,__LINE__,dev->interrupt_reason_ve1);
			//}
	...
	case VDI_IOCTL_GET_INSTANCE_POOL:
	{
		...
		if (ret == 0) {
			if (s_instance_pool.base != 0) {
				DPRINTK("%s [%d]s_instance_pool(base:0x%lx,virt:0x%lx,phys:0x%lx,size:%d).\n", DEV_NAME, __LINE__,s_instance_pool.base,s_instance_pool.virt_addr,s_instance_pool.phys_addr,s_instance_pool.size);
	...
	case VDI_IOCTL_GET_COMMON_MEMORY:
	{
		...
		if (s_common_memory.base != 0) {
			DPRINTK("%s [%d]s_common_memory(base:0x%lx,virt:0x%lx,phys:0x%lx,size:%d)\n",DEV_NAME,__LINE__,s_common_memory.base,s_common_memory.virt_addr,s_common_memory.phys_addr,s_common_memory.size);
...
int vdi_ioctl_get_instance_pool(vpudrv_buffer_t *vdb)
{
	int ret = 0;
	DPRINTK("%s [+] [%d]%s\n",DEV_NAME,__LINE__,__func__);
...
EXPORT_SYMBOL(vdi_ioctl_get_instance_pool);

int vdi_ioctl_get_register_info(vpudrv_buffer_t *vdb)
{
	DPRINTK("%s [+] [%d]%s\n",DEV_NAME,__LINE__,__func__);
...
EXPORT_SYMBOL(vdi_ioctl_get_register_info);

int vdi_ioctl_set_rtk_clk_gating(vpu_clock_info_t* clockInfo)
{
	DPRINTK("%s [+] [%d]%s\n",DEV_NAME,__LINE__,__func__);
	if (clockInfo == NULL)
	{
...
EXPORT_SYMBOL(vdi_ioctl_set_rtk_clk_gating);

int vdi_ioctl_get_common_memory(vpudrv_buffer_t *vdb)
{
	int ret = 0;
	DPRINTK("%s [+] [%d]%s\n",DEV_NAME,__LINE__,__func__);
	...
	if (s_common_memory.base != 0)
	{
		...
		if ((s_common_memory_mmap_kernel_buffer == 0) && (s_common_memory.virt_addr == 0))
		{
			s_common_memory.virt_addr = pu_mmap_kernel_buffer(s_common_memory.phys_addr, s_common_memory.size);
			if (s_common_memory.virt_addr == 0)
			{
				pr_err("%s [%d]%s.pu_mmap_kernel_buffer() fail.\n",DEV_NAME,__LINE__,__func__);
				return -EFAULT;
			}
			DPRINTK("%s [%d]%s.pu_mmap_kernel_buffer() s_common_memory(virt:0x%lx)\n",DEV_NAME,__LINE__,__func__,s_common_memory.virt_addr);
			s_common_memory_mmap_kernel_buffer = 1;
		}
		*vdb = s_common_memory;
	} else {
		s_common_memory = *vdb;
		...
		if (vpu_alloc_dma_buffer(&s_common_memory) != -1)
		{
			...
			if (s_common_memory.virt_addr == 0)
			{
				...
				memset(&s_common_memory, 0, sizeof(vpudrv_buffer_t));
				pr_err("%s [%d]pu_mmap_kernel_buffer() fail.\n",DEV_NAME,__LINE__);
				return -EFAULT;
			}
			DPRINTK("%s [%d]%s.pu_mmap_kernel_buffer() s_common_memory(virt:0x%lx)\n",DEV_NAME,__LINE__,__func__,s_common_memory.virt_addr);
			s_common_memory_mmap_kernel_buffer = 1;
...
EXPORT_SYMBOL(vdi_ioctl_get_common_memory);

ssize_t vdi_write_bit_firmware(vpu_bit_firmware_info_t *buf, size_t len)
{
	DPRINTK("%s [+] [%d]%s\n",DEV_NAME,__LINE__,__func__);
...
EXPORT_SYMBOL(vdi_write_bit_firmware);

int vdi_ioctl_allocate_physical_memory(void *filp, vpudrv_buffer_t *vdb)
{
	int ret = 0;
	vpudrv_buffer_pool_t *vbp;
	...
	if (ret == 0) {
		...
		vbp->vb.virt_addr = pu_mmap_kernel_buffer(vbp->vb.phys_addr, vbp->vb.size);
		if (vbp->vb.virt_addr == 0)
		{
		...
		*vdb = vbp->vb;
		...
		vbp->filp = (struct file *)filp;
		spin_lock(&s_vpu_lock);
		list_add(&vbp->list, &s_vbp_head);
		DPRINTK("%s [%d]%s.list_add vbp->list:0x%lx\n",DEV_NAME,__LINE__,__func__,&vbp->list);
		spin_unlock(&s_vpu_lock);
...
EXPORT_SYMBOL(vdi_ioctl_allocate_physical_memory);

int vdi_ioctl_free_physical_memory(vpudrv_buffer_t *vdb)
{
	int ret = 0;
	vpudrv_buffer_pool_t *vbp, *n;
	...
	if (ret == 0)
	{
		if (vdb->base)
		{
			pu_unmap_kernel_buffer(vdb->virt_addr, vdb->phys_addr);
		...
		list_for_each_entry_safe(vbp, n, &s_vbp_head, list)
		{
			...
			{
				list_del(&vbp->list);
				DPRINTK("%s [%d]%s.list_del vbp->list:0x%lx\n",DEV_NAME,__LINE__,__func__,&vbp->list);
				kfree(vbp);
				break;
			}
		}
...
EXPORT_SYMBOL(vdi_ioctl_free_physical_memory);

int vdi_ioctl_allocate_physical_memory_no_mmap(void *filp, vpudrv_buffer_t *vdb)
{
	int ret = 0;
	vpudrv_buffer_pool_t *vbp;
...
EXPORT_SYMBOL(vdi_ioctl_allocate_physical_memory_no_mmap);

int vdi_ioctl_free_physical_memory_no_mmap(vpudrv_buffer_t *vdb)
{
	int ret = 0;
	vpudrv_buffer_pool_t *vbp, *n;
	...
	if (ret == 0)
	{
		if (vdb->base)
		{
			DPRINTK("%s [%d]%s.vdb(base:0x%lx,virt:0x%lx,phys:0x%lx,size:%d)\n",DEV_NAME,__LINE__,__func__,vdb->base,vdb->virt_addr,vdb->phys_addr,vdb->size);
			vpu_free_dma_buffer(vdb);
		}
...
EXPORT_SYMBOL(vdi_ioctl_free_physical_memory_no_mmap);

int vdi_ioctl_open_instance(void *filp, vpudrv_inst_info_t *inst_info)
{
	int ret = 0;
	vpudrv_instanace_list_t *vil, *n;
...
EXPORT_SYMBOL(vdi_ioctl_open_instance);

int vdi_ioctl_close_instance(vpudrv_inst_info_t *inst_info)
{
	int ret = 0;
	vpudrv_instanace_list_t *vil, *n;
...
EXPORT_SYMBOL(vdi_ioctl_close_instance);

int vdi_ioctl_wait_interrupt(vpudrv_intr_info_t *intr_info)
{
	int ret = 0;
	...
	/* VE1 */
	if (intr_info->core_idx == 0)
	{
		smp_rmb();
		ret = wait_event_interruptible_timeout(s_interrupt_wait_q_ve1, atomic_read(&s_interrupt_flag_ve1) != 0, msecs_to_jiffies(intr_info->timeout));
		if (!ret)
		{
			pr_err("%s [%d]%s.wait_event_interruptible_timeout() fail.ret:%d\n",DEV_NAME,__LINE__,__func__,ret);
			ret = -ETIME;
			return ret;
		}
		...
		if (signal_pending(current))
		{
			pr_err("%s [%d]%s.signal_pending() fail\n",DEV_NAME,__LINE__,__func__);
			ret = -ERESTARTSYS;
			return ret;
		}

		//DPRINTK("[VPUDRV] s_interrupt_flag_ve1(%d), reason(0x%08lx)\n", atomic_read(&s_interrupt_flag_ve1), dev->interrupt_reason_ve1);
		//DPRINTK("%s [%d]s_interrupt_flag_ve1:%d.reason:0x%08lx\n",DEV_NAME,__LINE__,atomic_read(&s_interrupt_flag_ve1),dev->interrupt_reason_ve1);
		atomic_set(&s_interrupt_flag_ve1, 0);
		intr_info->intr_reason = dev->interrupt_reason_ve1;
		dev->interrupt_reason_ve1 = 0;
		ret = 0;
	}

	//DPRINTK("%s [-] [%d]%s.intr_reason:0x%x\n",DEV_NAME,__LINE__,__func__,intr_info->intr_reason);
	return ret;
}
EXPORT_SYMBOL(vdi_ioctl_wait_interrupt);
...
static int vpu_probe(struct platform_device *pdev)
{
	...
	ret = kfifo_alloc(&s_interrupt_pending_q_ve1, MAX_INTERRUPT_QUEUE*sizeof(unsigned long), GFP_KERNEL);
	if (ret) {
		pr_err("%s %d.kfifo_alloc failed 0x%x\n",DEV_NAME,__LINE__,ret);
	}
	...
	s_ve1_irq = irq;
	pr_info("%s %d.s_ve1_irq:%d want to register ve1_irq_handler\n",DEV_NAME,__LINE__,s_ve1_irq);
...
static int vpu_remove(struct platform_device *pdev)
{
	...
	if (s_common_memory.base) {
		if (s_common_memory_mmap_kernel_buffer == 1)
		{
			DPRINTK("%s [%d]pu_unmap_kernel_buffer() s_common_memory(base:0x%lx,virt:0x%lx,phys:0x%lx,size:%d)\n",DEV_NAME,__LINE__,s_common_memory.base,s_common_memory.virt_addr,s_common_memory.phys_addr,s_common_memory.size);
			pu_unmap_kernel_buffer(s_common_memory.virt_addr, s_common_memory.phys_addr);
			s_common_memory.virt_addr = 0;
			s_common_memory_mmap_kernel_buffer = 0;
		}
		vpu_free_dma_buffer(&s_common_memory);
		s_common_memory.base = 0;
	}
