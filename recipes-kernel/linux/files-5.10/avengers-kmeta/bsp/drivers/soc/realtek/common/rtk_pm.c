...
static int rtk_pm_suspend(struct device *dev)
{
	...
	dev_pm->device_param->dev_pm_triggered = SUSPEND_FLAG;

	return 0;
}

static int rtk_pm_suspend_noirq(struct device *dev)
{
	struct pm_private *dev_pm = dev_get_drvdata(dev);
	unsigned int dco_mode = dev_pm->dco_mode;
...
static int rtk_pm_resume_noirq(struct device *dev)
{
	struct pm_private *dev_pm = dev_get_drvdata(dev);
	unsigned int dco_mode = dev_pm->dco_mode;
	int ret = 0;
...
static const struct dev_pm_ops rtk_pm_ops = {
	...
	.suspend = rtk_pm_suspend,
	.suspend_noirq = rtk_pm_suspend_noirq,
	.resume_noirq = rtk_pm_resume_noirq,
};
