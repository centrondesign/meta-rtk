...
int rtk_pu_alloc(struct device *dev, size_t size,
		 struct dma_buf_attachment **attach,  struct sg_table **table,
		 void **ion_phy, unsigned int alloc_flags)
{
	int ret;
...
EXPORT_SYMBOL(rtk_pu_alloc);

int pu_alloc_dma_buffer(struct device *dev, unsigned int size,
			 unsigned long *phys_addr, unsigned long *base,
			 unsigned int mem_type)
{
    ...
    do {
        ...
        int flags = (RTK_FLAG_SCPUACC | RTK_FLAG_HWIPACC | RTK_FLAG_NONCACHED);
        ...
        switch(mem_type)
        {
            case VE_SECURE_NORMAL:
                {
                    ...
                    flags = (RTK_FLAG_SCPUACC | RTK_FLAG_HWIPACC | RTK_FLAG_NONCACHED | RTK_FLAG_VE_SPEC);
                    break;
                }
            ...
            default:
                {
                    heap = RTK_ION_HEAP_MEDIA_MASK;
                    flags = (RTK_FLAG_SCPUACC | RTK_FLAG_HWIPACC | RTK_FLAG_NONCACHED);
                    break;
                }
        }

        error = rtk_pu_alloc(dev, size, &attach, &table, (void **)&dat, flags);
        if (error != 0)
        {
            printk(KERN_ERR "[PUWRAP] ion_alloc allocation error"
                    " size=%d\n", size);
            break;
        }

        *base = (unsigned long)attach->dmabuf; // we didn't need base address on user space, but we also need it has value to do detection
        ...
        *phys_addr = (unsigned long)dat;
        ...
        pu_ion_buffer[i].attach = attach;
        pu_ion_buffer[i].dmabuf = (unsigned long)attach->dmabuf;
        pu_ion_buffer[i].table = table;
...
void pu_free_dma_buffer(unsigned long base, unsigned long phys_addr)
{
    ...
    do {
        ...
        attach = pu_ion_buffer[i].attach;
        dmabuf = (struct dma_buf *)pu_ion_buffer[i].dmabuf;
        table = pu_ion_buffer[i].table;
        if (dmabuf != NULL)
        {
