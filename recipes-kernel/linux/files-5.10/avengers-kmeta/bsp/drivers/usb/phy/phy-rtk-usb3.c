...
do_toggle:
	...
	if (phy_data->check_rx_front_end_offset) {
		...
		if (((rx_offset_range & REG_0x0B_RX_OFFSET_RANGE_MASK) ==
			...
			dev_warn(phy->dev, "Don't update rx_offset_range (rx_offset_code=0x%x, rx_offset_range=0x%x)\n",
...
static int rtk_usb3phy_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < phyN; i++) {
		...
		if (of_property_read_bool(sub_node, "realtek,check_rx_front_end_offset"))
