...
struct dma_buf_attachment *rtk_compress_rpc_attach = NULL;
struct dma_buf_attachment *compr_inRing_rpc_attach = NULL;
struct dma_buf_attachment *compr_inband_rpc_attach = NULL;
struct dma_buf_attachment *compr_dwnstrm_rpc_attach = NULL;
struct dma_buf_attachment *compr_outRing_rpc_attach[8] = {NULL};
struct dma_buf_attachment *refclock_rpc_attach = NULL;

struct dma_buf_attachment *rawdelay_handle_rpc_attach = NULL;
struct dma_buf_attachment *rawdelay_handle2_rpc_attach = NULL;
struct dma_buf_attachment *rawdelay_handle3_rpc_attach = NULL;
...
struct rtk_runtime_stream {
	...
	void *vaddr_rpc;
	struct dma_buf_attachment *rpc_attach_rpc;
};
...
static void rtk_compr_rpc_free(struct dma_buf_attachment *attach)
{
	struct dma_buf *dmabuf;

	pr_err("%s free dmabuf\n", __func__);
...
static int rtk_dma_buf_get(struct device *dev, struct dma_buf *dmabuf,
...

{
	...
	*ion_phy = (void *)sg_phys((*table)->sgl);
	...
	dev_err(dev, "%s *ion_phy = 0x%x , *ion_virt = 0x%x \n", __func__,
		 *ion_phy, *ion_virt);
...
static int rtk_compr_rpc_alloc(struct device *dev, size_t size,
			 struct dma_buf_attachment **attach,
			 struct sg_table **table,
			 void **ion_phy, void **ion_virt)
{
	...
	ret = rtk_dma_buf_get(dev, dmabuf, attach, table, ion_phy, ion_virt);
	if (ret) {
		dev_err(dev, "Failed to rtk_dma_buf_get\n");
		goto rheap_err;
	}
...
int createAudioComponent(struct device *dev, struct rtk_runtime_stream *stream)
{
...
int configOutput(struct device *dev, struct rtk_runtime_stream *stream)
{
	//config i2s
...
void destroyRingBuf(struct rtk_runtime_stream *stream)
{
	...
	if (refclock_rpc_attach && refclock_from_video == 0) {
		dma_buf_vunmap(refclock_rpc_attach->dmabuf,
				refclock_rpc_attach->dmabuf->vmap_ptr);
		rtk_compr_rpc_free(refclock_rpc_attach);
		refclock_rpc_attach = NULL;
	}

	if (compr_inRing_rpc_attach) {
		dma_buf_vunmap(compr_inRing_rpc_attach->dmabuf,
				compr_inRing_rpc_attach->dmabuf->vmap_ptr);
		rtk_compr_rpc_free(compr_inRing_rpc_attach);
		compr_inRing_rpc_attach = NULL;
	}

	if (compr_inband_rpc_attach) {
		dma_buf_vunmap(compr_inband_rpc_attach->dmabuf,
				compr_inRing_rpc_attach->dmabuf->vmap_ptr);
		rtk_compr_rpc_free(compr_inband_rpc_attach);
		compr_inband_rpc_attach = NULL;
	}

	if (compr_dwnstrm_rpc_attach) {
		dma_buf_vunmap(compr_dwnstrm_rpc_attach->dmabuf,
				compr_dwnstrm_rpc_attach->dmabuf->vmap_ptr);
		rtk_compr_rpc_free(compr_dwnstrm_rpc_attach);
		compr_dwnstrm_rpc_attach = NULL;
	}

	if (compr_outRing_rpc_attach[0]) {
		for (ch = 0; ch < AUDIO_DEC_OUTPIN; ch++) {
			dma_buf_vunmap(compr_outRing_rpc_attach[ch]->dmabuf,
				compr_outRing_rpc_attach[ch]->dmabuf->vmap_ptr);
			rtk_compr_rpc_free(compr_outRing_rpc_attach[ch]);
			compr_outRing_rpc_attach[ch] = NULL;
		}
	}
}

int createRingBuf(struct device *dev, struct rtk_runtime_stream *stream,
		 unsigned int buffer_size)
{
	...
	ret = rtk_compr_rpc_alloc(dev, buffer_size,
	...
	compr_inRing_rpc_attach = attach;
	stream->virDecInRing = vaddr;
	...
	ret = rtk_compr_rpc_alloc(dev, DEC_INBAND_BUFFER_SIZE,
				 &attach, &table, (void **)&dat,
				 (void **)&vaddr);
	...
	compr_inband_rpc_attach = attach;
	stream->virInbandRing = vaddr;
	...
	ret = rtk_compr_rpc_alloc(dev, DEC_DWNSTRM_BUFFER_SIZE,
				 &attach, &table, (void **)&dat,
				 (void **)&vaddr);
	if (ret) {
		pr_err("[%s %d alloc fail]\n", __func__, __LINE__);
	...
	compr_dwnstrm_rpc_attach = attach;
	stream->virDwnRing = vaddr;
	...
	for (ch = 0; ch < AUDIO_DEC_OUTPIN; ch++) {
		ret = rtk_compr_rpc_alloc(dev, decOutSize,
				 &attach, &table, (void **)&dat,
				 (void **)&vaddr);
		if (ret) {
			pr_err("[%s %d alloc fail]\n", __func__, __LINE__);
		...
		compr_outRing_rpc_attach[ch] = attach;
		stream->virDecOutRing[ch] = vaddr;
...
static int snd_card_compr_open(struct snd_compr_stream *cstream)
{
	...
	struct device *dev = &cstream->device->dev;
	...
	ret = rtk_compr_rpc_alloc(dev,
				 sizeof(struct rtk_runtime_stream),
				 &attach, &table, (void **)&dat,
				 (void **)&vaddr);
	if (ret) {
		pr_err("[%s %d alloc fail]\n", __func__, __LINE__);
	...
	rtk_compress_rpc_attach = attach;
	stream = (struct rtk_runtime_stream *)vaddr;
	...
	stream->rpc_attach_rpc = attach;
	stream->phy_addr_rpc = dat;
	stream->vaddr_rpc = vaddr;

	if (createAudioComponent(dev, stream)) {
	...
	if (rawdelay_mem == NULL) {
		...
		rawdelay_handle_rpc_attach = attach;
		phy_rawdelay = dat;
		rawdelay_mem = vaddr ;
	...
	if (rawdelay_mem2 == NULL) {
		...
		rawdelay_handle2_rpc_attach = attach;
		phy_rawdelay2 = dat;
		rawdelay_mem2 = vaddr;
	...
	if (rawdelay_mem3 == NULL) {
		ret = rtk_compr_rpc_alloc(dev,
					 ...
					 (void **)&vaddr);
		if (ret) {
			pr_err("[%s %d_alloc fail]\n", __func__, __LINE__);
			goto fail;
		}

		rawdelay_handle3_rpc_attach = attach;
		phy_rawdelay3 = dat;
		rawdelay_mem3 = vaddr;
	...
	return 0;
fail:
	if (rawdelay_handle_rpc_attach) {
		dma_buf_vunmap(rawdelay_handle_rpc_attach->dmabuf,
				 rawdelay_mem);
		rtk_compr_rpc_free(rawdelay_handle_rpc_attach);
		rawdelay_mem = NULL;
		rawdelay_handle_rpc_attach = NULL;
	}

	if (rawdelay_handle2_rpc_attach) {
		dma_buf_vunmap(rawdelay_handle2_rpc_attach->dmabuf,
				 rawdelay_mem2);
		rtk_compr_rpc_free(rawdelay_handle2_rpc_attach);
		rawdelay_mem2 = NULL;
		rawdelay_handle2_rpc_attach = NULL;
	}

	if (rawdelay_handle3_rpc_attach) {
		dma_buf_vunmap(rawdelay_handle3_rpc_attach->dmabuf,
				 rawdelay_mem3);
		rtk_compr_rpc_free(rawdelay_handle3_rpc_attach);
		rawdelay_mem3 = NULL;
		rawdelay_handle3_rpc_attach = NULL;
	}

	if (stream->rpc_attach_rpc) {
		dma_buf_vunmap(stream->rpc_attach_rpc->dmabuf,
				 stream->vaddr_rpc);
		rtk_compr_rpc_free(stream->rpc_attach_rpc);
		stream->vaddr_rpc = NULL;
		stream->rpc_attach_rpc = NULL;
	}

	if (rtk_compress_rpc_attach) {
		dma_buf_vunmap(rtk_compress_rpc_attach->dmabuf, stream);
		rtk_compr_rpc_free(rtk_compress_rpc_attach);
		stream = NULL;
		rtk_compress_rpc_attach = NULL;
	}
...
static int snd_card_compr_free(struct snd_compr_stream *cstream)
{
	...
	dma_buf_vunmap(stream->rpc_attach_rpc->dmabuf, stream->vaddr_rpc);
	rtk_compr_rpc_free(stream->rpc_attach_rpc);
...
static int snd_card_compr_set_params(struct snd_compr_stream *cstream, struct snd_compr_params *params)
{
	...
	if (createRingBuf(dev, stream, buffer_size)) {
		pr_err("[%s %d] create Audio Component failed\n", __func__, __LINE__);
		return -1;
	}
	...
	if (params->codec.reserved[0] > 0) { /* with hw av sync */
		...
		dmabuf = dma_buf_get(refclock_handle);
		if (IS_ERR(dmabuf)) {
			dev_err(dev, "Failed to dma_buf_get\n");
			return PTR_ERR(dmabuf);
		}
		ret = rtk_dma_buf_get(dev, dmabuf,
				 &attach, &table,
				 (void **)&dat, (void **)&vaddr);
		if (ret) {
			pr_err("[ALSA %s %d fail]\n", __func__, __LINE__);
		...
		refclock_rpc_attach = attach;
		stream->phyRefclock = dat;
		pr_info("stream->phyRefclock:%lx\n", (unsigned long)stream->phyRefclock);

		stream->refclock = (struct REFCLOCK *)vaddr;
