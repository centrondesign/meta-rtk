...
extern int send_rpc(int opt, uint32_t command, uint32_t param1, uint32_t param2, uint32_t * retval);
 ...
 ******************************************************************************************************************/
	unsigned int decInbandData[64];     /* inband ring at the first, because of 128 align for HIFI */
	...
	struct RINGBUFFER_HEADER *decInRing;     /* big endian, uncache, in DEC-AO path, inring of DEC */
	...
	phys_addr_t phy_decInRing;
	phys_addr_t phy_decInbandRing;
	phys_addr_t phy_decOutRing[8];
	struct dma_buf_attachment *rpc_attach;
	struct dma_buf_attachment *rpc_attach_rpc;    // dma buffer for doing suspend
	struct dma_buf_attachment *decOutData_attach[8]; // dma buffer for decoder out
	struct dma_buf_attachment *g_ShareMemPtr_dat_attach;
	struct dma_buf_attachment *g_ShareMemPtr_dat2_attach;
	struct dma_buf_attachment *g_ShareMemPtr_dat3_attach;
	struct dma_buf_attachment *decInRing_attach;
	struct dma_buf_attachment *decInbandRing_attach;
	struct dma_buf_attachment *decOutRing_attach[8];
...
struct snd_card_RTK_capture_pcm {
	...
	phys_addr_t phy_addr_rpc;       //physical address for doing suspend
	phys_addr_t phy_nAIRing[8];
	phys_addr_t phy_nPTSRingHdr;
	phys_addr_t phy_nLPCMRing;
	struct ALSA_MEM_INFO nPTSMem;
	struct AUDIO_RINGBUF_PTR_64 nPTSRing;
	struct timespec64 ts;
	struct dma_buf_attachment *rpc_attach;
	struct dma_buf_attachment *rpc_attach_rpc;   //dma buffer for doing suspend
	struct dma_buf_attachment *enc_in_attach[8]; //dma buffer for encoder in
	struct dma_buf_attachment *nAIRing_attach[8];
	struct dma_buf_attachment *enc_lpcm_attach;  //dma buffer for lpcm
	struct dma_buf_attachment *enc_pts_attach;
	struct dma_buf_attachment *nPTSRingHdr_attach;
	struct dma_buf_attachment *nLPCMRing_attach;
	void *vaddr_rpc;  //virtual address for doing suspend
