...
static struct dma_buf_attachment *enc_pts_rpc_attach;
...
static int rtk_snd_secure_alloc(struct device *dev, size_t size,
			 struct dma_buf_attachment **attach,
			 struct sg_table **table,
			 void **ion_phy, void **ion_virt)
{
	...
	alloc_flags = RTK_FLAG_SCPUACC | RTK_FLAG_ACPUACC |
...
static int snd_realtek_hw_capture_malloc_ring(struct snd_pcm_runtime *runtime)
{
	...
	for (ch = 0; ch < runtime->channels ; ++ch) {
		if (dpcm->pAIRingData[ch]) {
			...
			if (dpcm->enc_in_rpc_attach[ch]) {
				dma_buf_vunmap(
					dpcm->enc_in_rpc_attach[ch]->dmabuf,
					dpcm->pAIRingData[ch]);
				rtk_snd_free(dpcm->enc_in_rpc_attach[ch]);
				dpcm->enc_in_rpc_attach[ch] = NULL;
			}
			dpcm->pAIRingData[ch] = NULL;
		}

		if (dpcm->source_in == ENUM_AIN_I2S_LOOPBACK) {
			ret = rtk_snd_secure_alloc(dev, RTK_ENC_AI_BUFFER_SIZE,
		...
		dpcm->enc_in_rpc_attach[ch] = attach;
...
static int snd_realtek_hw_capture_malloc_lpcm_ring(struct snd_pcm_runtime *runtime)
{
	...
	dpcm->enc_lpcm_rpc_attach = attach;
...
static int snd_realtek_hw_capture_free_ring(struct snd_pcm_runtime *runtime)
{
	...
	if (dpcm->nAIFormat == AUDIO_ALSA_FORMAT_16BITS_LE_LPCM ||
		...
		if (dpcm->enc_lpcm_rpc_attach) {
			dma_buf_vunmap(dpcm->enc_in_rpc_attach[ch]->dmabuf,
				dpcm->enc_in_rpc_attach[ch]->dmabuf->vmap_ptr);
			rtk_snd_free(dpcm->enc_lpcm_rpc_attach);
			dpcm->enc_lpcm_rpc_attach = NULL;
		}
	}

#ifdef CAPTURE_USE_PTS_RING
	// free AI pts ring
	if (enc_pts_rpc_attach && snd_open_ai_count == 1) {
		dma_buf_vunmap(enc_pts_rpc_attach->dmabuf,
			enc_pts_rpc_attach->dmabuf->vmap_ptr);
		rtk_snd_free(enc_pts_rpc_attach);
		enc_pts_rpc_attach = NULL;
	}
#endif
...
static int snd_card_playback_open(struct snd_pcm_substream *substream)
{
	...
	ret = rtk_snd_alloc(dev, SHARE_MEM_SIZE,
					 &attach, &table, (void **)&dat,
					 (void **)&vaddr);
	if (ret) {
		pr_err("[%s %d alloc fail]\n", __func__, __LINE__);
	...
	ret = rtk_snd_alloc(dev, SHARE_MEM_SIZE_LATENCY,
					 &attach, &table, (void **)&dat,
					 (void **)&vaddr);
	if (ret) {
		pr_err("[%s %d alloc fail]\n", __func__, __LINE__);
...
static int snd_card_playback_ioctl(struct snd_pcm_substream *substream,  unsigned int cmd, void *arg)
{
	...
	case SNDRV_PCM_IOCTL_CLR_DMA_BUF:
		...
		pr_info("%s %d Set up clear flag to %d\n", __func__, __LINE__, dpcm->clr_dma_buf_flag);
		break;
	default:
		return snd_pcm_lib_ioctl(substream, cmd, arg);
		break;
			...
			struct snd_pcm_hw_params *hw_params)
{
	...
	pr_info("[ALSA %s demand %d Bytes PAGE_ALIGN %zu Bytes]\n", __func__,
				params_buffer_bytes(hw_params), buffer_size);
...
void snd_realtek_hw_playback_volume_work(struct work_struct *work)
{
	...
	RPC_TOAGENT_SET_VOLUME(dev, mars->mixer_volume[MIXER_ADDR_MASTER][0]);
}
