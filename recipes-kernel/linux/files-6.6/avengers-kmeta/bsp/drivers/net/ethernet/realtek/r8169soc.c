...
struct rtl8169_storm_ctrl {
	u8 type; /* 0:disable, 1:pkt_limit, 2:rate_limit */
	u16 limit;
};
				  ...
				  size_t count, loff_t *pos)
{
	...
	if (tp->output_mode != OUTPUT_EMBEDDED_PHY) {
		pr_debug("only take effect when using embedded PHY\n");
		goto out;
	}
...
static int wol_pkt_read_proc(struct seq_file *m, void *v)
{
	...
	len = min(rtl_ocp_read(tp, 0xD23E), (u32)WOL_BUF_LEN);
	for (i = 0; i < len; i++) {
		if ((i % 16) == 0)
			seq_puts(m, "\n");
		else if ((i % 8) == 0)
				     ...
				     size_t count, loff_t *pos)
{
	...
	if (buffer && !copy_from_user(tmp, buffer, len)) {
		...
		if (ret) {
			pr_err("invalid pkt_type token[%s], ret 0x%x\n", t, ret);
			goto usage;
		}

		if (pkt_type > 2) {
			pr_err("invalid pkt_type [%d] (pkt_type <= 2)\n", pkt_type);
			goto usage;
		}
		...
		if (ret) {
			pr_err("invalid limit_type token[%s], ret 0x%x\n", t, ret);
			goto usage;
		}

		if (limit_type > 2) {
			pr_err("invalid limit_type [%d] (limit_type <= 2)\n", limit_type);
			goto usage;
		}
		...
		if (ret) {
			pr_err("invalid limit token[%s], ret 0x%x\n", t, ret);
			goto usage;
		}
...
static void rtd119x_reset_phy_gmac(struct rtl8169_private *tp)
{
	...
	/* pre-init clk */
	if ((tp->status & RTL_STATUS_REINIT) == 0) {
...
static int rtl_init_one(struct platform_device *pdev)
{
	...
	do {
		...
		if (!entry) {
			pr_info(PFX "procfs:create /proc/net/eth0/r8169/eth_led failed\n");
			break;
		}
		...
		if (tp->chip->features & RTL_FEATURE_STORM_CTRL) {
			...
			if (!entry) {
				pr_info(PFX "procfs:create /proc/net/eth0/r8169/storm_ctrl failed\n");
				break;
			}
		}
	} while (0);
...
#define RETRY_MAX	10
	...
	eth_hw_addr_set(ndev, tp->mac_addr);
