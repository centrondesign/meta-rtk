...
static inline int __clk_pll_set_oc_en(struct clk_pll *clkp)
{
	...
	lockdep_assert_held(&pll_reg_lock);

	val = clk_regmap_read(&clkp->clkr, clkp->ssc_ofs + 0x0);
	if ((val & 0x7) == 0x5)
		return 0;
	clk_regmap_update_bits(&clkp->clkr, clkp->ssc_ofs + 0x0, 0x7, 0x5);
...
static inline int __clk_pll_set_pow_on_common(struct clk_pll *clkp)
{
	...
	u32 pow = clk_pll_get_pow_offset(clkp);

	lockdep_assert_held(&pll_reg_lock);

	if (clkp->pow_set_rs)
		clk_regmap_update_bits(clkr, clkp->pll_ofs, clkp->rs_mask, clkp->rs_val);

	clk_regmap_update_bits(clkr, clkp->pll_ofs + pow, 0x7, 0x5);
	clk_regmap_update_bits(clkr, clkp->pll_ofs + pow, 0x7, 0x7);

	if (clkp->pow_set_analog)
		clk_regmap_update_bits(clkr, clkp->pll_ofs, clkp->analog_mask, clkp->analog_val);

	udelay(200);

	if (clkp->pow_set_pi_bps)
		clk_regmap_update_bits(clkr, clkp->pll_ofs, 0x10, 0);

	clk_regmap_update_bits(clkr, clkp->pll_ofs + pow, 0x7, 0x3);
...
static inline int __clk_pll_set_pow_on_mno_v2(struct clk_pll *clkp)
{
	...
	if (mask)
		clk_regmap_update_bits(clkr, clkp->pll_ofs, mask, val);
	udelay(200);
...
static inline int __clk_pll_set_pow_off(struct clk_pll *clkp)
{
	...
	clk_regmap_update_bits(clkr, clkp->pll_ofs + pow, 0x7, 0x4);
	if (clkp->pll_type != CLK_PLL_TYPE_NF_SSC)
		return 0;

	if (clkp->pow_set_pi_bps)
		clk_regmap_update_bits(clkr, clkp->pll_ofs, 0x10, 0x10);
	return 0;
}
...
static u32 __clk_pll_freq_raw_get(struct clk_pll *clkp)
{
	u32 val = 0;
...
static inline int __clk_pll_freq_set(struct clk_pll *clkp, u32 val)
{
	int ret = 0;
	u32 mask = clk_pll_get_freq_mask(clkp);

	lockdep_assert_held(&pll_reg_lock);

	switch (clkp->pll_type) {
	case CLK_PLL_TYPE_MNO:
		clk_regmap_update_bits(&clkp->clkr, clkp->pll_ofs, mask, val);
		break;
	...
	case CLK_PLL_TYPE_NF:
		clk_regmap_update_bits(&clkp->clkr, clkp->pll_ofs + 0x4, mask, val);
		if (__clk_pll_get_pow(clkp) > 0) {
	...
	case CLK_PLL_TYPE_NF_SSC:
		clk_regmap_update_bits(&clkp->clkr, clkp->ssc_ofs + 0x0, 0x7, 0x4);
		clk_regmap_update_bits(&clkp->clkr, clkp->ssc_ofs + 0x4, mask, val);
...
static void __clk_pll_div_set(struct clk_pll_div *clkpd, u32 val)
{
	u32 m = (BIT(clkpd->div_width) - 1) << clkpd->div_shift;
	u32 s = clkpd->div_shift;

	clk_regmap_update_bits(&clkpd->clkp.clkr, clkpd->div_ofs, m, val << s);
}

static u32 __clk_pll_div_get(struct clk_pll_div *clkpd)
{
	u32 m = (BIT(clkpd->div_width) - 1) << clkpd->div_shift;
	u32 s = clkpd->div_shift;
