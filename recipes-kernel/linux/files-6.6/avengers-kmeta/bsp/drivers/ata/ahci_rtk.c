...
static int rtk_ahci_probe(struct platform_device *pdev)
{
	...
	syscon_np = of_parse_phandle(node, "realtek,satawrap", 0);

	priv->wrapper = device_node_to_regmap(syscon_np);
	...
	for_each_available_child_of_node(node, child) {
		...
		powerio = devm_fwnode_gpiod_get(dev, of_fwnode_handle(child), "sata", GPIOD_OUT_HIGH, child->name);
		if (!IS_ERR(powerio)) {
			gpiod_put(powerio);
		} else {
	...
	if (!of_property_read_u32(dev->of_node, "hostinit-mode", &priv->hostinit) && !priv->hostinit) {
		pr_err("special mode - ignore host init\n");
		return 0;
	}
...
static int rtk_ahci_suspend(struct device *dev)
{
	...
	struct ahci_host_priv *hpriv = (host) ? host->private_data : NULL;
	struct rtk_ahci_priv *priv = (hpriv) ? hpriv->plat_data : NULL;
	struct device_node *child;
	int rc;
	struct gpio_desc *powerio;
	...
	for_each_available_child_of_node(dev->of_node, child) {
		powerio = devm_fwnode_gpiod_get(dev, of_fwnode_handle(child), "sata-gpios", GPIOD_OUT_LOW, child->name);
		if (!IS_ERR(powerio)) {
			gpiod_put(powerio);
		}
	}
...
static int rtk_ahci_resume(struct device *dev)
{
	...
	for_each_available_child_of_node(dev->of_node, child) {
		powerio = devm_fwnode_gpiod_get(dev, of_fwnode_handle(child), "sata-gpios", GPIOD_OUT_HIGH, child->name);
		if (!IS_ERR(powerio)) {
			gpiod_put(powerio);
		}
	}
