...
static int ws_panel_probe(struct i2c_client *i2c)
{
	...
	/*
	 * The panel only ACKs once it is powered.  Check the first write:
	 * ws_panel_i2c_write() is void and merely logs, so without this a
	 * missing panel would still register a connector that always reports
	 * connected with hard-coded modes.
	 */
	ret = i2c_smbus_write_byte_data(i2c, 0xc0, 0x01);
	if (ret < 0) {
		dev_info(dev, "no panel responding at 0x%02x (%d)\n",
			 i2c->addr, ret);
		return -ENODEV;
	}
	...
	if (ret) {
		/*
		 * rtk_dsi_host_attach() returns -EPROBE_DEFER until the DRM device
		 * is registered, and this module is loaded ~140ms before that
		 * happens.  Returning 0 would mark the probe successful, so it
		 * would never be retried and the DSI device would stay unattached,
		 * leaving the connector disconnected for the rest of the boot.
		 *
		 * drm_panel_add() is not devm-managed, so undo it first: ts is
		 * freed on the error path while the global panel_list would still
		 * point at it, and the retry then makes of_drm_find_panel() walk
		 * into freed memory.
		 */
