...
void rtk_tp_reset_filters(struct demux_info *dmx, struct rtktpfei *fei)
{
       ...
       struct dvb_demux_feed *feed = fei->dvbdmxfeed;
...
void rtk_set_ts_input_select(struct demux_info *dmx)
{
	...
	if (dmx->input_sel == TS_IN_TSIO) {
		...
		if ((read_reg32(regs->reg_TF_CNTL) & TP_TF_CNTL_MODE_BIT) != 0) {
			/* MTP mode */
			write_reg32(regs->reg_TF_FRMCFG,
					(read_reg32(regs->reg_TF_FRMCFG) &
					~TP_TF_FRMCFG_LOCKNO(0xF)) |
...
static void rtktpfe_alloc_ddr_q(struct rtktpfei *fei)
{
	...
	for (i = 0; i < TP_PID_FILTER_COUNT; i++) {
		...
		filter->ddr_q_len = ALIGN(ddr_queue_size, TP_BUFFER_ALIGNMENT);

		rheap_setup_dma_pools(dev, "rtk_media_heap", flags, __func__);
		filter->ddr_q_virt_addr = dma_alloc_coherent(dev,
					 filter->ddr_q_len,
					 &filter->ddr_q_phy_addr, GFP_KERNEL |
					 __GFP_NOWARN);

		if (!filter->ddr_q_virt_addr) {
...
free_all:
	while (--i > 0) {
		filter = &fei->fi[i];
		if (filter->ddr_q_virt_addr) {
			dma_free_coherent(dev, ddr_queue_size,
...
static void rtktpfe_free_ddr_q(struct rtktpfei *fei)
{
	...
	for (i = 0; i < TP_PID_FILTER_COUNT; i++) {
		...
		if (filter->ddr_q_virt_addr) {
			dma_free_coherent(dev, filter->ddr_q_len,
				 filter->ddr_q_virt_addr,
				 filter->ddr_q_phy_addr);
		}
		filter->ddr_q_len = 0;
...
static void rtktpfe_alloc_TPA1_mmbuf(struct rtktpfei *fei)
{
	...
	dmx->mmbuf_len = ALIGN(MM_BUFFER_SIZE, TP_BUFFER_ALIGNMENT);

	rheap_setup_dma_pools(dev, "rtk_media_heap", flags, __func__);
	dmx->mmbuf_virt_addr = dma_alloc_coherent(fei->dev, dmx->mmbuf_len,
					&dmx->mmbuf_phy_addr, GFP_KERNEL |
					 __GFP_NOWARN);

	if (!dmx->mmbuf_virt_addr) {
...
static void rtktpfe_free_TPA1_mmbuf(struct rtktpfei *fei)
{
	...
	if (dmx->mmbuf_virt_addr) {
		dma_free_coherent(dev, dmx->mmbuf_len,
				 dmx->mmbuf_virt_addr,
				 dmx->mmbuf_phy_addr);
	}
