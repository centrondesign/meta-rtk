...
#define TP_FLUSH_BUF_RANGE(d, p, l) rtk_tp_ion_sync_range(d, p, l)
static int rtk_tp_ion_sync_range(struct demux_info *dmx,
				 unsigned long phys, size_t len)
{
	...
	struct dma_buf *dmabuf = dmx->mmbuf_dmabuf;
	...
	if (!(buffer->flags & ION_FLAG_NONCACHED)) {
		...
		if ((addr = rtk_tp_ion_pa(buffer)) != 0) {
			...
			dma_addr_t paddr = dma_map_page(dev,
						sg_page(buffer->sg_table->sgl),
						offset,
						len, DMA_TO_DEVICE);
			dma_sync_single_for_device(dev, phys, len, DMA_TO_DEVICE);
			if (paddr)
				dma_unmap_page(dev, paddr, len, DMA_TO_DEVICE);
		}
	}
...
#else /* USE_ION_BUFFER */
#define TP_FLUSH_BUF_RANGE(d, p, l) rtk_tp_dma_sync_range(d, p, l)
static int rtk_tp_dma_sync_range(struct demux_info *dmx,
				 unsigned long phys, size_t len)
{
	phys_addr_t addr;
	u64 dma_mask = 0xffffffff;
	...
	struct device *dev = fei->dev;
	dev->dma_mask = &dma_mask;
	if ((addr = (phys_addr_t)sg_phys(dmx->mmbuf_sgt->sgl)) != 0) {
		size_t offset = phys - addr;
		dma_addr_t paddr = dma_map_page(dev,
					sg_page(dmx->mmbuf_sgt->sgl),
					offset,
					len, DMA_TO_DEVICE);
		dma_sync_single_for_device(dev, phys, len, DMA_TO_DEVICE);
		if (paddr)
...
// empty function
#define TP_FLUSH_BUF_RANGE(d, p, l) (void)(p)

#endif /* FLUSH_USER_MTP */
			       ...
			       size_t len)
{
	...
	if (split > 0) {
		...
		TP_FLUSH_BUF_RANGE(dmx, phys + offset, split);
	...
	TP_FLUSH_BUF_RANGE(dmx, phys + offset, todo);
