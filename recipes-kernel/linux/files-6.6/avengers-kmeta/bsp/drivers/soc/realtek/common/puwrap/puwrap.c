		 ...
		 void **ion_phy, unsigned int alloc_flags)
{
	...
	*ion_phy = (void *)sg_phys((*table)->sgl);
	...
	dev_dbg(dev, "%s *ion_phy = 0x%lx \n", __func__, (long unsigned int) *ion_phy);
			 ...
			 unsigned int mem_type)
{
	...
	do {
		...
		handle = rtk_pu_alloc(dev, size, &attach, &table, (void **)&dat, flags);
		if (handle)
		{
			pr_err("[PUWRAP] ion_alloc allocation error size=%d\n", size);
			break;
		}
