...
static int of_dpi_set_ddr_ssc(struct dpi_device *dpi)
{
	struct device *dev = dpi->dev;
	int len, ret;

	if (!of_find_property(dev->of_node, "ddr-ssc", &len))
		return -EINVAL;

	if (len > sizeof(dpi->ssc_en))
		return -EINVAL;

	if((ret = of_property_read_u32_array(dev->of_node, "ddr-ssc", &dpi->ssc_en, len/4)))
		return ret;

	dpi_reg_update_bits(dpi, DPI_DLL_CRT_SSC0, 0x00000002, dpi->ssc_en << 1);

	pr_info("%-20s: ddr_ssc_en = 0x%x\n", __func__, dpi->ssc_en);

	return 0;
}

static int of_dpi_set_ddr_speed(struct dpi_device *dpi)
{
	struct device *dev = dpi->dev;
	int len, ret, i, dll_crt_ssc, ddr_speed_cur;

	if (!of_find_property(dev->of_node, "ddr-speed", &len))
		return -EINVAL;

	if (len > sizeof(dpi->ddr_speed))
		return -EINVAL;

	if((ret = of_property_read_u32_array(dev->of_node, "ddr-speed", &dpi->ddr_speed, len/4)))
		return ret;

	dpi_reg_read(dpi, DPI_DLL_CRT_SSC3, &dll_crt_ssc);

	ddr_speed_cur = (dll_crt_ssc >> 8) & 0xff;

	if(dpi->ddr_speed == 0) dpi->ddr_speed = ddr_speed_cur;
	else if(dpi->ddr_speed > ddr_speed_cur) i = 1;
	else i = -1;

	while(ddr_speed_cur != dpi->ddr_speed) {
		ddr_speed_cur += i;
		dpi_reg_write(dpi, DPI_DLL_CRT_SSC3, ((ddr_speed_cur << 0x8) | (ddr_speed_cur - 1) | (dll_crt_ssc & 0xffff0000)));
		dpi_reg_update_bits(dpi, DPI_DLL_CRT_SSC3, 0x04000000, 0x04000000);
		dpi_reg_update_bits(dpi, DPI_DLL_CRT_SSC3, 0x04000000, 0x00000000);
	}

	pr_info("%-20s: ddr_speed = 0x%x\n", __func__, dpi->ddr_speed);
...
static inline int of_dpi_read_temp_range(struct dpi_device *dpi)
{
	...
	pr_debug("%-20s: hot_temp = %d\n", __func__, dpi->hot_temp);
	pr_debug("%-20s: cold_temp = %d\n", __func__, dpi->cold_temp);

	return 0;
}

static const struct dpi_quirks default_quirks = {0, 0};
static const struct dpi_quirks hank_quirks = {1, 0};
static const struct dpi_quirks stark_quirks = {0, 1};

static const struct dpi_quirks *setup_quirks(void)
{
	const struct soc_device_attribute *match = NULL;
        struct soc_device_attribute attrs[] = {
		{ .family = "Realtek Hank",  .soc_id = "RTD1315", .data = &hank_quirks, },
		{ .family = "Realtek Hank",  .soc_id = "RTD1317", .data = &hank_quirks, },
		{ .family = "Realtek Hank",  .soc_id = "RTD1319", .data = &hank_quirks, },
		{ .family = "Realtek Stark", .soc_id = "RTD1619B", .data = &stark_quirks, },
		{ .family = "Realtek Stark", .soc_id = "RTD1315C", .data = &stark_quirks, },
		{}
        };

	match = soc_device_match(attrs);
	return match ? match->data : &default_quirks;
}
...
static int dpi_probe(struct platform_device *pdev)
{
	...
	if((ret = of_dpi_set_ddr_speed(dpi)))
		return ret;

	if((ret = of_dpi_set_ddr_ssc(dpi)))
		return ret;
