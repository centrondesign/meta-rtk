...
static ssize_t scd_dev_read(struct file* file, char __user* buff, size_t size, loff_t* ofst)
{
	...
	if (ret > 0) {
		...
		res = copy_to_user((unsigned char __user *)buff, tmp, ret);
		if (res < 0) {
...
static long scd_dev_ioctl(struct file* file, unsigned int cmd, unsigned long arg)
{
	...
	scd_baud_rate baud_rate; //scd_ifd_param ifd_param;
	scd_tx_rx_session session;
	scd_tx_rx_session *p_session;
	int         ret;
	...
	switch (cmd) {
	case SCD_SET_BAUD_RATE: //BAUD RATE:
		if (copy_from_user(&baud_rate, (scd_baud_rate __user *)arg, sizeof(baud_rate)))
			return -EFAULT;

		baud_rate.ret = drv->set_baud_rate(dev, baud_rate.clk, baud_rate.etu);

		// return error code
		return copy_to_user((scd_baud_rate __user *)arg, &baud_rate, sizeof(baud_rate));
	case SCD_GET_BAUD_RATE:

		if (copy_from_user(&baud_rate, (scd_baud_rate __user *)arg, sizeof(baud_rate)))
			return -EFAULT;

		baud_rate.ret = drv->get_baud_rate(dev,
								(unsigned long *)&baud_rate.clk,
								(unsigned long *)&baud_rate.etu);
	...
	*/
	...
	case SCD_GET_PROTOCOL:
		...
		if (drv->get_protocol(dev, (unsigned long *)&protocol.val) < 0)
			return -EFAULT;
	...
	case SCD_TX_RX_SESSION:

		if (copy_from_user(&session, (scd_tx_rx_session __user *)arg, sizeof(scd_tx_rx_session)))
			return -EFAULT;

		if( (ret = drv->tx_rx_session(dev, session.p_tx, session.p_rx, session.tx_len, session.rx_len, session.flags, session.wwt, session.end_stamp_ms)) < 0)
			return -EFAULT;

		return copy_to_user((scd_tx_rx_session __user *)arg, &session, sizeof(scd_tx_rx_session));

	case SCD_CARD_IO_COMMAND:
		p_session = (scd_tx_rx_session __user *)arg;
		get_user(session.tx_len, &p_session->tx_len);
		//get_user(session.rx_len, &p_session->rx_len);
		get_user(session.flags,  &p_session->flags);
		get_user(session.wwt,    &p_session->wwt);
		get_user(session.end_stamp_ms, &p_session->end_stamp_ms);
		if(session.tx_len)
			ret = copy_from_user(session.p_tx, p_session->p_tx, session.tx_len);

		ret = drv->card_io_command(dev, session.p_tx, session.p_rx, session.tx_len, &session.rx_len,
								session.flags, session.wwt, session.end_stamp_ms);
		if( ret != SC_SUCCESS)
			return ret;

		put_user(session.rx_len, &p_session->rx_len);
		if(session.rx_len>0)
			ret = copy_to_user(p_session->p_rx, session.p_rx, session.rx_len);
		return 0;
...
int __init scd_dev_module_init(void)
{
	...
	scd_dev_class = class_create("scd-dev");
