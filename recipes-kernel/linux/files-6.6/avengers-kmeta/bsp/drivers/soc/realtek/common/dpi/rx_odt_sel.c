...
static int dpi_rx_odt_sel_of_init(struct device *dev, struct dpi_rx_odt_sel_priv *priv)
{
	int ret, len;

	if ((ret = dpi_rx_odt_sel_read_reg_values(dev, "ddr-type", &priv->ddr_type_q, &len)))
		return ret;

	if ((ret = dpi_rx_odt_sel_read_reg_values(dev, "odt-sel-idx", &priv->odt_idx_q, &len)))
		return ret;

	if ((ret = dpi_rx_odt_sel_read_reg_values(dev, "state-0-value", &priv->odt_val_q_0, &len)))
		return ret;

	ret = dpi_rx_odt_sel_read_reg_values(dev, "state-1-value", &priv->odt_val_q_1, &len);
