...
} scd_protocol; // oo>> 32/64 bit issue, no use.
...
typedef struct
{
	...
	// get/set clk/etu in one ioctrl.
	int (*set_baud_rate)(scd_device *dev, unsigned long clk, unsigned long etu);
	int (*get_baud_rate)(scd_device *dev, unsigned long *p_clk, unsigned long *p_etu);

	//-- add these to avoid confusing in SCControl(GetParam/SetParm)
	int (*set_protocol)(scd_device *dev, unsigned long val);
	int (*get_protocol)(scd_device *dev, unsigned long *p_val);
	...
	// for chili
	//int (*tx_rx_session)(scd_device *dev, unsigned char *p_tx, unsigned char *p_rx,
	//	unsigned int tx_len, unsigned int rx_len, unsigned int flags,
	//	unsigned long wwt, unsigned long end_stamp_ms);
	int (*tx_rx_session)(scd_device *dev, uint8_t *p_tx, uint8_t *p_rx,
		uint32_t tx_len, uint32_t rx_len, uint32_t flags,
		uint64_t wwt, uint64_t end_stamp_ms);

	int (*card_io_command)(scd_device *dev, uint8_t *p_tx, uint8_t *p_rx,
