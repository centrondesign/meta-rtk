				      ...
				      struct device_attribute *attr, char *buf)
{
	struct usbport_monitor_port *port = container_of(dev,
		     struct usbport_monitor_port, dev);
	struct usb_device *usb = port->hub;

	return sprintf(buf, "%s at %s\n", dev_name(&usb->dev),
			 dev_name(usb->bus->controller->parent)) + 1;
}
					  ...
					  void *data)
{
	...
	list_for_each_entry(port, &usbport_data->ports, list) {
		if (usb_dev->parent == port->hub &&
		    usb_dev->portnum == port->portnum) {
			dev_info(&usb_dev->dev, "usbport monitor: %s attach to %s\n",
					  ...
					  void *data)
{
	...
	list_for_each_entry(port, &usbport_data->ports, list) {
		if (usb_dev->parent == port->hub &&
		    usb_dev->portnum == port->portnum) {
			dev_info(&usb_dev->dev, "usbport monitor: %s detach from %s\n",
				    dev_name(&usb_dev->dev), port->port_name);
...
static void __autosuspend_device(struct usb_device *udev)
{
	int	status;
...
static int __autoresume_device(struct usb_device *udev)
{
	int	status;
...
static int __usb_get_status(struct usb_device *dev)
{
	int ret;
...
static __maybe_unused int __get_device_descriptor(struct usb_device *dev, unsigned int size)
{
...
static void usbport_monitor_check_work(struct kthread_work *work)
{
	...
	list_for_each_entry_safe(port, tmp, &usbport_data->ports, list) {
		if (port->observed && port->attached_usb) {
			...
			__autoresume_device(udev);
			ret = __usb_get_status(udev);
			if (ret == -EPROTO) {
				int rc;

				dev_err(&udev->dev,
					    "%s: check device %s fail (ret=%d)\n",
					    __func__, dev_name(&udev->dev), ret);
				rc = usb_lock_device_for_reset(udev, NULL);
				if (rc >= 0) {
					dev_err(&udev->dev,
						    "%s: reset device %s (rc=%d)\n",
						    __func__, dev_name(&udev->dev), rc);
					mutex_unlock(&usbport_data->lock);
					...
					dev_err(&udev->dev,
						    "%s: lock device %s fail (rc=%d)\n",
						    __func__, dev_name(&udev->dev), rc);
				}
			} else {
				dev_dbg(&udev->dev, "%s: check device %s OK (ret=%d)\n",
				    __func__, dev_name(&udev->dev), ret);
			}
			__autosuspend_device(udev);
		}
	}
	mutex_unlock(&usbport_data->lock);
			       ...
			       void *data)
{
	...
	case USB_DEVICE_ADD:
		mutex_lock(&usbport_data->lock);
		dev_info(&usb_dev->dev, "Attach to controller %s\n",
			 dev_name(usb_dev->bus->controller->parent));
	...
	case USB_DEVICE_REMOVE:
		...
		dev_info(&usb_dev->dev, "Detach from controller %s\n",
			 dev_name(usb_dev->bus->controller->parent));
		mutex_unlock(&usbport_data->lock);
		return NOTIFY_OK;
	}
...
{
	struct usbport_monitor_data *usbport_data;
	struct class *usbport_monitor_class = NULL;
	int err = 0;
	...
	usbport_monitor_class = class_create("usbport_monitor");
...
static void __exit usbport_monitor_exit(void)
{
	...
	kthread_cancel_delayed_work_sync(&usbport_data->check_work);
