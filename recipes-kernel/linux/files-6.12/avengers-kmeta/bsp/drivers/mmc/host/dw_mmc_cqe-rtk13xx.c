...
int dw_mci_rtk13xx_write_protect_cmd(struct mmc_host *mmc, u32 args, bool is_wrtie_protect)
{
...
int dw_mci_rtk13xx_query_protect_cmd(struct mmc_host *mmc, unsigned long addr, u32 *src)
{
        struct mmc_request mrq = {};
        struct mmc_command cmd = {};
...
static ssize_t rtk13xx_protect_region_setting_dev_show(struct device *dev,
                        struct device_attribute *attr, char *buf)
{
	...
	if (ret) {
        ...
        dw_mci_rtk13xx_query_protect_cmd(mmc, priv->protect_start, &src);
...
static ssize_t rtk13xx_protect_region_setting_dev_store(struct device *dev,
	...
	if (ret) {
        ...
        if (buf[0]=='Q') {
                ...
                for (args = 0; args < max_sectors; args += (wpg_unit * 0x20)) {
                        dw_mci_rtk13xx_query_protect_cmd(mmc, args, &src);
                ...
                for (args = 0; args < max_sectors; args += wpg_unit) {
                        dw_mci_rtk13xx_write_protect_cmd(mmc, args, 0);
                }
        } else if(buf[0]=='S') {
                dev_info(dev, "%s set the eMMC protect group  regions...\n", __func__);
                dw_mci_rtk13xx_write_protect_cmd(mmc, priv->protect_start, 1);
        }
...
DEVICE_ATTR(rtk13xx_protect_region_setting, S_IRUGO | S_IWUSR,
                rtk13xx_protect_region_setting_dev_show, rtk13xx_protect_region_setting_dev_store);

static ssize_t rtk13xx_protect_region_start_dev_show(struct device *dev,
                                                struct device_attribute *attr, char *buf)
{
        struct dw_mci *host = dev_get_drvdata(dev);
        struct dw_mci_rtkemmc_host *priv = host->priv;
...
static ssize_t rtk13xx_protect_region_start_dev_store(struct device *dev,
                struct device_attribute *attr, const char *buf, size_t count)
{
        struct dw_mci *host = dev_get_drvdata(dev);
        struct dw_mci_rtkemmc_host *priv = host->priv;
...
DEVICE_ATTR(rtk13xx_protect_region_start, S_IRUGO | S_IWUSR,
                rtk13xx_protect_region_start_dev_show, rtk13xx_protect_region_start_dev_store);

static ssize_t rtk13xx_protect_region_unit_dev_show(struct device *dev,
                                                struct device_attribute *attr, char *buf)
{
        struct dw_mci *host = dev_get_drvdata(dev);
        struct mmc_host *mmc = host->slot->mmc;
...
static ssize_t rtk13xx_protect_region_unit_dev_store(struct device *dev,
                struct device_attribute *attr, const char *buf, size_t count)
{
        return count;
}

DEVICE_ATTR(rtk13xx_protect_region_unit, S_IRUGO | S_IWUSR,
                rtk13xx_protect_region_unit_dev_show, rtk13xx_protect_region_unit_dev_store);

static ssize_t
rtk13xx_tuning_info_dev_show(struct device *dev, struct device_attribute *attr, char *buf)
{
        struct dw_mci *host = dev_get_drvdata(dev);
        struct mmc_host *mmc = host->slot->mmc;
        ...
        if(mmc->caps2 & MMC_CAP2_HS400_1_8V) {
                dw_mci_rtk13xx_hs400_complete(mmc);
                size = sprintf(buf, "The eMMC hs400 tuning finished !!!\n");
        }
        else if(mmc->caps2 & MMC_CAP2_HS200_1_8V_SDR) {
                dw_mci_rtk13xx_execute_tuning(slot,MMC_SEND_TUNING_BLOCK_HS200);
...
static ssize_t rtk13xx_tuning_info_dev_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
...
DEVICE_ATTR(rtk13xx_tuning_info, S_IRUGO | S_IWUSR,
                rtk13xx_tuning_info_dev_show, rtk13xx_tuning_info_dev_store);
...
static int dw_mci_rtk13xx_execute_tuning(struct dw_mci_slot *slot, u32 opcode)
{
	...
	if (mmc->ios.timing == MMC_TIMING_MMC_HS400) {
		...
		dw_mci_rtk13xx_phase_tuning(host, 0x6, 0xff);
		pr_info("set tx_best to 0x6 (fixed) for hs400\n");
		ret = 0;
		goto out;
	}
	else if(mmc->ios.timing == MMC_TIMING_MMC_HS200){
		...
		dw_mci_rtk13xx_phase_tuning(host, 0x1f, 0xff);
		pr_info("set tx_best to 0x1f(fixed) for hs200\n");
		ret = 0;
		goto out;
	}
	else {
...
static int dw_mci_rtk_probe(struct platform_device *pdev)
{
	...
	ret = device_create_file(&pdev->dev, &dev_attr_rtk13xx_protect_region_setting);
        if (ret < 0) {
                dev_err(&pdev->dev, "device_create_setting_file fail (ret=%d)\n",
                            ret);
        }
        ret = device_create_file(&pdev->dev, &dev_attr_rtk13xx_protect_region_start);
        if (ret < 0) {
                dev_err(&pdev->dev, "device_create_start_file fail (ret=%d)\n",
                        ret);
        }
        ret = device_create_file(&pdev->dev, &dev_attr_rtk13xx_protect_region_unit);
        if (ret < 0) {
                dev_err(&pdev->dev, "device_create_len_file fail (ret=%d)\n",
                        ret);
        }
        ret = device_create_file(&pdev->dev, &dev_attr_rtk13xx_tuning_info);
        if (ret < 0) {
...
void dw_mci_rtk13xx_remove(struct platform_device *pdev)
{
        struct dw_mci *host = platform_get_drvdata(pdev);

        device_remove_file(&pdev->dev, &dev_attr_rtk13xx_protect_region_setting);
        device_remove_file(&pdev->dev, &dev_attr_rtk13xx_protect_region_start);
        device_remove_file(&pdev->dev, &dev_attr_rtk13xx_protect_region_unit);
        device_remove_file(&pdev->dev, &dev_attr_rtk13xx_tuning_info);
