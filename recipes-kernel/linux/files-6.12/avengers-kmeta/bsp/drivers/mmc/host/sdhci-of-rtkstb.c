...
static void rtkstb_set_sd_power(struct sdhci_host *host, int enable)
{
	...
	if (!rtk_host->sd_pwr_gpio) {
		dev_err(dev, "%s: sd power not found\n", __func__);
		return;
	}
...
static void sdhci_rtkstb_hw_init(struct sdhci_host *host)
{
	...
	if (host->mmc->caps2 & MMC_CAP2_AVOID_3_3V)
		rtkstb_sdhci_pad_power_ctrl(host, MMC_SIGNAL_VOLTAGE_180);
...
static u8 rtkstb_sdhci_search_final_phase(struct sdhci_host *host, u32 phase_map, int tx)
{
	...
	if (len_final == MAXIMUM_CONTINUE_LENGTH)
		final_phase = DEFAULT_PHASE;
	else if (len_final > MINIMUM_CONTINUE_LENGTH)
...
static int sdhci_rtkstb_probe(struct platform_device *pdev)
{
	...
	rtk_host->sd_pwr_gpio = devm_gpiod_get_optional(&pdev->dev, "sd-power", GPIOD_OUT_LOW);
	...
	if (device_property_read_bool(&pdev->dev, "init-avoid-3_3v"))
		host->mmc->caps2 |= MMC_CAP2_AVOID_3_3V;
...
static void sdhci_rtkstb_remove(struct platform_device *pdev)
{
