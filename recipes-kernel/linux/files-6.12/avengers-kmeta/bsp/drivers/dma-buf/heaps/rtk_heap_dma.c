...
static pgprot_t dma_rheap_pgprot(struct device *dev, pgprot_t prot, unsigned long attrs)
{
#ifdef CONFIG_ARM
...
static int dma_rheap_get_sgtable(struct device *dev, struct sg_table *sgt,
                 void *cpu_addr, dma_addr_t dma_addr, size_t size,
                 unsigned long attrs)
{
	struct page *page;
...
static int dma_rheap_map_sg(struct device *dev, struct scatterlist *sgl, int nents,
...
static void dma_rheap_unmap_sg(struct device *dev, struct scatterlist *sgl,
				 ...
				 unsigned long heap_flags, const char *caller)
{
	...
	list_for_each_entry(best_fit, &best_list, hlist) {
		...
		strscpy(dma_pool->caller, caller, sizeof(dma_pool->caller));
