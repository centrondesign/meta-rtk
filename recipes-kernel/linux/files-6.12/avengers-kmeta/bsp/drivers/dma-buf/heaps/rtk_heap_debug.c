...
static struct proc_dir_entry *rheap_proc_root = NULL;
		...
		list_for_each_entry_safe(rtk_heap_task, tmp, &ch->task_list,
			...
			if (list_entry_is_head(tmp_task, &ptask_list, list)) {
				tmp_task = (struct rtk_heap_task *)kzalloc(
					...
					GFP_KERNEL);
				strscpy(tmp_task->comm, rtk_heap_task->comm,
...
static const struct proc_ops proc_rtk_heap_summay_fops = {
	.proc_open = rtk_heap_summay_fops_open,
	.proc_read = seq_read,
...
static const struct proc_ops proc_rtk_task_summay_fops = {
	.proc_open = rtk_task_summary_fops_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};
...
void rheap_procfs_init(void)
{
        ...
        if (!rheap_proc_root) {
                rheap_proc_root = proc_mkdir("rtk_heap", NULL);
                if (!rheap_proc_root){
                        pr_err("Failed to create /proc/rtk_heap\n");
                        return;
                }
        }
        if (!proc_create("heap_summary", 0444, rheap_proc_root,
                                &proc_rtk_heap_summay_fops)) {
                pr_err("Failed to create /proc/rtk_heap/heap_summary\n");
        }
        if (!proc_create("task_summary", 0444, rheap_proc_root,
                                &proc_rtk_task_summay_fops)) {
                pr_err("Failed to create /proc/rtk_heap/task_summary\n");
        }

}
EXPORT_SYMBOL(rheap_procfs_init);
