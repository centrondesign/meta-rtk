...
/* Channel count for video-detection is 1 even it's actually 2 in cascade-mode */
#define PR2K_SIGNAL_DETECT_CH_COUNT 1
...
#define MAX_CH_CNT (8)
#define NUM_VALID_BYTES (4 + (MAX_CH_CNT*2))
/* The common struct for rtk v4l2_event to monitor video signals */
struct rtk_v4l2_event_payload {
...
struct pr2000 {
	...
	u8 signal_status; /* Signal status */
...
/* Offset for P0_10 in arrays, pr2k_*_setting[] */
#define P0X10_OFFSET_HD 1

/*
 * P1_11 is suggested to set 0x7 if auto_bgnd is enabled, but our test results
 * are both okay with 0x0 or 0x7. We don't wanna force the video-input with
 * MAN_IFMT_*, so 0x0 is set as the default.
 */
static const struct regval pr2k_720p25_setting[] = {
	{PAGE_SEL, 0x00},
	{0x10, 0x83}, /* P0X11_OFFSET_HD */
	{0x11, 0x00},
	{0x12, 0x00},
	{0x13, 0x00},
	{0x14, 0x21},
...
/* Offset for P0_10 in arrays, p0_main_*i[] */
#define P0X10_OFFSET_SD 0
...
static void pr2k_soft_reset(struct v4l2_subdev *sd)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static void pr2k_notify_status_event(struct v4l2_subdev *sd,
	u8 st, u8 pre_st)
{
	struct v4l2_event ev;
	struct rtk_v4l2_event_payload payload;
	struct pr2000 *pr2k = to_pr2k(sd);
	...
	payload.ch_cnt = PR2K_SIGNAL_DETECT_CH_COUNT;
	payload.sigbits = st;
	payload.prev_sigbits = pre_st;
	BUILD_BUG_ON(sizeof(payload) > sizeof(ev.u.data));
	memcpy(ev.u.data, &payload, sizeof(payload));

	dev_info(pr2k->dev, "Notify signal=%d prev=%d\n", st, pre_st);

	v4l2_subdev_notify_event(sd, &ev);
}

/* This funcion does not only as pr2k_signal_det does
 * but notifying the change of the signal of vid_status
 */
static void pr2k_signal_check(struct pr2000 *pr2k, struct v4l2_subdev *sd)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	u8 pre_sig;
	u8 vid_status0;
	...
	if (vid_status1 == 0xff) {
		...
		if (VID_STATUS0_get_DET_VIDEO(vid_status0)) {
			...
			dev_vdbg(pr2k->dev, "Format std=%s ref=%s res=%s\n",
				str_ifmt_std[VID_STATUS0_get_DET_IFMT_STD(vid_status0)&0x3],
				str_ifmt_ref[VID_STATUS0_get_DET_IFMT_REF(vid_status0)&0x7],
				str_ifmt_res[VID_STATUS0_get_DET_IFMT_RES0(vid_status0)&0x3]);
		}
	} else {
		pr2k->pll_locked = false;
		pr2k->video_detected = false;

		dev_vdbg(pr2k->dev, "vid_status0=0x%02x\n", vid_status0);
		dev_vdbg(pr2k->dev, "vid_status1=0x%02x\n", vid_status1);
	}

	if (pr2k->video_detected)
		pr2k->signal_status = VIN_SIGNAL_READY;
	else
		pr2k->signal_status = VIN_SIGNAL_NONE;

	if (pre_sig != pr2k->signal_status)
		pr2k_notify_status_event(sd, pr2k->signal_status, pre_sig);
}
...
static bool pr2k_signal_det(struct v4l2_subdev *sd)
{
	...
	if (vid_status1 == 0xff) {
		...
		dev_info(pr2k->dev, "VID_STATUS0=0x%02x\n", vid_status0);
		dev_info(pr2k->dev, "VID_STATUS1=0x%02x\n", vid_status1);
		pr2k->pll_locked = false;
		pr2k->video_detected = false;
	}
			...
			struct device_attribute *attr, char *buf)
{
	...
	struct  pr2000 *pr2k = to_pr2k(sd);
	unsigned char pr2k_status = 0x0;

	mutex_lock(&pr2k->detect_lock);
	pr2k_signal_check(pr2k, sd);
	mutex_unlock(&pr2k->detect_lock);

	pr2k_status = pr2k->video_detected;
...
static int pr2k_sysfs_create(struct v4l2_subdev *sd)
{
	int ret;
	struct pr2000 *pr2k = to_pr2k(sd);

	ret = device_create_file(pr2k->dev, &dev_attr_channels_state);
	if (ret)
		dev_err(pr2k->dev, "create sysfs channels_state failed: %d\n", ret);

	return ret;
}


static void pr2k_sysfs_remove(struct v4l2_subdev *sd)
{
	struct pr2000 *pr2k = to_pr2k(sd);

	device_remove_file(pr2k->dev, &dev_attr_channels_state);
}
...
static void pr2k_detect_work(struct work_struct *work)
{
	struct pr2000 *pr2k = container_of(work, struct pr2000,
					      detect_work.work);
	struct v4l2_subdev *sd = &pr2k->sd;
	...
	if (pr2k->detect_mode == DETECT_MODE_POLLING)
		schedule_delayed_work(&pr2k->detect_work, msecs_to_jiffies(100));
}

static int pr2k_subscribe_event(struct v4l2_subdev *sd,
			struct v4l2_fh *fh,
			struct v4l2_event_subscription *sub)
{
	...
	case RTK_V4L2_EVENT_SIGNAL_CHANGE:
		dev_info(pr2k->dev, "%s RTK_V4L2_EVENT_SIGNAL_CHANGE\n", __func__);
		return v4l2_event_subscribe(fh, sub, 0, NULL);
	default:
		dev_err(pr2k->dev, "%s unsupport event\n", __func__);
		return -EINVAL;
	}
}

static int pr2k_setup_polling_mode(struct pr2000 *pr2k)
{
	struct v4l2_subdev *sd = &pr2k->sd;

	/* The format might be set from video-input-config and this part is to
	 * determine it from P0_10 and show whether the configured format is correct
	 */
	if (pr2k->format > FMT_HD_960P30) {
		u8 man_ifmt;

		/* Read man_ifmt at P0_10 */
		pr2k_write_reg(sd, PAGE_SEL, 0x00);
		man_ifmt = pr2k_read_reg(sd, 0x10);

		dev_info(pr2k->dev, "Polling setup man_ifmt=%02x\n", man_ifmt);

		if (man_ifmt == p0_main_480i[P0X10_OFFSET_SD])
			pr2k->format = FMT_SD_480I;
		else if (man_ifmt == p0_main_576i[P0X10_OFFSET_SD])
			pr2k->format = FMT_SD_576I;
		else if (man_ifmt == pr2k_1080p30_setting[P0X10_OFFSET_HD].val)
			pr2k->format = FMT_HD_1080P30;
		else if (man_ifmt == pr2k_720p25_setting[P0X10_OFFSET_HD].val)
			pr2k->format = FMT_HD_720P25;
		else if (man_ifmt == pr2k_720p30_setting[P0X10_OFFSET_HD].val)
			pr2k->format = FMT_HD_720P30;
		else
			dev_dbg(pr2k->dev, "Polling setup format is not set\n");

		if (pr2k->format < FMT_UNDEFINED)
			dev_info(pr2k->dev, "Configured format=%s is found\n",
				str_cam_fmt[pr2k->format]);
	} else {
		dev_info(pr2k->dev, "Polling setup format=%s\n", str_cam_fmt[pr2k->format]);
	}
...
static const struct v4l2_subdev_core_ops pr2k_core_ops = {
	...
	.subscribe_event = pr2k_subscribe_event,
...
static void pr2k_s_detection(struct pr2000 *pr2k)
{
	if (pr2k->detect_mode == DETECT_MODE_POLLING) {
		pr2k_setup_polling_mode(pr2k);
		schedule_delayed_work(&pr2k->detect_work, msecs_to_jiffies(100));
	} else {
		cancel_delayed_work_sync(&pr2k->detect_work);
	}
}

static int pr2k_s_ctrl(struct v4l2_ctrl *c)
{
	struct pr2000 *pr2k = container_of(c->handler, struct pr2000, hdl);

	switch (c->id) {
	case V4L2_CID_RTK_CAMERA_VIDEOSIGNAL_DETECT:
		pr2k->detect_mode = c->val;
		dev_info(pr2k->dev, "s_ctrl detect_mode=%d\n", c->val);
		pr2k_s_detection(pr2k);
		break;
	default:
		return -EINVAL;
	}
...
static const struct v4l2_ctrl_ops pr2k_ctrl_ops = {
	.s_ctrl = pr2k_s_ctrl,
};

static struct v4l2_ctrl_config pr2k_detect_mode = {
	.ops = &pr2k_ctrl_ops,
...
static int pr2k_probe(struct i2c_client *client)
{
	...
	/* Initialize the status of the video signal */
	pr2k->signal_status = VIN_SIGNAL_NONE;

	mutex_init(&pr2k->detect_lock);

	pr2k->rst_gpio = devm_gpiod_get(dev, "reset", GPIOD_OUT_LOW);
	...
	if (ret < 0 || pr2k->format > FMT_HD_960P30) {
		dev_info(dev, "Camera format is undefined, ret=%d\n", ret);
		pr2k->format = FMT_UNDEFINED;
	} else {
	...
	ret = of_property_read_u32(dev->of_node, "detect-mode", &pr2k->detect_mode);
	if (ret < 0) {
		/* Default to disabled mode if not specified */
		pr2k->detect_mode = DETECT_MODE_DISABLED;
		dev_info(dev, "detect-mode not specified, detection disabled\n");
	} else {
		/* Validate detection mode */
		if (pr2k->detect_mode != DETECT_MODE_DISABLED &&
		    pr2k->detect_mode != DETECT_MODE_POLLING) {
	...
	pr2k->sd.flags |= V4L2_SUBDEV_FL_HAS_DEVNODE|V4L2_SUBDEV_FL_HAS_EVENTS;

	v4l2_ctrl_handler_init(&pr2k->hdl, 1);
	v4l2_ctrl_new_custom(&pr2k->hdl, &pr2k_detect_mode, NULL);
	...
	if (pr2k->format < FMT_UNDEFINED && pr2k->rst_gpio != NULL) {
		/* s_power always returns 0 */
		/* pixelplus-cn suggests that 5ms should be enough and
		 * we raise up to 20ms for tolerance.
		 */
		pr2k_s_power(&pr2k->sd, false);
		msleep(20);
		pr2k_s_power(&pr2k->sd, true);
		ret = pr2k_format_cfg(&pr2k->sd);
		if (ret) {
			dev_err(pr2k->dev, "init with format=%d failed\n", pr2k->format);
			goto free_hdl;
		}
	}

	INIT_DELAYED_WORK(&pr2k->detect_work, pr2k_detect_work);

	if (pr2k->detect_mode == DETECT_MODE_POLLING) {
		ret = pr2k_setup_polling_mode(pr2k);
		if (ret) {
		...
		schedule_delayed_work(&pr2k->detect_work, msecs_to_jiffies(500));
	} else {
		dev_info(dev, "Signal detection is disabled\n");
	}

	pr2k_sysfs_create(&pr2k->sd);

	if (IS_ENABLED(CONFIG_DEBUG_FS))
		pr2k_setup_dbgfs(pr2k);

	dev_info(dev, "pr2k probe completed successfully\n");
	return 0;

free_hdl:
	v4l2_ctrl_handler_free(&pr2k->hdl);
...
static void pr2k_remove(struct i2c_client *client)
{
	struct v4l2_subdev *sd = i2c_get_clientdata(client);
	struct pr2000 *pr2k = to_pr2k(sd);

	pr2k_sysfs_remove(sd);
	/* Cancel delayed work if detection is enabled */
	if (pr2k->detect_mode != DETECT_MODE_DISABLED)
		cancel_delayed_work_sync(&pr2k->detect_work);

	/* Destroy mutex */
	mutex_destroy(&pr2k->detect_lock);
}
