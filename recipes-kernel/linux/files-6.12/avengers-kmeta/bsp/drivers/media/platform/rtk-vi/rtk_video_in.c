...
#define KENT_MAX_CH    CH_1
#define PRINCE_MAX_CH  CH_3
...
#define CASCAD_VI_1_VIDEO_NR  21
#define CASCAD_VI_2_VIDEO_NR  22
#define CASCAD_VI_3_VIDEO_NR  23
...
static struct v4l2_queryctrl rtk_vi_v4l2_ctrls[] = {
	{
		.id = RTK_V4L2_CID_VI_MIRROR,
...
static const u32 rtk_vi_ctrls_num = ARRAY_SIZE(rtk_vi_v4l2_ctrls);
...
static int rtk_vi_get_video_nr(struct rtk_vi *vi)
{
	...
	case CH_1:
		if (vi->cascade_mode == CASCADE_OFF)
			...
			video_nr = CASCAD_VI_1_VIDEO_NR;
		break;
	case CH_2:
		video_nr = CASCAD_VI_2_VIDEO_NR;
		break;
	case CH_3:
		video_nr = CASCAD_VI_3_VIDEO_NR;
		break;
	default:
		video_nr = -1;
		break;
				...
				struct v4l2_format *f)
{
	...
	wait_event_interruptible_timeout(vi->detect_wait,
		vi->detect_done, msecs_to_jiffies(700));
	...
	if (!vi->detect_done) {
		if (vi->soc.id == SOC_ID_PRINCE) {
			vi->hw_ops->dump_registers(vi, vi->soc.fc, vi->soc.cascade);
			vi->hw_ops->dump_registers(vi, vi->soc.decoder, vi->soc.bt_p_img);
		} else if (vi->soc.id == SOC_ID_KENT) {
			vi->hw_ops->dump_registers(vi, vi->soc.fc, vi->soc.crc_ctrl);
		}
	}
				     ...
				     struct v4l2_dv_timings *timings)
{
	...
	if (timings->bt.width < VI_MIN_WIDTH || timings->bt.width > VI_MAX_WIDTH ||
		timings->bt.height < VI_MIN_HEIGHT || timings->bt.height > VI_MAX_HEIGHT)
		return -EINVAL;
	...
	vi->src_width = timings->bt.width;
	vi->src_height = timings->bt.height;
	vi->is_interlace = timings->bt.interlaced ? 1:0;

	dev_info(vi->dev, "Set src width=%u height=%u interlaced=%u\n",
		vi->src_width, vi->src_height, vi->is_interlace);
				     ...
				     struct v4l2_dv_timings *timings)
{
	...
	timings->type = V4L2_DV_BT_656_1120;
	timings->bt.width = vi->src_width;
	timings->bt.height = vi->src_height;
	timings->bt.interlaced = (u32)vi->is_interlace;

	dev_dbg(vi->dev, "%s width=%u height=%u interlaced=%u\n", __func__,
		timings->bt.width, timings->bt.height, timings->bt.interlaced);
...
static int rtk_vi_vb2_ioctl_dqbuf(struct file *file, void *priv,
...
static int rtk_vi_vidioc_queryctrl(struct file *file, void *fh,
				struct v4l2_queryctrl *queryctrl)
{
	...
	dev_dbg(vi->dev, "v4l2_queryctrl id=0x%08x\n", queryctrl->id);

	if (V4L2_CTRL_ID2WHICH(queryctrl->id) != V4L2_CID_PRIVATE_BASE)
		return ret;

	for (i = 0; i < rtk_vi_ctrls_num; i++) {
		if (rtk_vi_v4l2_ctrls[i].id == queryctrl->id) {
			memcpy(queryctrl, &rtk_vi_v4l2_ctrls[i],
			       sizeof(struct v4l2_queryctrl));
			if (rtk_vi_v4l2_ctrls[i].id == RTK_V4L2_CID_VI_MIRROR)
				queryctrl->default_value = (vi->ext_flags & RTK_EXT_DFT_MIRROR) ? true:false;
			ret = 0;
			break;
		}
	}
...
static int rtk_vi_vidioc_g_ctrl(struct file *file, void *fh,
				struct v4l2_control *ctrl)
{
	struct rtk_vi *vi = video_drvdata(file);

	if (ctrl->id != RTK_V4L2_CID_VI_MIRROR)
		return -EINVAL;

	ctrl->value = (vi->en_mirror == true);

	return 0;
}

static int rtk_vi_vidioc_s_ctrl(struct file *file, void *fh,
				struct v4l2_control *ctrl)
{
	struct rtk_vi *vi = video_drvdata(file);
	...
	if (vi->soc.id == SOC_ID_KENT)
		return -ESRCH;

	dev_info(vi->dev, "Set v4l2_control id=0x%08x value=%d\n",
		ctrl->id, ctrl->value);
...
static const struct v4l2_ioctl_ops rtk_vi_video_ioctls = {
	...
	/* Control handling */
	.vidioc_queryctrl = rtk_vi_vidioc_queryctrl,
	.vidioc_g_ctrl = rtk_vi_vidioc_g_ctrl,
	.vidioc_s_ctrl = rtk_vi_vidioc_s_ctrl,
};
...
static void rtk_vi_recovery_reset(struct rtk_vi *vi, u8 ch_index)
{
	...
	/* Skip reset if not stream on yet */
	if (vi->sequence == 0)
		return;
...
static irqreturn_t rtk_vi_irq_handler(int irq, void *dev_id)
{
	...
	if ((inst_a & VI_INTST_mac_ovr_mask) ||
		...
		dev_err_ratelimited(vi->dev, "Error: DMA out of range, inst_a=0x%08x inst_b=0x%08x\n",
			inst_a, inst_b);
...
static ssize_t vi_dbg_show(struct device *dev,
...
static int rtk_vi_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(dev->of_node, "pad-cfg",
				&vi->pad_cfg);
	if (ret < 0 || vi->pad_cfg > PAD_CFG_11)
		vi->pad_cfg = PAD_CFG_00;
	...
	ret = of_property_read_u32(dev->of_node, "cascade-mode",
				&vi->cascade_mode);
	if (ret < 0 || vi->cascade_mode > CASCADE_4CH || vi->separate_mode != SEP_NONE)
	...
	if (soc_device_match(rtk_soc_prince))
		vi->soc.id = SOC_ID_PRINCE;
	else
		vi->soc.id = SOC_ID_KENT;

	if ((vi->soc.id == SOC_ID_PRINCE) &&
		(vi->cascade_mode != CASCADE_OFF))
		vi->max_ch = PRINCE_MAX_CH;
	else
		vi->max_ch = KENT_MAX_CH;

	if (vi->soc.id == SOC_ID_KENT)
		vi->ext_flags = vi->ext_flags & (~RTK_EXT_DFT_MIRROR);

	if (vi->ext_flags)
		dev_info(dev, "ext_flags=0x%08x\n", vi->ext_flags);
	...
	if (ch_index > vi->max_ch) {
		dev_err(dev, "Failed to set ch_index, ch_index %u > max_ch %u\n",
			ch_index, vi->max_ch);
		ret = -EINVAL;
		goto err_exit;
	}
	...
	dev_info(vi->dev, "bt_mode=%u pad_cfg=%u separate_mode=%u cascade_mode=%u\n",
		vi->bt_mode, vi->pad_cfg, vi->separate_mode, vi->cascade_mode);
...
static void rtk_vi_remove(struct platform_device *pdev)
{
	// TODO:
}
