...
static int rtk_mipicsi_check_hw_init(struct rtk_mipicsi *mipicsi)
{
	int ret;
...
static int phy_check_get(void *data, u64 *val)
{
	...
	if (rtk_mipicsi_check_hw_init(mipicsi)) {
		*val = 0;
		goto exit;
	}
...
static int prbs_chk_mode_set(void *data, u64 val)
{
	struct rtk_mipicsi *mipicsi = data;
	struct rtk_mipicsi_debug *debug = &mipicsi->debug;
...
static int prbs_chk_mode_get(void *data, u64 *val)
{
	struct rtk_mipicsi *mipicsi = data;
	struct rtk_mipicsi_debug *debug = &mipicsi->debug;

	*val = (u64)debug->prbs_chk_mode;

	return 0;
}

DEFINE_DEBUGFS_ATTRIBUTE(mipicsi_dbg_prbs_chk_mode_fops, prbs_chk_mode_get, prbs_chk_mode_set,
			 "%llu\n");

static int mipicsi_dbg_prbs_chk_result_show(struct seq_file *s, void *v)
{
	struct rtk_mipicsi *mipicsi = s->private;
	...
	ret = mipicsi->hw_ops->prbs_check(mipicsi, debug->prbs_chk_mode);
	if (ret == 0)
		seq_puts(s, "FAIL: No signal\n");
	else if (ret < 0)
		seq_printf(s, "FAIL: err_cnt=%d\n", -ret);
	else
		seq_printf(s, "PASS: det_cnt=%d\n", ret);

exit:
	return 0;
}

DEFINE_SHOW_ATTRIBUTE(mipicsi_dbg_prbs_chk_result);
...
void rtk_mipicsi_setup_dbgfs(struct rtk_mipicsi *mipicsi)
{
	...
	if ((mipicsi->ch_index == CH_0) || (mipicsi->conf->en_group_dev) ||
		...
		debug->phy_check_ms = DEFAULT_PHY_CHECK_MS;
		debug->prbs_chk_mode = PRBS7_LANE0;
		...
		debugfs_create_file("prbs_chk_mode", 0644, debug->debugfs_dir, mipicsi,
			&mipicsi_dbg_prbs_chk_mode_fops);

		debugfs_create_file("prbs_chk_result", 0444, debug->debugfs_dir, mipicsi,
			&mipicsi_dbg_prbs_chk_result_fops);
	}
}
