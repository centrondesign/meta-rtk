			  ...
			  unsigned int width, unsigned int height)
{
	...
	case V4L2_PIX_FMT_RGB24:
		...
		break;
	default:
		npp_err("%s, Not support pixelformat = %c%c%c%c\n", __func__,
	...
	npp_dbg("%s, pix_mp->pixelformat = %c%c%c%c\n", __func__,
		pix_mp->pixelformat&0xFF, (pix_mp->pixelformat>>8)&0xFF,
		(pix_mp->pixelformat>>16)&0xFF, (pix_mp->pixelformat>>24)&0xFF);
	npp_dbg("%s, pix_mp->plane_fmt[0].bytesperline = %d\n", __func__,
		 pix_mp->plane_fmt[0].bytesperline);
	npp_dbg("%s, pix_mp->plane_fmt[0].sizeimage = %d\n", __func__,
...
static int npp_try_fmt_cap(struct v4l2_fh *fh, struct v4l2_format *f)
{
	...
	if (!npp_fmt) {
		npp_err("%s, unknown capture format %c%c%c%c width=%d height=%d quantization=%d\n",
		__func__,
		pix_mp->pixelformat&0xff, (pix_mp->pixelformat>>8)&0xff,
		(pix_mp->pixelformat>>16)&0xff, (pix_mp->pixelformat>>24)&0xff,
	...
	npp_dbg("%s, pixelformat=%c%c%c%c width=%d height=%d\n", __func__,
			pix_mp->pixelformat&0xFF, (pix_mp->pixelformat>>8)&0xFF,
			(pix_mp->pixelformat>>16)&0xFF, (pix_mp->pixelformat>>24)&0xFF,
...
static int npp_s_fmt_cap(struct v4l2_fh *fh, struct v4l2_format *f)
{
	...
	if (ret) {
		npp_err("%s, npp_try_fmt_cap fail", __func__);
		return ret;
	}
...
static int _npp_set_plock_unlocked(int offset)
{
	...
	if ((offset >= 0) && (offset < PLOCK_MAX_BUFFER_INDEX)) {
		...
		*lock_p_vaddr = PLOCK_QPEND;
		...
		npp_err("[%s] unexpected lock offset\n", __func__);
		return -1;
	}
	return 0;
}
...
static int npp_stop_streaming(struct vb2_queue *q)
{
	...
	if (ret < 0) {
		npp_err("[%s] cancel_work_sync failed\n", __func__);
		return ret;
	}
	...
	if (ret < 0) {
		npp_err("[%s] npp_run_stop failed\n", __func__);
		return ret;
	}

	for (i = 0; i < CAP_BUF_COUNT_MAX; i++) {
		npp_dbg("[%s] before stream off nppctx->plock_offset[%d] =%d\n",
			__func__, i, nppctx->plock_offset[i]);
		npp_dbg("[%s] before stream off nppctx->buffer_status[%d] =%d\n",
			__func__, i, nppctx->buffer_status[i]);
						...
						unsigned char format, int bufferNum)
{
	...
	if (vb->index >= CAP_BUF_COUNT_MAX) {
		npp_err("%s, index =%d, Out off supporting buffer num, only support %d buffers\n",
	...
	if (ctx->plock_offset[vb->index] == -1 && ctx->buffer_status[vb->index] == DEFAULT) {
		...
		npp_info("%s, bufferNum = %d,width = %d,height = %d,format = %d\n", __func__, bufferNum,
		capture_info.width, capture_info.height, capture_info.format);
		npp_info("%s, crop_x = %d,crop_y = %d,crop_width = %d,crop_height = %d\n", __func__,
		capture_info.crop_x, capture_info.crop_y, capture_info.crop_width, capture_info.crop_height);

		ret = _npp_add_wb_buffer(dev->dev, &capture_info, vb->index, ctx->rpc_opt, bufferNum);
		if (ret < 0)
			return ret;
...
static struct npp_buf_object *_npp_buffer_create(struct videc_dev *v_dev)
{
...
static int npp_get_plock_offset(dma_addr_t paddr)
{
	dma_addr_t lock_base_paddr = lock_info->paddr;
	int offset = -1;

	if (paddr < lock_base_paddr) {
		npp_err("[%s] unexpected lock paddr\n", __func__);
		goto finish;
	}
...
static int npp_get_frame(struct npp_ctx *nppctx)
{
	int ret;
	int lock_offset;
	...
	if (lock_offset >= 0) {
		...
		if(nppctx->excution_count_pts > 0)
			nppctx->ptsDelta = (((uint64_t) object.PTSH << 32) | object.PTSL) - nppctx->ptsLast;
		nppctx->ptsLast = (((uint64_t) object.PTSH << 32) | object.PTSL);
		nppctx->ptsSum += nppctx->ptsDelta;
		nppctx->excution_count_pts++;
		npp_dbg("ptsDelta:%llu, ptsLast:%llu, ptsSum:%llu, excution_count:%d\n",
		nppctx->ptsDelta, nppctx->ptsLast, nppctx->ptsSum, nppctx->excution_count_pts);
#endif

#ifdef ENABLE_NPP_MEASURE_VSYNC
		vsyncNew = ktime_get_ns();
		if(nppctx->excution_count_vsync > 0)
			nppctx->vsyncDelta = vsyncNew - nppctx->vsyncLast;
		nppctx->vsyncLast = vsyncNew;
		nppctx->vsyncSum += nppctx->vsyncDelta;
		nppctx->excution_count_vsync++;
		npp_dbg("vsyncDelta:%llu, vsyncLast:%llu, vsyncSum:%llu, excution_count:%d\n",
		nppctx->vsyncDelta, nppctx->vsyncLast, nppctx->vsyncSum, nppctx->excution_count_vsync);
#endif
	}
...
static void npp_get_frame_worker(struct work_struct *frame_work)
{
	...
	if (empty) {
		//npp_dbg( "%s, list empty , skip frame\n", __func__);
		return;
	}
	...
	npp_dbg("%s, VB2 done,buf_index = %d seq_cap = %u timestamp(ns) = %llu\n",
...
int npp_object_dma_allcate_capture( struct drv_npp_ctx *ctx, int bufferNum)
{
	...
	if(buf_obj->vaddr != NULL) {
		npp_err("%s, buf_obj->vaddr already allocate\n", __func__);
		return -ENOMEM;
	}
	...
	if (!buf_obj->vaddr) {
		npp_err("%s, failed to allocate buf_obj buffer\n", __func__);
		return -ENOMEM;
	}
...
void npp_set_capture_rheap_pools(struct videc_dev *v_dev)
{
	unsigned int flags;
	char *name = "rtk_media_heap";
...
void *npp_alloc_context_capture(struct v4l2_fh *fh, struct videc_dev *v_dev)
{
	...
	if (!ctx) {
		npp_err("%s, allocate npp ctx failed\n", __func__);
		return NULL;
	}

	ctx->cap_q_buf_obj = _npp_buffer_create(v_dev);

	if (!ctx->cap_q_buf_obj) {
		npp_err("%s, allocate npp cap_q_buf_obj failed\n", __func__);
		kfree(ctx);
		return NULL;
	}
...
static int _npp_ept_deinit(struct rtk_krpc_ept_info *krpc_ept_info)
{
	...
	krpc_ept_info_put(krpc_ept_info);
	return 0;
}
...
static int _npp_send_rpc(struct device *dev, struct rtk_krpc_ept_info *krpc_ept_info,
...
static int _npp_rpc_init_ringbuffer(struct RPC_RINGBUFFER *ringbuffer, int opt)
{
	...
	unsigned int offset, rpc_ret = S_OK;
	unsigned int command;
	struct RPCRES_LONG *retval;
	void *rpc_buffer;

	rpc_buffer = rpc_info->vaddr;
	memset(rpc_buffer, 0, RPC_BUFFER_SIZE);
	rpc = rpc_buffer;
...
int npp_rpc_buffer_allocate(struct videc_dev *dev)
{
	...
	if (!rpc_info->vaddr) {
		npp_err("%s, allocate RPC buffer failed\n", __func__);
		npp_rpc_buffer_release(dev);
		return -1;
	}
...
int npp_ringbuffer_buffer_allocate(struct videc_dev *dev)
{
	...
	if (!rb_icq){
		npp_err("%s, allocate rb_icq buffer failed\n", __func__);
		goto free_buffer;
	}
	...
	if (!rb_icq->vaddr) {
		npp_err("%s, allocate rb_icq dma buffer failed\n", __func__);
		goto free_buffer;
	}
	...
	if (!rb_wb){
		npp_err("%s, allocate rb_wb buffer failed\n", __func__);
		goto free_buffer;
	}
	...
	if (!rb_wb->vaddr) {
		npp_err("%s, allocate rb_wb dma buffer failed\n", __func__);
		goto free_buffer;
	}
...
free_buffer:
	npp_ringbuffer_buffer_release(dev);
	return -1;
}
...
static int _npp_ringbuffer_initialize(struct videc_dev *dev)
{
	...
	if (ret < 0) {
		npp_err("%s, _npp_init_ringbuffer failed\n", __func__);
		return ret;
	}
	...
	if (ret < 0) {
		npp_err("%s, _npp_init_ringbuffer WB_PINID failed\n", __func__);
		return ret;
	}
...
int npp_destroy(struct videc_dev *dev)
{
	...
	if (ret < 0) {
		npp_err("%s, _npp_rpc_npp_destroy failed\n", __func__);
		retval = ret;
	}

	ret = _npp_rpc_destroy(dev->rpc_opt);
	if (ret < 0) {
		npp_err("%s, _npp_rpc_destroy failed\n", __func__);
		retval = ret;
	}
	...
	if (IS_ENABLED(CONFIG_RPMSG_RTK_RPC)) {
		if (rpc_opt == RPC_AUDIO) {
			...
			if (IS_ERR(rpc_info->acpu_ept_info)) {
				...
				if (ret == -EPROBE_DEFER) {
					npp_warn("%s, krpc ept info not ready, retry\n", __func__);
				} else {
					npp_warn("%s, cannot get hifi ept info: %d\n", __func__, ret);
					rpc_info->acpu_ept_info = NULL;
				}
				return ret;
			}

			ret = _npp_ept_init(rpc_info->acpu_ept_info);
			if (ret) {
				npp_err("%s _npp_ept_init fail: %d\n", __func__, ret);
				if (rpc_info->acpu_ept_info)
			...
			if (IS_ERR(rpc_info->hifi_ept_info)) {
				...
				if (ret == -EPROBE_DEFER) {
					npp_warn("%s, hifi ept info not ready, retry\n", __func__);
				} else {
					npp_warn("%s, cannot get hifi ept info: %d\n", __func__, ret);
					rpc_info->hifi_ept_info = NULL;
				}
				return ret;
			}

			ret = _npp_ept_init(rpc_info->hifi_ept_info);
			if (ret) {
				npp_err("%s _npp_ept_init fail: %d\n", __func__, ret);
				if (rpc_info->hifi_ept_info)
					krpc_ept_info_put(rpc_info->hifi_ept_info);
				return ret;
			}
		}
	} else {
		npp_warn("%s, CONFIG_RPMSG_RTK_RPC not enable\n", __func__);
		return -1;
	}
...
int npp_rpc_init_check(struct videc_dev *dev)
{
	...
	if (ret < 0) {
		npp_warn("%s, _npp_krpc_init not finished ret = %d, rpc not ready\n", __func__, ret);
	} else {
		if (dev->rpc_opt == RPC_AUDIO)
			_npp_ept_deinit(rpc_info->acpu_ept_info);
		else
...
int npp_initialize(struct videc_dev *dev)
{
	...
	ret = _npp_krpc_init(dev->rpc_opt);
	if (ret < 0) {
		npp_err("%s, _npp_krpc_init failed\n", __func__);
		return ret;
	}

	ret = _npp_rpc_create(dev->rpc_opt);
	if (ret < 0) {
		npp_err("%s, _npp_rpc_create failed\n", __func__);
		goto rpc_deinit;
	}

	ret = _npp_rpc_npp_init(dev->rpc_opt);
	if (ret < 0) {
		npp_err("%s, _npp_rpc_npp_init failed\n", __func__);
		goto rpc_destroy;
	}

	ret = _npp_ringbuffer_initialize(dev);
	if (ret < 0) {
		npp_err("%s, _npp_ringbuffer_initialize failed\n", __func__);
		goto npp_destroy;
	}
...
static int _npp_querydisplaywin(int opt)
{
	...
	unsigned int offset, rpc_ret = S_OK, result;
	unsigned int command;
	void *rpc_buffer;
...
int npp_run_start(struct videc_dev *dev)
{
	...
	ret = _npp_allocate_lock_buffer(dev->dev);
	if (ret < 0) {
		npp_err("%s, _npp_allocate_lock_buffer failed\n", __func__);
		return -ENOMEM;
	}

	ret = _npp_rpc_config(VSYNC_V1_NPU_PP, dev->rpc_opt);
	if (ret < 0) {
		npp_err("%s, _npp_rpc_config failed\n", __func__);
		_npp_allocate_lock_buffer(dev->dev);
		return -EIO;
	}
...
int npp_run_stop(struct videc_dev *dev)
{
	...
	_npp_free_lock_buffer(dev->dev);

	/* Pause */
	ret = _npp_rpc_pause(dev->rpc_opt);
	if (ret < 0) {
		npp_err("%s, _npp_rpc_pause failed\n", __func__);
		return ret;
	}
	...
	if (ret < 0) {
		npp_err("%s, _npp_rpc_stop failed\n", __func__);
		return ret;
	}
...
#ifdef ENABLE_NPP_MEASURE_PTS
void npp_pts_measure(void *ctx)
{
	struct npp_ctx *ctx_npp = (struct npp_ctx *)ctx;
	uint64_t avg_tick, avg_us, avg_ms_int, avg_ms_frac;

	avg_tick = ctx_npp->ptsSum/(ctx_npp->excution_count_pts-1);
	avg_us = div_u64(avg_tick * 1000000ULL, 90000ULL);

	avg_ms_int  = avg_us / 1000;
	avg_ms_frac = avg_us % 1000;

	npp_dbg("%s, avg_tick: %llu, ptsSum:%llu, excution_count:%d\n",
		__func__, avg_tick,  ctx_npp->ptsSum, ctx_npp->excution_count_pts);

	npp_info("%s, Average Execution Time: %llu.%03llu ms, avg_us =%llu\n",
		__func__, avg_ms_int, avg_ms_frac, avg_us);
}
#endif

#ifdef ENABLE_NPP_MEASURE_VSYNC
void npp_vsync_measure(void *ctx)
{
	struct npp_ctx *ctx_npp = (struct npp_ctx *)ctx;
	uint64_t avg_ns, avg_us, avg_ms_int, avg_ms_frac;

	avg_ns = (ctx_npp->vsyncSum)/(ctx_npp->excution_count_vsync-1);
	avg_us = div_u64(avg_ns, 1000ULL);

	avg_ms_int  = avg_us / 1000;
	avg_ms_frac = avg_us % 1000;

	npp_dbg("%s, avg_ns: %llu, vsyncSum:%llu, excution_count:%d\n",
		__func__, avg_ns,  ctx_npp->vsyncSum, ctx_npp->excution_count_vsync);
