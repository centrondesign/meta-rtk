...
#include <trace/events/rtk_mipi_csi_trace.h>
		...
		u8 top_index, u8 lane_flags)
{
	 ...
	 * All tap = 0xF*16*UI (Maximum value of hi_interval is 0xF for KENT)
	 * PRINCE supports up to 0xFF for extended timing flexibility.
	 * The time of one UI = 1/1.5G = 0.66667ns
	 */
	if (mipicsi->soc_id == SOC_ID_KENT) {
		regmap_write(mipicsi->phyreg, MIPI0_DPHY_HSTERM0 + offset,
			MIPI0_DPHY_HSTERM0_hl_interval_1(mipicsi->hl_interval) |
			MIPI0_DPHY_HSTERM0_hl_interval_0(mipicsi->hl_interval));
		regmap_write(mipicsi->phyreg, MIPI0_DPHY_HSTERM1 + offset,
			MIPI0_DPHY_HSTERM1_hl_interval_3(mipicsi->hl_interval) |
			MIPI0_DPHY_HSTERM1_hl_interval_2(mipicsi->hl_interval));
	} else {
		regmap_write(mipicsi->phyreg, MIPI0_DPHY_HSTERM0 + offset,
			P_MIPI0_DPHY_HSTERM0_hl_interval_1(mipicsi->hl_interval) |
			P_MIPI0_DPHY_HSTERM0_hl_interval_0(mipicsi->hl_interval));
		regmap_write(mipicsi->phyreg, MIPI0_DPHY_HSTERM1 + offset,
			P_MIPI0_DPHY_HSTERM1_hl_interval_3(mipicsi->hl_interval) |
			P_MIPI0_DPHY_HSTERM1_hl_interval_2(mipicsi->hl_interval));
	}
		...
		u8 top_index, u8 lane_flags)
{
	...
	if (mipicsi->soc_id == SOC_ID_KENT) {
		reg_addr = MIPI0_APHY_REG43;
		lane0_mask = MIPI0_APHY_REG43_offset_cal_lane0_mask;
		lane1_mask = MIPI0_APHY_REG43_offset_cal_lane1_mask;
		lane2_mask = MIPI0_APHY_REG43_offset_cal_lane2_mask;
		lane3_mask = MIPI0_APHY_REG43_offset_cal_lane3_mask;
		ck_mask = MIPI0_APHY_REG43_offset_cal_ck_mask;
	} else {
		reg_addr = MIPI0_APHY_REG54;
		lane0_mask = MIPI0_APHY_REG54_reg_offset_forcek_lane0_mask;
		lane1_mask = MIPI0_APHY_REG54_reg_offset_forcek_lane1_mask;
		lane2_mask = MIPI0_APHY_REG54_reg_offset_forcek_lane2_mask;
		lane3_mask = MIPI0_APHY_REG54_reg_offset_forcek_lane3_mask;
		ck_mask = MIPI0_APHY_REG54_reg_offset_forcek_ckk_mask;
	}

	offset = get_phy_offset(top_index);
	regmap_read(mipicsi->phyreg,  reg_addr + offset, &reg_val);
	...
	regmap_write(mipicsi->phyreg, reg_addr + offset,
			reg_val | cal_mask);
		...
		u8 top_index, u8 lane_flags, u8 enable)
{
	...
	if ((mipicsi->soc_id != SOC_ID_KENT) && (top_index > MIPI_TOP_0))
		return;
		...
		u8 top_index, u8 rtune)
{
	...
	if (mipicsi->soc_id == SOC_ID_KENT) {
		...
		rtk_mask_write(mipicsi->phyreg, MIPI0_APHY_REG56 + offset,
			MIPI0_APHY_REG56_reg_rx_vgagain_stg1_ckk_mask |
			MIPI0_APHY_REG56_reg_rx_vgagain_stg1_lane3_mask |
			MIPI0_APHY_REG56_reg_rx_vgagain_stg1_lane2_mask |
			MIPI0_APHY_REG56_reg_rx_vgagain_stg1_lane1_mask |
			MIPI0_APHY_REG56_reg_rx_vgagain_stg1_lane0_mask,
			MIPI0_APHY_REG56_reg_rx_vgagain_stg1_ckk(rtune_gray) |
			MIPI0_APHY_REG56_reg_rx_vgagain_stg1_lane3(rtune_gray) |
			MIPI0_APHY_REG56_reg_rx_vgagain_stg1_lane2(rtune_gray) |
			MIPI0_APHY_REG56_reg_rx_vgagain_stg1_lane1(rtune_gray) |
			MIPI0_APHY_REG56_reg_rx_vgagain_stg1_lane0(rtune_gray));
	}
}
		...
		u8 top_index)
{
	...
	if (mipicsi->soc_id == SOC_ID_KENT) {
		...
		regmap_read(mipicsi->phyreg, MIPI0_APHY_REG56 + offset, &reg_val);
		rtune_gray = MIPI0_APHY_REG56_get_reg_rx_vgagain_stg1_ckk(reg_val);
	}
...
static void rtk_mipi_set_swap(struct rtk_mipicsi *mipicsi,
	u8 top_index, u32 swap_config)
{
	u32 offset;
	u32 lane_sel;
	u8 pn_swap_clk;
	...
	if (swap_config == (FLAG_NO_LANE_SWAP | SWAP_SEQ_LANE_0_1_2_3)) {
		dev_info(mipicsi->dev, "No SWAP, skip TOP_%u SWAP setting\n", top_index);
		return;
	}

	pn_swap_clk = (swap_config & FLAG_LANE_CK_SWAP) ? 1:0;
	pn_swap_lane3 = (swap_config & FLAG_LANE_3_SWAP) ? 1:0;
	pn_swap_lane2 = (swap_config & FLAG_LANE_2_SWAP) ? 1:0;
	pn_swap_lane1 = (swap_config & FLAG_LANE_1_SWAP) ? 1:0;
	pn_swap_lane0 = (swap_config & FLAG_LANE_0_SWAP) ? 1:0;

	lane_sel = swap_config & 0xFF;

	if ((lane_sel != SWAP_SEQ_LANE_0_1_2_3) && (lane_sel != SWAP_SEQ_LANE_3_1_2_0) &&
		(lane_sel != SWAP_SEQ_LANE_1_0_2_3) && (lane_sel != SWAP_SEQ_LANE_3_0_2_1) &&
		(lane_sel != SWAP_SEQ_LANE_0_3_2_1) && (lane_sel != SWAP_SEQ_LANE_1_3_2_0) &&
		(lane_sel != SWAP_SEQ_LANE_0_2_1_3) && (lane_sel != SWAP_SEQ_LANE_3_2_1_0) &&
		(lane_sel != SWAP_SEQ_LANE_2_0_1_3) && (lane_sel != SWAP_SEQ_LANE_3_0_1_2) &&
		(lane_sel != SWAP_SEQ_LANE_0_3_1_2) && (lane_sel != SWAP_SEQ_LANE_2_3_1_0) &&
		(lane_sel != SWAP_SEQ_LANE_2_1_0_3) && (lane_sel != SWAP_SEQ_LANE_3_1_0_2) &&
		(lane_sel != SWAP_SEQ_LANE_1_2_0_3) && (lane_sel != SWAP_SEQ_LANE_3_2_0_1) &&
		(lane_sel != SWAP_SEQ_LANE_2_3_0_1) && (lane_sel != SWAP_SEQ_LANE_1_3_0_2) &&
		(lane_sel != SWAP_SEQ_LANE_0_1_3_2) && (lane_sel != SWAP_SEQ_LANE_2_1_3_0) &&
		(lane_sel != SWAP_SEQ_LANE_1_0_3_2) && (lane_sel != SWAP_SEQ_LANE_2_0_3_1) &&
		(lane_sel != SWAP_SEQ_LANE_0_2_3_1) && (lane_sel != SWAP_SEQ_LANE_1_2_3_0)) {
		dev_err(mipicsi->dev, "Wrong swap_config=0x%08x, skip TOP_%u SWAP setting\n",
			swap_config, top_index);
		return;
	}
	...
	rtk_mask_write(mipicsi->phyreg, MIPI0_APHY_REG43 + offset,
		...
		MIPI0_APHY_REG43_pn_swap_clk(pn_swap_clk) |
		MIPI0_APHY_REG43_pn_swap_lane3(pn_swap_lane3) |
		MIPI0_APHY_REG43_pn_swap_lane2(pn_swap_lane2) |
		MIPI0_APHY_REG43_pn_swap_lane1(pn_swap_lane1) |
		MIPI0_APHY_REG43_pn_swap_lane0(pn_swap_lane0));

	/* swap_config Bit[7:0]  */
	rtk_mask_write(mipicsi->phyreg, MIPI0_DPHY_LANE_SEL + offset,
		MIPI0_DPHY_LANE_SEL_lane3_sel_mask |
		MIPI0_DPHY_LANE_SEL_lane2_sel_mask |
		MIPI0_DPHY_LANE_SEL_lane1_sel_mask |
		MIPI0_DPHY_LANE_SEL_lane0_sel_mask,
		lane_sel);

	dev_info(mipicsi->dev, "Set TOP_%u swap_config=0x%08x\n", top_index, swap_config);
}
		...
		const struct rtk_mipicsi_phy *phy, u8 top_index, u8 lane_flags)
{
	...
	rtk_mask_write(mipicsi->phyreg, MIPI0_APHY_REG0 + offset,
		MIPI0_APHY_REG0_hs_cm_mask,
		MIPI0_APHY_REG0_hs_cm(phy->hs_cm));
	...
	rtk_mask_write(mipicsi->phyreg, MIPI0_APHY_REG13 + offset,
		...
		MIPI0_APHY_REG13_offset_range_lane2(phy->offset_range_lane2) |
		MIPI0_APHY_REG13_offset_range_lane1(phy->offset_range_lane1) |
		MIPI0_APHY_REG13_offset_range_lane0(phy->offset_range_lane0));
	...
	rtk_mipi_set_swap(mipicsi, top_index, mipicsi->swap_config);
...
static void rtk_mipi_app_size_cfg(struct rtk_mipicsi *mipicsi, u8 ch_index)
{
	...
	dev_info(mipicsi->dev, "%s channel%u src_width=%u src_height=%u dst_width=%u dst_height=%u  mode=%s\n",
		__func__, ch_index,
...
exit:
	...
	trace_mipicsi_app_size_cfg(ch_index, mipicsi->src_width, mipicsi->src_height,
		mipicsi->dst_width, mipicsi->dst_height, mipicsi->mode,
		mipicsi->line_pitch, mipicsi->header_pitch, mipicsi->video_size);
}
		...
		u8 ch_index)
{
	...
	case CH_2:
		offset = get_phy_offset(MIPI_TOP_0);
		regmap_read(mipicsi->phyreg,  MIPI0_DPHY_FRAME_CNT_CH2 + offset, &reg_val);
		frame_cnt = reg_val & 0xFF;
		regmap_read(mipicsi->phyreg,  MIPI0_DPHY_LINE_CNT_CH2 + offset, &reg_val);
		line_cnt = reg_val & 0xFFF;
		regmap_read(mipicsi->phyreg,  MIPI0_DPHY_PIX_CNT_CH2 + offset, &reg_val);
		pixel_cnt = reg_val & 0xFFF;
		regmap_read(mipicsi->phyreg,  MIPI0_DPHY_FRAME_HEIGHT_CH2 + offset, &reg_val);
		frame_height = reg_val & 0xFFF;
		regmap_read(mipicsi->phyreg,  MIPI0_DPHY_FRAME_WIDTH_CH2 + offset, &reg_val);
		frame_width = reg_val & 0xFFF;
		regmap_read(mipicsi->phyreg,  MIPI0_DPHY_MIPI2ISP_DEBUG + offset, &reg_val);
		errcnt = MIPI0_DPHY_MIPI2ISP_DEBUG_get_errcnt(reg_val);

		/* Clear count */
		regmap_write(mipicsi->phyreg, MIPI0_DPHY_FRAME_CNT_CH2 + offset,
			MIPI0_DPHY_FRAME_CNT_CH2_frame_cnt_clr_ch2(1));
		regmap_write(mipicsi->phyreg,  MIPI0_DPHY_LINE_CNT_CH2 + offset,
			MIPI0_DPHY_LINE_CNT_CH2_line_cnt_clr_ch2(1));
		regmap_write(mipicsi->phyreg,  MIPI0_DPHY_PIX_CNT_CH2 + offset,
			MIPI0_DPHY_PIX_CNT_CH2_pix_cnt_clr_ch2(1));
		regmap_write(mipicsi->phyreg,  MIPI0_DPHY_FRAME_HEIGHT_CH2 + offset,
			MIPI0_DPHY_FRAME_HEIGHT_CH2_frame_height_clr_ch2(1));
		regmap_write(mipicsi->phyreg,  MIPI0_DPHY_FRAME_WIDTH_CH0 + offset,
			MIPI0_DPHY_FRAME_WIDTH_CH2_frame_width_clr_ch2(1));
		regmap_write(mipicsi->phyreg,  MIPI0_DPHY_MIPI2ISP_DEBUG + offset,
			MIPI0_DPHY_MIPI2ISP_DEBUG_errcnt_clr(1));
		break;
	case CH_3:
		offset = get_phy_offset(MIPI_TOP_0);
		regmap_read(mipicsi->phyreg,  MIPI0_DPHY_FRAME_CNT_CH3 + offset, &reg_val);
		frame_cnt = reg_val & 0xFF;
		regmap_read(mipicsi->phyreg,  MIPI0_DPHY_LINE_CNT_CH3 + offset, &reg_val);
		line_cnt = reg_val & 0xFFF;
		regmap_read(mipicsi->phyreg,  MIPI0_DPHY_PIX_CNT_CH3 + offset, &reg_val);
		pixel_cnt = reg_val & 0xFFF;
		regmap_read(mipicsi->phyreg,  MIPI0_DPHY_FRAME_HEIGHT_CH3 + offset, &reg_val);
		frame_height = reg_val & 0xFFF;
		regmap_read(mipicsi->phyreg,  MIPI0_DPHY_FRAME_WIDTH_CH3 + offset, &reg_val);
		frame_width = reg_val & 0xFFF;
		regmap_read(mipicsi->phyreg,  MIPI0_DPHY_MIPI2ISP_DEBUG + offset, &reg_val);
		errcnt = MIPI0_DPHY_MIPI2ISP_DEBUG_get_errcnt(reg_val);

		/* Clear count */
		regmap_write(mipicsi->phyreg, MIPI0_DPHY_FRAME_CNT_CH3 + offset,
			MIPI0_DPHY_FRAME_CNT_CH3_frame_cnt_clr_ch3(1));
		regmap_write(mipicsi->phyreg,  MIPI0_DPHY_LINE_CNT_CH3 + offset,
			MIPI0_DPHY_LINE_CNT_CH3_line_cnt_clr_ch3(1));
		regmap_write(mipicsi->phyreg,  MIPI0_DPHY_PIX_CNT_CH3 + offset,
			MIPI0_DPHY_PIX_CNT_CH3_pix_cnt_clr_ch3(1));
		regmap_write(mipicsi->phyreg,  MIPI0_DPHY_FRAME_HEIGHT_CH3 + offset,
			MIPI0_DPHY_FRAME_HEIGHT_CH3_frame_height_clr_ch3(1));
		regmap_write(mipicsi->phyreg,  MIPI0_DPHY_FRAME_WIDTH_CH0 + offset,
			MIPI0_DPHY_FRAME_WIDTH_CH3_frame_width_clr_ch3(1));
		regmap_write(mipicsi->phyreg,  MIPI0_DPHY_MIPI2ISP_DEBUG + offset,
			MIPI0_DPHY_MIPI2ISP_DEBUG_errcnt_clr(1));
		break;
	default:
	...
	if (mipicsi->en_dump_frame_state) {
		dev_info(mipicsi->dev, "ch%u width=%u height=%u errcnt=%u frame_cnt=%u line_cnt=%u pixel_cnt=%u\n",
			ch_index, frame_width, frame_height, errcnt, frame_cnt, line_cnt, pixel_cnt);
		mipicsi->en_dump_frame_state = false;
	}
...
/**
 * rtk_mipi_csi_prbs_check - Analog PRBS Check
 * @chk_mode: 0 - Pattern PRBS7, LANE0
 *    1 - Pattern PRBS7, LANE1
 *    2 - Pattern PRBS7, LANE2
 *    3 - Pattern PRBS7, LANE3
 *    4 - Pattern PRBS9, LANE0
 *    5 - Pattern PRBS9, LANE1
 *    6 - Pattern PRBS9, LANE2
 *    7 - Pattern PRBS9, LANE3
 *
 * @return: 0 - Fail, no signal;
 *  Negative value - Fail, errro count;
 *  Positive value - Pass, detect count
 */
static int rtk_mipi_csi_prbs_check(struct rtk_mipicsi *mipicsi, u8 chk_mode)
{
	int result = 0;
	u32 chk_detcnt = 0;
	u32 chk_errcnt = 0;
	...
	if ((mipicsi->soc_id == SOC_ID_KENT) || (chk_mode > PRBS9_LANE3))
		goto exit;

	lane_sel = chk_mode & 0x3;
	pattern = (chk_mode > PRBS7_LANE3) ? 1:0;

	rtk_mask_write(mipicsi->phyreg, MIPI1_APHY_PRBS_CHK_CTRL,
			MIPI1_APHY_PRBS_CHK_CTRL_prbs_chk_lane_sel_mask |
			MIPI1_APHY_PRBS_CHK_CTRL_prbs_chk_mode_mask |
			MIPI1_APHY_PRBS_CHK_CTRL_prbs_chk_enable_mask,
			MIPI1_APHY_PRBS_CHK_CTRL_prbs_chk_lane_sel(lane_sel) |
			MIPI1_APHY_PRBS_CHK_CTRL_prbs_chk_mode(pattern) |
			MIPI1_APHY_PRBS_CHK_CTRL_prbs_chk_enable(1));

	/* Minimum delay = 16 CSI digital clock(661.5MHz)= 1.512ns*16=24.192ns */
	usleep_range(2, 3);

	regmap_read(mipicsi->phyreg, MIPI1_APHY_PRBS_CHK_DETCNT, &chk_detcnt);
	regmap_read(mipicsi->phyreg, MIPI1_APHY_PRBS_CHK_ERRCNT, &chk_errcnt);

	rtk_mask_write(mipicsi->phyreg, MIPI1_APHY_PRBS_CHK_CTRL,
		MIPI1_APHY_PRBS_CHK_CTRL_prbs_chk_enable_mask,
		MIPI1_APHY_PRBS_CHK_CTRL_prbs_chk_enable(0));

	rtk_mask_write(mipicsi->phyreg, MIPI1_APHY_PRBS_CHK_CTRL,
		MIPI1_APHY_PRBS_CHK_CTRL_prbs_chk_result_clr_mask,
		MIPI1_APHY_PRBS_CHK_CTRL_prbs_chk_result_clr(1));
...
static const struct rtk_mipicsi_ops mipicsi_ops = {
	...
	.prbs_check = rtk_mipi_csi_prbs_check,
};
...
int rtk_mipi_csi_hw_deinit(struct rtk_mipicsi *mipicsi)
{
	if (g_crt.phy_inited && g_crt.crt_obtained) {
		...
		if (mipicsi->soc_id == SOC_ID_KENT) {
			...
			rtk_mipi_app_ctrl_g(mipicsi, CH_3, DISABLE);
			rtk_mipi_interrupt_ctrl_g(mipicsi, CH_3, DISABLE);
		}
...
int rtk_mipi_csi_hw_init(struct rtk_mipicsi *mipicsi)
{
	...
	if ((mipicsi->soc_id == SOC_ID_KENT) && (mipicsi->mirror_mode != MIRROR_DIS))
		return -EINVAL;

	if ((mipicsi->soc_id == SOC_ID_PRINCE) &&
		((mipicsi->ch_index == CH_0) || (mipicsi->ch_index == CH_1))) {
		if ((mipicsi->src_width > 3840) || (mipicsi->src_height > 2160))
			return -EINVAL;
	} else {
		if ((mipicsi->src_width > 1920) || (mipicsi->src_height > 1600))
			return -EINVAL;
	}
