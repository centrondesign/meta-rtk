...
#ifndef __NPP_INBAND_H__
#define __NPP_INBAND_H__
...
struct VIDEO_VO_NPP_WB_PICTURE_OBJECT {
...
struct NPP_PICTURE_OBJECT_CAPTURE {
	struct INBAND_CMD_PKT_HEADER header;
...
enum NPP_IMAGE_SCAN_MODE {
  /* top	field data stored in even lines of a frame buffer */
	INTERLEAVED_TOP_FIELD = 0,
   /* bottom field data stored in odd	lines of a frame buffer */
	INTERLEAVED_BOT_FIELD,
  /* top field data stored consecutlively */
	CONSECUTIVE_TOP_FIELD,
	/* in all lines of a field buffer */
	CONSECUTIVE_BOT_FIELD,	/* bottom field data stored consecutlively */
	/* in all lines of a field buffer */
	CONSECUTIVE_FRAME,	/* progressive frame data stored consecutlively */
	/* in all lines of a frame buffer */
	INTERLEAVED_TOP_FIELD_422,
	/* top	field data stored in even lines of a frame buffer */
	INTERLEAVED_BOT_FIELD_422,
	/* bottom field data stored in odd	lines of a frame buffer */
	CONSECUTIVE_TOP_FIELD_422,
	/* top	field data stored consecutlively in all lines of a field buffer */
	CONSECUTIVE_BOT_FIELD_422,
	/* bottom field data stored consecutlively in all lines of a field buffer */
	CONSECUTIVE_FRAME_422,
	/* progressive frame with 4:2:2 chroma */
	TOP_BOTTOM_FRAME,
	/* top field in the 0~height/2-1, bottom field in the height/2~height-1 in the frame */
...
struct VIDEO_VO_PICTURE_OBJECT_NEW {
		struct INBAND_CMD_PKT_HEADER header;
		unsigned int version;
     ...
     */
		...
		/* 3D Blu-ray dependent-view sub-picture plane offset metadata as defined in BD spec
		 * sec. 9.3.3.6. Valid only when Y_BufId_Right and C_BufId_Right are both valid
		 */
		unsigned int subPicOffset;
		...
		/* 'rtk3', according to VO spec: 10bits Pixel Packing mode selection,
		...
		/*for transcode interlaced feild use.*/
