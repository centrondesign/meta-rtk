...
#define MIPICSI_MAX_HEIGHT  1600
#define MIPICSI_DFT_WIDTH   1920
#define MIPICSI_DFT_HEIGHT  1080
...
static struct v4l2_queryctrl mipicsi_v4l2_ctrls[] = {
	...
	{
		.id = RTK_V4L2_CID_MIPI_STOP_RESTART,
...
static void rtk_mipicsi_stop(struct rtk_mipicsi *mipicsi);
static void rtk_mipicsi_restart(struct rtk_mipicsi *mipicsi);
...
static void rtk_mipicsi_dma_pool(struct rtk_mipicsi *mipicsi)
{
	if (mipicsi == NULL)
		return;
	...
	dma_coerce_mask_and_coherent(mipicsi->dev, DMA_BIT_MASK(32));
	set_dma_ops(mipicsi->dev, &rheap_dma_ops);

	rheap_setup_dma_pools(mipicsi->dev, "rtk_media_heap",
...
static int mipicsi_start_streaming(struct vb2_queue *q, unsigned int count)
{
	...
	mipicsi->en_dump_frame_state = true;
	...
	mipicsi->stream_state = STREAM_STATE_CAPTURING;
...
static void mipicsi_stop_streaming(struct vb2_queue *q)
{
	...
	mipicsi->sequence = 0;
	mipicsi->stream_state = STREAM_STATE_IDLE;
				...
				struct v4l2_format *f)
{
	...
	dev_info(mipicsi->dev, "Set dst fmt %ux%u\n",
				...
				void *fh, struct v4l2_frmivalenum *fival)
{
	...
	dev_dbg(mipicsi->dev, "frmivalenum %ufps\n",
				     ...
				     struct v4l2_dv_timings *timings)
{
	...
	if (timings->bt.width < MIPICSI_MIN_WIDTH || timings->bt.width > MIPICSI_MAX_WIDTH ||
		timings->bt.height < MIPICSI_MIN_HEIGHT || timings->bt.height > MIPICSI_MAX_HEIGHT)
		return -EINVAL;
	...
	mipicsi->src_width = timings->bt.width;
	mipicsi->src_height = timings->bt.height;

	dev_info(mipicsi->dev, "Set src width=%u height=%u\n",
				     ...
				     struct v4l2_dv_timings *timings)
{
	...
	timings->type = V4L2_DV_BT_656_1120;
	timings->bt.width = mipicsi->src_width;
	timings->bt.height = mipicsi->src_height;
				...
				struct v4l2_control *ctrl)
{
	...
	switch (ctrl->id) {
	case RTK_V4L2_CID_MIPI_COMPENC:
		ctrl->value = (mipicsi->mode == DATA_MODE_COMPENC);
		break;
	case RTK_V4L2_CID_MIPI_MIRROR:
		ctrl->value = (mipicsi->mirror_mode == MIRROR_EN);
		break;
	case RTK_V4L2_CID_MIPI_STOP_RESTART:
		ctrl->value = mipicsi->stream_state;
		break;
	default:
		ret = -EINVAL;
	}
				...
				struct v4l2_control *ctrl)
{
	...
	if ((mipicsi->soc_id == SOC_ID_KENT) &&
		(ctrl->id == RTK_V4L2_CID_MIPI_MIRROR)) {
		ret = -ESRCH;
		goto exit;
	}
	...
	case RTK_V4L2_CID_MIPI_COMPENC:
		...
		dev_info(mipicsi->dev, "Set DATA_MODE=%s\n", mipicsi->mode ? "COMPENC" : "LINE");
		break;
	case RTK_V4L2_CID_MIPI_MIRROR:
		if (ctrl->value)
			mipicsi->mirror_mode = MIRROR_EN;
		else
			mipicsi->mirror_mode = MIRROR_DIS;

		dev_info(mipicsi->dev, "Set MIRROR_MODE=%s\n", mipicsi->mirror_mode ? "EN" : "DIS");
		break;
	case RTK_V4L2_CID_MIPI_STOP_RESTART:
		switch (ctrl->value) {
		case STREAM_STATE_STOP:
			if (mipicsi->stream_state != STREAM_STATE_CAPTURING) {
				ret = -ENOEXEC;
				goto exit;
			}

			spin_lock_irqsave(&mipicsi->v4l2_dev.lock, flags);
			rtk_mipicsi_stop(mipicsi);
			mipicsi->stream_state = STREAM_STATE_STOP;
			spin_unlock_irqrestore(&mipicsi->v4l2_dev.lock, flags);

			dev_info(mipicsi->dev, "STREAM_STATE: CAPTURING -> STOP\n");
			break;
		case STREAM_STATE_CAPTURING:
			if (mipicsi->stream_state != STREAM_STATE_STOP) {
				ret = -ENOEXEC;
				goto exit;
			}
			spin_lock_irqsave(&mipicsi->v4l2_dev.lock, flags);
			rtk_mipicsi_restart(mipicsi);
			mipicsi->stream_state = STREAM_STATE_CAPTURING;
			spin_unlock_irqrestore(&mipicsi->v4l2_dev.lock, flags);

			dev_info(mipicsi->dev, "STREAM_STATE: STOP -> CAPTURING\n");
			break;
		default:
			ret = -EINVAL;
			break;
...
static int rtk_mipicsi_probe(struct platform_device *pdev)
{
	...
	if (soc_device_match(rtk_soc_prince))
		mipicsi->soc_id = SOC_ID_PRINCE;
	else
		mipicsi->soc_id = SOC_ID_KENT;

	dev_info(mipicsi->dev, "soc_id=%u en_group_dev=%s\n",
		mipicsi->soc_id,
	...
	if ((mipicsi->soc_id == SOC_ID_KENT) && (mipicsi->mirror_mode != MIRROR_DIS)) {
		dev_info(mipicsi->dev, "This soc does not support mirror_mode\n");
		mipicsi->mirror_mode = MIRROR_DIS;
	}

	ret = of_property_read_u32(dev->of_node, "swap-config",
				&mipicsi->swap_config);
	if (ret < 0 || mipicsi->swap_config > (FLAG_ALL_LANE_SWAP | SWAP_SEQ_LANE_0_1_2_3) ||
		mipicsi->swap_config < (FLAG_NO_LANE_SWAP | SWAP_SEQ_LANE_3_2_1_0))
		mipicsi->swap_config = (FLAG_NO_LANE_SWAP | SWAP_SEQ_LANE_0_1_2_3);

	ret = of_property_read_u32(dev->of_node, "hl-interval",
				&mipicsi->hl_interval);
	if (ret < 0) {
		mipicsi->hl_interval = DPHY_HL_INTERVAL_DEFAULT;
	} else {
		/* Validate based on SoC type */
		if (mipicsi->soc_id == SOC_ID_KENT) {
			if (mipicsi->hl_interval > DPHY_HL_INTERVAL_MAX_KENT) {
				dev_warn(dev, "hl-interval 0x%x exceeds max 0x%x for KENT, using default 0x%x\n",
					mipicsi->hl_interval, DPHY_HL_INTERVAL_MAX_KENT,
					DPHY_HL_INTERVAL_DEFAULT);
				mipicsi->hl_interval = DPHY_HL_INTERVAL_DEFAULT;
			}
		} else {
			if (mipicsi->hl_interval > DPHY_HL_INTERVAL_MAX_PRINCE) {
				dev_warn(dev, "hl-interval 0x%x exceeds max 0x%x for PRINCE, using default 0x%x\n",
					mipicsi->hl_interval, DPHY_HL_INTERVAL_MAX_PRINCE,
					DPHY_HL_INTERVAL_DEFAULT);
				mipicsi->hl_interval = DPHY_HL_INTERVAL_DEFAULT;
			}
		}
	...
	dev_info(dev, "src_width=%u src_height=%u\n",
		mipicsi->src_width, mipicsi->src_height);
	dev_info(dev, "skew_mode=%u mirror_mode=%u swap_config=0x%08x\n",
		mipicsi->skew_mode, mipicsi->mirror_mode, mipicsi->swap_config);
	dev_info(dev, "hl_interval=0x%x\n", mipicsi->hl_interval);
...
static void rtk_mipicsi_remove(struct platform_device *pdev)
{
	// TODO:
}
