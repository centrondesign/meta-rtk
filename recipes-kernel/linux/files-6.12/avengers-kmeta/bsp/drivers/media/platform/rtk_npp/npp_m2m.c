			  ...
			  unsigned int width, unsigned int height)
{
	...
	case V4L2_PIX_FMT_NV12:
		if ((drvctx->params.mipi_csi_param.mipi_csi_input == 1) ||
			...
			npp_dbg("%s, y_header_size = %d, c_header_size = %d\n",
				__func__, mipi_csi_y_header_size, mipi_csi_c_header_size);
			npp_dbg("%s, actual alloc sizeimage = %d, ideal sizeimage = %d\n",
				__func__, sizeimage,
...
bool eos, bool no_frame)
{
	...
	if (!v4l2_buf) {
		npp_warn("%s, No dst buffer to remove\n", __func__);
		return -ENOBUFS;
	}
		...
		if (v4l2_m2m_num_src_bufs_ready(fh->m2m_ctx)) {
			for (;;) {
				...
				if (!ret) {
					v4l2_buf = v4l2_m2m_src_buf_remove(fh->m2m_ctx);
					if (!v4l2_buf) {
						npp_err("%s, No src buffer to remove\n", __func__);
						break;
					}
...
static int threadrun(void *data)
{
	...
	while (1) {
		...
		if (kthread_should_stop() || (ret == -ERESTARTSYS)) {
			if (ret == -ERESTARTSYS)
				npp_err("%s(), ret = -ERESTARTSYS, exit\n", __func__);
			return 1;
		}
...
static int npp_try_fmt_out(struct v4l2_fh *fh, struct v4l2_format *f)
{
	...
	if (!npp_fmt) {
		npp_err("%s, unknown output format %c%c%c%c width=%d height=%d quantization=%d\n",
		__func__,
		pix_mp->pixelformat&0xff, (pix_mp->pixelformat>>8)&0xff,
		(pix_mp->pixelformat>>16)&0xff, (pix_mp->pixelformat>>24)&0xff,
...
static int npp_s_fmt_out(struct v4l2_fh *fh, struct v4l2_format *f)
{
	...
	if (ret) {
		npp_err("%s, npp_try_fmt_out fail", __func__);
		return ret;
	}
...
static int npp_start_streaming(struct vb2_queue *q, uint32_t count)
{
	...
	if (V4L2_TYPE_IS_OUTPUT(q->type)) {
		...
		((out_pixelformat>>0)&0xff), ((out_pixelformat>>8)&0xff),
		((out_pixelformat>>16)&0xff), ((out_pixelformat>>24)&0xff));
		nppctx->is_out_started = 1;
	} else {
		nppctx->seq_cap = 0;
		nppctx->memory_cap = q->memory;
		nppctx->thread_cap = kthread_run(threadcap, fh, "capthread %c%c%c%c",
		((cap_pixelformat>>0)&0xff), ((cap_pixelformat>>8)&0xff),
		((cap_pixelformat>>16)&0xff), ((cap_pixelformat>>24)&0xff));
...
static int npp_run(struct v4l2_fh *fh)
{
	...
	if (nppctx->memory_out == V4L2_MEMORY_USERPTR) {
		...
		npp_dbg("%s, USERPTR, src_paddr = 0x%llx, dst_paddr = 0x%llx\n",
		...
		npp_dbg("%s, MMAP, src_paddr = 0x%llx, dst_paddr = 0x%llx\n",
		 __func__, src_paddr, dst_paddr);
	} else {
		src_vaddr =
		vb2_plane_vaddr(&nppctx->out_v4l2_buf->vb2_buf, 0) +
		...
		vb2_dma_contig_plane_dma_addr(&nppctx->cap_v4l2_buf->vb2_buf, 0);
		// cap physical address
		npp_dbg("%s, DMA, src_paddr = 0x%llx, dst_paddr = 0x%llx\n",
		 __func__, src_paddr, dst_paddr);
	}
	...
	if (v_dev->target_soc == STARK) {
		...
		case V4L2_PIX_FMT_NV16:
			...
			break;
		default:
			npp_err("%s, Unsupported format=%c%c%c%c\n", __func__,
	...
	npp_dbg("%s, mutex_lock npp_ctx = %p\n", __func__, nppctx);
	...
	npp_dbg("%s, width(%d), height(%d), yuv_sample(%d), bit_depth(%d)\n",
	...
	if (crop_rect.left != 0  ||
		...
		crop_rect.width != 0 ||
		crop_rect.height != 0) {
		npp_dbg("%s, crop win: x(%d), y(%d), w(%d), h(%d)\n",
	...
	npp_dbg("%s, input(%d x %d), output(%d x %d)\n",
	...
	spin_lock_irqsave(&nppctx->npp_spin_lock, flags);
	nppctx->out_q_buf_obj->done = 0;
	npp_dbg("%s, out_q_buf_obj->done = 0\n", __func__);
	spin_unlock_irqrestore(&nppctx->npp_spin_lock, flags);
	...
	if (kthread_should_stop() || (ret == -ERESTARTSYS)) {
		if (ret == -ERESTARTSYS)
			npp_err("%s(%d) [%p] wait event error !!!\n", __func__, __LINE__, nppctx);
	...
	nppctx->excution_count_time++;
	npp_info("NPP Execution time: %lld us\n", nppctx->delta_ns / 1000);
#endif
...
void npp_set_m2m_rheap_pools(struct videc_dev *v_dev)
{
...
int npp_object_dma_allcate_m2m(struct drv_npp_ctx *ctx, int type)
{
	...
	if V4L2_TYPE_IS_OUTPUT(type) {
		...
		size = sizeimage*2 + MAX_HEADER_SIZE;
	} else {
		mutex_lock(&nppctx->npp_mutex);
		sizeimage = nppctx->cap.fmt.pix_mp.plane_fmt[0].sizeimage;
		mutex_unlock(&nppctx->npp_mutex);
...
void *npp_alloc_context_m2m(struct v4l2_fh *fh, struct videc_dev *dev)
{
	...
	ctx->out_q_buf_obj = _npp_buffer_create(dev);
	if (!ctx->out_q_buf_obj) {
		npp_err("%s, allocate npp out_q_buf_obj failed\n", __func__);
		kfree(ctx);
		return NULL;
	}

	ctx->cap_q_buf_obj = _npp_buffer_create(dev);
	...
	if (dev->dev_open_cnt_m2m == 0) {
		if (request_irq(dev->irq, npp_isr, IRQF_SHARED, "NPU_PP", (void *)dev) != 0) {
			npp_err("%s, request_irq fail\n", __func__);
			_npp_buffer_free(ctx->out_q_buf_obj);
			_npp_buffer_free(ctx->cap_q_buf_obj);
			kfree(ctx);
			return NULL;
		}
		npp_info("%s, request_irq done\n", __func__);
	}
#endif

	npp_info("%s, npp_ctx = %p\n", __func__, ctx);
	return ctx;
}
...
void npp_free_context_m2m(struct videc_dev *dev, void *ctx)
{
	...
	npp_info("%s, free out_q_buf_obj\n", __func__);

	_npp_buffer_free(ctx_npp->cap_q_buf_obj);
	npp_info("%s, free cap_q_buf_obj\n", __func__);
	...
	if (dev->dev_open_cnt_m2m == 0) {
		...
		if (dev->target_soc == STARK) {
			...
			npp_info("%s, not need to do hw reset and close clock\n", __func__);
		}
	}
...
void npp_time_measure(void *ctx)
{
	...
	avg_ns = ctx_npp->sum_ns/ctx_npp->excution_count_time;
	avg_ms_whole = avg_ns / 1000000;
	avg_ms_decimal = (avg_ns % 1000000) / 1000;

	npp_info("%s, Average Execution Time: %llu.%03llu ms\n",
		__func__, avg_ms_whole, avg_ms_decimal);
}
#endif
