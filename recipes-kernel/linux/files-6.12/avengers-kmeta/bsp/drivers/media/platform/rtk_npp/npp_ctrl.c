...
static void _WriteReg32(unsigned int addr, unsigned int val, unsigned int NPP_MODE_reg)
{
...
static unsigned int _ReadReg32(unsigned int addr, unsigned int NPP_MODE_reg)
{
	unsigned int ofst = addr - NPP_MODE_reg;
...
static void _hw_rst_WriteReg32(void __iomem *rst_reg_base, unsigned int base,
...
static unsigned int _hw_rst_ReadReg32(void __iomem *rst_reg_base, unsigned int base, unsigned int addr)
{
	unsigned int ofst = addr - base;
...
static void _SetVideoScalingCoeffs16p(unsigned short *coeff, int delta_phase,
...
static void _NPP_Set_EngineGo(enum TARGET_SOC target_soc)
{
...
static void _NPP_Set_EngineStop(enum TARGET_SOC target_soc)
{
	const struct RegisterAddresses *regs = get_register_addresses(target_soc);
	unsigned int NPP_INTST_reg = regs->NPP_INTST;
	unsigned int NPP_MODE_reg = regs->NPP_MODE;
...
static void _NPP_Get_INTStatus(unsigned char *is_finish, enum TARGET_SOC target_soc)
{
...
unsigned int NPP_NPP_HwClock(unsigned char enable_clock, enum TARGET_SOC target_soc)
{
	...
	npp_dbg("SYS_REG_CLOCK_ENABLE7 = 0x%x\n",
	 _hw_rst_ReadReg32(npp_reset_reg_base, base_reg_addr, SYS_REG_CLOCK_ENABLE7));
	...
	if (target_soc == KENT) {
		...
		npp_dbg("SYS_REG_SOFT_RESET_MMU_SECURE = 0x%x\n",
		_hw_rst_ReadReg32(npp_reset_reg_base, base_reg_addr, SYS_REG_SOFT_RESET_MMU_SECURE));
	}
#endif
...
unsigned int NPP_Init(enum TARGET_SOC target_soc)
{
	...
	if (Npp_Reg_Base == NULL) {
		npp_err("%s, npp register ioremap failed\n", __func__);
...
unsigned int NPP_Set_InputImageFormat(struct Npp_InputImageFormat *src_input, enum TARGET_SOC target_soc)
{
	...
	if (src_input->yuv_format >= PACKED_YUYV && src_input->yuv_format <= PACKED_VYUY) {
		...
		npp_dbg("NPP_VI_PACKED(0x%x) = 0x%x\n",
	...
	npp_dbg("NPP_VI(0x%x) = 0x%x, VI_SIZE(0x%x) = 0x%x\n",
	NPP_VI_reg, _ReadReg32(NPP_VI_reg, NPP_MODE_reg), reg_NPP_VI_SIZE, _ReadReg32(reg_NPP_VI_SIZE, NPP_MODE_reg));
	npp_dbg("VI_SEQ_PITCH_C_Y(0x%x) = 0x%x, VI_SEQ_PITCH_C_C(0x%x) = 0x%x\n",
...
unsigned int NPP_Set_InputDataBuffer(struct Npp_InputBufInfo *input_buffer, enum TARGET_SOC target_soc)
{
	...
	if (target_soc == STARK) {
		...
		if (vi_yuv_packed == 1) {
			...
			npp_dbg("NPP_VI(0x%x) = 0x%x, VI_SEQ_SA_C_Y(0x%x) = 0x%x\n",
	...
	npp_dbg("NPP_VI(0x%x) = 0x%x\n", NPP_VI_reg, _ReadReg32(NPP_VI_reg, NPP_MODE_reg));
	npp_dbg("VI_SEQ_SA_C_Y(0x%x) = 0x%x, VI_SEQ_SA_C_C(0x%x) = 0x%x\n",
	 NPP_VI_SEQ_SA_C_Y_reg, _ReadReg32(NPP_VI_SEQ_SA_C_Y_reg, NPP_MODE_reg),
...
unsigned int NPP_Set_OutputImageFormat(struct Npp_OutputImageFormat *tgt_output, enum TARGET_SOC target_soc)
{
	...
	if (target_soc == STARK || target_soc == KENT) {
		...
		npp_dbg("NPP_NORM_R(0x%x) = 0x%x, NPP_NORM_G(0x%x) = 0x%x, NPP_NORM_B(0x%x) = 0x%x\n",
		...
		npp_dbg("NPP_NORM_M_R(0x%x) = 0x%x, NPP_NORM_M_G(0x%x) = 0x%x, NPP_NORM_M_B(0x%x) = 0x%x\n",
		...
		npp_dbg("NPP_NORM_S_R(0x%x) = 0x%x, NPP_NORM_S_G(0x%x) = 0x%x, NPP_NORM_S_B(0x%x) = 0x%x\n",
		...
		npp_dbg("NPP_NORM_MAX_R(0x%x) = 0x%x, NPP_NORM_MAX_G(0x%x) = 0x%x, NPP_NORM_MAX_B(0x%x) = 0x%x\n",
		...
		npp_dbg("NPP_NORM_MIN_R(0x%x) = 0x%x, NPP_NORM_MIN_G(0x%x) = 0x%x, NPP_NORM_MIN_B(0x%x) = 0x%x\n",
	...
	npp_dbg("NPP_WB(0x%x) = 0x%x, WB_SEQ_PITCH(0x%x) = 0x%x, NPP_NORM(0x%x) = 0x%x\n",
...
unsigned int NPP_Set_OutputRGBFormat(enum NPP_RGB_FORMAT rgb_format, enum TARGET_SOC target_soc)
{
	...
	if (rgb_format == SINGLE_PLANE_RGB) {
		...
		npp_dbg("rgb_format = %d, set NPP_WB_fmt_mask(0x%x)\n",
		 rgb_format, wb_config_value);
	}

	npp_dbg("rgb_format = %d, NPP_WB(0x%x) = 0x%x\n",
...
unsigned int NPP_Set_OutputDataBuffer(struct Npp_OutputBufInfo *output_buffer, enum TARGET_SOC target_soc)
{
	...
	npp_dbg("rgb_format = %d, WB_SEQ_SA_R(0x%x) = 0x%x, WB_SEQ_SA_G(0x%x) = 0x%x,WB_SEQ_SA_B(0x%x) = 0x%x\n",
...
unsigned int NPP_Set_ScalerCoefficient(struct Npp_ImageScalingInfo *image_scaling, enum TARGET_SOC target_soc)
{
	...
	npp_dbg("VI_VSYI(0x%x)  = 0x%x, VI_VSCI(0x%x)  = 0x%x, VI_HSI(0x%x) = 0x%x\n",
...
void NPP_Set_MIPICSI_ImageFormat(struct Npp_MIPICSI_ImageFormat *mipi_csi_image_param, enum TARGET_SOC target_soc)
{
	...
	npp_dbg("VI_TVVE(0x%x) = 0x%x\n",
	reg_NPP_VI_TVVE, _ReadReg32(reg_NPP_VI_TVVE, NPP_MODE_reg));
	npp_dbg("TVVE_HEAD(0x%x) = 0x%x, TVVE_DATA(0x%x) = 0x%x\n",
	...
	npp_dbg("TVVE_CROP(0x%x) = 0x%x, TVVE_CROP_POS(0x%x) = 0x%x\n",
...
void NPP_Set_MIPICSI_InputDataBuffer(struct Npp_MIPI_CSI_InputBufInfo *mipi_csi_input_buf, enum TARGET_SOC target_soc)
{
	...
	npp_dbg("TVVE_HEAD_LU(0x%x) = 0x%x, TVVE_HEAD_CH(0x%x) = 0x%x\n",
	...
	npp_dbg("TVVE_DATA_LU(0x%x) = 0x%x, TVVE_DATA_CH(0x%x) = 0x%x\n",
...
void NPP_Set_SecureVideoPath(unsigned char enable_svp, enum TARGET_SOC target_soc)
{
	...
	npp_dbg("enable_svp = %d, NPP_DMA_PRT_MODE(0x%x) = 0x%x\n",
