...
#define IS_LATTEST(x)  (((x) & LATTEST) >> 4)
#define INIT_ADDR_MASK (0xF << 8)
#define GET_INIT_ADDR(x) (((x) & INIT_ADDR_MASK) >> 8)
#define INIT_ADDR_MASK_NEW (0xF << 12)
#define GET_INIT_ADDR_NEW(x) (((x) & INIT_ADDR_MASK_NEW) >> 12)

#define GET_LOGICAL_ADDR(x) (((x) & LOG_ADDR_MASK) >> LOG_ADDR_SHIFT)

#define RX_DEST_ADDR_MASK (0xF << 8)
#define GET_RX_DEST_ADDR(x) (((x) & RX_DEST_ADDR_MASK) >> 8)

#define RX_RCVE_ST_MASK (0xF << 4)
#define GET_RX_RCVE_ST(x) (((x) & RX_RCVE_ST_MASK) >> 4)

#define SEL_TX_LOGICAL_ADDR(x) ((x) << LOG_ADDR_SEL_SHIFT)
...
struct rtk_cec_dev {
	...
	unsigned int		wakeup_cold;
	unsigned int		clear_osd;

	enum cec_state		rx;
	enum cec_state		tx;
	unsigned char		log_addr[4];
	unsigned char		num_log_addrs;
	unsigned char		valid_addrs;
...
static void rtk_cec_set_divider(struct rtk_cec_dev *cec)
{
	...
	if (soc_device_match(rtk_soc_kent) || soc_device_match(rtk_soc_prince)) {
		switch (cec->num_log_addrs) {
		case 1:
			reg = LOG_ADDR0_EN;
			break;
		case 2:
			reg = LOG_ADDR0_EN | LOG_ADDR1_EN;
			break;
		case 3:
			reg = LOG_ADDR0_EN | LOG_ADDR1_EN | LOG_ADDR2_EN;
			break;
		case 4:
			reg = LOG_ADDR0_EN | LOG_ADDR1_EN | LOG_ADDR2_EN | LOG_ADDR3_EN;
			break;
		default:
			break;
		}
		writel(reg, cec->reg + CEC_CR1);
	}
}
...
static void rtk_cec_enable(struct rtk_cec_dev *cec)
{
	...
	if (soc_device_match(rtk_soc_kent) || soc_device_match(rtk_soc_prince))
		reg |= (1 << CEC_MODE_SHIFT);//1 enable,3 self test, UNREG_ACK_EN remove on kent
	else
			       ...
			       unsigned int size, unsigned char *buf)
{
	...
	if (soc_device_match(rtk_soc_kent) || soc_device_match(rtk_soc_prince)) {
		if (IS_BROADCAST(readl(cec->reg + CEC_RXCR0))) {
			...
			dev_dbg(cec->dev, "%s initiator la: %d, follower la: 0xf", __func__, GET_INIT_ADDR_NEW(readl(cec->reg + CEC_RXST)));
			rxfifo[0] |= GET_INIT_ADDR_NEW(readl(cec->reg + CEC_RXST)) << 4 |
				0xF;
		} else {

			dev_dbg(cec->dev, "%s initiator la: %d, follower la: %d", __func__, GET_INIT_ADDR_NEW(readl(cec->reg + CEC_RXST)), GET_RX_DEST_ADDR(readl(cec->reg + CEC_RXST)));
			rxfifo[0] |= GET_INIT_ADDR_NEW(readl(cec->reg + CEC_RXST)) << 4 |
				GET_RX_DEST_ADDR(readl(cec->reg + CEC_RXST));
		}

		rcve_st = GET_RX_RCVE_ST(readl(cec->reg + CEC_RXST));

		for (i = 0; i < cec->num_log_addrs; i++)
			dev_dbg(cec->dev, "%s RCVE_ST LA(%i): 0x%x", __func__, i, (rcve_st & (0x1 << i)) >> i);

		dev_dbg(cec->dev, "%s  CEC_RXST: 0x%x", __func__, readl(cec->reg + CEC_RXST));
...
static void send_cec_text_view_on(struct rtk_cec_dev *cec);
static void cec_clear_osd_name(struct rtk_cec_dev *cec);

static int rtk_cec_set_log_addr(struct rtk_cec_dev *cec, u8 addr)
{
	unsigned int reg;
	...
	if (addr == CEC_LOG_ADDR_INVALID) {
		...
		cec->valid_addrs = 0;
		dev_dbg(cec->dev, "%s CEC_LOG_ADDR_INVALID\n", __func__);
		return 0;
	}
...
static int rtk_cec_set_log_addr_new(struct rtk_cec_dev *cec, u8 addr)
{
	unsigned int reg, i, free_idx;
	bool is_valid;

	free_idx = cec->num_log_addrs;
	...
	if (addr == CEC_LOG_ADDR_INVALID) {
		...
		reg = readl(cec->reg + CEC_CR1);
		reg &= ~(LOG_ADDR1_MASK | LOG_ADDR2_MASK | LOG_ADDR3_MASK);
		writel(reg, cec->reg + CEC_CR1);
	...
	for (i = 0; i < cec->num_log_addrs; i++) {
		is_valid = cec->valid_addrs & (1 << i);

		if (free_idx == cec->num_log_addrs && !is_valid) {
			free_idx = i;
			dev_err(cec->dev, "%s free_idx: %d", __func__, free_idx);
		}

		if (is_valid && cec->log_addr[i] == addr) {
			dev_err(cec->dev, "%s same la\n", __func__);
			return 0;
		}

	}
	if (i == cec->num_log_addrs) {
		i = free_idx;
		if (i == cec->num_log_addrs) {
			dev_err(cec->dev, "%s no available la\n", __func__);
			return -ENXIO;
		}
	}
	cec->log_addr[i] = addr;
	...
	case 0:
		...
		reg |= (addr & 0xf) << LOG_ADDR_SHIFT;
		writel(reg, cec->reg + CEC_CR0);
		dev_dbg(cec->dev, "%s CEC_CR0 0 0x%x\n", __func__, readl(cec->reg + CEC_CR0));
		break;
	case 1:
		reg = readl(cec->reg + CEC_CR1);
		reg &= ~(LOG_ADDR1_MASK);
		reg |= (addr & 0xf) << LOG_ADDR1_SHIFT;
		writel(reg, cec->reg + CEC_CR1);
		dev_dbg(cec->dev, "%s CEC_CR1 1 0x%x\n", __func__, readl(cec->reg + CEC_CR1));
		break;
	case 2:
		reg = readl(cec->reg + CEC_CR1);
		reg &= ~(LOG_ADDR2_MASK);
		reg |= (addr & 0xf) << LOG_ADDR2_SHIFT;
		writel(reg, cec->reg + CEC_CR1);
		dev_dbg(cec->dev, "%s CEC_CR1 2 0x%x\n", __func__, readl(cec->reg + CEC_CR1));
		break;
	case 3:
		reg = readl(cec->reg + CEC_CR1);
		reg &= ~(LOG_ADDR3_MASK);
		reg |= (addr & 0xf) << LOG_ADDR3_SHIFT;
		writel(reg, cec->reg + CEC_CR1);
		dev_dbg(cec->dev, "%s CEC_CR1 3 0x%x\n", __func__, readl(cec->reg + CEC_CR1));
		break;
	}
...
static int rtk_cec_transmit(struct rtk_cec_dev *cec, struct cec_msg *msg)
{
	...
	reg = TX_ADDR_EN | (cec->log_addr[0] << TX_LOG_ADDR_SHIFT)
...
static int rtk_cec_transmit_new(struct rtk_cec_dev *cec, struct cec_msg *msg)
{
	unsigned int txfifo[TX_FIFO_NUM];
	unsigned int reg;
	int remain;
	int i;
	int len;
	int init_la = cec_msg_initiator(msg);
	int dest_la = cec_msg_destination(msg);
	int la_index = cec->num_log_addrs;
	bool is_valid;
	...
	for (i = 0; i < cec->num_log_addrs; i++) {
		is_valid = cec->valid_addrs & (1 << i);
		if (is_valid && cec->log_addr[i] == init_la) {
			la_index = i;
			dev_dbg(cec->dev, "%s log_addr[%d]: 0x%x\n", __func__, i, cec->log_addr[i]);
		}
	}

	if (la_index < cec->num_log_addrs) {
		reg = readl(cec->reg + CEC_CR1);
		reg &= ~(LOG_ADDR_SEL_MASK);
		reg |= SEL_TX_LOGICAL_ADDR(la_index);
		writel(reg, cec->reg + CEC_CR1);
		dev_dbg(cec->dev, "%s CEC_CR1 la_index:%d CEC_CR1: 0x%x\n", __func__, la_index, readl(cec->reg + CEC_CR1));
	} else {
		dev_err(cec->dev, "%s cannot find la!", __func__);
	}

	memcpy(txfifo, (msg->msg + 1), len);
	for (i = 0; i < len; i += 4) {
		dev_dbg(cec->dev, "%s len: %d, txfifo[%d]: 0x%x\n", __func__, len, i, txfifo[i >> 2]);
		writel(__cpu_to_be32(txfifo[i >> 2]), cec->reg + CEC_TXDR1 + i);
	}

	writel(ADD_CNT | len, cec->reg + CEC_TXDR0);

	dev_dbg(cec->dev, "%s initiator la: %d, follower la: %d\n", __func__, init_la, dest_la);
	reg = TX_ADDR_EN | (init_la << TX_LOG_ADDR_SHIFT)
		| (dest_la << TX_DEST_ADDR_SHIFT)
		| TX_EN | TX_INT_EN;
	writel(reg, cec->reg + CEC_TXCR0);
...
static int rtk_cec_adap_log_addr(struct cec_adapter *adap, u8 addr)
{
	...
	if (soc_device_match(rtk_soc_kent) || soc_device_match(rtk_soc_prince))
		ret = rtk_cec_set_log_addr_new(cec, addr);
	else
		ret = rtk_cec_set_log_addr(cec, addr);

	if (cec->log_addr[0] > 0 && cec->log_addr[0] < 15 && !cec->woken_up) {
		send_cec_text_view_on(cec);
		cec->woken_up = true;
	}
				 ...
				 u32 signal_free_time, struct cec_msg *msg)
{
	...
	if (soc_device_match(rtk_soc_kent) || soc_device_match(rtk_soc_prince))
		ret = rtk_cec_transmit_new(cec, msg);
	else
		ret = rtk_cec_transmit(cec, msg);
...
static irqreturn_t rtk_cec_irq_handler(int irq, void *priv)
{
	...
	if (reg & RX_INT) {
		if (reg & RX_FIFO_OV) {
			...
			if (!IS_LATTEST(readl(cec->reg + CEC_RTCR0))) {
...
static void send_cec_text_view_on(struct rtk_cec_dev *cec)
{
	...
	if (cec->wakeup_en == 0 || cec->wakeup_cold == 0) {
		return;
	}
	...
	msg.msg[0] = 0x0;
	msg.msg[1] = 0x0D;
	rtk_cec_adap_transmit(cec->adap, 0, 0, &msg);
	msleep(100);
}

static void cec_clear_osd_name(struct rtk_cec_dev *cec)
{
	rtk_cec_adap_log_addr(cec->adap, 0xf);
}
...
static int rtk_cec_wakeup_cold_proc_show(struct seq_file *m, void *v)
{
	struct rtk_cec_dev *cec = m->private;

	seq_printf(m, "wakeup quiescent: %d\n", cec->wakeup_cold);

	return 0;
}

static int rtk_cec_clear_osd_proc_show(struct seq_file *m, void *v)
{
	struct rtk_cec_dev *cec = m->private;

	seq_printf(m, "clear osd: %d\n", cec->clear_osd);
...
static int rtk_cec_wakeup_cold_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, rtk_cec_wakeup_cold_proc_show, pde_data(inode));
}

static int rtk_cec_clear_osd_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, rtk_cec_clear_osd_proc_show, pde_data(inode));
}
...
static ssize_t rtk_cec_wakeup_cold_proc_write(struct file *file, const char __user *buffer,
					size_t count, loff_t *ppos)
{
	struct rtk_cec_dev *cec = pde_data(file_inode(file));
	int i;
...
static ssize_t rtk_cec_clear_osd_proc_write(struct file *file, const char __user *buffer,
					size_t count, loff_t *ppos)
{
	struct rtk_cec_dev *cec = pde_data(file_inode(file));
	int i;
	...
	dev_info(cec->dev, "clear_osd: %d\n", cec->clear_osd);

	return count;
}

static struct proc_dir_entry *root_pe;
static struct proc_dir_entry *wakeup_pe;
static struct proc_dir_entry *wakeup_cold_pe;
static struct proc_dir_entry *clear_osd_pe;

static const struct proc_ops rtk_wakeup_pe_proc_fops = {
...
static const struct proc_ops rtk_cec_wakeup_cold_proc_fops = {
	.proc_open	= rtk_cec_wakeup_cold_proc_open,
	.proc_read	= seq_read,
	.proc_write	= rtk_cec_wakeup_cold_proc_write,
	.proc_release	= single_release,
};

static const struct proc_ops rtk_clear_osd_pe_proc_fops = {
	.proc_open	= rtk_cec_clear_osd_proc_open,
	.proc_read	= seq_read,
	.proc_write	= rtk_cec_clear_osd_proc_write,
	.proc_release	= single_release,
};
...
static int rtk_cec_proc_create(struct rtk_cec_dev *cec)
{
	...
	wakeup_pe = proc_create_data("rtk/cec_wakeup", 0644, NULL,
					&rtk_wakeup_pe_proc_fops, cec);
	if (!wakeup_pe)
		return -ENOMEM;

	wakeup_cold_pe = proc_create_data("rtk/cec_wakeup_cold", 0644, NULL,
						&rtk_cec_wakeup_cold_proc_fops, cec);
	if (!wakeup_cold_pe)
		return -ENOMEM;

	clear_osd_pe = proc_create_data("rtk/cec_clear_osd", 0644, NULL,
					&rtk_clear_osd_pe_proc_fops, cec);
	if (!clear_osd_pe)
		return -ENOMEM;
...
static int rtk_cec_probe(struct platform_device *pdev)
{
	...
	prop = of_get_property(dev->of_node, "num-la", &size);
	if (prop) {
		cec->num_log_addrs = of_read_number(prop, 1);
		dev_info(dev, "support %d logical addresses\n", cec->num_log_addrs);
	} else {
		dev_info(dev, "support 1 logical address\n");
		cec->num_log_addrs = 1;
	}
	...
	if (soc_device_match(rtk_soc_stark) || soc_device_match(rtk_soc_parker)
		|| soc_device_match(rtk_soc_morbius) || soc_device_match(rtk_soc_kent)
		|| soc_device_match(rtk_soc_prince)) {
	...
	cec->adap = cec_allocate_adapter(&rtk_cec_adap_ops, cec, CEC_NAME,
		...
		| CEC_CAP_TRANSMIT | CEC_CAP_PASSTHROUGH, cec->num_log_addrs);
...
static void rtk_cec_remove(struct platform_device *pdev)
{
...
static int rtk_cec_suspend(struct device *dev)
{
	...
	if (dev_pm->version == RTK_PCPU_VERSION_V2) {
		...
		for (i = 0; i < cec->num_log_addrs; i++) {
			is_valid = cec->valid_addrs & (1 << i);
			if (is_valid && cec->log_addr[i] == adap->log_addrs.log_addr[i]) {
				pcpu_data_v2->logical_addr[i] = adap->log_addrs.log_addr[i];
				dev_info(cec->dev, "wakeup la[%d]: 0x%x\n", i, adap->log_addrs.log_addr[i]);
			}
		}
		...
		pcpu_data_v1->standby_cec_wakeup_off = cec->rpu_value;
