...
#define RPC_INT_VE3S_V2 BIT(8)
#define RPC_INT_VE3S_WRITE_EN BIT(9)
...
struct remote_cpu_info {
	...
	int from_rcpu_intr_write_en_bit;
	int intr_en;
	int big_endian;
	int id;
...
static struct class *rpmsg_info_class;
...
static void handle_intr_data(unsigned long data)
{
...
static void handle_kern_data(unsigned long data)
{
...
static irqreturn_t rtk_rcpu_ve3_isr(int irq, void *data)
{
	...
	if (check_notify_flag(rcpu) && !(intr_st & rcpu->info->from_rcpu_intr_bit)) {
		if (rcpu->info->from_rcpu_intr_write_en_bit)
			regmap_write(rcpu->rcpu_intr_regmap, 0x88, rcpu->info->from_rcpu_intr_write_en_bit);
		else
...
static void rcpu_set_flag(struct rtk_rcpu *rcpu, uint32_t flag)
{
	writel(__cpu_to_be32(flag), rcpu->sync_flag);
}

static uint32_t rcpu_get_flag(struct rtk_rcpu *rcpu)
{
...
static ssize_t ringbuf_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct rtk_rpmsg_channel *channel = dev_get_drvdata(dev);
	int ringSize;
	int i;
	uint32_t *addr;
	volatile uint32_t *ringIn, *ringOut, *ringStart, *ringEnd, *ringBuf;
	int len = 0;
	int max_len = PAGE_SIZE;
	...
	len += scnprintf(buf + len, max_len - len, "name: %s\n", channel->name);
	len += scnprintf(buf + len, max_len - len, "TX ringBuffer");
    len += scnprintf(buf + len, max_len - len, "RingBuf: %x\n", *ringBuf);
    len += scnprintf(buf + len, max_len - len, "RingStart: %x\n", *ringStart);
    len += scnprintf(buf + len, max_len - len, "RingIn: %x\n", *ringIn);
    len += scnprintf(buf + len, max_len - len, "RingOut: %x\n", *ringOut);
    len += scnprintf(buf + len, max_len - len, "RingEnd: %x\n", *ringEnd);
	...
	for (i = 0; i < ringSize; i += 16) {
		...
		if (channel->rcpu->info->big_endian)
			len += scnprintf(buf + len, max_len - len, "%x: %08x %08x %08x %08x\n",
							*ringStart + i, ntohl(*(addr + 0)), ntohl(*(addr + 1)),
							ntohl(*(addr + 2)), ntohl(*(addr + 3)));
		else
			len += scnprintf(buf + len, max_len - len, "%x: %08x %08x %08x %08x\n",
							*ringStart + i, *(addr + 0), *(addr + 1), *(addr + 2),
							*(addr + 3));
	}
	...
	len += scnprintf(buf + len, max_len - len, "name: %s\n", channel->name);
	len += scnprintf(buf + len, max_len - len, "RX ringBuffer");
    len += scnprintf(buf + len, max_len - len, "RingBuf: %x\n", *ringBuf);
    len += scnprintf(buf + len, max_len - len, "RingStart: %x\n", *ringStart);
    len += scnprintf(buf + len, max_len - len, "RingIn: %x\n", *ringIn);
    len += scnprintf(buf + len, max_len - len, "RingOut: %x\n", *ringOut);
...
static DEVICE_ATTR_RO(ringbuf);

static struct attribute *rpmsg_attrs[] = {
	&dev_attr_ringbuf.attr,
	NULL
};

static const struct attribute_group rpmsg_group = {
	.attrs = rpmsg_attrs,
};

static const struct attribute_group *rpmsg_groups[] = {
	&rpmsg_group,
	NULL
};
							...
							struct dentry *rpmsg_dir)
{
	...
	channel->class_dev = device_create_with_groups(rpmsg_info_class, rcpu->dev,
												  MKDEV(0, 0), channel, rpmsg_groups,
												  "%s", channel->name);
...
static void rtk_rcpu_remove(struct platform_device *pdev)
{
...
static const struct remote_cpu_info rtd1635_ve3_info = {
	.name = "ve3-rpc",
	.id = VE3_ID,
	.big_endian = 1,
	.to_rcpu_intr_bit = RPC_INT_SVE3,
	.from_rcpu_intr_bit = RPC_INT_VE3S_V2,
	.from_rcpu_intr_write_en_bit = RPC_INT_VE3S_WRITE_EN,
	.intr_en = RPC_INT_VE3_EN,
	.isr = &rtk_rcpu_ve3_isr,
	.send_interrupt = &rpmsg_send_ve3_interrupt,
};
...
static const struct of_device_id rtk_rcpu_of_match[] = {
	...
	{ .compatible = "realtek,rtd1635-ve3-rpmsg", .data = &rtd1635_ve3_info },
	{},
};
...
{
	rpmsg_info_class = class_create("rpmsg_info");
	if (IS_ERR(rpmsg_info_class))
        return PTR_ERR(rpmsg_info_class);
...
static void __exit rtk_rcpu_exit(void)
{
	...
	if (rpmsg_info_class)
		class_destroy(rpmsg_info_class);
}
