...
static struct clk_pll rtd1315e_pll_dcsb = {
	...
	.freq_tbl  = rtd1315e_bus_tbl,
	.freq_mask = FREQ_NF_MASK,
	.clkr.hw.init = CLK_HW_INIT("pll_dcsb", "osc27m", &clk_pll_ops, CLK_GET_RATE_NOCACHE),
};
...
static struct clk *rtd1319d_cc_clk_sysh;

static int rtd1319d_cc_clk_ve2_notifier_cb(struct notifier_block *nb, unsigned long event,
					   void *data)
{
	struct clk_notifier_data *cnd = data;
	struct clk *clk_sysh = rtd1319d_cc_clk_sysh;
	struct clk *clk_ve2 = cnd->clk;
	struct clk *clk_ve2_parent = clk_get_parent(clk_ve2);
	unsigned long cur_freq = clk_get_rate(clk_sysh);
	unsigned long new_freq = 0;
	unsigned long rounded_freq = 0;
	int ret = 0;

	if (clk_is_match(clk_sysh, clk_ve2_parent))
		return NOTIFY_DONE;

	switch (event) {
        case PRE_RATE_CHANGE:
                if (cnd->old_rate > cnd->new_rate)
                        new_freq = cnd->new_rate;
                break;
	...
	rounded_freq = clk_round_rate(clk_sysh, new_freq);

	pr_debug("%pC: freq: cur=%lu, new=%lu, rounded=%lu\n",
		 clk_sysh, cur_freq, new_freq, rounded_freq);

	if (rounded_freq && cur_freq != rounded_freq)
		ret = clk_set_rate(clk_sysh, rounded_freq);

	return notifier_from_errno(ret);
}

static struct notifier_block rtd1319d_cc_clk_ve2_notifier_nb = {
	.notifier_call = rtd1319d_cc_clk_ve2_notifier_cb,
};

static struct clk *get_clk_by_id(const struct rtk_clk_desc *desc, unsigned int id)
{
	struct clk_hw *hw;

	if (id >= desc->clk_data->num)
		return NULL;
	hw = desc->clk_data->hws[id];

	return hw ? hw->clk : NULL;
}

static void rtd1319d_cc_setup_clk_sysh_constraint(struct device *dev,
						  const struct rtk_clk_desc *desc)
{
	struct clk *clk_sysh = get_clk_by_id(desc, RTD1319D_CRT_CLK_SYSH);
	struct clk *clk_ve2 = get_clk_by_id(desc, RTD1319D_CRT_CLK_VE2);
	int ret = -EINVAL;

	if (!clk_sysh || !clk_ve2) {
		dev_notice(dev, "%s: invalid clk\n", __func__);
		return;
	}

	rtd1319d_cc_clk_sysh = clk_sysh;
	ret = clk_notifier_register(clk_ve2, &rtd1319d_cc_clk_ve2_notifier_nb);
	dev_info(dev, "%pC is %sconstrained by %pC\n", clk_sysh, ret < 0 ? "not " : "", clk_ve2);
}
...
static int rtd1319d_cc_probe(struct platform_device *pdev)
{
	...
	ret = rtk_clk_probe(pdev, desc);
	if (ret)
		return ret;

	rtd1319d_cc_setup_clk_sysh_constraint(&pdev->dev, desc);
	return 0;
}
