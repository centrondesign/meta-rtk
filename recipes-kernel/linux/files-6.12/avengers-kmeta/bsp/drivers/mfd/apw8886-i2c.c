...
static inline unsigned long apw8886_i2c_get_driver_data(struct i2c_client *client)
{
	...
	return (unsigned long)i2c_get_match_data(client);
}
...
static struct apw888x_device *apw8886_adev;

static void apw8886_poweroff(void)
{
	dev_info(apw8886_adev->dev, "trigger soft poweroff\n");
	regmap_write(apw8886_adev->regmap, APW8886_REG_SYS_CONTROL, APW8886_SYS_CONTROL_SOFTOFF);
}
...
static int apw8886_i2c_probe(struct i2c_client *client)
{
	...
	if (of_device_is_system_power_controller(dev->of_node)) {
		dev_info(dev, "register as system power controller\n");
		apw8886_adev = adev;
		pm_power_off = apw8886_poweroff;
	}
...
{
	if (pm_power_off == apw8886_poweroff) {
		apw8886_adev = NULL;
		pm_power_off = NULL;
	}
	return;
