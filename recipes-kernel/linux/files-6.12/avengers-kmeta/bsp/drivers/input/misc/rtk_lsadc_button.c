...
static int rtk_lsdac_button_vstate(struct rtk_lsadc_button_data *button, const u8 *val)
{
	...
	for (i = 0; i < MAX_CHANNEL; i++) {
		for (j = 0; j < MAX_CHANNEL_KEY; j++) {
			if (in_range(val[i], button->button_info[i][j].vs_min, button->button_info[i][j].vs_max) && button->button_info[i][j].vs_key > 0) {
				key = button->button_info[i][j].vs_key;
				break;
			}
		}
		if (key != -1)
				 ...
				 struct rtk_lsadc_button_data *button, const u8 *val)
{
	...
	if (button->pre_vs != -1 && vs != button->pre_vs)
		return;

	hrtimer_cancel(&button_dev->timer);
	spin_lock_irqsave(&button_dev->lock, flags);
	button->times_count++;
	if (button->times_count <= 40) {
		spin_unlock_irqrestore(&button_dev->lock, flags);
		hrtimer_start(&button_dev->timer, ms_to_ktime(20), HRTIMER_MODE_REL);
		return;
	}
	if (vs >= 0 && button->pre_vs == -1 && button->keycode[vs] >= 0)  {
...
static int rtk_lsdac_button_handle(const void *data, void *private)
{
	...
	if(data == NULL || private == NULL) {
		pr_err("rtk_lsdac_button_handle param error\n");
		return -EINVAL;
	}
				   ...
				   struct rtk_lsadc_button_data *button)
{
	int ret, len;
	int i, j;
	bool NOT_USE_CH[MAX_CHANNEL] = {0};
	u32 val[RTK_LSADC_BUTTON_MAX_INFO];
	char property_name[10];
	struct property *prop = NULL;

	button->button_nums = 0;
	memset(button->keycode, -1, sizeof(button->keycode));
	memset(button->button_info, 0, sizeof(button->button_info));

	for (i = 0; i < MAX_CHANNEL; i++) {
		snprintf(property_name, sizeof(property_name), "ch%d-key", i);
		prop = of_find_property(np, property_name, NULL);
		if (prop) {
			if (prop->length / sizeof(u32) % 3 != 0)
				return dev_err_probe(button_dev->dev, -EINVAL, "%pOF: ch%d key config error\n", np, i);

			len = prop->length / sizeof(u32) / 3;
			if(len > MAX_CHANNEL_KEY)
				return dev_err_probe(button_dev->dev, -EINVAL, "%pOF: ch%d key config error\n", np, i);

			ret = of_property_read_u32_array(np, property_name, val, len * 3);
			if (ret)
				return dev_err_probe(button_dev->dev, -EINVAL, "%pOF: read ch%d-key error\n", np, i);

			for(j = 0; j < len; j++) {
				button->button_info[i][j].vs_min = val[j * 3];
				button->button_info[i][j].vs_max = val[j * 3 + 1];
				button->button_info[i][j].vs_key = val[j * 3 + 2];
				button->keycode[button->button_nums + j] = val[j * 3 + 2];
			}

			button->button_nums += len;
		} else {
			NOT_USE_CH[i] = true;
		}
	}
	...
	return dev_err_probe(button_dev->dev, -EINVAL, "%pOF: not find any key config\n", np);
}
...
static int rtk_lsdac_button_unset_lsadc_cmpblk(struct rtk_lsadc_button_device *button_dev)
{
	...
	for (i = 0; i < MAX_CHANNEL; i++) {
		for (j = 0; j < MAX_CHANNEL_KEY; j++) {
			ret = snprintf(name, sizeof(name), "cmpblk%d_raw", j);
