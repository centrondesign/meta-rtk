...
void rtk_drm_ringbuffer_reset(struct rtk_drm_ringbuffer *rb, bool is_cacheable)
{
	...
	rtk_drm_tag_ringbuffer_header_ptrs_init(
		&rb->shm_ringheader, rb->virt + rb->header_offset, is_cacheable);

	*rb->shm_ringheader.p_begin_addr = cpu_to_rpmsg32(rpdev, (u32)rb->addr);
	*rb->shm_ringheader.p_size      = cpu_to_rpmsg32(rpdev, rb->header_offset);
	*rb->shm_ringheader.p_buffer_id   = cpu_to_rpmsg32(rpdev, 1);
	*rb->shm_ringheader.p_write_ptr   = *rb->shm_ringheader.p_begin_addr;
	*rb->shm_ringheader.p_read_ptr = *rb->shm_ringheader.p_begin_addr;
}

int rtk_drm_ringbuffer_alloc(struct drm_device *drm, struct rpmsg_device *rpdev,
			     struct rtk_drm_ringbuffer *rb, u32 buffer_size, bool is_cacheable)
{
	rb->drm = drm;
	rb->rpdev = rpdev;
	...
	rtk_drm_ringbuffer_reset(rb, is_cacheable);
	return  0;
}
...
int rtk_drm_ringbuffer_write(struct rtk_drm_ringbuffer *rb, void *cmd, u32 size)
{
	...
	buf_r = rpmsg32_to_cpu(rb->rpdev, *rb->shm_ringheader.p_read_ptr);
	buf_w = rpmsg32_to_cpu(rb->rpdev, *rb->shm_ringheader.p_write_ptr);
	buf_b = rpmsg32_to_cpu(rb->rpdev, *rb->shm_ringheader.p_begin_addr);
	buf_s = rpmsg32_to_cpu(rb->rpdev, *rb->shm_ringheader.p_size);
	buf_l = buf_b + buf_s;
	...
	*rb->shm_ringheader.p_write_ptr = cpu_to_rpmsg32(rb->rpdev, buf_w);
	return 0;
}
			    ...
			    bool update_read_ptr)
{
	...
	if (update_read_ptr) {
		...
		*rb->shm_ringheader.p_read_ptr = cpu_to_rpmsg32(rb->rpdev, buf_r);
	}
	return 0;
}
