...
static u32 rtk_edp_crt_read(struct rtk_edp *edp, u32 reg)
{
	unsigned int val;
...
// stb needs symbol: drm_property_create_blob, drm_property_replace_global_blob
#ifdef CONFIG_CHROME_PLATFORMS
static int rtk_edp_update_dpcd_property(struct rtk_edp *edp)
{
	int ret;

	ret = drm_property_replace_global_blob(edp->drm_dev,
				 &edp->dpcd_blob,
				 ...
				 edp->dpcd_property);
	if (ret)
		dev_err(edp->dev, "replace dpcd blob failed");

	return ret;
}

static int rtk_edp_update_link_status_property(struct rtk_edp *edp)
{
	int ret;

	ret = drm_property_replace_global_blob(edp->drm_dev,
				 &edp->link_status_blob,
				 DP_LINK_STATUS_SIZE,
				 edp->link_status,
				 &edp->connector.base,
				 edp->link_status_property);
	if (ret)
		dev_err(edp->dev, "replace link status blob failed");
...
// stb needs symbol: drm_property_create_blob, drm_property_replace_global_blob
#ifdef CONFIG_CHROME_PLATFORMS
	rtk_edp_update_dpcd_property(edp);
#endif
	return ret;
}
...
static int rtk_edp_get_link_status(struct rtk_edp *edp)
{
	...
	rtk_edp_update_link_status_property(edp);
	return 0;
}
...
{
	u32 phy_pow = rtk_edp_crt_read(edp, SYS_EDPTX_PHY_CTRL0)
				 & SYS_EDPTX_PHY_CTRL0_REG_POW_EDP_mask;

	return __clk_is_enabled(edp->clk_edp) || __clk_is_enabled(edp->clk_edptx) || phy_pow;
}
...
static int poll_hpd(struct rtk_edp *edp)
{
	...
	struct drm_connector *connector;
	struct drm_device *dev __attribute__((unused));
	bool is_short_pulse;
	bool is_long_pulse;
...
static int rtk_edp_get_eq_pattern(struct rtk_edp *edp)
{
	return (edp->test_train_pattern) ? edp->test_train_pattern :
						 ...
						 struct rtk_edp_train_signal signals[4])
{
	...
	int eq_pattern = rtk_edp_get_eq_pattern(edp);

	dev_info(edp->dev, "[%s] start train eq, pattern: %d\n", __func__, eq_pattern);
...
static void rtk_car_edp_enc_enable(struct drm_encoder *encoder)
{
	struct drm_display_mode *mode = &encoder->crtc->state->adjusted_mode;
	struct rtk_edp *edp = to_rtk_edp(encoder);
	...
	dev_info(edp->dev, "edp: car enc enable");
	...
	hw_init_rpc.link_rate    = edp->link_rate;
	hw_init_rpc.lane_count   = edp->lane_count;
	hw_init_rpc.bpc          = edp->bpc;
...
static void rtk_car_edp_enc_disable(struct drm_encoder *encoder)
{
	struct rtk_edp *edp = to_rtk_edp(encoder);
	struct rtk_rpc_info *rpc_info = edp->rpc_info;
	struct rpc_hw_init_display_out_interface hw_init_rpc;
	...
	dev_info(edp->dev, "edp: car enc disable\n");
...
static const struct drm_encoder_helper_funcs rtk_car_edp_encoder_helper_funcs = {
	.mode_fixup = rtk_edp_enc_mode_fixup,
	.mode_set   = rtk_edp_enc_mode_set,
	.enable     = rtk_car_edp_enc_enable,
	.disable    = rtk_car_edp_enc_disable,
...
static void rtk_edp_stop_detect_hpd(struct rtk_edp *edp)
{
	struct device *dev = edp->dev;
...
(struct drm_connector *connector, bool force)
{
	...
	if (edp->edp_data->type == RTK_AUTOMOTIVE_TYPE)
		edp->connected = true;
...
static int rtk_car_edp_get_modes(struct rtk_edp *edp)
{
	struct drm_connector *connector;
	struct rtk_rpc_info *rpc_info = edp->rpc_info;
	struct drm_display_mode *mode;
...
static int rtk_edp_conn_atomic_check(struct drm_connector *connector,
					  struct drm_atomic_state *state)
{
	...
	if (old_connector_state->crtc) {
		...
		if (edp->is_autotest) {
			...
			dev_info(edp->dev, "atomic check format changed");
		}
	}
...
static int rtk_edp_conn_set_property(struct drm_connector *connector,
				...
				uint64_t val)
{
	...
	ret = -EINVAL;
	if (property == edp->link_rate_property) {
		ret = 0;
		edp->link_rate =
			 (val == LINK_RATE_1_62) ? DP_LINK_RATE_1_62 :
			 (val == LINK_RATE_2_7) ? DP_LINK_RATE_2_7 :
			 (val == LINK_RATE_5_4) ? DP_LINK_RATE_5_4 :
			 LINK_RATE_UNSPECIFIED;
		if (edp->link_rate > LINK_RATE_UNSPECIFIED) {
			edp->is_autotest = true;
			dev_info(edp->dev, "[%s] link rate: %d(prop: %llu)\n",
				 __func__, edp->link_rate, val);
		}
	} else if (property == edp->lane_count_property) {
		ret = 0;
		edp->lane_count =
			 (val == LANE_COUNT_1) ? LANE_COUNT_1 :
			 (val == LANE_COUNT_2) ? LANE_COUNT_2 :
			 LANE_COUNT_UNSPECIFIED;
		if (edp->lane_count > LANE_COUNT_UNSPECIFIED) {
			edp->is_autotest = true;
			dev_info(edp->dev, "[%s] lane count: %u(prop: %llu)\n",
				 __func__, edp->lane_count, val);
		}
	} else if (property == edp->train_pattern_property) {
		ret = 0;
		edp->test_train_pattern =
			 (val == TRAIN_PATTERN_2) ? DP_TRAINING_PATTERN_2 :
			 (val == TRAIN_PATTERN_3) ? DP_TRAINING_PATTERN_3 :
			 TRAIN_PATTERN_UNSPECIFIED;
		if (edp->test_train_pattern > TRAIN_PATTERN_UNSPECIFIED) {
			edp->is_autotest = true;
			dev_info(edp->dev, "[%s] test_train_pattern: %d(prop: %llu)\n",
				 __func__, edp->test_train_pattern, val);
		}
	}
...
static int rtk_edp_conn_get_property(struct drm_connector *connector,
				...
				uint64_t *val)
{
	...
	if (property == edp->link_rate_property) {
		*val =
			 (edp->link_rate == DP_LINK_RATE_1_62) ? LINK_RATE_1_62 :
			 (edp->link_rate == DP_LINK_RATE_2_7) ? LINK_RATE_2_7 :
			 (edp->link_rate == DP_LINK_RATE_5_4) ? LINK_RATE_5_4 :
			 LINK_RATE_UNSPECIFIED;
		return 0;
	} else if (property == edp->lane_count_property) {
		*val =
			 (edp->lane_count == LANE_COUNT_1) ? LANE_COUNT_1 :
			 (edp->lane_count == LANE_COUNT_2) ? LANE_COUNT_2 :
			 LANE_COUNT_UNSPECIFIED;
		return 0;
	} else if (property == edp->train_pattern_property) {
		tmp = rtk_edp_get_eq_pattern(edp);
		*val =
			 (tmp == DP_TRAINING_PATTERN_2) ? TRAIN_PATTERN_2 :
...
//	.dpms = drm_atomic_helper_connector_dpms,
	...
	.atomic_set_property = rtk_edp_conn_set_property,
	.atomic_get_property = rtk_edp_conn_get_property,
...
static struct drm_connector_helper_funcs rtk_edp_connector_helper_funcs = {
	...
	.atomic_check = rtk_edp_conn_atomic_check,
};
...
static int edp_aux_isr(struct rtk_edp *edp)
{
	unsigned int val;
	int ret = 0;
	int i;
...
static int rtk_edp_register(struct drm_device *drm, struct rtk_edp *edp)
{
	...
	drm_encoder_helper_add(encoder, edp->edp_data->helper_funcs);
...
// stb needs symbol: drm_property_create_blob, drm_property_replace_global_blob
	...
	/* link rate property */
	prop = drm_property_create_enum(edp->drm_dev, 0, "link rate",
				 link_rate_list,
				 ARRAY_SIZE(link_rate_list));
	if (!prop) {
		dev_err(edp->dev, "create link rate enum property failed");
		return -ENOMEM;
	}
	edp->link_rate_property = prop;
	drm_object_attach_property(&edp->connector.base, prop, LINK_RATE_UNSPECIFIED);

	/* lane count property */
	prop = drm_property_create_enum(edp->drm_dev, 0, "lane count",
				 lane_count_list,
				 ARRAY_SIZE(lane_count_list));
	if (!prop) {
		dev_err(edp->dev, "create lane count enum property failed");
		return -ENOMEM;
	}
	edp->lane_count_property = prop;
	drm_object_attach_property(&edp->connector.base, prop, LANE_COUNT_UNSPECIFIED);

	/* train pattern property */
	prop = drm_property_create_enum(edp->drm_dev, 0, "train pattern",
				 train_pattern_list,
				 ARRAY_SIZE(train_pattern_list));
	if (!prop) {
		dev_err(edp->dev, "create train pattern enum property failed");
		return -ENOMEM;
	}
	edp->train_pattern_property = prop;
	drm_object_attach_property(&edp->connector.base, prop, TRAIN_PATTERN_UNSPECIFIED);

// stb needs symbol: drm_property_create_blob, drm_property_replace_global_blob
#ifdef CONFIG_CHROME_PLATFORMS
	/* DPCD property */
	prop = drm_property_create(edp->drm_dev,
				DRM_MODE_PROP_IMMUTABLE | DRM_MODE_PROP_BLOB,
				"DPCD(0x0)", 0);
	if (!prop) {
		dev_err(edp->dev, "create dpcd property failed");
		return -ENOMEM;
	}
	edp->dpcd_property = prop;
	drm_object_attach_property(&edp->connector.base, prop, 0);
	dpcd_blob = drm_property_create_blob(edp->drm_dev,
				 DP_RECEIVER_CAP_SIZE, edp->rx_cap);
	if (IS_ERR(dpcd_blob)) {
		dev_err(edp->dev, "create dpcd blob failed");
		return PTR_ERR(dpcd_blob);
	}
	edp->dpcd_blob = dpcd_blob;
	drm_object_property_set_value(&edp->connector.base,
				 edp->dpcd_property, dpcd_blob->base.id);
	...
	prop = drm_property_create(edp->drm_dev,
				...
				"link status(0x202)", 0);
	if (!prop) {
		dev_err(edp->dev, "create link status property failed");
		return -ENOMEM;
	}
	edp->link_status_property = prop;
	drm_object_attach_property(&edp->connector.base, prop, 0);
	link_status_blob = drm_property_create_blob(edp->drm_dev,
				 DP_LINK_STATUS_SIZE, edp->link_status);
	if (IS_ERR(link_status_blob)) {
		dev_err(edp->dev, "create link status blob failed");
		return PTR_ERR(link_status_blob);
	}
	edp->link_status_blob = link_status_blob;
	drm_object_property_set_value(&edp->connector.base,
				 edp->link_status_property, link_status_blob->base.id);
#endif

	return 0;
}

static int rtk_edp_parse_dt(struct rtk_edp *edp)
{
				 ...
				 void *data)
{
	...
	ret = rtk_edp_parse_dt(edp);
	if (ret) {
		dev_err(edp->dev, "rtk_edp_parse_dt fail\n");
		return ret;
	}
...
static const struct rtk_edp_platform_data rtk_edp_data = {
	.type = RTK_NORMAL_TYPE,
	.get_modes = rtk_1920_edp_get_modes,
	.helper_funcs = &rtk_edp_encoder_helper_funcs,
};

static const struct rtk_edp_platform_data rtk_car_edp_data = {
	.type = RTK_AUTOMOTIVE_TYPE,
	.get_modes = rtk_car_edp_get_modes,
	.helper_funcs = &rtk_car_edp_encoder_helper_funcs,
};
...
static const struct of_device_id rtk_edp_dt_ids[] = {
	...
	{
		.compatible = "realtek,rtk-car-edp",
		.data = &rtk_car_edp_data,
	},
	{},
};
...
static int rtk_edp_probe(struct platform_device *pdev)
{
	...
	edp->edp_data = of_device_get_match_data(&pdev->dev);
...
static void rtk_edp_remove(struct platform_device *pdev)
{
