...
static const struct rtk_edid_quirk {
	...
	/* Pioneer  VSX-LX302 */
	{ "PIO", 4455, RTK_EDID_QUIRK_HDMI_MODE, 0x0},
};
...
static int rtk_do_ovt_modes(struct drm_connector *connector, u8 *db, u8 len)
{
	 ...
	 * and flags 2X, 4X, 6X, 8X, and FR144 shall be considered cleared.
	 */
	if (len == 1) {
		frame_rate_factor |= 0x3;
		frame_rate += 0x08;
	}
	...
	for (i = 0; i < ARRAY_SIZE(rtk_ovt_info_list); i++) {
		if (rid == rtk_ovt_info_list[i].rid &&
			...
			switch (rtk_ovt_info_list[i].frame_rate) {
			case 0x1:
				fp10s = 24;
			break;
			case 0x2:
			...
			switch (rtk_ovt_info_list[i].frame_rate_factor) {
			case 0x1:
				fp10s *= 5;
			break;
			case 0x2:
			...
			if ((fp10s != 250) && (fp10s != 500) && (fp10s != 1000) &&
				(fp10s != 2000) && (fp10s != 3000) && (fp10s != 4000)) {
				modes += rtk_add_fractional_modes(connector, &rtk_ovt_info_list[i].rtk_ovt_mode);
			}
		}
	}
	return modes;
	...
	for (i = 1; i <= edid->extensions; i++) {
		...
		for_each_cea_db(block, j, start, end) {
			...
			if (cea_db_tag(db) == VIDEO_FORMAT_DATA_BLOCK) {
				lvfd = db[1] & 0x03;
				hdmi->edid_info.vfdb_y420 = db[1] & 0x80;
				hdmi->edid_info.vfdb_ntsc = db[1] & 0x40;
				vfd_len = lvfd + 1;
				vfd_num = (dbl-1) / vfd_len;
				dev_dbg(hdmi->dev, "vfdb size: %u, lvfd: %d, y420: %s, ntsc: %s",
					dbl, lvfd, hdmi->edid_info.vfdb_y420 ? "Y":"N",
					hdmi->edid_info.vfdb_ntsc ? "Y":"N");
