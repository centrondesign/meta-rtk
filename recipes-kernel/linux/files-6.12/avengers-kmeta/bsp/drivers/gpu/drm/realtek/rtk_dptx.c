...
static int rtk_dptx_update_dpcd_property(struct rtk_dptx *dptx)
{
	int ret;

	ret = drm_property_replace_global_blob(dptx->drm_dev,
				 &dptx->dpcd_blob,
				 ...
				 dptx->dpcd_property);
	if (ret)
		dev_err(dptx->dev, "replace dpcd blob failed");

	return ret;
}

static int rtk_dptx_update_link_status_property(struct rtk_dptx *dptx)
{
	int ret;

	ret = drm_property_replace_global_blob(dptx->drm_dev,
				 &dptx->link_status_blob,
				 DP_LINK_STATUS_SIZE,
				 dptx->link_status,
				 &dptx->connector.base,
				 dptx->link_status_property);
	if (ret)
		dev_err(dptx->dev, "replace link status blob failed");
...
static void rtk_dptx_reset(struct rtk_dptx *dptx)
{
	dev_info(dptx->dev, "dptx: reset\n");
...
static int rtk_dptx_get_link_status(struct rtk_dptx *dptx)
{
	...
	rtk_dptx_update_link_status_property(dptx);
...
// stb needs symbol: drm_property_create_blob, drm_property_replace_global_blob
#ifdef CONFIG_CHROME_PLATFORMS
	rtk_dptx_update_dpcd_property(dptx);
#endif
...
static int rtk_dptx_get_eq_pattern(struct rtk_dptx *dptx)
{
	return (dptx->test_train_pattern) ? dptx->test_train_pattern :
		 /* TPS4 fails on MSI Modern MD271UL Monitor, temp remove it */
						 ...
						 struct rtk_dptx_train_signal signals[4])
{
	...
	uint8_t eq_pattern = rtk_dptx_get_eq_pattern(dptx);
...
static int rtk_dptx_enable(struct rtk_dptx *dptx)
{
	...
	/* No need to reset value during fallback */
	if (dptx->is_fallback_mode)
		return 0;
...
static int rtk_dptx_get_modes(struct rtk_dptx *dptx)
{
	...
	if (connector->force == DRM_FORCE_ON) {
		...
		dev_info(dptx->dev, "dptx force mode, num_modes (%d)\n",
				num_modes);
	} else {
		if (edid) {
...
(struct drm_connector *connector, struct drm_display_mode *mode)
{
	...
	if (vic >= VIC_3840X2160P24) {
		dev_dbg(dptx->dev, "dptx mode clock high (%d) >= (%d)\n",
				...
				uint64_t val)
{
	...
	if (property == dptx->link_rate_property) {
		...
		dptx->link_rate =
			 ...
			 LINK_RATE_UNSPECIFIED;
		if (dptx->link_rate > LINK_RATE_UNSPECIFIED) {
			dptx->is_autotest = true;
			dev_info(dptx->dev, "[%s] link rate: %d(prop: %llu)\n",
				 __func__, dptx->link_rate, val);
		}
		...
		dptx->lane_count =
			 ...
			 LANE_COUNT_UNSPECIFIED;
		if (dptx->lane_count > LANE_COUNT_UNSPECIFIED) {
			dptx->is_autotest = true;
			dev_info(dptx->dev, "[%s] lane count: %u(prop: %llu)\n",
				 __func__, dptx->lane_count, val);
		}
	} else if (property == dptx->train_pattern_property) {
		ret = 0;
		dptx->test_train_pattern =
				...
				uint64_t *val)
{
	...
	if (property == dptx->link_rate_property) {
		*val =
			 (dptx->link_rate == DP_LINK_RATE_1_62) ? LINK_RATE_1_62 :
			 (dptx->link_rate == DP_LINK_RATE_2_7) ? LINK_RATE_2_7 :
			 (dptx->link_rate == DP_LINK_RATE_5_4) ? LINK_RATE_5_4 :
			 (dptx->link_rate == DP_LINK_RATE_8_1) ? LINK_RATE_8_1 :
			 LINK_RATE_UNSPECIFIED;
		return 0;
	} else if (property == dptx->lane_count_property) {
		*val =
			 (dptx->lane_count == LANE_COUNT_1) ? LANE_COUNT_1 :
			 (dptx->lane_count == LANE_COUNT_2) ? LANE_COUNT_2 :
			 (dptx->lane_count == LANE_COUNT_4) ? LANE_COUNT_4 :
			 LANE_COUNT_UNSPECIFIED;
		return 0;
	} else if (property == dptx->train_pattern_property) {
		tmp = rtk_dptx_get_eq_pattern(dptx);
		*val =
...
static int dptx_aux_isr(struct rtk_dptx *dptx)
{
	unsigned int val;
	int ret = 0;
	int i;
...
static void rtk_dptx_pd_event_work(struct work_struct *work)
{
	...
	rtk_dptx_hdcp_detect_status(&dptx->hdcp.hdcp1_tee);
...
// stb needs symbol: drm_property_create_blob, drm_property_replace_global_blob
#ifdef CONFIG_CHROME_PLATFORMS
	/* DPCD property */
	prop = drm_property_create(dptx->drm_dev,
				DRM_MODE_PROP_IMMUTABLE | DRM_MODE_PROP_BLOB,
				"DPCD(0x0)", 0);
	if (!prop) {
		dev_err(dptx->dev, "create dpcd property failed");
		return -ENOMEM;
	}
	dptx->dpcd_property = prop;
	drm_object_attach_property(&dptx->connector.base, prop, 0);
	dpcd_blob = drm_property_create_blob(dptx->drm_dev,
				 DP_RECEIVER_CAP_SIZE, dptx->rx_cap);
	if (IS_ERR(dpcd_blob)) {
		dev_err(dptx->dev, "create dpcd blob failed");
		return PTR_ERR(dpcd_blob);
	}
	dptx->dpcd_blob = dpcd_blob;
	drm_object_property_set_value(&dptx->connector.base,
				 dptx->dpcd_property, dpcd_blob->base.id);
	...
	prop = drm_property_create(dptx->drm_dev,
				...
				"link status(0x202)", 0);
	if (!prop) {
		dev_err(dptx->dev, "create link status property failed");
		return -ENOMEM;
	}
	dptx->link_status_property = prop;
	drm_object_attach_property(&dptx->connector.base, prop, 0);
	link_status_blob = drm_property_create_blob(dptx->drm_dev,
				 DP_LINK_STATUS_SIZE, dptx->link_status);
	if (IS_ERR(link_status_blob)) {
		dev_err(dptx->dev, "create link status blob failed");
		return PTR_ERR(link_status_blob);
	}
	dptx->link_status_blob = link_status_blob;
	drm_object_property_set_value(&dptx->connector.base,
				 dptx->link_status_property, link_status_blob->base.id);
#endif
				 ...
				 void *data)
{
	...
	ret = rtk_dptx_hdcp_init(dptx, 1);
	if (ret)
		dev_err(dptx->dev, "dptx hdcp init fail\n");
...
static const struct rtk_dptx_platform_data rtk_dptx_data = {
...
static const struct rtk_dptx_platform_data rtk_car_dptx_data = {
