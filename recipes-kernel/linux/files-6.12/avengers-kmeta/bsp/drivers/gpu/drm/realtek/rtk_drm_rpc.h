...
struct graphic_object {
	 ...
	 * @ext1:
	 *
	 * ext1=afbc_block_split if mode is afbc
	 * ext1=cu_size          if mode is afrc
	 */
	unsigned int ext1;
	 ...
	 * @ext2:
	 *
	 * ext2=afbc_yuv_transform if mode is afbc
	 * ext2=stride_en          if mode is afrc
	 */
	unsigned int ext2;
};
...
struct tag_ringbuffer_header_ptrs {
...
struct tag_refclock_ptrs {
	long long *p_rcd;
	unsigned int *p_rcd_ext;
	long long *p_gpts_timeout;
	long long *p_video_system_pts;
	long long *p_audio_system_pts;
	long long *p_video_rpts;
	long long *p_audio_rpts;
	unsigned int *p_video_context;
	unsigned int *p_audio_context;
	struct tag_master_ship *p_mastership;
	unsigned int *p_video_free_run_threshold;
	unsigned int *p_audio_free_run_threshold;
	long long *p_master_gpts;
	int *p_audio_fullness;
	int *p_audio_pause_flag;
	int *p_vo_underflow;
	int *p_ao_underflow;
	int *p_video_end_of_segment;
	int *p_audio_end_of_segment;
};

static inline
void rtk_drm_tag_ringbuffer_header_ptrs_init(struct tag_ringbuffer_header_ptrs *ptrs,
...
static inline
void rtk_drm_tag_refclock_ptrs_init(struct tag_refclock_ptrs *ptrs,
				    void *base, bool cacheable)
{
	if (!cacheable) {
		ptrs->p_rcd = base + 0;
		ptrs->p_rcd_ext = base + 8;
		ptrs->p_gpts_timeout = base + 16;
		ptrs->p_video_system_pts = base + 24;
		ptrs->p_audio_system_pts = base + 32;
		ptrs->p_video_rpts = base + 40;
		ptrs->p_audio_rpts = base + 48;
		ptrs->p_video_context = base + 56;
		ptrs->p_audio_context = base + 60;
		ptrs->p_mastership = base + 64;
		ptrs->p_video_free_run_threshold = base + 68;
		ptrs->p_audio_free_run_threshold = base + 72;
		ptrs->p_master_gpts = base + 80;
		ptrs->p_audio_fullness = base + 88;
		ptrs->p_audio_pause_flag = base + 92;
		ptrs->p_vo_underflow = base + 96;
		ptrs->p_ao_underflow = base + 100;
		ptrs->p_video_end_of_segment = base + 104;
		ptrs->p_audio_end_of_segment = base + 108;
	} else {
		ptrs->p_rcd = base + 128;
		ptrs->p_rcd_ext = base + 136;
		ptrs->p_gpts_timeout = base + 0;
		ptrs->p_video_system_pts = base + 8;
		ptrs->p_audio_system_pts = base + 256;
		ptrs->p_video_rpts = base + 16;
		ptrs->p_audio_rpts = base + 264;
		ptrs->p_video_context = base + 24;
		ptrs->p_audio_context = base + 272;
		ptrs->p_mastership = base + 384;
		ptrs->p_video_free_run_threshold = base + 28;
		ptrs->p_audio_free_run_threshold = base + 32;
		ptrs->p_master_gpts = base + 144;
		ptrs->p_audio_fullness = base + 276;
		ptrs->p_audio_pause_flag = base + 36;
		ptrs->p_vo_underflow = base + 40;
		ptrs->p_ao_underflow = base + 280;
		ptrs->p_video_end_of_segment = base + 44;
		ptrs->p_audio_end_of_segment = base + 284;
	}
}
...
int rpc_set_out_interface(struct rtk_rpc_info *rpc_info,
			    struct rpc_set_display_out_interface *arg);
int rpc_query_out_fw_memory_config(struct rtk_rpc_info *rpc_info,
