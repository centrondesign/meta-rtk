...
static void hdcp2_disable_encryption(struct rtk_hdmi *hdmi, bool is_reauth)
{
	struct rtk_hdcp *hdcp = &hdmi->hdcp;
	struct rtk_hdcp2_tee *hdcp2_tee = &hdcp->hdcp2_tee;
	...
	if (hdcp2_tee->hdcp2_tee_ops &&
	   hdcp2_tee->hdcp2_tee_ops->disable_hdcp2_cipher)
		hdcp2_tee->hdcp2_tee_ops->disable_hdcp2_cipher(hdcp2_tee, is_reauth);
...
static int rtk_hdcp_auth(struct rtk_hdmi *hdmi)
{
	...
	ret = hdcp1_tee_ops->hdcp1_set_encryption(hdcp1_tee, HDCP_ENC_ON, NOT_HDCP_REAUTH);

	return ret;
}

static void _rtk_hdcp_disable(struct rtk_hdmi *hdmi, bool is_reauth)
{
	int ret;
	struct rtk_hdcp *hdcp = &hdmi->hdcp;
	struct rtk_hdcp1_tee *hdcp1_tee = &hdcp->hdcp1_tee;
	...
	ret = hdcp1_tee_ops->hdcp1_set_encryption(hdcp1_tee, HDCP_ENC_OFF, is_reauth);
...
static int _rtk_hdcp_enable(struct rtk_hdmi *hdmi)
{
	...
	_rtk_hdcp_disable(hdmi, NOT_HDCP_REAUTH);
...
static void _rtk_hdcp2_disable(struct rtk_hdmi *hdmi, bool is_reauth)
{
	struct rtk_hdcp *hdcp = &hdmi->hdcp;

	DRM_DEBUG_DRIVER("HDCP2 is being disabled");

	hdcp2_disable_encryption(hdmi, is_reauth);
...
static int rtk_hdcp_check_link(struct rtk_hdmi *hdmi)
{
	...
	_rtk_hdcp_disable(hdmi, IS_HDCP_REAUTH);
...
static int rtk_hdcp2_check_link(struct rtk_hdmi *hdmi)
{
	...
	_rtk_hdcp2_disable(hdmi, IS_HDCP_REAUTH);
	hdcp->value = DRM_MODE_CONTENT_PROTECTION_DESIRED;
...
int rtk_hdcp_enable(struct rtk_hdmi *hdmi)
{
	 ...
	 */
	if (hdcp->hdcp2_supported && !hdcp->force_hdcp14 && !hdmi->is_dvi_mode) {
		struct rtk_hdcp2_tee *hdcp2_tee = &hdcp->hdcp2_tee;
...
int rtk_hdcp_disable(struct rtk_hdmi *hdmi)
{
	...
	if (hdcp->hdcp2_encrypted)
		_rtk_hdcp2_disable(hdmi, NOT_HDCP_REAUTH);
...
static ssize_t hdcp_info_show(struct device *dev,
...
static int rtk_hdcp_check_hdcp_ops(const struct rtk_hdcp_ops *hdcp_ops)
{
	if (!hdcp_ops)
		return -EINVAL;
