...
#define HIFI_VO_SYNC_FLAG 0x9810060c
#define KR4_VO_SYNC_FLAG  0x9801a588
#define HIFI_VO_ON_FLAG   0x9801a604
...
static int drm_ept_init(struct rtk_krpc_ept_info *krpc_ept_info)
{
	int ret = 0;
...
static int drm_send_rpc(struct device *dev, struct rtk_krpc_ept_info *krpc_ept_info, char *buf, int len, uint32_t *retval)
{
	int ret = 0;
...
static int rpc_video_run_cmd(struct rtk_rpc_info *rpc_info, unsigned int instance,
				...
				struct rpc_privateinfo_result *result)
{
	...
	if (result) {
		if (opt != RPC_AUDIO)
			...
			result->private_info[0] = htonl(retval->private_info[0]);
	}
...
int rtk_rpc_init(struct device *dev, struct rtk_rpc_info *rpc_info, int of_index)
{
	...
	if (rpc_info->krpc_vo_opt == RPC_HIFI) {
		val = readl(ioremap(HIFI_VO_ON_FLAG, 0x1));
		if ((val & (1 << 21)) && (val & (1 << 23))) {
			dev_info(dev, "VO HIFI is on\n");
			rpc_info->vo_sync_flag = ioremap(HIFI_VO_SYNC_FLAG, 0x1);
			writel(0, rpc_info->vo_sync_flag);
		}
	} else if (rpc_info->krpc_vo_opt == RPC_KR4) {
		rpc_info->vo_sync_flag = ioremap(KR4_VO_SYNC_FLAG, 0x1);
		writel(0, rpc_info->vo_sync_flag);
	} else
	...
	if (lock_id > 0 || (IS_ENABLED(CONFIG_HWSPINLOCK) && lock_id == 0)) {
		...
		if (lock) {
			...
			if (priv->hwlock)
				rpc_info->hwlock = priv->hwlock;
		}

		if (rpc_info->hwlock == NULL)
			dev_err(dev, "failed to request hwlock%d\n", lock_id);

		dev_info(dev, "%s use hwlock%d\n",
			krpc_names[rpc_info->krpc_vo_opt], lock_id);
	} else {
		if (lock_id != -ENOENT)
