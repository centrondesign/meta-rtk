...
static int rtk_hdmi_set_5v(struct rtk_hdmi *hdmi,
...
static bool is_rtk_ovt_mode(struct drm_display_mode *mode)
{
	unsigned int fractional_clock;
	unsigned int clock;
	int i;
	...
	for (i = 0; i < ARRAY_SIZE(rtk_ovt_info_list); i++) {
		clock = rtk_ovt_info_list[i].rtk_ovt_mode.clock;
		fractional_clock = DIV_ROUND_CLOSEST(clock * 1000, 1001);
		if (mode->hdisplay == rtk_ovt_info_list[i].rtk_ovt_mode.hdisplay &&
			mode->vdisplay == rtk_ovt_info_list[i].rtk_ovt_mode.vdisplay &&
			(mode->clock == clock || mode->clock == fractional_clock))
			return true;
	}
...
static int rtk_dmt_to_vic(struct rtk_hdmi *hdmi, struct drm_display_mode *mode)
{
	...
	cur_fp10s = get_frame_per_10s(hdmi, mode);
	for (i = 0; i < ARRAY_SIZE(rtk_dmt_info_list); i++) {
		dmt_mode = rtk_dmt_info_list[i].rtk_dmt_modes;
		list_fp10s = get_frame_per_10s(hdmi, &dmt_mode);
		if (mode->hdisplay == dmt_mode.hdisplay &&
			mode->vdisplay == dmt_mode.vdisplay &&
			cur_fp10s == list_fp10s) {
			return rtk_dmt_info_list[i].dmt_id;
		}
	}
...
static bool is_rtk_dmt_mode(struct rtk_hdmi *hdmi, struct drm_display_mode *mode)
{
	struct drm_display_mode dmt_mode;
	int cur_fp10s;
	int list_fp10s;
...
static int rtk_ovt_to_vic(struct rtk_hdmi *hdmi, struct drm_display_mode *mode)
{
	int i;
	int rid = 0;
	int fr = 0;
	...
	for (i = 0; i < ARRAY_SIZE(rtk_ovt_info_list); i++) {
		if (mode->hdisplay == rtk_ovt_info_list[i].rtk_ovt_mode.hdisplay &&
			mode->vdisplay == rtk_ovt_info_list[i].rtk_ovt_mode.vdisplay &&
			mode->clock == rtk_ovt_info_list[i].rtk_ovt_mode.clock) {
			rid = rtk_ovt_info_list[i].rid;
			fp10s = get_frame_per_10s(hdmi, mode);
			switch (fp10s) {
			case 239:
				fr = OVT_FR_23HZ;
...
static bool is_same_scramble_status(struct rtk_hdmi *hdmi, unsigned int tmdsconfig)
{
	...
	/* TODO: Use drm_scdc_get_scrambling_status when fix symbol issue */
			...
			struct drm_display_mode *mode)
{
	...
	if ((hdmi->connector.force == DRM_FORCE_ON) ||
		(colorspace == HDMI_COLORSPACE_YUV422)) {
		bpc = requested_bpc;
		goto exit;
	}
...
static unsigned char get_tmds_pll_mode(struct rpc_display_output_format format)
{
	...
	case VIC_1280x1024P60:
		pll_mode = PLL_108MHZ;
		break;
	case VIC_2560x1600P60RB:
	case VIC_2560x1600P60:
		pll_mode = PLL_348p5MHZ;
		break;
	default:
		pll_mode = PLL_INVALIDMHZ;
		break;
...
static void enable_hdmitx_pll_power(struct rtk_hdmi *hdmi, unsigned char pll_mode)
{
...
static void active_hdmitx_pll_control(struct rtk_hdmi *hdmi)
{
	unsigned int reg_val;
	...
	struct drm_display_mode *mode, void *avi_buffer)
{
	...
	/* TODO: Use drm_hdmi_avi_infoframe_content_type when fix symbol issue */
	...
	struct drm_display_mode *mode)
{
	...
	if ((hdmi->edid_info.edid_quirks & RTK_EDID_QUIRK_HDMI_MODE) ||
		hdmi->edid_info.sink_is_hdmi || is_rtk_non_cea_mode(mode) ||
		hdmi->connector.force == DRM_FORCE_ON || hdmi->edid_cache == NULL) {
		tvsystem.info_frame.hdmiMode = VO_HDMI_ON;
		hdmi->is_dvi_mode = false;
	} else {
		tvsystem.info_frame.hdmiMode = VO_DVI_ON;
		hdmi->is_dvi_mode = true;
	}
	...
	struct drm_display_mode *mode)
{
	...
	if ((hdmi->edid_info.edid_quirks & RTK_EDID_QUIRK_HDMI_MODE) ||
		(hdmi->ext_flags & RTK_EXT_SKIP_EDID_AUDIO) ||
		(hdmi->connector.force == DRM_FORCE_ON) ||
	...
	if (is_rtk_ovt_mode(mode)) {
		format.vic = rtk_ovt_to_vic(hdmi, mode);
		format.cmd_version = 1;
	} else if (is_rtk_dmt_mode(hdmi, mode)) {
		format.vic = rtk_dmt_to_vic(hdmi, mode);
		format.cmd_version = 1;
	} else {
...
static bool rtk_hdmi_update_hpd_state(struct rtk_hdmi *hdmi)
{
	...
	if ((hdmi->hpd_state == 0) && (hpd == 1)) {
		if (!IS_ERR_OR_NULL(hdmi->edid_cache) &&
			!hdmi->is_edid_override) {
			kfree(hdmi->edid_cache);
			hdmi->edid_cache = NULL;
		}

		if (!hdmi->is_edid_override)
...
static int rtk_hdmi_init_properties(struct rtk_hdmi *hdmi)
{
	...
	hdmi->request_auto_mode = HDMI_AUTO_DISABLE;
	hdmi->auto_mode = HDMI_AUTO_DISABLE;
...
static int rtk_hdmi_conn_set_property(struct drm_connector *connector,
...
static int rtk_hdmi_conn_get_property(struct drm_connector *connector,
...
static int rtk_hdmi_conn_get_modes(struct drm_connector *connector)
{
	...
	list_for_each_entry(mode, &connector->probed_modes, head) {
		...
		if (rtk_hdmi_valid_vic(vic) ||
			is_rtk_ovt_mode(mode) ||
			is_rtk_dmt_mode(hdmi, mode)) {
			match_rtk_mode = true;
			break;
		}
	}
...
static ssize_t edid_override_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	...
	if (hdmi->edid_cache == NULL) {
		...
		count = (hdmi->edid_cache->extensions + 1) * EDID_LENGTH;
		memcpy(buf, (u8 *)hdmi->edid_cache, count);
	}

	return count;
}

static ssize_t edid_override_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	hdmi->is_edid_override = false;

	if (count == 5 && !strncmp(buf, "reset", 5)) {
		ret = drm_connector_update_edid_property(connector, NULL);
	} else if (count < EDID_LENGTH ||
		   EDID_LENGTH * (1 + edid->extensions) > count)
		ret = -EINVAL;
	else {
		hdmi->edid_cache = kmemdup(buf, count, GFP_KERNEL);
		if (!hdmi->edid_cache)
			return -ENOMEM;

		ret = drm_connector_update_edid_property(connector, hdmi->edid_cache);
		if (!ret)
			hdmi->is_edid_override = true;
	}

	return count;
}

static DEVICE_ATTR_RW(edid_override);
...
skip_5v_gpio:
	...
	hdmi->is_edid_override = false;
	ret = device_create_file(hdmi->dev, &dev_attr_edid_override);
...
static void rtk_hdmi_remove(struct platform_device *pdev)
{
