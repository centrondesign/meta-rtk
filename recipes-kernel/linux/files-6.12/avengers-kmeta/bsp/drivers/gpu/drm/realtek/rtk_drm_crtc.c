...
int rtk_crtc_ioctl_get_mixer_id(struct drm_device *dev, void *data, struct drm_file *file)
{
	...
	DRM_INFO("rtk_crtc_ioctl: get_mixer_id\n");
...
int rtk_crtc_get_display_panel_usage_by_mixer(struct rtk_drm_crtc *rtk_crtc)
{
	...
	DRM_INFO("rtk_crtc: get_display_panel_usage_by_mixer\n");
...
static int rtk_crtc_rpc_bind(struct rtk_drm_crtc *rtk_crtc, struct rtk_drm_private *priv)
{
	...
	if (rpc_info->krpc_vo_opt != RPC_KR4) {
		DRM_INFO("skip rpc bind when opt is %s\n",
				...
				struct drm_atomic_state *old_crtc_state)
{
	...
	if (rtk_crtc->event) {
		...
		DRM_DEBUG_DRIVER("pending_needs_vblank\n");
	}
	spin_unlock_irqrestore(&drm->event_lock, flags);
}
...
static irqreturn_t rtk_crtc_isr(int irq, void *dev_id)
{
...
static void rtk_crtc_remove(struct platform_device *pdev)
{
