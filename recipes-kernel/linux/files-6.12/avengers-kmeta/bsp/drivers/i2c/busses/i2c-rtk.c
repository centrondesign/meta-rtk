...
#ifdef SDA_DEL_EN
static int SDA_DEL_SHIFT[] = { 0x84, 0x80, 0x80, 0x84, 0x88, 0x8C, 0x90, 0xF0, 0x0 };
...
static int rtk_i2c_bus_get(struct i2c_adapter *adap)
{
	...
	dummy_reg = ioremap(0x9801a304, 0x4);
retry:
	ret = hwspin_lock_timeout_irqsave(priv->hwlock, 10, &flags);
	if (ret)
		goto exit;
	...
	if (reg & (1 << adap->nr)) {
		if (retry++ < 10) {
			hwspin_unlock_irqrestore(priv->hwlock, &flags);
			msleep(10);
			goto retry;
		} else {
			pr_err("%s bus busy, get fail\n", __func__);
			ret = -EBUSY;
			goto unlock;
		}
	}
	reg |= 1 << adap->nr;
	writel(reg, dummy_reg);
unlock:
	hwspin_unlock_irqrestore(priv->hwlock, &flags);
exit:
	iounmap(dummy_reg);
...
static int rtk_i2c_bus_put(struct i2c_adapter *adap)
{
	struct rtk_i2c_dev *priv = i2c_get_adapdata(adap);
	void __iomem *dummy_reg;
	unsigned long flags;
	...
	reg = readl(dummy_reg);
	if (reg & (1 << adap->nr))
		writel((reg & ~(1 << adap->nr)), dummy_reg);
	else
		pr_err("%s fail, already put???\n", __func__);
...
static int rtk_i2c_init(struct rtk_i2c_dev *priv, int getbus)
{
	struct i2c_adapter *adap = &priv->adap;
	int ret;

	if (getbus == 1) {
		ret = rtk_i2c_bus_get(adap);
		if (ret) {
			pr_err("%s bus get fail\n", __func__);
			return ret;
		}
	}
	...
	ret = rtk_i2c_set_speed(priv);

	if (getbus == 1)
		rtk_i2c_bus_put(adap);
...
static int rtk_i2c_generic_scl_recovery(struct i2c_adapter *adap)
{
...
static int rtk_i2c_probe(struct platform_device *pdev)
{
	...
	strscpy(priv->adap.name, "realtek i2c adapter", sizeof(priv->adap.name));
