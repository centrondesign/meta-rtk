...
#define SMC_CMD_VCPU_STOP  0x8200050a
...
#define IPC_PPC_VE3_OFF		0x7000		/* power off ve3 */
#define IPC_PPC_VE3_ON		0x7008		/* power on ve3 */
...
struct rproc_pm_ops {
	int (*pm_init)(struct rtk_fw_rproc *data);
			    ...
			    u32 cate, u32 opcode, u32 arg)
{
	...
	ret = regmap_read_poll_timeout(rtk_rproc->intr_regmap, intr_offset, val,
				       !(val & intr_bit), POLLING_TIME, 1000 * POLLING_TIME);
	if (ret) {
		dev_err(rtk_rproc->dev, "send %s interrupt timeout\n", dst ? "pcpu" : "rproc");
		return ret;
	}
...
static int dlsr_load(struct rproc *rproc, const struct firmware *fw)
{
	...
	if (rmem->size < fw->size) {
		dev_err(rtk_rproc->dev, "Reserved memory(%llx) is insufficient for firmware (%zx)\n",
			rmem->size, fw->size);
		ret = -EINVAL;
		goto free_mem;
	}
...
static int rtd1625_kr4_attach(struct rproc *rproc)
{
	...
	device_remove_file(rtk_rproc->dev, &dev_attr_verify_state);
...
static int rtd1635_ve3_start(struct rproc *rproc)
{
	struct rtk_fw_rproc *rtk_rproc = rproc->priv;
	struct rtk_ipc_shm __iomem *ipc = (void __iomem *)IPC_SHM_VIRT;
	const struct pm_info *pm = rtk_rproc->info->pm;
	...
	dev_info(&rproc->dev, "ve3 bring up\n");

	ipc->ve3_rpc_flag = 0xffffffff;
	ret = send_ipc_command(rtk_rproc, TO_PCPU, IPC_CMD_BLOCKING, IPC_CATE_PPC,
			       pm->poweron_cmd, VE3_A_ENTRY);
	...
	while (ipc->ve3_rpc_flag && timeout--)
		mdelay(10);

	if (timeout <= 0) {
		dev_info(&rproc->dev, "ve3 boot timeout\n");
		ipc->ve3_rpc_flag = 0;
		return -EINVAL;
	}
...
static int rtd1625_rproc_suspend(struct rtk_fw_rproc *rtk_rproc)
{
	...
	if (pm->stop_cmd) {
		arm_smccc_smc(pm->stop_cmd, 0, 0, 0, 0, 0, 0, 0, &res);
		ret = (unsigned int)res.a0;
		if (ret) {
...
static int rtd1635_ve3_suspend(struct rtk_fw_rproc *rtk_rproc)
{
	const struct pm_info *pm;
	unsigned int val;
	int ret = 0;
	...
	regmap_read(rtk_rproc->ve3_regmap, HEVC_ENC_TOP_CR44, &val);
	if (val & rtk_rproc->info->pm->to_rproc_intr_bit)
		regmap_write(rtk_rproc->ve3_regmap, HEVC_ENC_TOP_CR44,
		    val & ~rtk_rproc->info->pm->to_rproc_intr_bit);

out:
	return ret;
};

static int rtd1635_ve3_resume(struct rtk_fw_rproc *rtk_rproc)
{
	const struct pm_info *pm = rtk_rproc->info->pm;
	unsigned int val;
	int ret = 0;
...
static int rproc_pm_init(struct rtk_fw_rproc *rtk_rproc)
{
	int ret = 0;
	int val;
...
static int ve3_pm_init(struct rtk_fw_rproc *rtk_rproc)
{
	int ret = 0;
	int val;
	...
	rtk_rproc->ve3_regmap = syscon_regmap_lookup_by_phandle(rtk_rproc->dev->of_node, "ve3-syscon");
	if (IS_ERR(rtk_rproc->ve3_regmap)) {
		dev_err(rtk_rproc->dev, "cannot get ve3 regmap\n");
		ret = -EINVAL;
		goto err;
	}
...
static const struct rproc_pm_ops rtd1635_ve3_pm_ops = {
	.pm_init = ve3_pm_init,
	.suspend = rtd1635_ve3_suspend,
	.resume = rtd1635_ve3_resume,
};
...
static const struct rproc_ops rtd1635_ve3_ops = {
	.load = rtd1619b_ve3_fw_load,
	.start = rtd1635_ve3_start,
	.stop = rtd1619b_ve3_stop,
};
...
static const struct pm_info ve3_pm = {
	.poweron_cmd = IPC_PPC_VE3_ON,
	.poweroff_cmd = IPC_PPC_VE3_OFF,
	.intr_offset = 0xa80,
	.intr_en_offset = 0xa84,
	.to_pcpu_intr_bit = BIT(3),
	.to_pcpu_ipc_regoff = 0x470,
...
static const struct rtk_fw_info rtd1635_ve3_info = {
	.ops = &rtd1635_ve3_ops,
	.pm = &ve3_pm,
	.pm_ops = &rtd1635_ve3_pm_ops,
};
...
static int rtk_register_rproc(struct device *dev, struct device_node *node)
{
	...
	if (rtk_rproc->info->pm && rtk_rproc->info->pm_ops->pm_init) {
		ret = rtk_rproc->info->pm_ops->pm_init(rtk_rproc);
		if (ret)
			goto put_rproc;
	}
	ret = rproc_add(rproc);
...
put_rproc:
	device_remove_file(dev, &dev_attr_verify_state);
	rproc_free(rproc);
out:
	return ret;
}
...
static void rtk_rproc_remove(struct platform_device *pdev)
{
	struct rproc *rproc = platform_get_drvdata(pdev);

	device_remove_file(&pdev->dev, &dev_attr_verify_state);
	rproc_del(rproc);
	rproc_free(rproc);
}
...
static const struct of_device_id rtk_rproc_of_match[] = {
	...
	{ .compatible = "realtek,rtd1635-rproc-ve3", .data = &rtd1635_ve3_info },
