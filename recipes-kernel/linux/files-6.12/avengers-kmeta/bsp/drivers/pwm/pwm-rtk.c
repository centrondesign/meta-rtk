...
static inline struct rtk_pwm_chip *to_rtk_pwm_chip(struct pwm_chip *chip)
{
...
static int set_real_freq_by_target_freq(struct rtk_pwm_chip *pc, int hwpwm,
				 int target_freq)
{
...
static int set_real_period_by_target_period(struct rtk_pwm_chip *pc, int hwpwm,
...
static int set_real_freq_by_target_div(struct rtk_pwm_chip *pc, int hwpwm,
...
static int rtk_pwm_probe(struct platform_device *pdev)
{
	...
	chip = devm_pwmchip_alloc(&pdev->dev, NUM_PWM_V3, sizeof(*pwm));
	if (IS_ERR(chip))
		return PTR_ERR(chip);
	pwm = to_rtk_pwm_chip(chip);
	...
	if (ret < 0) {
		dev_err_probe(&pdev->dev, ret, "sysfs_create_group() failed\n");
		return ret;
	}
...
static void rtk_pwm_remove(struct platform_device *pdev)
{
