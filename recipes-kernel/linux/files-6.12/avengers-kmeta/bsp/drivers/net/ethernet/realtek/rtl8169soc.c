...
#define RTL8169SOC_VERSION "2.1.3"
...
static inline int __int_mdio_read(struct rtl8169_private *tp, int reg)
{
	...
	value = rtl_loop_wait_high(tp, &rtl_phyar_cond, 25, 20) ?
		RTL_R32(tp, PHYAR) & PHY_DATA_MASK : -ETIMEDOUT;

	if (value == -ETIMEDOUT) {
		if (net_ratelimit())
			netdev_err(tp->dev, "MDIO timeout\n");
		value = 0;
	}
...
static inline void __int_mdio_write(struct rtl8169_private *tp, int reg,
				    int value)
{
	/* write reg */
	RTL_W32(tp, PHYAR,
		0x80000000 | (reg & 0x1f) << 16 | (value & PHY_DATA_MASK));
...
static int int_ocp_mdio_read(struct rtl8169_private *tp, int page, int reg)
{
	...
	value = rtl_loop_wait_low(tp, &rtl_int_phyar_cond, 25, 20) ?
		rtl_ocp_read(tp, 0xDE08) & PHY_DATA_MASK : -ETIMEDOUT;
...
static void int_ocp_mdio_write(struct rtl8169_private *tp, int page, int reg,
			       int value)
{
	if (page != CURRENT_MDIO_PAGE)
		rtl_ocp_write(tp, 0xDE0C, page);
...
static inline int __ext_mdio_read(struct rtl8169_private *tp, int reg)
{
	...
	value = rtl_loop_wait_high(tp, &rtl_ephyar_cond, 10, 100) ?
		RTL_R32(tp, EPHYAR) & EPHYAR_DATA_MASK : -ETIMEDOUT;
...
static inline void __ext_mdio_write(struct rtl8169_private *tp, int reg,
				    int value)
{
	/* write reg */
...
static void ext_mdio_write(struct rtl8169_private *tp, int page, int reg,
			   int value)
{
	tp->chip->mdio_lock(tp);
...
static int ext_ocp_mdio_read(struct rtl8169_private *tp, int page, int reg)
{
	...
	value = rtl_loop_wait_low(tp, &rtl_ext_phyar_cond, 25, 20) ?
		rtl_ocp_read(tp, 0xDE28) & PHY_DATA_MASK : -ETIMEDOUT;
...
static void ext_ocp_mdio_write(struct rtl8169_private *tp, int page, int reg,
			       int value)
{
	if (page != CURRENT_MDIO_PAGE)
		rtl_ocp_write(tp, 0xDE2C, page);
...
static inline void rtl_phy_write(struct rtl8169_private *tp, int page, int reg,
				 u32 val)
{
...
static void int_mmd_write(struct rtl8169_private *tp, u32 dev_addr,
...
static void ext_mmd_write(struct rtl8169_private *tp, u32 dev_addr,
			  u32 reg_addr, u32 value)
{
	tp->chip->mdio_lock(tp);
...
static void int_ocp_mmd_write(struct rtl8169_private *tp, u32 dev_addr,
			      u32 reg_addr, u32 value)
{
	/* address mode */
...
static u32 int_ocp_mmd_read(struct rtl8169_private *tp, u32 dev_addr,
			    u32 reg_addr)
{
	/* address mode */
	int_ocp_mdio_write(tp, 0, 13, (0x1f & dev_addr));
...
static void ext_ocp_mmd_write(struct rtl8169_private *tp, u32 dev_addr,
			      u32 reg_addr, u32 value)
{
	/* address mode */
...
static u32 ext_ocp_mmd_read(struct rtl8169_private *tp, u32 dev_addr,
			    u32 reg_addr)
{
	/* address mode */
	ext_ocp_mdio_write(tp, 0, 13, (0x1f & dev_addr));
...
static void rtl_mmd_write(struct rtl8169_private *tp, u32 dev_addr,
			  u32 reg_addr, u32 value)
{
...
static void __rtl_eri_write(struct rtl8169_private *tp, int addr, u32 mask,
			    u32 val, int type)
{
...
static void __rtl8169_set_wol(struct rtl8169_private *tp, u32 wolopts)
{
	if (wolopts & WAKE_MAGIC)
		...
		tp->wol_enable &= ~WOL_MAGIC;
	tp->dev->ethtool->wol_enabled = (tp->wol_enable & WOL_MAGIC) ? 1 : 0;
	device_set_wakeup_enable(tp_to_dev(tp), tp->dev->ethtool->wol_enabled);
}
...
static struct sk_buff *rtl_gen_loopback_pkt(struct rtl8169_private *tp,
					    int len)
{
	struct sk_buff *skb;
	u16 type;
...
static struct r8169soc_chip_info rtk_prince_info;
...
static void rtd139x_load_otp_content(struct rtl8169_private *tp)
{
	...
	if (IS_ERR(buf)) {
		dev_err(tp_to_dev(tp), "failed to load OTP content: para\n");
		goto set_idac;
	}
...
set_idac:
	...
	if (IS_ERR(buf)) {
		dev_err(tp_to_dev(tp), "failed to load OTP content: idac\n");
		return;
	}
...
static void rtd16xx_load_otp_content(struct rtl8169_private *tp)
{
	...
	if (IS_ERR(buf)) {
		dev_err(tp_to_dev(tp), "failed to load OTP content: rc_k\n");
		goto set_r_amp_cal;
	}
...
set_r_amp_cal:
	...
	if (IS_ERR(buf)) {
		dev_err(tp_to_dev(tp), "failed to load OTP content: r_amp_k\n");
		return;
	}
...
static void rtd16xxb_load_otp_content(struct rtl8169_private *tp)
{
	...
	if (IS_ERR(buf)) {
		dev_err(tp_to_dev(tp),
			"failed to load OTP content: rc_r_amp_cal\n");
		return;
	}
...
static void rtd1625_load_otp_content(struct rtl8169_private *tp)
{
	...
	buf = r8169soc_read_otp(tp, "etn_rc_cal");
	if (IS_ERR(buf)) {
		dev_err(tp_to_dev(tp),
			"failed to load OTP content: etn_rc_cal\n");
		goto set_etn_idac;
	}

	/* Set RS (for TX) from OTP 0x98032500[18:16] */
	otp = (buf[0] & (0x7 << 0)) >> 0;
	tmp = otp ^ RTD1625_RS_DEFAULT;
	tmp = (int_mdio_read(tp, 0x0bd0, 21) & ~(0x7 << 13)) | (tmp << 13);
	int_mdio_write(tp, 0x0bd0, 21, tmp);

	/* Set RSW (for RX) from OTP 0x98032500[21:19] */
	otp = (buf[0] & (0x7 << 3)) >> 3;
	tmp = otp ^ RTD1625_RSW_DEFAULT;
	tmp = (int_mdio_read(tp, 0x0bd0, 20) & ~(0x7 << 13)) | (tmp << 13);
	int_mdio_write(tp, 0x0bd0, 20, tmp);
...
set_etn_idac:
	...
	buf = r8169soc_read_otp(tp, "etn_idac");
	if (IS_ERR(buf)) {
		dev_err(tp_to_dev(tp),
			"failed to load OTP content: etn_idac\n");
		return;
	}

	/* Set idac_fine_mdi from OTP 0x98032500[4:0] */
	otp = (buf[0] & (0x1F << 0)) >> 0;
	tmp = otp ^ RTD1625_IDAC_FINE_MDI_DEFAULT;
	tmp += tp->amp_k_offset;
	tmp = (int_mdio_read(tp, 0x0bc0, 23) & ~(0x1f << 0)) | (tmp << 0);
	int_mdio_write(tp, 0x0bc0, 23, tmp);
	...
	/* Set idac_fine_mdix from OTP 0x98032500[9:5] */
	otp = ((buf[0] | (buf[1] << 8)) & (0x1F << 5)) >> 5;
	tmp = otp ^ RTD1625_IDAC_FINE_MDIX_DEFAULT;
	tmp += tp->amp_k_offset;
	tmp = (int_mdio_read(tp, 0x0bc0, 23) & ~(0x1f << 8)) | (tmp << 8);
	int_mdio_write(tp, 0x0bc0, 23, tmp);
...
/* PRINCE */
static void rtk_prince_reset_phy_gmac(struct rtl8169_private *tp)
{
	struct clk *clk_etn_sys   = clk_get(&tp->pdev->dev, "etn_sys");
	struct clk *clk_etn_250m  = clk_get(&tp->pdev->dev, "etn_250m");
	struct clk *clk_iso_misc  = clk_get(&tp->pdev->dev, "iso_misc");
	struct clk *clk_sgmii     = clk_get(&tp->pdev->dev, "sgmii");
	struct clk *clk_sgmii_sys = clk_get(&tp->pdev->dev, "sgmii_sys");
	struct reset_control *rstc_gmac =
		reset_control_get_exclusive(&tp->pdev->dev, "gmac");
	struct reset_control *rstc_misc =
		reset_control_get_exclusive(&tp->pdev->dev, "misc");
	struct reset_control *rstc_sgmii =
		reset_control_get_exclusive(&tp->pdev->dev, "sgmii");
	struct reset_control *rstc_sgmii_reg =
		reset_control_get_exclusive(&tp->pdev->dev, "sgmii_reg");
	struct reset_control *rstc_sds_phy =
		reset_control_get_exclusive(&tp->pdev->dev, "sds_phy");
	struct reset_control *rstc_sgmii_mdio =
		reset_control_get_exclusive(&tp->pdev->dev, "sgmii_mdio");

	/* pre-init clk bits */
	if (!test_bit(RTL_STATUS_REINIT, tp->status)) {
		clk_prepare_enable(clk_etn_sys);
		clk_prepare_enable(clk_etn_250m);
		clk_prepare_enable(clk_sgmii);
		clk_prepare_enable(clk_sgmii_sys);
	}
	...
	/* 0x98000050[11:10]=0x3: Clock enable for MISC */
	clk_prepare_enable(clk_iso_misc);
	...
	/* set int PHY addr = 1 */
	/* set ext PHY addr as dts setting */
	regmap_write(tp->m2tmx_base, PRINCE_M2TMX_PWRCUT_ETN,
		     0x04612703 | ((tp->ext_phy_id & 0x1f) << 21));

	/* 0x98000050[13:12]=0x2: Deassert clk_en_sgmii */
	clk_disable_unprepare(clk_sgmii);

	/* 0x9804f198[3:2]=0x0: Deassert SGMII CMU_EN and CPHY_MBIAS_EN */
	regmap_update_bits_base(tp->m2tmx_base, PRINCE_M2TMX_ETN_SGPHY_MISC,
				GENMASK(3, 2), 0, NULL, false, true);

	fsleep(15);

	/* 0x9804f194[10]=0x0: Deassert RSTB_N_SGMII */
	regmap_update_bits_base(tp->m2tmx_base, PRINCE_M2TMX_ETN_MISC,
				BIT(10), 0, NULL, false, true);

	fsleep(15);

	/* 0x98000000[11:10]=0x2: Assert rstn_sds_phy */
	reset_control_assert(rstc_sds_phy);

	fsleep(15);

	/* 0x9804f198[5]=0x0: Deassert SGMII CA18PC_EN */
	regmap_update_bits_base(tp->m2tmx_base, PRINCE_M2TMX_ETN_SGPHY_MISC,
				BIT(5), 0, NULL, false, true);

	fsleep(15);

	/* 0x9804f198[4]=0x0: Deassert SGMII CA08PC_EN */
	regmap_update_bits_base(tp->m2tmx_base, PRINCE_M2TMX_ETN_SGPHY_MISC,
				BIT(4), 0, NULL, false, true);

	/* 0x9800005c[1:0]=0x2: Deassert clk_en_sgmii_sys */
	clk_disable_unprepare(clk_sgmii_sys);

	/* 0x98000000[15:14]=0x2: Deassert rstn_sgmii_mdio */
	reset_control_assert(rstc_sgmii_mdio);

	/* 0x98000000[9:6]=0xa: Assert rstn_sgmii, rstn_sgmii_reg */
	reset_control_assert(rstc_sgmii);
	reset_control_assert(rstc_sgmii_reg);
	...
	reset_control_put(rstc_gmac);
	reset_control_put(rstc_misc);
	reset_control_put(rstc_sgmii);
	reset_control_put(rstc_sgmii_reg);
	reset_control_put(rstc_sds_phy);
	reset_control_put(rstc_sgmii_mdio);
	clk_put(clk_etn_sys);
	clk_put(clk_etn_250m);
	clk_put(clk_iso_misc);
	clk_put(clk_sgmii);
	clk_put(clk_sgmii_sys);
}

static void rtk_prince_pll_clock_init(struct rtl8169_private *tp)
{
	struct device *d = tp_to_dev(tp);
	struct clk *clk_etn_sys   = clk_get(&tp->pdev->dev, "etn_sys");
	struct clk *clk_etn_250m  = clk_get(&tp->pdev->dev, "etn_250m");
	...
	/* 0x98000000[9:6]=0xf: Deassert rstn_sgmii, rstn_sgmii_reg */
	reset_control_deassert(rstc_sgmii);
	reset_control_deassert(rstc_sgmii_reg);

	/* 0x98000000[15:14]=0x3: Deassert rstn_sgmii_mdio */
	reset_control_deassert(rstc_sgmii_mdio);

	/* 0x9800005c[1:0]=0x3: Assert clk_en_sgmii_sys */
	clk_prepare_enable(clk_sgmii_sys);

	/* 0x9804f198[4]=0x1: Assert SGMII CA08PC_EN */
	regmap_update_bits_base(tp->m2tmx_base, PRINCE_M2TMX_ETN_SGPHY_MISC,
				BIT(4), BIT(4), NULL, false, true);

	fsleep(15);

	/* 0x9804f198[5]=0x1: Assert SGMII CA18PC_EN */
	regmap_update_bits_base(tp->m2tmx_base, PRINCE_M2TMX_ETN_SGPHY_MISC,
				BIT(5), BIT(5), NULL, false, true);

	fsleep(15);

	/* 0x98000000[11:10]=0x3: Deassert rstn_sds_phy */
	reset_control_deassert(rstc_sds_phy);

	fsleep(15);

	/* 0x9804f194[10]=0x1: Wait RSTB_N of SGMII PHY is high */
	regmap_read(tp->m2tmx_base, PRINCE_M2TMX_ETN_MISC, &val);
	tmp = 0;
	while (0 == (val & BIT(10))) {
		tmp += 1;
		fsleep(1000);
		if (tmp >= MAC_INIT_TIMEOUT) {
			dev_err(d, "RSTB_N_SGMII is not ready\n");
			set_bit(RTL_STATUS_HW_FAIL, tp->status);
			goto ERR_OUT;
		}
		regmap_read(tp->m2tmx_base, PRINCE_M2TMX_ETN_MISC, &val);
	...
	if (tmp < MAC_INIT_TIMEOUT)
		dev_info(d, "ETN wait %d ms for RSTB_N of SGMII PHY\n", tmp);

	/* 0x9804f198[3:2]=0x3: Set SGMII CMU_EN and CPHY_MBIAS_EN */
	regmap_update_bits_base(tp->m2tmx_base, PRINCE_M2TMX_ETN_SGPHY_MISC,
				GENMASK(3, 2), GENMASK(3, 2), NULL, false, true);

	/* 0x981c980c[24]=0x1: Wait SGMII PHY clock ready */
	regmap_read(tp->sds_regif_base, PRINCE_SDS_LINK, &val);
	tmp = 0;
	while (0 == (val & BIT(24))) {
		tmp += 1;
		fsleep(1000);
		if (tmp >= MAC_INIT_TIMEOUT) {
			dev_err(d, "sds0_clk_rdy is not ready\n");
			set_bit(RTL_STATUS_HW_FAIL, tp->status);
			goto ERR_OUT;
		}
		regmap_read(tp->sds_regif_base, PRINCE_SDS_LINK, &val);
	...
	if (tmp < MAC_INIT_TIMEOUT)
		dev_info(d, "ETN wait %d ms for SGMII PHY clock ready\n", tmp);
	...
	while (0 == (val & BIT(8))) {
		...
		fsleep(1000);
		if (tmp >= MAC_INIT_TIMEOUT) {
			dev_err(d, "clk_rdy_sgmii is not ready\n");
			set_bit(RTL_STATUS_HW_FAIL, tp->status);
			goto ERR_OUT;
		}
		regmap_read(tp->sds_regif_base, PRINCE_SDS_LINK, &val);
	...
	if (tmp < MAC_INIT_TIMEOUT)
		dev_info(d, "ETN wait %d ms for clk_rdy_sgmii\n", tmp);
...
static void rtk_prince_load_otp_content(struct rtl8169_private *tp)
{
}

static void rtk_prince_phy_iol_tuning(struct rtl8169_private *tp)
{
}

static int rtk_prince_serdes_init(struct rtl8169_private *tp)
{
	struct device *d = tp_to_dev(tp);
	u32 tmp;
	u32 val;
	...
	regmap_update_bits_base(tp->sds_regif_base, PRINCE_SDS_MISC,
				BIT(16), BIT(16), NULL, false, true);

	/* Update DPHY reg */
	/* reg_0x981c9010 = 0x0B81 */
	regmap_write(tp->sds_dphy_base, PRINCE_SDS_ANA04, 0x00000B81);

	/* reg_0x981c9010 = 0x4B81 */
	regmap_write(tp->sds_dphy_base, PRINCE_SDS_ANA04, 0x00004B81);

	/* reg_0x981c902c = 0xF010 */
	regmap_write(tp->sds_dphy_base, PRINCE_SDS_ANA0B, 0x0000F010);

	/* reg_0x981c9030 = 0x0170 */
	regmap_write(tp->sds_dphy_base, PRINCE_SDS_ANA0C, 0x00000170);

	/* reg_0x981c9034 = 0x6088 */
	regmap_write(tp->sds_dphy_base, PRINCE_SDS_ANA0D, 0x00006088);

	/* reg_0x981c9038 = 0x4009 */
	regmap_write(tp->sds_dphy_base, PRINCE_SDS_ANA0E, 0x00004009);

	/* reg_0x981c903c = 0x4180 */
	regmap_write(tp->sds_dphy_base, PRINCE_SDS_ANA0F, 0x00004180);

	/* reg_0x981c9040 = 0x04AE */
	regmap_write(tp->sds_dphy_base, PRINCE_SDS_ANA10, 0x000004AE);

	/* reg_0x981c9044 = 0x6000 */
	regmap_write(tp->sds_dphy_base, PRINCE_SDS_ANA11, 0x00006000);

	/* reg_0x981c9094 = 0x4864 */
	regmap_write(tp->sds_dphy_base, PRINCE_SDS_ANA25, 0x00004864);

	/* 0x981c8008[9:8]=0x3: Set serdes force N-way enable */
	regmap_update_bits_base(tp->sds_base, PRINCE_SDS_REG2,
				GENMASK(9, 8), GENMASK(9, 8), NULL, false, true);

	/* 0x9804f180[4]=0x1: Set GMAC to SGMII interface */
	regmap_update_bits_base(tp->m2tmx_base, PRINCE_M2TMX_PWRCUT_ETN,
				BIT(4), BIT(4), NULL, false, true);
	...
	regmap_read(tp->sds_regif_base, PRINCE_SDS_MISC, &val);
	tmp = 0;
	while (0 == (val & BIT(12))) {
		tmp += 1;
		fsleep(1000);
		if (tmp >= MAC_INIT_TIMEOUT) {
			dev_err(d, "SGMII link down\n");
			set_bit(RTL_STATUS_HW_FAIL, tp->status);
			ret = -EBUSY;
			goto ERR_OUT;
		}
	...
	if (tmp < MAC_INIT_TIMEOUT)
		dev_info(d, "ETN wait %d ms for sds_link_ok\n", tmp);
...
static void rtk_prince_mdio_init(struct rtl8169_private *tp)
{
	struct device *d = tp_to_dev(tp);
	u32 tmp;

	/* adjust PHY electrical characteristics */
	rtk_prince_phy_iol_tuning(tp);

	/* load OTP contents (R-K, Amp)
	 */
	rtk_prince_load_otp_content(tp);

	if (rtk_prince_serdes_init(tp) != 0) {
		dev_err(d, "SERDES init failed\n");
		set_bit(RTL_STATUS_HW_FAIL, tp->status);
		return;
	}
	...
	tmp = rtl_ocp_read(tp, 0xde10) & ~GENMASK(7, 6);
	tmp |= BIT(6); /* MDC freq = 8.9MHz */
	rtl_ocp_write(tp, 0xde10, tmp);
	/* OCP 0xDE30[7:6]=0x1: set external PHY MDC freq = 8.9MHZ */
	tmp = rtl_ocp_read(tp, 0xde30) & ~GENMASK(7, 6);
	tmp |= BIT(6); /* MDC freq = 8.9MHz */
	rtl_ocp_write(tp, 0xde30, tmp);
	...
	while (rtl_phy_read(tp, 0x0a43, 31) != 0x0a43) {
		...
		fsleep(1000);
		if (tmp >= 2000) {
			dev_err(d, "External SGMII PHY not found, current = 0x%02x\n",
				rtl_phy_read(tp, 0x0a43, 31));
			set_bit(RTL_STATUS_HW_FAIL, tp->status);
			return;
		}
	...
	if (tmp < 2000)
		dev_info(d, "Wait %d ms for SGMII PHY ready\n", tmp);
}

static void rtk_prince_eee_set(struct rtl8169_private *tp, bool enable)
{
	if (enable) {
		/* enable eee auto fallback function */
		rtl_phy_write(tp, 0x0a4b, 17,
		...
		rtl_phy_write(tp, 0x0a43, 25,
			      rtl_phy_read(tp, 0x0a43, 25) | BIT(4) | BIT(2));
		/* EEE Tw_sys_tx timing adjustment,
		 * make sure MAC would send data after PHY wake up
		 */
		/* default 0x001F, 100M EEE */
		rtl_ocp_write(tp, 0xe04a, 0x002f);
		/* default 0x0011, 1000M EEE */
...
static void rtk_prince_dump_regs(struct seq_file *m, struct rtl8169_private *tp)
{
	u32 val;
	...
	regmap_read(tp->sds_base, PRINCE_SDS_REG2, &val);
	seq_printf(m, "SDS_REG2\t[0x981c8008] = %08x\n", val);

	regmap_read(tp->sds_dphy_base, PRINCE_SDS_ANA0C, &val);
	seq_printf(m, "SDS_ANA0C\t[0x981c9030] = %08x\n", val);
	regmap_read(tp->sds_dphy_base, PRINCE_SDS_ANA0D, &val);
	seq_printf(m, "SDS_ANA0D\t[0x981c9034] = %08x\n", val);
	regmap_read(tp->sds_dphy_base, PRINCE_SDS_ANA0E, &val);
	seq_printf(m, "SDS_ANA0E\t[0x981c9038] = %08x\n", val);
	regmap_read(tp->sds_dphy_base, PRINCE_SDS_ANA0F, &val);
	seq_printf(m, "SDS_ANA0F\t[0x981c903c] = %08x\n", val);
	regmap_read(tp->sds_dphy_base, PRINCE_SDS_ANA10, &val);
	seq_printf(m, "SDS_ANA10\t[0x981c9040] = %08x\n", val);
	regmap_read(tp->sds_dphy_base, PRINCE_SDS_ANA11, &val);
	seq_printf(m, "SDS_ANA11\t[0x981c9044] = %08x\n", val);

	regmap_read(tp->m2tmx_base, PRINCE_M2TMX_PWRCUT_ETN, &val);
	seq_printf(m, "M2TMX_PWRCUT\t[0x9804f180] = %08x\n", val);
	regmap_read(tp->m2tmx_base, PRINCE_M2TMX_ETN_MISC, &val);
	seq_printf(m, "M2TMX_ETN_MISC\t[0x9804f194] = %08x\n", val);
	regmap_read(tp->m2tmx_base, PRINCE_M2TMX_ETN_SGPHY_MISC, &val);
	seq_printf(m, "M2TMX_SGPHY_MISC\t[0x9804f198] = %08x\n", val);
...
static struct r8169soc_chip_info rtk_prince_info = {
	...
	.mdio_lock		= rtd139x_mdio_lock,
	.mdio_unlock		= rtd139x_mdio_unlock,
	.reset_phy_gmac		= rtk_prince_reset_phy_gmac,
	.pll_clock_init		= rtk_prince_pll_clock_init,
	.mdio_init		= rtk_prince_mdio_init,
	.mac_mcu_patch		= rtd1625_mdns_mac_mcu_patch,
	.wakeup_set		= rtl_mdns_wakeup_set,
	.eee_set		= rtk_prince_eee_set,
	.dump_regs		= rtk_prince_dump_regs,
	.jumbo_max		= JUMBO_1K,
	.jumbo_tx_csum		= false,
	.led_cfg		= 0x170000A9,
	.features		= RTL_FEATURE_TX_NO_CLOSE |
...
static int rtl_init_one(struct platform_device *pdev)
{
	...
	/* it is not used by rtd119x, rtd129x, and rtk_prince */
	/* it should be valid for rtd139x, rtd16xx, and rtd13xx */
	...
	/* it should be valid for rtd139x, rtd16xx, rtk_prince */
	...
	/* it should be valid for rtd1625 and rtk_prince */
	...
	/* sds dphy regs 0x981c9000 (optional) */
	/* it should be valid for rtk_prince */
	tp->sds_dphy_base = syscon_regmap_lookup_by_phandle(pdev->dev.of_node, "realtek,sds_dphy");
	if (IS_ERR_OR_NULL(tp->sds_dphy_base)) {
		if (tp->chip == &rtk_prince_info) {
			dev_err(&pdev->dev,  "Fail to get sds_dphy_base address\n");
			rc = -ENODEV;
			goto err_out_netdev;
		}
	} else {
		dev_dbg(&pdev->dev,  "Get sds_dphy_base address\n");
	}

	/* sds regif regs 0x981c9800 (optional) */
	/* it should be valid for rtk_prince */
	tp->sds_regif_base = syscon_regmap_lookup_by_phandle(pdev->dev.of_node,
							     "realtek,sds_regif");
	if (IS_ERR_OR_NULL(tp->sds_regif_base)) {
		if (tp->chip == &rtk_prince_info) {
			dev_err(&pdev->dev,  "Fail to get sds_regif_base address\n");
			rc = -ENODEV;
			goto err_out_netdev;
		}
	} else {
		dev_dbg(&pdev->dev,  "Get sds_regif_base address\n");
	}
	...
	device_set_wakeup_enable(&pdev->dev, tp->dev->ethtool->wol_enabled);
