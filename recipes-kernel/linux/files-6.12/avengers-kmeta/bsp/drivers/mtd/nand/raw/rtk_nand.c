...
static void rtk_nf_set_nand_info(struct rtk_nf *nf, char *item)
{
	...
	if ((s = strstr(item, "ds:")) != NULL) {
		...
		nf_ids->chipsize = *&nf_ids->chipsize >> 20;
	}
				    ...
				    const u8 *buf, int page, int mode)
{
	...
	if (buf != buffer->dataBuf && !((uintptr_t)buf & 0xFFF)) {
	...
	if (!((uintptr_t)chip->oob_poi & 0xFFF)){
				...
				int page, int access_mode)
{
	...
	memset(buffer->tmpBuf, 0xff, mtd->writesize + mtd->oobsize);
/*
...
static int rtk_read_status_on_die(struct nand_chip *chip)
{
        struct rtk_nf *nf = nand_get_controller_data(chip);
        void __iomem *base = nf->regs;
        unsigned int ecc_max = 0;
        unsigned int ecc_num = 0;
        unsigned int i = 0;
	int ret = 0;

        writel(0x4, base + REG_DATA_TL0);
        writel(0x80, base + REG_DATA_TL1); //Set data transfer count[13:8], SRAM path and access mode

        //Set PP
        writel(0x0, base + REG_READ_BY_PP); //Data read to DRAM from NAND through PP
        writel(0x1, base + REG_PP_CTL0);
        writel(0x0, base + REG_PP_CTL1);

        //Set command
        writel(0x7a, base + REG_ND_CMD);
        ...
        for (i = 0; i < 4; i++) {
                ecc_num = ((readl(base + REG_ND_PA0) >> (8 * i)) & 0xf);
                if (ecc_num > ecc_max)
                        ecc_max = ecc_num;
        }
				   ...
				   int oob_on, int mode, int phase)
{
	...
	if (blank_check & 0x2) {
		ret = 1;
	} else if (g_id_chain == 0x1590da98 && ((g_id_chain_2 & 0xffff) == 0x16f6)) { //toshiba read status need extra check
		eccNum = rtk_read_status_on_die(chip);
		if (eccNum > 4) {
			dev_err(nf->dev, "RTK %s on_die debug: ecc_num:%d, page:%u\n", __func__, eccNum, page);
			writel(NF_BLANK_CHK_blank_ena(1) | NF_BLANK_CHK_read_ecc_xnor_ena(0),
				map_base + REG_BLANK_CHK);
			return -2;
		}
				  ...
				  struct device_node *np)
{
	...
	nf->chipsize =  nanddev_target_size(&chip->base);
...
static int rtk_nf_probe(struct platform_device *pdev)
{
	...
	dev_err(dev, "success to init nand chip\n");
	return 0;
...
static void rtk_nf_remove(struct platform_device *pdev)
{
...
static int __init __maybe_unused rtk_nf_nand_info_setup(char *line)
{
	strscpy(g_rtk_nandinfo_line, line, sizeof(g_rtk_nandinfo_line));
