...
/* MSI int handler */
static irqreturn_t rtd_handle_mac_msi_irq(struct rtd_pcie_port *pp)
{
	int i, pos, irq;
	unsigned long val;
...
/* MSI int handler */
static irqreturn_t rtd_handle_intx_irq(struct rtd_pcie_port *pp)
{
	int pos, irq;
	u32 status;
	unsigned long val;
...
static irqreturn_t rtd13xx_handle_wrapper_msi_irq(int this_irq, void *devid)
{
...
static irqreturn_t rtd_handle_wrapper_msi_irq(struct rtd_pcie_port *pp)
{
	int pos, irq;
...
static irqreturn_t rtd_handle_wrapper_msix_irq(struct rtd_pcie_port *pp)
{
	int i, pos, irq;
	unsigned long val;
	u32 status, num_ctrls;
...
static int rtd_pcie_allocate_domains(struct rtd_pcie_port *pp)
{
...
static void __maybe_unused rtd_pcie_free_msi(struct rtd_pcie_port *pp)
{
	if (pp->msi_irq) {
...
static void __iomem *rtd_pcie_host_map_bus(struct pci_bus *bus, unsigned int devfn, int where)
{
	struct rtd_pcie_port *pp = bus->sysdata;
...
static int __maybe_unused rtd_pci_get_host_bridge_resources(struct rtd_pcie_port *pp,
...
static void setup_rc(struct rtd_pcie_port *pp)
{
	int val;
...
static int ldo_adjust_voltage_get(struct device *dev)
{
	int val;
	struct nvmem_cell *cell;

	cell = nvmem_cell_get(dev, "pcie_cal");
	if (!IS_ERR(cell)) {
		unsigned char *buf;
		size_t buf_size;
		...
		dev_info(dev, "missing pcie_cal resource, fallback to default\n");
		val = 0;
	}
...
static int ldo_power_init(struct rtd_pcie_port *pp)
{
	int adjust_volt;
	int otp_val;
	u32 val;
	...
	otp_val = ldo_adjust_voltage_get(pp->dev);
	if (otp_val & LDO_EFUSE_VALID_BIT)
		adjust_volt = otp_val & GENMASK(3, 0);
	else
		adjust_volt = 0x4;

	val = readl(pp->ctrl_base + PCIE_LDO_PARA);
	val = (val & ~GENMASK(11, 8)) | (adjust_volt << 8);
	writel(val, pp->ctrl_base + PCIE_LDO_PARA);
	writel(EN_BG | EN_IB, pp->ctrl_base + PCIE_LDO_CTRL);

	ret = readl_poll_timeout(pp->ctrl_base + PCIE_LDO_STS, val,
				 (val & BG_IBHN_BIAS_OK), 5, 100);
	if (ret) {
		dev_err(pp->dev, "wait BG_IBHN_BIAS_OK on timeout\n");
		return -EBUSY;
	}

	val = readl(pp->ctrl_base + PCIE_LDO_CTRL);

	writel(val | EN_LDO, pp->ctrl_base + PCIE_LDO_CTRL);

	ret = readl_poll_timeout(pp->ctrl_base + PCIE_LDO_STS, val,
				 (val & LDO_POWER_OK), 10, 500);
	if (ret) {
		dev_err(pp->dev, "wait LDO_POWER_OK on timeout\n");
		return -EBUSY;
	}
...
static void ldo_power_exit(struct rtd_pcie_port *pp)
{
    u32 val;
    int ret;
    ...
    val = readl(pp->ctrl_base + PCIE_LDO_CTRL);
    val &= ~EN_LDO;
    writel(val, pp->ctrl_base + PCIE_LDO_CTRL);

    ret = readl_poll_timeout(pp->ctrl_base + PCIE_LDO_STS, val,
                             !(val & LDO_POWER_OK), 10, 500);
    if (ret) {
        dev_err(pp->dev, "wait LDO_POWER_OK off timeout\n");
    }
    ...
    ret = readl_poll_timeout(pp->ctrl_base + PCIE_LDO_STS, val,
                             !(val & BG_IBHN_BIAS_OK), 5, 100);
    if (ret) {
        dev_err(pp->dev, "wait BG_IBHN_BIAS_OK off timeout\n");
    }
}

static int rtd1635_pcie0_init(struct rtd_pcie_port *pp)
{
	int ret = 0;
	...
	ret = ldo_power_init(pp);
	if (ret) {
		dev_err(pp->dev, "ldo power init failed\n");
		return ret;
	}
	clk_disable_unprepare(pp->pcie_clk);
	phy_power_on(pp->pcie_phy);
...
static int rtd1635_pcie0_init2(struct rtd_pcie_port *pp)
{
	int val;
	...
	/*workaround: MAC clock is stalled by default, must toggle clock gate register to enable.*/
	writel(0x1, pp->ctrl_base + PCIE_ORDER_CTRL_OFF);
	writel(0x3, pp->ctrl_base + PCIE_ORDER_CTRL_OFF);

	val = readl(pp->ctrl_base + GEN3_RELATED_OFF);
	writel(val & ~BIT(15), pp->ctrl_base + GEN3_RELATED_OFF);

	return 0;
}

static int rtd1635_pcie0_deinit(struct rtd_pcie_port *pp)
{
	int ret = 0;
...
static const struct rtd_pcie_ops rtd1635_pcie0_ops = {
	.name = "pcie0",
	.msi_ops = &rtd_mac_msi_ops,
	.get_resource = rtd1625_pcie0_get_res,
	.init = rtd1635_pcie0_init,
	.init2 = rtd1635_pcie0_init2,
	.deinit = rtd1635_pcie0_deinit,
	.hwinit = rtd1625_pcie0_hw_init,
	.get_ca_type = rtd_get_ca_type,
};
...
static const struct of_device_id rtd_pcie_match_table[] = {
	...
	{.compatible = "realtek,rtd1635-pcie-slot0", .data = &rtd1635_pcie0_ops},
	{},
};
