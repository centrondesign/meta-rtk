...
static int rtk_rtc_init(struct rtk_rtc_device *rdev)
{
	unsigned long flags;
	unsigned int val;
	int ret = 0;
...
static int rtk_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	/* when use hsec interrupt, don't round to min */
	if (!rdev->use_hsec && alrm->time.tm_sec) {
	...
	rdev->alarm_left_hsec = 0;
	if (rdev->use_hsec) {
		rdev->alarm_left_hsec = (alarm_time - now) * 2 + 3;
		dev_dbg(rdev->dev, "%s: set alarm_left_hsec to %llu\n",
			__func__, rdev->alarm_left_hsec);
	}
...
static irqreturn_t rtk_rtc_alarm_irq_handler(int irq, void *data)
{
	...
	if (rdev->use_hsec && rdev->alarm_left_hsec == 0) {
		dev_dbg(rdev->dev, "%s: alarm not enabled\n", __func__);
		return IRQ_HANDLED;
	}

	if (rdev->use_hsec && --rdev->alarm_left_hsec > 5) {
		dev_dbg(rdev->dev, "%s: alarm not timedout (alarm_left_hsec=%llu)\n",
			__func__, rdev->alarm_left_hsec + 1);
		return IRQ_HANDLED;
	}
