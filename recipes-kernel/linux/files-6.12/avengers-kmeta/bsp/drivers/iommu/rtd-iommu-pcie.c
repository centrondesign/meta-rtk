...
#define MMU_TLB_AUTOINVAL 0xE60
		...
		struct device *dev)
{
	...
	regmap_write(rtd_iommu->base, MMU_PGTABLE_BASE, rtd_iommu->pgtable_paddr >> 4);
...
static int rtd_pcie_iommu_map(struct iommu_domain *domain, unsigned long iova,
			 phys_addr_t pa, size_t len, size_t count,
			      ...
			      size_t len, size_t count, struct iommu_iotlb_gather *gather)
{
	...
	if (rtd_iommu->autoinvalid) {
		regmap_write(rtd_iommu->base, MMU_TLB_AUTOINVAL, (iova >> 4) | 0x1);
	} else {
...
const struct iommu_ops rtd_pcie_iommu_ops = {
	...
	.default_domain_ops = &(const struct iommu_domain_ops) {
		...
		.map_pages	    = rtd_pcie_iommu_map,
		.unmap_pages	    = rtd_pcie_iommu_unmap,
		.iova_to_phys	= rtd_pcie_iommu_iova_to_phys,
		.free		= rtd_pcie_iommu_domain_free,
	}
};
...
static int rtd_pcie_iommu_probe(struct platform_device *pdev)
{
	...
	if (of_find_property(dev->of_node, "tlb-autoinvalid", NULL))
		rtd_iommu->autoinvalid = true;
	else
		rtd_iommu->autoinvalid = false;
