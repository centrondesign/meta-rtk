...
#define DLA_REMAP_ID				0x1f	/* 7d'31 */
...
#define XDI_REMAP_ID				0x28	/* 7d'40 */
#define DPRX_REMAP_ID				0x29	/* 7d'41 */

/* Kent: SYS, Prince: SYSH */
...
#define PQC_REMAP_ID				0x1b	/* 7d'39 */
...
DBUS_SYSH_EVENT_GROUP(dla,		DLA_REMAP_ID,		DBUS_CH_AUTO);
...
DBUS_SYSH_EVENT_GROUP(xdi,		XDI_REMAP_ID,		DBUS_CH_AUTO);
DBUS_SYSH_EVENT_GROUP(dprx,		DPRX_REMAP_ID,		DBUS_CH_AUTO);
DBUS_SYSH_EVENT_GROUP(jpeg_sysh,	JPEG_REMAP_ID,		DBUS_CH_AUTO);


/* SYS domain events */
DBUS_SYS_EVENT_GROUP_V2(aio,		AIO_REMAP_ID);
DBUS_SYS_EVENT_GROUP_V2(hif,		HIF_REMAP_ID);
DBUS_SYS_EVENT_GROUP_V2(jpeg_sys,	JPEG_REMAP_ID);
...
DBUS_SYS_EVENT_GROUP_V2(pqc,		PQC_REMAP_ID);
...
static struct attribute *rtk_1625_event_attrs[] = {
	...
	DBUS_SYS_EVENT_REF_GROUP_V2(jpeg_sys),
...
static struct attribute *rtk_1635_event_attrs[] = {
	DBUS_DRV_EVENT_REF(dbus_refresh),
	DBUS_DRV_EVENT_REF(dbus_overflow),
	...
	DBUS_SYSH_EVENT_REF_GROUP(hse_ree),
	DBUS_SYSH_EVENT_REF_GROUP(hse_tee),
	DBUS_SYSH_EVENT_REF_GROUP(xdi),
	DBUS_SYSH_EVENT_REF_GROUP(dprx),
	DBUS_SYSH_EVENT_REF_GROUP(jpeg_sysh),
	...
	DBUS_SYS_EVENT_REF_GROUP_V2(tpb),
	DBUS_SYS_EVENT_REF_GROUP_V2(tp),
	DBUS_SYS_EVENT_REF_GROUP_V2(pqc),
	NULL
};
...
static struct attribute_group rtk_1635_event_attr_group = {
	.name = "events",
	.attrs = rtk_1635_event_attrs,
};
...
static const struct attribute_group *rtk_1635_dbus_attr_groups[] = {
	/* must be Null-terminated */
	[RTK_PMU_ATTR_GROUP__COMMON] = &rtk_pmu_common_attr_group,
	[RTK_PMU_ATTR_GROUP__FORMAT] = &rtk_dbus_format_attr_group,
	[RTK_PMU_ATTR_GROUP__EVENT] = &rtk_1635_event_attr_group,
	[RTK_PMU_ATTR_GROUP__NUM] = NULL
};
/* channel control bit */
enum {
...
__get_client_ch(struct rtk_pmc_set *ps, unsigned int target)
{
	...
	case VE1_REMAP_ID:
		...
		break;
	case DLA_REMAP_ID:
		ch = test_bit(DLA_CH_SEL, &ch_reg);
		break;
	case PCIE0_REMAP_ID:
		ch = test_bit(PCIE0_CH_SEL, &ch_reg);
		break;
	case PCIE1_REMAP_ID:
	...
	case AUCPU1_REMAP_ID:
		ch = test_bit(AUCPU1_CH_SEL, &ch_reg);
		break;
	case NPUPP_REMAP_ID:
		ch = test_bit(NPUPP_CH_SEL, &ch_reg);
		break;
	case MIPICSI_REMAP_ID:
		ch = test_bit(MIPICSI_CH_SEL, &ch_reg);
		break;
	case VI_REMAP_ID:
		ch = test_bit(VI_CH_SEL, &ch_reg);
		break;
	case DPRX_REMAP_ID:
		ch = test_bit(DPRX_CH_SEL, &ch_reg);
		break;
	case XDI_REMAP_ID:
		ch = test_bit(XDI_CH_SEL, &ch_reg);
		break;
	case JPEG_REMAP_ID:
		ch = test_bit(JPEG_CH_SEL, &ch_reg);
		break;
	default:
		break;
	}
...
static const unsigned int rtk_1625_dbus_sysh_clients[] = {
	...
	ACPU_REMAP_ID, VE1_REMAP_ID, DLA_REMAP_ID, PCIE1_REMAP_ID,
...
static const unsigned int rtk_1635_dbus_sysh_clients[] = {
	VO1_REMAP_ID, NPUPP_REMAP_ID, SCPU_SW_REMAP_ID, SCPU_NW_REMAP_ID,
	ACPU_REMAP_ID, VE1_REMAP_ID, DLA_REMAP_ID, AUCPU0_REMAP_ID,
	PCIE0_REMAP_ID, MIPICSI_REMAP_ID, VI_REMAP_ID, VO2_REMAP_ID,
	DIP_REMAP_ID, HSE_REE_REMAP_ID, HSE_TEE_REMAP_ID, XDI_REMAP_ID,
	DPRX_REMAP_ID, JPEG_REMAP_ID,
};
...
static const unsigned int rtk_1635_dbus_sys_clients[] = {
	AIO_REMAP_ID, HIF_REMAP_ID, MIS_REMAP_ID, VTC_REMAP_ID,
	USB0_REMAP_ID, LSADC_REMAP_ID, MD_REMAP_ID, ETN_REMAP_ID,
	EMMC_REMAP_ID, NF_REMAP_ID, SD_REMAP_ID, CP_REMAP_ID,
	BGC_REMAP_ID, TPB_REMAP_ID, TP_REMAP_ID, PQC_REMAP_ID,
};
...
static const struct rtk_pmc_set_meta rtk_1635_ps_meta[] = {
	{
		.name = "1635 Dbus SYS",
		.compatible = "dbus-sys",
		.hw_ver = RTK_DBUS_BIT_ORDER_V2,
		...
		.nr_clients = ARRAY_SIZE(rtk_1635_dbus_sys_clients),
		.nr_pmcgs = ARRAY_SIZE(rtk_1625_dbus_sys_pmcgs),
		.nr_configs = ARRAY_SIZE(rtk_1625_dbus_sys_configs),
		.clients = rtk_1635_dbus_sys_clients,
		.pmcgs = rtk_1625_dbus_sys_pmcgs,
		.configs = rtk_1625_dbus_sys_configs,
	},
	{
		...
		.nr_clients = ARRAY_SIZE(rtk_1635_dbus_sysh_clients),
		.nr_pmcgs = ARRAY_SIZE(rtk_1625_dbus_sysh_pmcgs),
		.nr_configs = ARRAY_SIZE(rtk_1625_dbus_sysh_configs),
		.clients = rtk_1635_dbus_sysh_clients,
		.pmcgs = rtk_1625_dbus_sysh_pmcgs,
		.configs = rtk_1625_dbus_sysh_configs,
	},
	{
...
int rtk_1635_dbus_init(struct rtk_pmu *pmu, struct device_node *dt)
{
	int ret;
	unsigned long ctrl;
	...
	ret = rtk_dbus_pmu_init(pmu, dt,
				"rtk_1635_dbus_pmu",
				DBUS_CTRL_OFFSET,
				rtk_1635_dbus_attr_groups,
				rtk_1635_ps_meta,
				RTK_PMU_META_NR(rtk_1635_ps_meta));
...
EXPORT_SYMBOL(rtk_1635_dbus_init);
MODULE_LICENSE("GPL");
