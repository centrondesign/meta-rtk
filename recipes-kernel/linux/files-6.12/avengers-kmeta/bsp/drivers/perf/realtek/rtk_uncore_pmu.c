...
#define REG(_obj, _base)	((u32)((unsigned long)_base + _obj))
...
static u64
__rtk_pmu_update_event(struct rtk_pmc_set *ps, struct perf_event *event, bool overflow)
{
	...
	u64 prev_val, new_val;

	if (rtk_pmu_counter_auto_clear(ps->pmu)) {
		prev_raw_count = 0;
	...
	if (rtk_pmu_has_no_overflow(ps->pmu) &&
		...
		if (IS_ENABLED(CONFIG_RTK_PMU_DEV)) {
			...
			dev_notice(dev, "** ev:%#llx overflow, %dth sample\n",
				  event->attr.config,
				  rtk_event_pmc_sample(event));
			dev_notice(dev, "\tinterval:%llu, prev/new:%llu/%llu\n",
		...
		case RTK_PMC_NORMAL:
			...
			if (overflow && new_raw_count >= prev_raw_count)
				delta += ((unsigned long)rtk_event_pmc_mask(event) + 1);
		...
		case RTK_PMC_STATE:
			...
			break;
		case RTK_PMC_MIX:
			prev_val = (prev_raw_count >> rtk_event_pmc_offset(event)) &
				rtk_event_pmc_mask(event);
			new_val  = (new_raw_count >> rtk_event_pmc_offset(event)) &
				rtk_event_pmc_mask(event);

			delta = (new_val - prev_val) & rtk_event_pmc_mask(event);
			if (overflow && new_val >= prev_val)
				delta += ((unsigned long)rtk_event_pmc_mask(event) + 1);
...
{
	return __rtk_pmu_update_event(ps, event, false);
}

u64
rtk_pmu_update_event_overflow(struct rtk_pmc_set *ps, struct perf_event *event)
{
	return __rtk_pmu_update_event(ps, event, true);
}
EXPORT_SYMBOL(rtk_pmu_update_event_overflow);
...
rtk_ps_refresh_pmcg(struct rtk_pmc_set *ps, struct perf_event *event)
{
	...
	for_each_set_bit(bit,
		...
		dev_dbg(dev, "refresh pmcg %#x:%#x:%#x, events:%#llx\n",
		     ps->type, pmc.idx, bit,
		     event->attr.config);
	}
}
...
rtk_ps_refresh(struct rtk_pmc_set *ps)
{
	...
	for_each_set_bit(idx, &ps->used_mask, ps->meta->nr_pmcgs) {
		for_each_set_bit(bit,
			...
			dev_dbg(dev, "refresh pmc %#x:%#x:%#x, events:%#llx\n",
			     ps->type, idx, bit,
			     event->attr.config);
		}
	}
...
rtk_ps_read_pmc(struct rtk_pmc_set *ps, struct perf_event *event)
{
	...
	if (event->hw.state & PERF_HES_ARCH) {
		if (IS_ENABLED(CONFIG_RTK_PMU_DEV))
			dev_notice(dev, "PMC read skip, event state %x\n", event->hw.state);
		return;
	}
	...
	if (rtk_pmu_has_no_overflow(pmu) &&
		...
		if (IS_ENABLED(CONFIG_RTK_PMU_DEV))
			dev_notice(dev, "Overflow protect: ev:%#llx, val:%#llx\n", event->attr.config, curr);
	...
	if (ps->meta->nr_configs) {
		...
		if (IS_ENABLED(CONFIG_RTK_PMU_DEV)) {
			...
			WARN(target != rtk_event_target(event),
			     "Remove event mis-match %#x:%#x\n",
			     target, rtk_event_target(event));
		}
	}
...
rtk_ps_start_pmc(struct rtk_pmc_set *ps, struct perf_event *event)
{
	...
	dev_dbg(dev, "start ev:%#llx, pmc:%#x, count:%ld\n",
	     event->attr.config, event->hw.idx,
	...
	if (atomic_inc_return(&tracking->active) == 1) {
		dev_dbg(dev, "reset PMC set: %s, pmc config: %llx\n",
		     ps->name, config | rtk_event_config(event));
		     ...
		     struct perf_event *event)
{
	...
	if (!test_and_clear_bit(pmc.usage, &tracking->used_mask)) {
		dev_err(dev, "ev:%#llx already been removed\n", event->attr.config);
		return -EINVAL;
	}
...
rtk_event_destroy(struct perf_event *event)
{
	...
	if (IS_ENABLED(CONFIG_RTK_PMU_DEV)) {
		 ...
		 * here.
		 */
		dev_dbg(dev, "ev:%#llx is destroyed, count:%lld, hwc:%lld\n",
		     event->attr.config, count, prev_count);
	}
}
...
rtk_pmu_event_init(struct perf_event *event)
{
	...
	dev_dbg(dev, "try init ev:%#llx cpu-%d\n", event->attr.config, event->cpu);
	...
	if (rtk_pmu_need_hrtimer(pmu)) {
		...
		if (event->cpu != pmu->cpu)
			dev_err(dev, "ev:%#llx runs on CPU%d instead of CPU%d\n",
			       event->attr.config, event->cpu,
			       pmu->cpu);
	}
...
rtk_pmu_start_event(struct perf_event *event, int pmu_flags)
{
	...
	if (IS_ENABLED(CONFIG_RTK_PMU_DEV)) {
		...
		if (tracking->config != ps->pmc_config(event->attr.config))
			dev_err(dev, "tracking event mis-match, %#x-%#llx\n",
			       tracking->config, event->attr.config);
	}
...
rtk_pmu_add_event(struct perf_event *event, int flags)
{
	...
	dev_dbg(dev, "add ev:%#llx cpu-%d\n", event->attr.config, event->cpu);

	/* already inserted */
	if (hwc->idx != -1) {
		dev_err(dev, "ev:%#llx already inserted, hwc:%x\n", event->attr.config, hwc->idx);
		return -EBUSY;
	}
...
rtk_pmu_del_event(struct perf_event *event, int flags)
{
	...
	dev_dbg(dev, "del ev:%#llx on cpu-%d\n", event->attr.config, event->cpu);
		     ...
		     const struct rtk_pmc_set_meta *psm)
{
	...
	for_each_child_of_node(dt, child) {
		...
		ret = of_property_read_u32(child, "nr_defects",
					   &ps->nr_defects);
		if (ret == -EINVAL || ps->nr_defects == 0) {
			ps->nr_defects = 0;
			goto defect_complete;
		} else if (ret < 0) {
			dev_err(dev, "Parsing nr_defects of %s failed\n", psm->name);
			return ret;
		}
		...
		ret = of_property_read_u32_array(child, "defects", ps->defects,
						 ps->nr_defects);
		if (ret < 0) {
			dev_err(dev, "Parsing defect descriptions of %s failed\n", psm->name);
			return ret;
		}

defect_complete:
		ret = of_property_read_u32(child, "has_ext", &ps->has_ext);
		if (ret == -EINVAL || ps->has_ext == 0) {
			ps->has_ext = 0;
			goto ext_complete;
		} else if (ret < 0) {
			dev_err(dev, "Parsing has_ext of %s failed\n", psm->name);
			return ret;
		}
		ret = of_property_read_u32(child, "ext_info", &ps->ext_info);
		if (ret == -EINVAL) {
			dev_err(dev, "Missing ext_info of %s\n", psm->name);
			return ret;
		} else if (ret < 0) {
			dev_err(dev, "Parsing ext_info of %s failed\n", psm->name);
			return ret;
		}

ext_complete:
		of_property_read_u32(child, "remap_en", &ps->remap_en);
...
{
	struct device *dev = &rtk_pmu->pdev->dev;
...
__setup_cpuhp(struct rtk_pmu *rtk_pmu, const char *name)
{
	...
	if (ret < 0) {
		dev_err(dev, "Setup cpuhp state for %s failed, errno:%#x\n", name, ret);
		return ret;
	}

	rtk_pmu->cpuhp_state = ret;
	dev_info(dev, "%s- registered cpuhp stat=%d\n", name, ret);
		     ...
		     const struct of_device_id *of_table)
{
	...
	if (unlikely(pmu == NULL)) {
		dev_err(dev, "failed to allocate PMU device for %s!\n", pdev->dev.of_node->name);
		return -ENOMEM;
	}
	...
	dev_info(dev, "registered type=%d\n", pmu->pmu.type);
	return 0;
...
out_free:
	dev_err(dev, "failed to register %s, errno=%d!\n", of_node_full_name(node), ret);
	__pmu_free(pmu);
	return ret;
}
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("RTK UNCORE PMU support");
