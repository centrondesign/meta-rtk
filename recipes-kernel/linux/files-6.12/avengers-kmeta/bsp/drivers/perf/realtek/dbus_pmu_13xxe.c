...
/*
 * Realtek Dbus PMU driver for 13xxe
 *
...
static struct attribute *rtk_13xxe_event_attrs[] = {
...
static struct attribute_group rtk_13xxe_event_attr_group = {
	.name = "events",
	.attrs = rtk_13xxe_event_attrs,
};

static const struct attribute_group *rtk_13xxe_dbus_attr_groups[] = {
	/* must be Null-terminated */
	[RTK_PMU_ATTR_GROUP__COMMON] = &rtk_pmu_common_attr_group,
	[RTK_PMU_ATTR_GROUP__FORMAT] = &rtk_dbus_format_attr_group,
	[RTK_PMU_ATTR_GROUP__EVENT] = &rtk_13xxe_event_attr_group,
...
rtk_13xxe_arrange_pmc(struct rtk_pmu *pmu, u64 config)
{
...
static const unsigned int rtk_13xxe_dbus_sysh_clients[] = {
	VO_ID, SCPU_NW_ID, ACPU_ID, SCPU_SW_ID, VE1_ID, VE3_ID,
	PCIE1_ID, AUCPU_ID, DIP_ID, HSE_REE_ID, HSE_TEE_ID,
	SB1_ID, SB3_ID, SB4_ID, SB5_ID
};

/* address offset of Dbus sysh domain PMCG */
static const unsigned long rtk_13xxe_dbus_sysh_pmcgs[] = {
	0x0060, 0x0070, 0x0080, 0x0510, 0x0520, 0x530, 0x540, 0x550
};

/* address offset of configs of Dbus sysh domain PMCG */
static const unsigned long rtk_13xxe_dbus_sysh_configs[] = {
	0x0050, 0x0500, 0x504
};

static const unsigned int rtk_13xxe_dbus_sys_clients[] = {
	AIO_ID, HIF_ID, AEE_ID, MD_ID, ADE_ID, JPEG_ID,
	VTC_ID, USB3_ID, USB_ID, ETN_ID, EMMC_ID, NF_ID, SD_ID,
	CP_ID, BGC_ID, TP_ID,
};

/* address offset of Dbus sys domain(bridge) PMCG */
static const unsigned long rtk_13xxe_dbus_sys_pmcgs[] = {
	0x0090, 0x00a0, 0x00b0
};

/* address offset of configs of Dbus sys domain(bridge) PMCG */
static const unsigned long rtk_13xxe_dbus_sys_configs[] = {
	0x0054
};

static const unsigned int rtk_13xxe_dbus_ch_clients[] = {
	DBUS_CH_0, DBUS_CH_1
};

/* address offset of Dbus output channel statistics PMCG */
static const unsigned long rtk_13xxe_dbus_ch_pmcgs[] = {
	0x0034, 0x0040
};

static const unsigned int rtk_13xxe_dbus_drv_clients[] = {
	RTK_DRV_OVERFLOW, DBUS_REFRESH
};

static const struct rtk_pmc_set_meta rtk_13xxe_ps_meta[] = {
	{
		.name = "13xxe Dbus SYS",
		.compatible = "dbus-sys",
		...
		.nr_clients = ARRAY_SIZE(rtk_13xxe_dbus_sys_clients),
		.nr_pmcgs = ARRAY_SIZE(rtk_13xxe_dbus_sys_pmcgs),
		.nr_configs = ARRAY_SIZE(rtk_13xxe_dbus_sys_configs),
		.clients = rtk_13xxe_dbus_sys_clients,
		.pmcgs = rtk_13xxe_dbus_sys_pmcgs,
		.configs = rtk_13xxe_dbus_sys_configs,
	},
	{
		.name = "13xxe Dbus SYSH",
		...
		.nr_clients = ARRAY_SIZE(rtk_13xxe_dbus_sysh_clients),
		.nr_pmcgs = ARRAY_SIZE(rtk_13xxe_dbus_sysh_pmcgs),
		.nr_configs = ARRAY_SIZE(rtk_13xxe_dbus_sysh_configs),
		.clients = rtk_13xxe_dbus_sysh_clients,
		.pmcgs = rtk_13xxe_dbus_sysh_pmcgs,
		.configs = rtk_13xxe_dbus_sysh_configs,
	},
	{
		.name = "13xxe Dbus CH",
		.compatible = "dbus-ch",
		...
		.nr_clients = ARRAY_SIZE(rtk_13xxe_dbus_ch_clients),
		.nr_pmcgs = ARRAY_SIZE(rtk_13xxe_dbus_ch_pmcgs),
		.nr_configs = 0,
		.clients = rtk_13xxe_dbus_ch_clients,
		.pmcgs = rtk_13xxe_dbus_ch_pmcgs,
		.configs = NULL,
	},
	{
		.name = "13xxe Dbus DRV",
		...
		.nr_clients = ARRAY_SIZE(rtk_13xxe_dbus_drv_clients),
		.nr_pmcgs = DBUS_EV_NUM,
		.nr_configs = 0,
		.clients = rtk_13xxe_dbus_drv_clients,
		.pmcgs = NULL,
		.configs = NULL,
	},
	{},
...
int rtk_13xxe_dbus_init(struct rtk_pmu *pmu, struct device_node *dt)
{
	int ret;
	unsigned long ctrl;
	...
	ret = rtk_dbus_pmu_init(pmu, dt,
				"rtk_13xxe_dbus_pmu",
				DBUS_CTRL_OFFSET,
				rtk_13xxe_dbus_attr_groups,
				rtk_13xxe_ps_meta,
				RTK_PMU_META_NR(rtk_13xxe_ps_meta));

	if (!ret) {
		pmu->arrange_pmc = rtk_13xxe_arrange_pmc;
...
EXPORT_SYMBOL(rtk_13xxe_dbus_init);
MODULE_LICENSE("GPL");
