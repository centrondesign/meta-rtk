...
__pmc_to_ctrl_bit(struct rtk_pmu *pmu, union rtk_pmc_desc pmc)
{
	...
	dev_err(dev, "do invalid PMC(%x) to bit translation\n", pmc.val);
		      ...
		      unsigned long ov_status1)
{
	...
	size = ps->meta->nr_pmcgs * DDRC_OV_PMCG_WIDTH;
	  ...
	  struct perf_event *event)
{
	...
	dev_dbg(dev, "%s- set hwc, pmc idx:%d, usage:%d\n", ps->name, pmc.idx, pmc.usage);
		      ...
		      struct perf_event *event)
{
	...
	if (rtk_pmu_need_update_ctrl(ps->pmu)) {
		...
		dev_dbg(dev, "ev:%#llx, pmc=%d:%d:%d update ctrl:%#lx, update:%#x\n",
		       ...
		       struct perf_event *event)
{
	...
	dev_dbg(dev, "ev:%#llx, ebase:%#lx\n", event->attr.config, rtk_event_pmc_addr(event));
}
...
static const struct of_device_id rtk_ddrc_of_device_ids[] = {
	...
	},
	{
		...
		.data = rtk_13xxd_ddrc_init
	},
	{
		.compatible = "realtek,rtk-1625-ddrc-pmu",
		.data = rtk_1625_ddrc_init
	},
	{
		.compatible = "realtek,rtk-1635-ddrc-pmu",
		.data = rtk_1635_ddrc_init
	},
	{},
};
...
static void rtk_ddrc_pmu_remove(struct platform_device *pdev)
{
