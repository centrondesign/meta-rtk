...
__pmc_to_ctrl_bit(struct rtk_pmc_set *ps, union rtk_pmc_desc pmc)
{
	...
	if (pmc.set == PMC_SET__DBUS_SYS)
		ret += ps->pmu->pmcss[PMC_SET__DBUS_CH]->meta->nr_pmcgs *
...
__pmc_to_ctrl_bit_v2(struct rtk_pmc_set *ps, union rtk_pmc_desc pmc)
{
	...
	ret = pmc.idx * DBUS_UPDATE_CTRL_WIDTH + pmc.usage;
	if (pmc.set == PMC_SET__DBUS_SYS)
		ret += ps->pmu->pmcss[PMC_SET__DBUS_CH]->meta->nr_pmcgs *
			DBUS_UPDATE_CTRL_WIDTH;
...
rtk_dbus_ps_enable_with_ov(struct rtk_pmc_set *ps, struct perf_event *event)
{
	...
	ctrl = rtk_readl(addr) | BIT(ps->pmc_to_ctrl_bit(ps, pmc));
...
static void
__set_remap(struct rtk_pmc_set *ps, u32 val)
{
	int i;
	const struct rtk_pmc_set_meta *psm = ps->meta;
	unsigned long base = (unsigned long)ps->base;

	for (i = 0; i < psm->nr_configs; i++)
		rtk_writel(base + psm->configs[i],
			   rtk_readl(base + psm->configs[i]) | val);
}

static void
rtk_dbus_remap_setup(struct rtk_pmu *pmu)
{
	if (pmu->pmcss[PMC_SET__DBUS_SYS]->remap_en)
		__set_remap(pmu->pmcss[PMC_SET__DBUS_SYS],
			    BIT(7) | BIT(15) | BIT(23));

	if (pmu->pmcss[PMC_SET__DBUS_SYSH]->remap_en)
		__set_remap(pmu->pmcss[PMC_SET__DBUS_SYSH],
	  ...
	  struct perf_event *event)
{
	...
	dev_dbg(dev, "event config %x:%x:%x:%x\n", conf.set, conf.idx, conf.offset, conf.width);
	...
	if (rtk_pmu_need_update_ctrl(ps->pmu)) {
		...
		dev_dbg(dev, "ev:%#llx update ctrl:%lx, update:%x\n",
	...
	dev_dbg(dev, "ev:%#llx, config:%#llx, cbase:%#lx, ebase:%#lx, rdpmc:%#x\n",
	     event->attr.config, rtk_event_config(event),
...
static void
__set_ov_callback(struct rtk_pmc_set *ps)
{
	struct rtk_pmu *pmu = ps->pmu;
	struct device *dev = &pmu->pdev->dev;

	switch (ps->meta->hw_ver) {
	case RTK_DBUS_BIT_ORDER_V1:
		ps->find_event_by_ov = __find_event_by_ov;
		ps->pmc_to_ctrl_bit = __pmc_to_ctrl_bit;
		break;
	case RTK_DBUS_BIT_ORDER_V2:
		ps->find_event_by_ov = __find_event_by_ov_v2;
		ps->pmc_to_ctrl_bit = __pmc_to_ctrl_bit_v2;
		break;
	default:
		dev_err(dev, "Invalide Dbus version(%u), use default bit mapping\n", ps->meta->hw_ver);
		ps->find_event_by_ov = __find_event_by_ov;
		ps->pmc_to_ctrl_bit = __pmc_to_ctrl_bit;
		break;
	}
		      ...
		      const struct rtk_pmc_set_meta *meta, int nr_ps)
{
	...
	if (rtk_pmu_has_no_overflow(pmu)) {
		...
		pmu->flags |= RTK_PMU_COUNTER_AUTO_CLEAR;
...
static const struct of_device_id rtk_pmu_of_device_ids[] = {
	...
	},
	{
		...
		.data = rtk_13xxd_dbus_init
	},
	{
		.compatible = "realtek,rtk-13xxe-dbus-pmu",
		.data = rtk_13xxe_dbus_init
	},
	{
		.compatible = "realtek,rtk-1625-dbus-pmu",
		.data = rtk_1625_dbus_init
	},
	{
		.compatible = "realtek,rtk-1635-dbus-pmu",
		.data = rtk_1635_dbus_init
	},
	{},
};
...
static void rtk_dbus_pmu_remove(struct platform_device *pdev)
{
