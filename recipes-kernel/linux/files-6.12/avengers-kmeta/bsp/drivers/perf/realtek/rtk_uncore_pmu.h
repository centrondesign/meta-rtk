...
#if IS_ENABLED(CONFIG_RTK_PMU_TICK_REFRESH)
...
#define RTK_PMU_COUNTER_AUTO_CLEAR	0x02
...
struct rtk_pmu;
struct rtk_pmc_set;
struct rtk_pmu_region;

struct rtk_pmc_set_meta {
	const char	*name;
	const char	*compatible;
	u32		hw_ver;		/* HW version */
	...
	struct rtk_pmu_region *regions;	/* region info */
};
...
struct rtk_pmc_set {
	...
	u32		ext_info;
	u32		remap_en;	/* indicate if remap ID is used */
	...
	struct perf_event *	(*find_event_by_ov)(struct rtk_pmc_set *ps,
						    int bit);
	int			(*pmc_to_ctrl_bit)(struct rtk_pmc_set *ps,
						   union rtk_pmc_desc pmc);
};
...
rtk_pmu_last_client(const struct rtk_pmc_set_meta *psm)
{
	return psm->clients[psm->nr_clients - 1];
}
...
#if IS_ENABLED(CONFIG_RTK_PMU_GATOR_COMPATIBLE)
static inline int
...
u32 rtk_read_counter(struct rtk_pmc_set *ps, struct perf_event *event);
u64 rtk_pmu_update_event_overflow(struct rtk_pmc_set *ps, struct perf_event *event);
...
compensate_event(struct rtk_pmc_set *ps, struct perf_event *ev)
{
	...
	rtk_pmu_update_event_overflow(ps, ev);
}
...
static inline int
rtk_pmu_counter_auto_clear(struct rtk_pmu *pmu)
{
	return pmu->flags & RTK_PMU_COUNTER_AUTO_CLEAR;
}
