...
static void rtk_tfc_ps_reset(struct rtk_pmc_set *ps, struct perf_event *event)
{
...
static void rtk_tfc_ps_enable(struct rtk_pmc_set *ps, struct perf_event *event)
{
	union rtk_pmc_desc pmc = rtk_event_pmc_desc(event);
	unsigned long addr;
	u32 ctrl;
...
static void rtk_tfc_ps_disable(struct rtk_pmc_set *ps, struct perf_event *event)
{
	union rtk_pmc_desc pmc = rtk_event_pmc_desc(event);
	unsigned long addr;
	u32 ctrl;
...
static int rtk_tfc_pmc_config(u64 config)
{
...
#define NOP_THRESHOLD	3
	...
	if (!rtk_pmu_has_no_overflow(pmu)) {
		...
		if (nop_count == 0) {
			...
			dev_err(dev, "interrupted without overflow for %d times", nop_count);
			nop_count = 0;
		}
	}
		...
		struct perf_event *event)
{
	...
	dev_dbg(dev, "%s- ev:%#llx set hwc, pmc idx:%d, usage:%d\n",
	     ps->name, event->attr.config, pmc.idx, pmc.usage);
	...
	dev_dbg(dev, "ev:%#llx, cconf:%#llx, cbase:%#lx, ebase:%#lx, rdpmc:%#x",
	...
	dev_dbg(dev, "ev:%#llx, pmc=%d:%d:%d update ctrl:%lx, update val:%x\n",
...
rtk_tfc_arrange_pmc(struct rtk_pmu *pmu, u64 config)
{
	...
	dev_dbg(dev, "%s- ev:%#llx, desc %#x:%#x:%#x\n",
	     ps->name, config, desc.set, desc.cat, desc.sid);
...
rtk_tfc_check_event(struct rtk_pmu *pmu, struct perf_event *event)
{
	...
	if (ret == false)
		dev_dbg(dev, "invalid ev:%#llx, sid:%#x, cat:%#x, set:%d\n",
		     event->attr.config, desc.sid, desc.cat,
		     desc.set);
...
static void rtk_tfc_pmu_remove(struct platform_device *pdev)
{
