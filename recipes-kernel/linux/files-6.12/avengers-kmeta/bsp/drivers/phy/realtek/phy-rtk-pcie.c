...
static int rtd_get_tx_swing(struct rtd_pcie_phy *rtd_phy)
{
	...
	if (!IS_ERR(cell)) {
		...
		dev_info(rtd_phy->dev, "missing tx_swing nvmem resource, skip\n");
	}
...
static void set_tx_matrix_table(struct rtd_pcie_phy *rtd_phy)
{
	...
	int value[32] = { 0x0, 0x821, 0x1042, 0x1863, 0x2084, 0x28A6, 0x30C7, 0x40E8,
			  0x4909, 0x512A, 0x594B, 0x616C, 0x698D, 0x71AE, 0x71CF, 0x79F0,
...
static int rtd1635_pcie0_phy_offset_calibrate(struct rtd_pcie_phy *rtd_phy)
{
	int cnt;
	int val;
	int offset_k_timeout = 10000;
	...
	write_mdio_parallel_reg(rtd_phy, 0x1990, 0x0554);
	write_mdio_parallel_reg(rtd_phy, 0x1a40, 0x4c04);
	write_mdio_parallel_reg(rtd_phy, 0x1982, 0x0011);
...
gen3_retry:
	...
	write_mdio_parallel_reg(rtd_phy, 0x1a40, 0x4c00);
	write_mdio_parallel_reg(rtd_phy, 0x1990, 0x0554);
	write_mdio_parallel_reg(rtd_phy, 0x1990, 0x0550);
	write_mdio_parallel_reg(rtd_phy, 0x1982, 0x0001);
	regmap_read(rtd_phy->pcie_base, PCIE_PHY_CTR, &val);
	regmap_write(rtd_phy->pcie_base, PCIE_PHY_CTR, val & ~GENMASK(7, 6));
...
static int rtd1635_pcie0_phy_init(struct phy *phy)
{
	struct rtd_pcie_phy *rtd_phy = phy_get_drvdata(phy);
	int ret = 0;
	...
	write_mdio_parallel_reg(rtd_phy, 0x97, 0x5240);
	write_mdio_parallel_reg(rtd_phy, 0x1144, 0xc00);
	write_mdio_parallel_reg(rtd_phy, 0x09, 0xa505);
	write_mdio_parallel_reg(rtd_phy, 0x59, 0xa505);
	write_mdio_parallel_reg(rtd_phy, 0xa9, 0xa505);
	write_mdio_parallel_reg(rtd_phy, 0x1d, 0x10);
	write_mdio_parallel_reg(rtd_phy, 0x6d, 0x10);
	write_mdio_parallel_reg(rtd_phy, 0xbd, 0x10);
	write_mdio_parallel_reg(rtd_phy, 0x1b, 0x60);
	write_mdio_parallel_reg(rtd_phy, 0x6b, 0x60);
	write_mdio_parallel_reg(rtd_phy, 0xbb, 0x60);
	write_mdio_parallel_reg(rtd_phy, 0x0, 0x1a0a);
	write_mdio_parallel_reg(rtd_phy, 0x13, 0x2002);
	write_mdio_parallel_reg(rtd_phy, 0x63, 0x2003);
	write_mdio_parallel_reg(rtd_phy, 0xb3, 0x2303);
	write_mdio_parallel_reg(rtd_phy, 0x1a6a, 0xf144);
	write_mdio_parallel_reg(rtd_phy, 0x1c, 0x3e2);
	write_mdio_parallel_reg(rtd_phy, 0x6c, 0x3e2);
	write_mdio_parallel_reg(rtd_phy, 0xbc, 0x3e2);
	write_mdio_parallel_reg(rtd_phy, 0x1114, 0xa800);

	regmap_write(rtd_phy->pcie_base, PIPE_CTL1, 40 << 8 | 12);
	set_tx_matrix_table(rtd_phy);

	ret = rtd1635_pcie0_phy_offset_calibrate(rtd_phy);
	if (ret)
		dev_err(rtd_phy->dev, "offset calibrate failed\n");
...
static const struct phy_ops rtd1635_pcie0_phy_ops = {
	.init		= rtd1635_pcie0_phy_init,
	.power_on	= rtd13xx_pcie_phy_power_on,
	.power_off	= rtd13xx_pcie_phy_power_off,
	.owner		= THIS_MODULE,
};
...
static const struct of_device_id rtd_pcie_phy_of_match[] = {
	...
	{ .compatible = "realtek,rtd1635-pcie-slot0-phy", .data = &rtd1635_pcie0_phy_ops},
	{ },
};
