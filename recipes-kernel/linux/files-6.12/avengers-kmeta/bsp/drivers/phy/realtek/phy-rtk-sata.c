...
static int rtk_sata_phy_init(struct phy *phy)
{
	...
	if (soc_device_match(rtk_soc_kylin)) {
		...
		write_mdio_reg(base, PHY_ADDR_ALL, 0xa, 0xc600);
		...
		write_mdio_reg(base, PHY_ADDR_ALL, 0x15, 0x1717);
		write_mdio_reg(base, PHY_ADDR_ALL, 0x2a, 0x0000);
		write_mdio_reg(base, PHY_ADDR_ALL, 0x16, 0x770);
		write_mdio_reg(base, PHY_ADDR_ALL, 0x10, 0x2900);
		write_mdio_reg(base, PHY_ADDR_ALL, 0xc, 0xc000);
		write_mdio_reg(base, PHY_ADDR_ALL, 0x17, 0x27);
		if (port->ssc_en) {
			write_mdio_reg(base, PHY_ADDR_ALL, 0x4, 0x738e);
			write_mdio_reg(base, PHY_ADDR_ALL, 0x8, 0x3591);
			write_mdio_reg(base, PHY_ADDR_ALL, 0x27, 0x0234);
		} else
		...
		if (port->ssc_en) {
			write_mdio_reg(base, PHY_ADDR_ALL, 0x8, 0x3591);
			write_mdio_reg(base, PHY_ADDR_ALL, 0x27, 0x024e);
			write_mdio_reg(base, PHY_ADDR_ALL, 0x4, 0xb38e);
			dev_info(priv->dev, "port%d ssc enable\n", port->index);
		}
	}
	...
	if (port->param_size) {
		...
		if (soc_device_match(rtk_soc_kylin)) {
			write_mdio_reg(base, PHY_ADDR_ALL, 0x20, 0x94a7);
			write_mdio_reg(base, PHY_ADDR_ALL, 0x21, 0x387a);
		} else if (soc_device_match(rtk_soc_hank)) {
...
static struct phy *rtk_sata_phy_xlate(struct device *dev, const struct of_phandle_args *args)
{
...
static int rtk_sata_phy_probe(struct platform_device *pdev)
{
	...
	for_each_available_child_of_node(node, child) {
		...
		of_property_read_u32(child, "spread-spectrum", &port->ssc_en);
