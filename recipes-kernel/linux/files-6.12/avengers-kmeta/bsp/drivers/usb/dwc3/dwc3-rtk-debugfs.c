...
#define read_dwc3_register(base, offset) \
	readl(base + offset - DWC3_GLOBALS_REGS_START)
...
static int dwc3_rtk_dwc_regdump_show(struct seq_file *s, void *unused)
{
	...
	for (i = 0; i < nregs; i++, regs++) {
		u32 val = read_dwc3_register(base, regs->offset);
...
void dwc3_rtk_regdump_init(struct dwc3_rtk_debug *rtk)
{
	...
	if (dwc3_node) {
		...
		for (i = 0; i < regs_size; i++, regs++)
			regs->default_val = read_dwc3_register(base, regs->offset);
...
void dwc3_rtk_debugfs_init(struct dwc3_rtk_debug *rtk)
{
	...
	debugfs_create_file("dwc3_regdump", S_IRUGO, debug_dir, rtk,
				   &dwc3_rtk_dwc_regdump_fops);

	debugfs_create_file("wrap_regdump", S_IRUGO, debug_dir, rtk,
...
static void dwc3_rtk_debug_remove(struct platform_device *pdev)
{
