	    ...
	    struct device_attribute *attr, char *buf)
{
	...
	return sprintf (buf, "%s to runTestMode\n"
		    "echo \"%d %d\" to run port1 HC_RESET command\n"
		    "echo \"%d %d\" to run port1 TEST_J command\n"
		    "echo \"%d %d\" ro run port1 TEST_K command\n"
		    "echo \"%d %d\" to run port1 TEST_SE0_NAK command\n"
		    "echo \"%d %d\" to run port1 TEST_PACKET command\n"
		    "echo \"%d %d\" to run port1 TEST_FORCE_ENABLE command\n"
		    "echo \"%d %d\" to run port1 SUSPEND/RESUME command\n"
		    "echo \"%d %d\" to run port1 "
		    "SINGLE_STEP_GET_DEVICE_DESCRIPTOR command\n"
		    "echo \"%d %d\" to run port1 PORT_RESET command\n",
		    dev_name(&udev->dev),
		    port_num, TEST_RESET, port_num, TEST_TEST_J, port_num, TEST_TEST_K,
		    port_num, TEST_TEST_SE0_NAK,
		    port_num, TEST_TEST_PACKET,
		    port_num, TEST_TEST_FORCE_ENABLE, port_num, TEST_SUSPEND_RESUME,
		    port_num, TEST_SINGLE_STEP_GET_DEVICE_DESCRIPTOR,
		    port_num, TEST_PORT_RESET);
...
static bool usbport_port_exist(struct usbport_data *usbport_data,
				 struct usb_device *usb_dev, int portnum)
{
	...
	list_for_each_entry(port, &usbport_data->ports, list) {
		if (usb_dev == port->hub &&
			...
			dev_dbg(&usb_dev->dev, "%s: %s in port_list, %s\n",
				    __func__,
...
static int usbport_add_usb_dev_ports(struct usbport_data *usbport_data,
				     struct usb_device *usb_dev)
{
	int i;
	...
	for (i = 1; i <= usb_dev->maxchild; i++)
		if (!usbport_port_exist(usbport_data, usb_dev, i))
...
static void usbport_attach_usb_dev_to_ports(struct usbport_data *usbport_data,
					    struct usb_device *usb_dev)
{
	struct usbport_port *port;
...
static void usbport_detach_usb_dev_from_ports(struct usbport_data *usbport_data,
					      struct usb_device *usb_dev)
{
	struct usbport_port *port;
...
static int usbport_add_usb_dev(struct usbport_data *usbport_data,
			       struct usb_device *usb_dev)
{
	dev_dbg(&usb_dev->dev, "%s: start add %s\n",
		    __func__, dev_name(&usb_dev->dev));

	mutex_lock(&usbport_data->lock);
	usbport_add_usb_dev_ports(usbport_data, usb_dev);
	usbport_attach_usb_dev_to_ports(usbport_data, usb_dev);
	mutex_unlock(&usbport_data->lock);

	return 0;
}

static int usbport_remove_usb_dev(struct usbport_data *usbport_data,
				  struct usb_device *usb_dev)
{
	dev_dbg(&usb_dev->dev, "%s: start remove %s\n",
		    __func__, dev_name(&usb_dev->dev));

	mutex_lock(&usbport_data->lock);
	usbport_detach_usb_dev_from_ports(usbport_data, usb_dev);
...
static int usbport_add_usb_dev_for_parent(struct usbport_data *usbport_data,
			       struct usb_device *usb_dev)
{
	int i;
	int count = 0;
	...
	parent = usb_dev->parent;
	while (parent) {
		count++;
		dev_dbg(&usb_dev->dev, "%s: get parent %s (count=%d)\n",
			    __func__, dev_name(&parent->dev), count);
	...
	for (i = count; i > 0; i--) {
		...
		if (!parent) {
			dev_err(&usb_dev->dev, "%s: get parent error for %s\n",
				__func__, dev_name(&usb_dev->dev));
			return -ENODEV;
		}

		dev_dbg(&usb_dev->dev, "%s: get parent %s (i=%d/count=%d)\n",
			    __func__, dev_name(&parent->dev), i, count);

		usbport_add_usb_dev(usbport_data, parent);
	}
	return 0;
}

__maybe_unused
static int usbport_scan_usb_dev(struct usb_device *usb_dev, void *data)
{
	struct usbport_data *usbport_data = (struct usbport_data *)data;

	dev_dbg(&usb_dev->dev, "%s: start scan %s\n",
		    __func__, dev_name(&usb_dev->dev));

	usbport_add_usb_dev(usbport_data, usb_dev);
			       ...
			       void *data)
{
	...
	switch (action) {
	case USB_DEVICE_ADD:
		usbport_add_usb_dev_for_parent(usbport_data, usb_dev);
		usbport_add_usb_dev(usbport_data, usb_dev);
		return NOTIFY_OK;
	case USB_DEVICE_REMOVE:
		usbport_remove_usb_dev(usbport_data, usb_dev);
		return NOTIFY_OK;
	}
...
static int __init usbport_test_init(void)
{
	...
	usbport_class = class_create("usbport_test");
	if (!usbport_class) {
		err = -ENOMEM;
		goto err_free;
	}
	...
	/* Note Android GKI does not support the usb_for_each_dev export symbol */
#if IS_BUILTIN(CONFIG_USB_HOST_TEST_MODE)
	usb_for_each_dev(usbport_data, usbport_scan_usb_dev);
#endif
