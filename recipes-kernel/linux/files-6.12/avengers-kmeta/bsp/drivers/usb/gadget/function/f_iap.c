...
static const char iap_shortname[] = "iap";
	...
	switch (cmd){
		case IAP_GET_DEVICE_STATE:
			if(dev->online)
				...
				online = 0;
			pr_info("iap_ioctl IAP_GET_DEVICE_STATE online=%d\n", online);
		}
		...
		unsigned intf, unsigned alt)
{
	...
	dev->online = 1;
	pr_info("iap_function_set_alt: online\n");
...
struct usb_function *iap_alloc(struct usb_function_instance *fi)
{
	...
	dev->function.suspend = iap_function_disable;
