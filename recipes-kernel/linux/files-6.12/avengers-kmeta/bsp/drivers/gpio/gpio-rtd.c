				   ...
				   unsigned int debounce)
{
	...
	if (!strcmp(data->info->name, "rtk_129x_iso_gpio")) {
		shift = 0;
		deb_val += 1;
		write_en = BIT(shift + 3);
	} else if (!strcmp(data->info->name, "rtk_129x_misc_gpio")){
		shift = (offset >> 4) * 0x4;
		deb_val += 1;
		write_en = BIT(shift + 3);
	} else {
...
static int rtd_gpio_get_direction(struct gpio_chip *chip, unsigned int offset)
{
	...
	ret = (val) ? GPIO_LINE_DIRECTION_OUT : GPIO_LINE_DIRECTION_IN;
...
static unsigned int rtd_misc_gpio_dir_offset(unsigned int offset)
{
	return 0x00 + (offset / 32) * 4;
}

static unsigned int rtd_misc_gpio_dato_offset(unsigned int offset)
{
	return 0x10 + (offset / 32) * 4;
}

static unsigned int rtd_misc_gpio_dati_offset(unsigned int offset)
{
	return 0x20 + (offset / 32) * 4;
}

static unsigned int rtd_misc_gpio_ie_offset(unsigned int offset)
{
	return 0x30 + (offset / 32) * 4;
}

static unsigned int rtd_misc_gpio_dp_offset(unsigned int offset)
{
	return 0x40 + (offset / 32) * 4;
}

static unsigned int rtd_misc_gpio_gpa_offset(unsigned int offset)
{
	...
	case 3:
		...
		break;
	default:
		pr_err("GPIO misc gpa offset(%d) over range\n", offset);
		break;
	}
	return ret;
}

static unsigned int rtd_misc_gpio_gpda_offset(unsigned int offset)
{
	unsigned int group = offset / 31;
	unsigned int ret = 0;
	...
	case 3:
		...
		break;
	default:
		pr_err("GPIO misc gpda offset(%d) over range\n", offset);
		break;
	}
	return ret;
}

static unsigned int rtd129x_misc_gpio_deb_offset(unsigned int offset)
{
	return 0x50;
}

static unsigned int rtd129x_iso_gpio_deb_offset(unsigned int offset)
{
       return 0x14;
}
...
static const struct rtd_gpio_info rtd129x_misc_gpio_info = {
	.name = "rtk_129x_iso_gpio",
	.gpio_base = 0,
	.num_gpios = 101,
	.dir_offset  = rtd_misc_gpio_dir_offset,
	.dato_offset = rtd_misc_gpio_dato_offset,
	.dati_offset = rtd_misc_gpio_dati_offset,
	.ie_offset = rtd_misc_gpio_ie_offset,
	.dp_offset = rtd_misc_gpio_dp_offset,
	.gpa_offset = rtd_misc_gpio_gpa_offset,
	.gpda_offset = rtd_misc_gpio_gpda_offset,
	.deb_offset = rtd129x_misc_gpio_deb_offset,
};

static const struct rtd_gpio_info rtd129x_iso_gpio_info = {
	.name = "rtk_129x_misc_gpio",
	.gpio_base = 101,
	.num_gpios = 35,
	.dir_offset  = rtd_iso_gpio_dir_offset,
	.dato_offset = rtd_iso_gpio_dato_offset,
	...
	.gpa_offset = rtd_iso_gpio_gpa_offset,
	.gpda_offset = rtd_iso_gpio_gpda_offset,
	.deb_offset = rtd129x_iso_gpio_deb_offset,
};
...
static const struct of_device_id rtd_gpio_of_matches[] = {
	...
	{ .compatible = "realtek,rtd129x-misc-gpio", .data = &rtd129x_misc_gpio_info },
	{ .compatible = "realtek,rtd129x-iso-gpio", .data = &rtd129x_iso_gpio_info },
	{ }
};
...
static int rtd_gpio_probe(struct platform_device *pdev)
{
	...
	ret = gpiochip_add_data(&data->gpio_chip, NULL);
	if (ret) {
