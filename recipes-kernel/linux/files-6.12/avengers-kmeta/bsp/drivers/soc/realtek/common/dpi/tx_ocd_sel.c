...
static int dpi_tx_ocd_sel_of_init(struct device *dev, struct dpi_tx_ocd_sel_priv *priv)
{
	int ret, len;

	if ((ret = dpi_tx_ocd_sel_read_reg_values(dev, "ddr-type", &priv->ddr_type_q, &len)))
		return ret;

	if ((ret = dpi_tx_ocd_sel_read_reg_values(dev, "ocd-sel-idx", &priv->ocd_idx_q, &len)))
		return ret;

	if ((ret = dpi_tx_ocd_sel_read_reg_values(dev, "state-0-value", &priv->ocd_val_q_0, &len)))
		return ret;

	ret = dpi_tx_ocd_sel_read_reg_values(dev, "state-1-value", &priv->ocd_val_q_1, &len);
