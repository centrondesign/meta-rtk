...
static struct devfreq_dev_profile vpu_defreq_profile;
static struct devfreq *vpu_devfreq;

static int vpu_target(struct device *dev, unsigned long *freq, u32 flags)
{
	unsigned long cuf_freq = clk_get_rate(vpu_clk);
...
static int vpu_init_devfreq(struct device *dev)
{
	...
	struct devfreq_dev_profile *profile = &vpu_defreq_profile;
	int ret;
	...
	profile->is_cooling_device = 1;
	profile->get_cur_freq = vpu_get_cur_freq;
	profile->target       = vpu_target;
	profile->initial_freq = clk_get_rate(vpu_clk);
	vpu_devfreq = devm_devfreq_add_device(dev, profile, "performance", NULL);
	return PTR_ERR_OR_ZERO(vpu_devfreq);
}
...
static int vpu_probe(struct platform_device *pdev)
{
	...
	if (vpu_devfreq)
		devfreq_suspend_device(vpu_devfreq);
...
static int rtk_ve1_runtime_resume(struct device *dev)
{
	...
	if (vpu_devfreq)
		devfreq_resume_device(vpu_devfreq);
	return 0;
}
