...
static int rtk_usb_clear_clock_reset(struct manager_data *data, bool check)
{
	struct device *dev = data->dev;
	struct reset_control *reset_usb_apply = usb_reset_get(dev->of_node, "apply");
	int i;
 ...
 *    |-> ehci/ohci
 */
static struct device *__get_usb_port_device_from_notify(struct manager_data *data,
				  ...
				  enum usb_role try_role, bool force_swap)
{
	...
	if (usb_role_switch_get_role(role_sw) == USB_ROLE_DEVICE) {
		port->auto_swap_pre_role = USB_ROLE_DEVICE;
		...
		port->auto_swap_pre_role = USB_ROLE_HOST;
		...
		port->auto_swap_pre_role = usb_role_switch_get_role(role_sw);
		return;
	}
...
static void auto_role_swap_func(struct work_struct *work)
{
	...
	if (usb_role_switch_get_role(role_sw) == USB_ROLE_DEVICE) {
		if (port->auto_swap_pre_role == USB_ROLE_DEVICE) {
			dev_info(dev, "port%d: stay in device mode.\n", port->port_num);
			goto out;
		}
		...
		if (port->auto_swap_pre_role == USB_ROLE_HOST) {
			dev_info(dev, "port%d: stay in host mode.\n", port->port_num);
			goto out;
		}
...
static void rtk_usb_manager_remove(struct platform_device *pdev)
{
	struct device		*dev = &pdev->dev;
	struct manager_data *data = dev_get_drvdata(dev);
	...
	rtk_usb_clear_clock_reset(data, 0);
	put_port_setting(data);
	if (data->usb_ctrl_compat_class)
		class_compat_unregister(data->usb_ctrl_compat_class);
...
static int rtk_usb_manager_suspend(struct device *dev)
{
	...
	if (!data->usb_iso_mode) {
		...
		if (soc_device_match(rtk_soc_kent)) {
			check = true;
			dev_dbg(dev, "Do not reset u3dp phy_reset.\n");
		}
		rtk_usb_clear_clock_reset(data, check);
	}
