...
static int rtk_ifcp_client_calibrate_dco(struct rtk_ifcp_client_data *mbc)
{
	if (!mbc->dco)
		return -EINVAL;

	clk_prepare_enable(mbc->dco);
	clk_disable_unprepare(mbc->dco);
	return 0;
}
...
static long rtk_ifcp_client_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case MBOX_CALIBRATE_DCO:
		return rtk_ifcp_client_calibrate_dco(mbc);
	default:
		return -ENOIOCTLCMD;
	};
	return 0;
...
static int rtk_ifcp_client_probe(struct platform_device *pdev)
{
	...
	mbc->dco = devm_clk_get_optional(dev, "dco");
	if (IS_ERR(mbc->dco)) {
		dev_warn(dev, "error %pe: failed to get clk_dco\n", mbc->dco);
		mbc->dco = NULL;
	}
