...
static int r_program_fd(unsigned long phys_addr, unsigned long *offset,
		 unsigned long *size)
{
	int ret_fd = -1;
...
static long compat_rpc_mem_ioctl(struct file *filp, unsigned int cmd,
			  unsigned long arg)
{
...
static int rtk_rpc_mem_init(void)
{
...
static void remote_alloc_reply(struct rtk_rpc_client *client, struct rpc_struct *rpc, unsigned long reply)
{
...
static unsigned int rpc_ion_alloc_handler_legacy(struct rtk_rpc_client *client, bool secure, const struct fw_alloc_parameter_legacy *param)
{
...
static unsigned int rpc_ion_alloc_handler(struct rtk_rpc_client *client, const struct fw_alloc_parameter *param)
{
	struct device *dev = client->dev;
	struct r_program_entry *rpc_entry;
	unsigned int reply_value = 0;
...
static void  kern_rpc_work(struct work_struct *work)
{
	...
	spin_lock_irqsave(&client->queue_lock, flags);
	if (skb_queue_empty(&client->queue)) {
		spin_unlock_irqrestore(&client->queue_lock, flags);
		return;
	}
