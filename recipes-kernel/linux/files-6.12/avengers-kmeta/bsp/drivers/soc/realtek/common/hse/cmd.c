		       ...
		       u16 dst_width, u16 dst_height, u16 src_width, u16 src_height, u16 colorSel)
{
	...
	{
		deltaPhase = src_height * (HSE_DELTA_PHASE_BASE) / dst_height;
		vDeltaPhase = (int32_t) deltaPhase;
		deltaPhaseMeta = vDeltaPhase;
	...
	{
		deltaPhase = src_width * (HSE_DELTA_PHASE_BASE) / dst_width;
		hDeltaPhase = (int32_t) deltaPhase;
		deltaPhaseMeta = hDeltaPhase;
		if(deltaPhaseMeta != deltaPhase) {
	...
	/* calculate coeffs */
	tmp_coef_4t16p = &(tmp_coef[0]);
	tmp_coef_8t16p = &(tmp_coef[32]);
