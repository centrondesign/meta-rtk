...
struct tee_shm *tee_shm_register_fd(struct tee_context *ctx, int fd)
{
	...
	ref->attach = dma_buf_attach(ref->dmabuf, &shm_teedev->dev);
	...
	mutex_lock(&shm_teedev->mutex);

	ref->shm.id = idr_alloc(&shm_teedev->idr, &ref->shm,
				1, 0, GFP_KERNEL);

	mutex_unlock(&shm_teedev->mutex);
