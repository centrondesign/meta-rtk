...
long prince_compat_vpu_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	long ret = 0;
	...
	case COMPAT_VDI_IOCTL_GET_INSTANCE_POOL:
	{
		...
		instance_pool = prince_vpu_get_instance_pool();
...
#else
			ret = prince_vpu_alloc_from_dmabuffer2();
			if (ret) {
				prince_vpu_sem_up();
				return -EFAULT;
			}
	...
	case COMPAT_VDI_IOCTL_GET_COMMON_MEMORY:
	{
		...
		common_memory = prince_vpu_get_common_memory();
		...
		if (common_memory->base != 0) {
			...
			if (prince_vpu_alloc_dma_buffer(common_memory) != 0)
				return -ENOMEM;
	...
	case COMPAT_VDI_IOCTL_GET_REGISTER_INFO: {
		...
		vpu_register = prince_vpu_get_vpu_register();
...
EXPORT_SYMBOL(prince_compat_vpu_ioctl);
