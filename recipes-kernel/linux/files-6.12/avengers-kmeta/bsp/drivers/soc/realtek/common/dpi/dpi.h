...
struct dpi_device {
	...
	uint32_t chip_id;
	const struct dpi_quirks *quirks;
	int base_temp;
	int swc_access;
...
{
	pr_debug("%s\n", __func__);
	dpi_reg_read(dpi, 0x90, &dpi->pll_ctl_5);
...
{
	pr_debug("%s\n", __func__);
	dpi_reg_write(dpi, 0x90, dpi->pll_ctl_5);
...
static inline int dpi_check_perm(struct dpi_device *dpi)
{
	...
	dpi->type = dpi_get_dram_type(dpi);

	if (dpi->quirks->check_secacc) {
