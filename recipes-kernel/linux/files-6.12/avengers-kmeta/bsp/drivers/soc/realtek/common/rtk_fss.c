...
static int fss_get_calibration_data(struct fss_device *fss, struct fss_calibration_data *data)
{
	...
	struct fss_calibration_data_cell *cell;
	int ret;
	...
	cell = &data->fss;
	cell->bitmap    = desc->sensor_bitmask;
	cell->cdl1_size = 4;
	cell->cdl1      = fss_reg_read(fss, SC_WRAP_DVFS_FSS_ST2);
	cell->min_cdl1  = fss_reg_read(fss, SC_WRAP_DVFS_FSS_ST3);
...
static int fss_v4_get_calibration_data(struct fss_device *fss, struct fss_calibration_data *data)
{
	...
	cell->bitmap    = desc->sensor_bitmask;
	cell->cdl0_size = 4;
	cell->cdl0      = fss_reg_read(fss, SC_WRAP_DVFS_FSS_ST1);
	cell->cdl1_size = 5;
	cell->min_cdl1  = fss_reg_read(fss, SC_WRAP_DVFS_FSS_ST3);

	cell = &data->dsu;
	cell->bitmap    = desc->dsu_sensor_bitmask;
	cell->cdl0_size = 4;
	cell->cdl0      = dsu_fss_reg_read(fss, SC_DSU_WRAP_SC_FSS_STAT_2) & 0xffff;
	cell->cdl1_size = 5;
	cell->min_cdl1  = dsu_fss_reg_read(fss, SC_DSU_WRAP_SC_FSS_STAT_3);
...
static int fss_get_hw_version(struct fss_device *fss)
{
	return fss->desc->get_calibration_data == fss_v4_get_calibration_data ? 4 : 1;
}
...
static void fss_remove(struct platform_device *pdev)
{
...
{
	return fss_get_hw_version(ctl->fss);
}
EXPORT_SYMBOL_GPL(fss_control_get_hw_version);
