...
extern struct tee_shm *tee_shm_register_fd(struct tee_context *ctx, int fd);
...
int tee_client_close_session(struct tee_context *ctx, u32 session)
{
	...
	ret = ctx->teedev->desc->ops->close_session(ctx, session);
			   ...
			   struct tee_param *param)
{
	...
	ret = ctx->teedev->desc->ops->invoke_func(ctx, arg, param);
			  ...
			  struct tee_ioctl_cancel_arg *arg)
{
	...
	ret = ctx->teedev->desc->ops->cancel_req(ctx, arg->cancel_id, arg->session);
...
struct bus_type rtk_tee_bus_type = {
	.name		= "tee",
	.match		= tee_client_device_match,
	.uevent		= tee_client_device_uevent,
};
EXPORT_SYMBOL_GPL(rtk_tee_bus_type);
...
static int __init tee_init(void)
{
	...
	rc = bus_register(&rtk_tee_bus_type);
	if (rc) {
...
{
	bus_unregister(&rtk_tee_bus_type);
	unregister_chrdev_region(tee_devt, TEE_NUM_DEVICES);
	class_destroy(tee_class);
	tee_class = NULL;
}
