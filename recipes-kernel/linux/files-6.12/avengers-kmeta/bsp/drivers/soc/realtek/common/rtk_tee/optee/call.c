...
#include <soc/realtek/optee_private.h>
#include "../tee_private.h"

#ifdef CONFIG_OPTEE_ACL_API
		       ...
		       struct tee_param *param)
{
...
#ifdef CONFIG_OPTEE_ACL_API
	if (optee_acl_list_check(RTK_ACL_FUNC_OPEN, 0, (void *)arg->uuid))
		return -EINVAL;
#endif
	...
	if (arg->ret == TEEC_SUCCESS) {
		if (optee_acl_is_enabled()) {
			if (!optee_acl_active_insert(msg_arg->session, (void *)arg->uuid, true)) {
				pr_err("[TEE_ACL] %s-%d: acl_active_insert fail, session:[0x%x]\n", __func__, __LINE__, msg_arg->session);
...
#ifdef CONFIG_OPTEE_ACL_API
	if (optee_acl_list_check(RTK_ACL_FUNC_CLOSE, session, NULL))
		return -EINVAL;
#endif
...
#ifdef CONFIG_OPTEE_ACL_API
	if (optee_acl_is_enabled()) {
		optee_acl_active_delete(session);
	}
#endif
...
#ifdef CONFIG_OPTEE_ACL_API
	if (optee_acl_list_check(RTK_ACL_FUNC_INVOKE, arg->session, NULL))
		return -EINVAL;
#endif
