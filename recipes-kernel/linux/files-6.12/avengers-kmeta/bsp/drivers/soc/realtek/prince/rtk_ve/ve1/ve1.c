...
#if !defined(DISABLE_NOTIFY_PCPU)
...
int prince_vpu_alloc_dma_buffer(vpudrv_buffer_t *vb)
{
	if (!vb)
		return -1;
	...
	DPRINTK("%s [%d]prince_vpu_alloc_dma_buffer.base:0x%lx.phys_addr:0x%lx.size:%d\n",DEV_NAME,__LINE__,vb->base,vb->phys_addr,vb->size);
	return 0;
}
...
void prince_vpu_free_dma_buffer(vpudrv_buffer_t *vb)
{
...
static int prince_vpu_free_instances(struct file *filp)
{
	...
	DPRINTK("%s prince_vpu_free_instances\n", DEV_NAME);
	...
	list_for_each_entry_safe(vil, n, &s_inst_list_head, list) {
		if (vil->filp == filp) {
			...
			DPRINTK("%s prince_vpu_free_instances detect instance crash instIdx=%d, coreIdx=%d, vip_base=%p, instance_pool_size_per_core=%d\n", DEV_NAME, (int)vil->inst_idx, (int)vil->core_idx, vip_base, (int)instance_pool_size_per_core);
			...
			if (vip) {
				...
				DPRINTK("%s prince_vpu_free_instances : force to destroy vdi_mutexes_base=%p in userspace \n", DEV_NAME, vdi_mutexes_base);
				if (vdi_mutexes_base) {
					int i;
					for (i = 0; i < 4; i++) {
...
void prince_vpu_clock_getting(vpu_clock_info_t *clockInfo)
{
	if (clockInfo->enable) {
...
int prince_vpu_down_interruptible(void)
{
	return down_interruptible(&s_vpu_sem);
}

void prince_vpu_sem_up(void)
{
	up(&s_vpu_sem);
}

void prince_vpu_open_ref_count_inc(void)
{
	s_vpu_open_ref_count++;
}

void prince_vpu_open_ref_count_dec(void)
{
	s_vpu_open_ref_count--;
}

void prince_vpu_add_vbp_list(vpudrv_buffer_pool_t *vbp, struct file *filp)
{
	vbp->filp = filp;
	spin_lock(&s_vpu_lock);
...
int prince_vpu_open_inst(vpudrv_inst_info_t *inst_info, struct file *filp)
{
	vpudrv_instanace_list_t *vil, *n;
...
void prince_vpu_close_inst(vpudrv_inst_info_t *inst_info)
{
	vpudrv_instanace_list_t *vil, *n;
...
void prince_vpu_get_inst_num(vpudrv_inst_info_t *inst_info)
{
	vpudrv_instanace_list_t *vil, *n;
...
void prince_vpu_get_total_inst_num(vpudrv_inst_info_t *inst_info)
{
	vpudrv_instanace_list_t *vil, *n;
...
void prince_vpu_free_mem(vpudrv_buffer_t *vb)
{
...
int prince_vpu_wait_init(vpu_drv_context_t *dev, vpudrv_intr_info_t *info)
{
	int ret = 0;
...
vpudrv_buffer_t *prince_vpu_get_instance_pool(void)
{
	return &s_instance_pool;
}

vpudrv_buffer_t *prince_vpu_get_common_memory(void)
{
	return &s_common_memory;
}

vpudrv_buffer_t *prince_vpu_get_vpu_register(void)
{
	return &s_vpu_register;
}

int prince_vpu_alloc_from_vm(void)
{
	int ret = 0;
...
int prince_vpu_alloc_from_dmabuffer2(void)
{
	int ret = 0;
...
static long vpu_ioctl(struct file *filp, u_int cmd, u_long arg)
{
	...
	case VDI_IOCTL_ALLOCATE_PHYSICAL_MEMORY:
	{
		...
		ret = prince_vpu_down_interruptible();
		if (ret != 0)
			return -EFAULT;
		...
		ret = prince_vpu_alloc_dma_buffer(&(vbp->vb));
		if (ret != 0) {
			kfree(vbp);
			prince_vpu_sem_up();
			return -ENOMEM;
		...
		prince_vpu_add_vbp_list(vbp, filp);
	...
	case VDI_IOCTL_WAIT_INTERRUPT:
	{
		...
		ret = prince_vpu_wait_init(dev, &info);
		if (ret != 0)
			return -EFAULT;
	...
	case VDI_IOCTL_GET_COMMON_MEMORY:
	{
		...
		if (s_common_memory.base != 0) {
			...
			ret = prince_vpu_alloc_dma_buffer(&s_common_memory);
			if (ret != 0)
				return -ENOMEM;
	...
	case VDI_IOCTL_OPEN_INSTANCE:
	{
		...
		ret = prince_vpu_open_inst(&inst_info, filp);
		if (ret)
			return -ENOMEM;

		/* flag just for that vpu is in opened or closed */
		prince_vpu_open_ref_count_inc();
	...
	case VDI_IOCTL_CLOSE_INSTANCE:
	{
		...
		prince_vpu_close_inst(&inst_info);

		/* flag just for that vpu is in opened or closed */
		prince_vpu_open_ref_count_dec();
	...
	case VDI_IOCTL_GET_INSTANCE_NUM:
	{
		...
		prince_vpu_get_inst_num(&inst_info);
	...
	case VDI_IOCTL_SET_RTK_CLK_GATING:
	{
		...
		prince_vpu_clock_getting(&clockInfo);
	...
	case VDI_IOCTL_GET_TOTAL_INSTANCE_NUM:
	{
		...
		if (ret)
			return -EFAULT;
		prince_vpu_get_total_inst_num(&inst_info);
...
static int vpu_release(struct inode *inode, struct file *filp)
{
	...
	if (ret == 0) {
		...
		prince_vpu_free_instances(filp);
		s_vpu_drv_context.open_count--;
		if (s_vpu_drv_context.open_count == 0) {
...
#else
				prince_vpu_free_dma_buffer(&s_instance_pool);
#endif /* USE_VMALLOC_FOR_INSTANCE_POOL_MEMORY */
			...
			if (s_common_memory.base) {
				...
				prince_vpu_free_dma_buffer(&s_common_memory);
...
#endif // #if !defined(DISABLE_NOTIFY_PCPU)
