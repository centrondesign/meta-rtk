...
static int ops_set_param(scd_device* dev, unsigned long id, unsigned long val)
{
...
#ifdef CLK_ETU_TRACE_mask
		//etu_from = (unsigned char)(val >> CLK_ETU_TRACE_shift_bit);
		//val = val & (~(CLK_ETU_TRACE_mask<<CLK_ETU_TRACE_shift_bit));
		CLK_ETU_TRACE_get(val, etu_from);
		SC_WARNING("clk(%lu) from %s\n", val,
			(etu_from==CLK_ETU_TRACE_cas_lib) ? "cas_lib" :
			(etu_from==CLK_ETU_TRACE_atr)     ? "atr" :
			(etu_from==CLK_ETU_TRACE_middle_reset) ? "mid_reset" :
			(etu_from==CLK_ETU_TRACE_scctrl) ? "scctrl" :
			(etu_from==CLK_ETU_TRACE_scd) ? "scd???" : "unknown" );
#endif // CLK_ETU_TRACE_mask
		//ret = mars_scd_set_clock(p_this, (unsigned long) val, CLK_ETU_FLAG_flush);
		MARS_SCD_SET_CLOCK(p_this, (unsigned long) val, CLK_ETU_FLAG_flush, ret);
		break;
	...
	case SC_PARAM_ETU:
		...
		SC_WARNING("etu(%lu) from %s\n", val,
			...
			(etu_from==CLK_ETU_TRACE_scd) ? "scd???" : "unknown" );
#endif  // CLK_ETU_TRACE_mask
		//ret = mars_scd_set_etu(p_this, (unsigned long) val, CLK_ETU_FLAG_flush );
		MARS_SCD_SET_ETU(p_this, (unsigned long) val, CLK_ETU_FLAG_flush, ret);
		break;
	...
	case SC_PARAM_RX_TIMEOUT:
		ret = mars_scd_set_rx_timeout(p_this, val);
		break;

	case SC_PARAM_PROTO:
		ret = mars_scd_set_protocol(p_this, val);
		break;

	case SC_PARAM_FLOW_CONTROL:
		ret = mars_scd_set_flowcontrol(p_this, val);
		break;

	case SC_PARAM_CAS:
		ret = mars_scd_set_cas(p_this, (SC_CAS)val);
		break;
...
static int ops_get_param(scd_device* dev, unsigned long id, unsigned long* p_val)
{
	...
	case SC_PARAM_RX_TIMEOUT:
		ret = mars_scd_get_rx_timeout(p_this, p_val);
		break;

	case SC_PARAM_PROTO:
		ret = mars_scd_get_protocol(p_this, p_val);
		break;

	case SC_PARAM_FLOW_CONTROL:
		ret = mars_scd_get_flowcontrol(p_this, p_val);
		break;

	case SC_PARAM_CAS:
		ret = mars_scd_get_cas(p_this, (SC_CAS*)p_val);
		break;
 ...
 *------------------------------------------------------------------*/
static
int ops_set_protocol(scd_device* dev, unsigned long val)
{
	mars_scd* p_this = (mars_scd*) scd_get_drvdata(dev);
 ...
 * Retn : SC_SUCCESS / SC_FAIL
 *------------------------------------------------------------------*/
static int ops_get_protocol(scd_device* dev, unsigned long* p_val)
{
	mars_scd* p_this = (mars_scd*) scd_get_drvdata(dev);
...
/*------------------------------------------------------------------
 * Func : ops_set_baud_rate
 *
 * Desc : set clk/etu in one shot
 *
 * Parm : dev, clk, etu
 *
 * Retn : SC_SUCCESS / SC_FAIL
 *------------------------------------------------------------------*/
static int ops_set_baud_rate(scd_device *dev, unsigned long clk, unsigned long etu)
{
	mars_scd* p_this = (mars_scd*) scd_get_drvdata(dev);
	int ret = SC_SUCCESS;
#ifdef CLK_ETU_TRACE_mask
	uint8_t clk_from, etu_from;

	CLK_ETU_TRACE_get(clk, clk_from);
	CLK_ETU_TRACE_get(etu, etu_from);
	SC_WARNING("baud(%d, %d) from (0x%x, 0x%x)\n", clk, etu, clk_from, etu_from);
#endif // CLK_ETU_TRACE_mask

	//ret = mars_scd_set_baud_rate(p_this, clk, etu, CLK_ETU_FLAG_flush);
	MARS_SCD_SET_BAUD_RATE(p_this, clk, etu, CLK_ETU_FLAG_flush, ret);

	return ret;
}

/*------------------------------------------------------------------
 * Func : ops_get_baud_rate
 *
 * Desc : get clk/etu in one shot
 *
 * Parm : dev, clk, etu
 *
 * Retn : SC_SUCCESS / SC_FAIL
 *------------------------------------------------------------------*/
static int ops_get_baud_rate(scd_device *dev, unsigned long *p_clk, unsigned long *p_etu)
{
	mars_scd* p_this = (mars_scd*) scd_get_drvdata(dev);
	return mars_scd_get_baud_rate(p_this, p_clk, p_etu);
}
...
/*------------------------------------------------------------------
 * Func : ops_tx_rx_session
 *
 * Desc : Speedup Tx and Rx session in some CAS
 *
 * Parm : dev  :
 *        *p_tx  : tx buffer
 *        *p_rx  : rx buffer
 *        tx_len : tx len, may equal to 0
 *        rx_len : rx len, should not 0
 *        flags  : enable flow ctrl, null filter and hack record.
 *        wwt    : max interval to get byte in rx.
 *        end_stamp_ms :
 * Retn : SC_SUCCESS / SC_FAIL
 *------------------------------------------------------------------*/
static int ops_tx_rx_session(scd_device *dev, uint8_t *p_tx, uint8_t *p_rx,
		uint32_t tx_len, uint32_t rx_len, uint32_t flags,
		uint64_t wwt, uint64_t end_stamp_ms)
{
	mars_scd* p_this = (mars_scd*) scd_get_drvdata(dev);

	Chili_TimeoutWwtSetup(wwt, end_stamp_ms);

	return Chili_TxRxSession(p_this, p_tx, tx_len, p_rx, rx_len, flags);
}

/*------------------------------------------------------------------
 * Func : ops_card_io_command
 *
 * Desc : Speedup Card IO command in Chili
 *
 * Parm : dev  :
 *        *p_tx  : tx buffer
 *        *p_rx  : rx buffer
 *        tx_len : tx len, may equal to 0
 *        rx_len : rx len, should not 0
 *        flags  : bToCard, TxRxHack.
 *        wwt    : max interval to get byte in rx.
 *        timeout_ms :
 * Retn : SC_SUCCESS / SC_FAIL
 *------------------------------------------------------------------*/
static int ops_card_io_command(scd_device *dev, uint8_t *p_tx, uint8_t *p_rx,
		uint32_t tx_len, uint32_t *p_rx_len, uint32_t flags,
		uint64_t wwt, uint64_t timeout_ms)
{
	mars_scd* p_this = (mars_scd*) scd_get_drvdata(dev);

	Chili_TimeoutWwtSetup(wwt, timeout_ms);

	return Chili_CardIoCommand(p_this, p_tx, tx_len, p_rx, p_rx_len, flags);
}
...
static scd_driver mars_scd_driver =
{
	...
	.xmit		   = ops_xmit,
	.read		   = ops_read,
	.tx_rx_session = ops_tx_rx_session,
	.card_io_command = ops_card_io_command,

	.set_baud_rate = ops_set_baud_rate,
	.get_baud_rate = ops_get_baud_rate,
};
...
static int setup_pwr_sel_pin_config(void)
{
...
static int rtk_smc_probe(struct platform_device *pdev)
{
	...
	const struct fwnode_handle *fwnode = dev_fwnode(&pdev->dev);

	memset(&dts_info, 0, sizeof(dts_info));

	dts_info.pwr_sel_pin_x2 = setup_pwr_sel_pin_config();
	...
	cmd_vcc_gpio = devm_fwnode_gpiod_get(&pdev->dev,
					     of_fwnode_handle(ls_ic_node),
					     "pin_cmd_vcc",
					     GPIOD_OUT_LOW,
					     "pin_cmd_vcc");
	...
	pwr_sel0_gpio = devm_fwnode_gpiod_get(&pdev->dev,
					      of_fwnode_handle(ls_ic_node),
					      "pin_pwr_sel0",
					      GPIOD_OUT_LOW,
					      "pin_pwr_sel0");
	...
	pwr_sel1_gpio = devm_fwnode_gpiod_get(&pdev->dev,
					      of_fwnode_handle(ls_ic_node),
					      "pin_pwr_sel1",
					      GPIOD_OUT_LOW,
					      "pin_pwr_sel1");
