...
static int rtk_pm_hifi_prepare(struct device *dev)
{
	...
	if (rtk_pm_wakeup_source_hifi_enabled(data->pm_dev) > 0) {
		ret = regulator_suspend_enable(data->supp->rdev, PM_SUSPEND_MEM);
		if (ret) {
			dev_warn(dev, "suspend enable is not supported by supply, ignore pd pin off\n");
			rtk_pm_ignore_pd_pin(data->pm_dev);
		}
	} else
		regulator_suspend_disable(data->supp->rdev, PM_SUSPEND_MEM);
	return 0;
}
...
static void rtk_pm_hifi_remove(struct platform_device *pdev)
{
