...
struct buflock_module {
    ...
    size_t                  max_handles;
    size_t                  release_timeout_cnt;

    struct mutex            independent_prelock;
};
...
static void buflock_handle_put(struct buflock_handle * handle)
{
    ...
    if (module) {
        mutex_lock(&module->handles_ref_lock);
        kref_put(&handle->ref, handle->destroy);
        mutex_unlock(&module->handles_ref_lock);
    } else {
        kref_put(&handle->ref, handle->destroy);
    }
}
                    ...
                    default:
                        {
                            ...
                            if (ktime_ms_delta(now, handle->sTime.destroy) > module->destroy_timeout_ms) {
#if 0
                                /*
                                 * Remove watchdog as requested by VFW.
                                 * Related JIRA: SW-7283, DHCDANVERS-925
                                 */
                                remove = true;
                                pr_err("%s Destroy handle(%p) timeout! ***FORCED RELEASE*** (id=%zu fwid=%u, status=%s)\n",
                                        ...
                                        handle->id, handle->getFWID(handle), getStatusStr(handle->getStatus(handle)));
#else
                                pr_err("%s Destroy handle(%p) TIMEOUT! Please check buffer status (id=%zu fwid=%u, status=%s)\n",
                                        MODULE_TAG, handle,
                                        handle->id, handle->getFWID(handle), getStatusStr(handle->getStatus(handle)));

                                if (handle->release_timeout_report) {
                                    module->release_timeout_cnt++;
                                    handle->release_timeout_report--;
                                }
#endif
                            }
                        }
...
static struct buflock_handle * buflock_module_handle_get_by_fwid (struct buflock_module * module, buflock_fwid_t fwid)
{
    ...
    mutex_lock(&module->handles_lock);
    {
        struct buflock_handle * handle, * tmp_handle;
        list_for_each_entry_safe(handle,  tmp_handle, &module->handles, list) {
            if (handle && handle->getFWID(handle) == fwid) {
                handle->get(handle);
                ret = handle;
                break;
            }
...
static void __exit buflock_module_exit(void)
{
    ...
    mutex_destroy(&module->handles_ref_lock);
    mutex_destroy(&module->clients_lock);
    mutex_destroy(&module->independent_prelock);
    ...
    do {
...

        {
            ...
            if (task)
                ...
                strscpy(ret->task_comm, "KTHREAD", sizeof(ret->task_comm));
...
{
    int ret = -1;
    mutex_lock(&client->module->independent_prelock);
    mutex_lock(&client->mutex);
    {
        struct buflock_slot * slot, * tmp_slot;
        list_for_each_entry_safe(slot,  tmp_slot, &client->slots, list) {
    ...
    mutex_unlock(&client->mutex);
    mutex_unlock(&client->module->independent_prelock);
    return ret;
}
...
static ssize_t buflock_fops_read (struct file *file, char *buf, size_t size, loff_t *f_pos)
{
    ...
    do  {
        ...
        s += scnprintf(s, str + size - s, "#%s phy=%08zx max=%zu checkloop_ms=%d"
			"\n destroy_timeout_ms=%d\n", MODULE_NAME,
			 (long unsigned int) module->uStatusPhyAddr,
			 module->max_handles,
			 module->release_loop_ms, module->destroy_timeout_ms);

        s += scnprintf(s, str + size - s, " RELEASE_TIMEOUT=%zu\n",
                module->release_timeout_cnt);

        s += scnprintf(s, str + size - s, "    clients:\n");
        mutex_lock(&module->clients_lock);
        {
            struct buflock_client * client, * tmp_client;
            list_for_each_entry_safe(client,  tmp_client, &module->clients, list) {
                s += scnprintf(s, str + size - s, "        %u %u [%s]\n", client->pid, client->tgid, client->task_comm);
                mutex_lock(&client->mutex);
                {
                    struct buflock_slot * slot, * tmp_slot;
                    list_for_each_entry_safe(slot,  tmp_slot, &client->slots, list) {
                        s += scnprintf(s, str + size - s, "            %p\n", slot->handle);
                    }
                }
                mutex_unlock(&client->mutex);
            }
        ...
        s += scnprintf(s, str + size - s, "    active handle:\n");
        {
            struct buflock_handle * handle, * tmp_handle;
            list_for_each_entry_safe(handle,  tmp_handle, &module->handles, list) {
                unsigned int lastSet_delta_us;
                lastSet_delta_us = ktime_us_delta(now, handle->sTime.lastSet) & -1U;
                s += scnprintf(s, str + size - s,"        %p (id=%08x, %.8s) lastSet(%5d.%03d ms)\n", handle,
        ...
        s += scnprintf(s, str + size - s, "    inactive handle:\n");
        {
            struct buflock_handle * handle, * tmp_handle;
            list_for_each_entry_safe(handle,  tmp_handle, &module->free_handles, list) {
                unsigned int lastSet_delta_us;
                ...
                s += scnprintf(s, str + size - s, "        %p (id=%08x, %.8s) lastSet(%5d.%03d ms), Destroy(%5d.%03d ms)\n", handle,
                        handle->getFWID(handle), getStatusStr(handle->getStatus(handle)),
                        lastSet_delta_us/1000, lastSet_delta_us%1000,
