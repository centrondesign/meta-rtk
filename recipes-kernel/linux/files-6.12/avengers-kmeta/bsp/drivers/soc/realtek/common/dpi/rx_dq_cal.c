...
static void rx_rd_dly_apply(struct dpi_rx_dq_cal_priv *priv)
{
	...
	for(slice = 0; slice < 4; ++slice)
	{
		for(sel = 0; sel < 4; ++sel) {
			...
			DELTA_P[2 * sel] = (dpi_rw_cal_out_1 >> 8) & 0xff;
			DELTA_P[2 * sel + 1] = (dpi_rw_cal_out_1 >> 24) & 0xff;
			DELTA_N[2 * sel] = (dpi_rw_cal_out_1 >> 0) & 0xff;
			DELTA_N[2 * sel + 1] = (dpi_rw_cal_out_1 >> 16) & 0xff;
		}
		...
		DELTA_P[8] = (dpi_rw_cal_out_1 >> 8) & 0xff;
		DELTA_N[8] = (dpi_rw_cal_out_1 >> 0) & 0xff;
