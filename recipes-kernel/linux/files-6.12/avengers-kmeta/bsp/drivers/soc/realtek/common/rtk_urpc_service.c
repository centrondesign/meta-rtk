...
static void add_rcpu(struct proc_rpc_stat *rpc_stat, int type)
{
...
static void remove_rcpu(struct proc_rpc_stat *rpc_stat, int type)
{
	switch (type) {
	case AUDIO_ID:
		rpc_stat->acpu = 0;
...
static long rtk_urpc_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	int ret = 0;
	pid_t g_pid;
	pid_t g_tgid;
...
static int urpc_send_rpc(struct rtk_krpc_ept_info *krpc_ept_info, char *buf, int len, uint32_t *retval)
{
	int ret = 0;
...
static int rtk_urpc_open(struct inode *inode, struct file *filp)
{
...
static int self_destroy_ept_init(struct rtk_krpc_ept_info *krpc_ept_info, char *name)
{
	int ret = 0;
