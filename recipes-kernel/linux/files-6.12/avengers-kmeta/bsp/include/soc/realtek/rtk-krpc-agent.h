...
#else
static inline int rtk_send_rpc(struct rtk_krpc_ept_info *krpc_info, char *buf, int len)
{
	return 0;
}

static inline struct rtk_krpc_ept_info *of_krpc_ept_info_get(struct device_node *np, int index)
{
	return ERR_PTR(-ENODEV);
}

static inline void krpc_ept_info_put(struct rtk_krpc_ept_info *krpc_ept_info)
{
}

static inline int krpc_info_init(struct rtk_krpc_ept_info *krpc_ept_info, char *name, krpc_cb cb)
{
	return 0;
}

static inline void krpc_info_deinit(struct rtk_krpc_ept_info *krpc_ept_info)
{
}

static inline void rtk_krpc_dump_ringbuf_info(struct rtk_krpc_ept_info *krpc_ept_info)
{
}
