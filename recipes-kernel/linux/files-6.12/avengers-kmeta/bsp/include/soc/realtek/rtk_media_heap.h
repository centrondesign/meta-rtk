...
extern struct mutex chplist_mutex;
...
extern void rheap_procfs_init(void);
extern int rheap_miscdev_register(void);
extern void rtk_dma_remap(struct page *page, size_t size, pgprot_t prot);
#else
#define rheap_dma_ops dma_dummy_ops
static inline struct dma_buf *rheap_alloc(char *name, unsigned long len,
					 unsigned long flags)
{
	return ERR_PTR(-ENODEV);
}
static inline void rheap_setup_dma_pools(struct device *dev, char *name,
				 unsigned long heap_flags, const char *caller) {}
static inline void rheap_debugfs_init(void) {}
#endif
