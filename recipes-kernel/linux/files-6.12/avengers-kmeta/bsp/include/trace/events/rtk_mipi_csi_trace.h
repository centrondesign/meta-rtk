...
TRACE_EVENT(mipicsi_app_crtl,
	...
	TP_printk("%.8s Ch%u, scaling_down=%.1s is_compenc=%.1s",
...
TRACE_EVENT(mipicsi_app_size_cfg,
	TP_PROTO(u8 ch_index, u32 src_width, u32 src_height, u32 dst_width, u32 dst_height,
		u8 mode, u32 line_pitch, u32 header_pitch, u32 video_size),
	TP_ARGS(ch_index, src_width, src_height, dst_width, dst_height,
		mode, line_pitch, header_pitch, video_size),
	TP_STRUCT__entry(
		__field(u8, ch_index)
	...
	TP_printk("Ch%u [%ux%u]->[%ux%u] %.7s line_pitch=%u header_pitch=%u video_size=%u",
