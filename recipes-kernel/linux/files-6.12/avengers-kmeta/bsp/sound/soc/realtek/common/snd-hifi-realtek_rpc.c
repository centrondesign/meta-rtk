...
int rpc_set_btpcm_config(phys_addr_t paddr, void *vaddr, struct rtk_snd_mixer *mixer)
{
	...
	memcpy(cmd, &mixer->btpcm_config, sizeof(struct rpc_btpcm_config));
...
int rpc_ao_config_mix_idx(struct snd_rtk_pcm *dpcm, struct snd_pcm_runtime *runtime)
{
	...
	cmd = (struct audio_rpc_aio_privateinfo_parameters_ext *)dpcm->vaddr_rpc;
	dat = dpcm->phy_addr_rpc;

	offset = get_rpc_alignment_offset(sizeof(struct audio_rpc_aio_privateinfo_parameters_ext));
	offset = ALIGN(offset, 128); /* HIFI 128 Align */

	memset(cmd, 0, sizeof(struct audio_rpc_aio_privateinfo_parameters_ext));
	res = (struct audio_rpc_privateinfo_returnval *)((unsigned long)cmd + offset);

	cmd->type = ENUM_PRIVATEINFO_AIO_AO_FLASH_PIN_MIX_CH_MAP;
	cmd->instance_id = dpcm->ao_agent_id;
	cmd->argate_info[1] = dpcm->ao_pin_id;
	cmd->argate_info[2] = 1;
	cmd->argate_info[3] = sizeof(*cmd);
...
int rpc_set_balance(struct device *dev, struct rtk_snd_mixer *mixer)
{
	...
	for (i=0; i<MAX_CAR_CH; i++)
		rpc->parameter[i] = 31 - mixer->balance[i];
...
/* set AO volume */
int rpc_set_volume(struct device *dev, int volume, int mode)
{
	...
	config->msd_id = AUDIO_CONFIG_CMD_VOLUME_CONFIG;
	config->value[2] = mode;
...
int rpc_config_subwoofer_level(struct device *dev, int id,
			       struct audio_subwoofer_level *subwoofer)
{
	...
	memset(config, 0, sizeof(struct audio_config_command));
	config->msd_id = AUDIO_CONFIG_CMD_SUBWOOFER_LEVEL_CONTROL;
	config->value[0] = subwoofer->enable;
	if (subwoofer->gain >= 0)
		config->value[1] = 1;
	config->value[2] = abs(subwoofer->gain);
	config->value[3] = subwoofer->mute;
	config->value[5] = id;
...
int rpc_config_spdif_mute(struct device *dev, int en, int id)
{
	struct audio_config_command *config = NULL;
	unsigned int *res;
	u32 ret = 0;
	...
	memset(config, 0, sizeof(struct audio_config_command));
	config->msd_id = AUDIO_CONFIG_CMD_AO_HW_SPDIF_SWITCH;
	config->value[0] = en;
	config->value[5] = id;
...
int rpc_config_i2s0_mute(struct device *dev, int en, int id)
{
	struct audio_config_command *config = NULL;
	unsigned int *res;
	u32 ret = 0;
	...
	memset(config, 0, sizeof(struct audio_config_command));
	config->msd_id = AUDIO_CONFIG_CMD_DAC_OUTPUT_SWITCH;
	config->value[0] = en;
	config->value[5] = id;
...
/* get AO volume */
int rpc_set_volume_new(struct device *dev, int volume, int mode, int id)
{
	struct audio_config_command *config = NULL;
	unsigned int *res;
	u32 ret = 0;
...
int rpc_config_aio_ctrl(struct device *dev, struct rtk_snd_mixer *mixer)
{
	struct audio_rpc_aio_privateinfo_parameters *rpc = NULL;
	u32 RPC_ret;
	phys_addr_t dat;
	...
	rpc->argate_info[0] = mixer->aio_ctrl;
	rpc->argate_info[1] = 1;
	rpc->argate_info[2] = 0;
	rpc->argate_info[3] = mixer->pin_id;

	pr_err("%s aio_ctrl = 0x%x\n", __func__, rpc->argate_info[0]);
...
int rpc_set_ao_param_eq(struct device *dev, struct audio_ao_param_eq *ao_param_eq)
{
	struct audio_ao_param_eq *rpc = NULL;
//	struct audio_rpc_privateinfo_returnval *res;
	u32 rpc_ret;
	unsigned int offset;
	int ret = -1;
	...
	rpc = (struct audio_ao_param_eq *)vaddr;

	memset(rpc, 0, sizeof(struct audio_ao_param_eq));
	memcpy(rpc, ao_param_eq, sizeof(struct audio_ao_param_eq));
	offset = get_rpc_alignment_offset(sizeof(struct audio_ao_param_eq));
	offset = ALIGN(offset, 128); /* HIFI 128 align */

	pr_info("%s enable 0x%x, ch %d, bandnum %d\n", __func__, rpc->enable[0], rpc->ch, rpc->band_num);
...
int rpc_set_sound_effect(struct device *dev, struct audio_sound_effect_header *hdr, int id)
{
	struct rpc_sound_effect *rpc = NULL;
	void *rpc_param;
	u32 rpc_ret, cmd;
	...
	if (hdr->command == STEREO_TO_MULTICH_CROSSOVER) {
		cmd = ENUM_KERNEL_RPC_STEREO_TO_MULTICH_CONFIG;
	} else if(hdr->command == LOUDNESS_V2) {
		cmd = ENUM_KERNEL_RPC_LOUDNESS_CONFIG;
	} else if(hdr->command == PARAM_EQ) {
		cmd = ENUM_KERNEL_RPC_EXT_EQ_CONFIG;
	} else if(hdr->command == SUBWOOFER_LEVEL) {
		cmd = ENUM_KERNEL_RPC_AUDIO_MULTI_AO_CONFIG;
	} else {
		pr_err("%s command %d not support\n", __func__, hdr->command);
		return -1;
	}
	...
	rpc = (struct rpc_sound_effect *)vaddr;

	memset(rpc, 0, sizeof(struct rpc_sound_effect));
	offset = ALIGN(sizeof(struct rpc_sound_effect), 128);

	rpc->instanceID = id;
	rpc->command_type = hdr->command_type;
	rpc->addr = dat + offset;
	rpc_param = (void *)(vaddr + offset);

	memset(rpc_param, 0, hdr->size);
	if (copy_from_user((void *)rpc_param, (void *)hdr->p, hdr->size)) {
		pr_err("%s %d copy sound effect data fail\n",__func__, __LINE__);
		ret = -ENODEV;
		goto exit;
	}

	offset2 = ALIGN(hdr->size, 128);
	offset += offset2;

	pr_info("%s send command %d, structure size 0x%x\n", __func__,
...
int rpc_set_loudness_config(struct device *dev, struct audio_loudness_config *config)
{
	struct audio_loudness_config *rpc = NULL;
	u32 rpc_ret;
	unsigned int offset;
	int ret = -1;
	phys_addr_t dat;
	...
	rpc = (struct audio_loudness_config *)vaddr;

	memset(rpc, 0, sizeof(struct audio_loudness_config));
	memcpy(rpc, config, sizeof(struct audio_loudness_config));
	offset = get_rpc_alignment_offset(sizeof(struct audio_loudness_config));
	offset = ALIGN(offset, 128); /* HIFI 128 align */

	pr_info("%s command_type 0x%x, ch %d, level select 0x%x\n", __func__, rpc->command_type, rpc->n_ch, rpc->loudness_level_select);
		  ...
		  struct snd_pcm_runtime *runtime)
{
	...
	/* version 1 */
	rpc->private_info[5] = runtime->channels;
	rpc->private_info[6] = dpcm->mixer->aec.mute_duration;
	rpc->private_info[7] = dpcm->mixer->aec.fade_duration;
	/* the last info is version */
	rpc->private_info[31] = AEC_RPC_VERSION_1;
