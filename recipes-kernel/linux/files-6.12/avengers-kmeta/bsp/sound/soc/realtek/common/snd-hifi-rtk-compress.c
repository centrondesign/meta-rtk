...
#define PTS_GAP_TH	5000
#define PTS_STOP_TH	50
#define STOP_UPDATE_CNT	200
...
struct rtk_compr_stream {
	...
	unsigned long pre_out_frames;

	bool err_pts;
	unsigned long pts_gap_pos;
	unsigned long pre_timestamp;
...
static unsigned long get_buffer_from_ring(unsigned char *upper,
				   unsigned char *lower,
				   unsigned char *rptr,
				   unsigned char *dest,
				   unsigned long size)
...
static void update_ring_ptr(unsigned char *upper, unsigned char *lower,
...
static long get_general_instance_id(long instanceID, long pinID)
{
	return ((instanceID & 0xFFFFF000) | (pinID & 0xFFF));
}

static void reset_pointer(struct rtk_compr_stream *rtk_stream)
{
	...
	rtk_stream->pre_out_frames = 0;
	rtk_stream->err_pts = false;
	rtk_stream->pts_gap_pos = 0;
	rtk_stream->pre_timestamp = 0;
	rtk_stream->total_byte = 0;
	rtk_stream->pts_stop_cnt = 0;
	rtk_stream->pts_gap_cnt = 0;
}

static int __maybe_unused get_sample_index(unsigned int sample_rate)
{
	int i;
	...
	if (rtk_stream->avsync_hdr_offset != -1) {
		...
		while (remain_len > 0) {
			...
			if (is_header) {
				...
				if (timestamp >= rtk_stream->pre_timestamp) {
					timestamp_delta = timestamp - rtk_stream->pre_timestamp;
					rtk_stream->pre_timestamp = timestamp;
				} else {
					timestamp_delta = 0;
					/* timestamp jumps back */
					rtk_stream->err_pts = true;
				}

				if (timestamp_delta > PTS_GAP_TH && !rtk_stream->compr_first_pts) {
					pr_debug("PTS gap at %llu",timestamp);
					rtk_stream->err_pts = true;
					rtk_stream->pts_gap_pos = timestamp;
				}
...
static int trigger_audio(struct rtk_compr_stream *rtk_stream, int cmd)
{
	switch (cmd) {
...
static void destroy_audio_component(struct rtk_compr_stream *rtk_stream)
{
...
static int create_audio_component(struct rtk_compr_stream *rtk_stream)
{
				  ...
				  struct snd_compr_tstamp *tstamp)
{
	...
	if (rtk_stream->pts_gap_pos) {
		pr_debug("PTS gap, stop upadte pcm_io_frames");
		tstamp->pcm_io_frames = rtk_stream->pre_out_frames;
		rtk_stream->pts_gap_cnt++;
		if (rtk_stream->pts_gap_cnt >= STOP_UPDATE_CNT) {
			rtk_stream->pts_gap_cnt = 0;
			rtk_stream->pts_gap_pos = 0;
		}
		goto pointer_end;
	...
	/* check if bitstream stall */
	if (rtk_stream->out_frames == rtk_stream->pre_out_frames)
		rtk_stream->pts_stop_cnt += 1;
	else
		rtk_stream->pts_stop_cnt = 0;

	if (rtk_stream->pts_stop_cnt >= PTS_STOP_TH) {
		pr_debug("bitstream stall without STOP");
		rtk_stream->pts_stop_cnt = 0;
		rtk_stream->err_pts = true;
	}

	if (rtk_stream->err_pts) {
		tstamp->pcm_io_frames = rtk_stream->total_byte>>2;
		goto pointer_end;
	}
	...
	rtk_stream->pre_out_frames = tstamp->pcm_io_frames;
pointer_end:
			       ...
			       char __user *buf, size_t count)
{
	...
	while (count > writable_size) {
		if (buffull_count++ > 5) {
			pr_err("writable_size %ld count %zu buffull_count %d return\n",
	...
	write_byte = direct_writedata(rtk_stream, (char *)buf, count, &hdr_size);
	if (write_byte < 0) {
	...
	rtk_stream->total_byte += write_byte;
	return write_byte + hdr_size;
}
...
static int rtk_create_compress_instance(struct snd_card *card,
				 int instance_idx,
				 char *id,
				 int direction)
{
...
static void rtk_compress_remove(struct platform_device *pdev)
{
