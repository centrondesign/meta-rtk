	...
	for_each_child_of_node(np, child) {
		...
		if (id == ENUM_DT_AO_I2S || id == ENUM_DT_AO_I2S1) {
			...
			if (ao_params->ao_id == AUDIO_OUT) {
				...
				if (of_property_read_u32(child, "realtek,ao-i2s-mclk", &val))
					val = 0;

				ao_params->i2s_mclk = val;
			}
		}
		cnt ++;
		ao_params->bitmap &= ~BIT(id);
...
static int rtk_aio_ctrl_init_remote_ao(struct device *dev)
{
	...
	params.argateInfo[4] = rtk_aio_ctrl_rpc_to_remote32(&aio->rpc, ao_params.i2s_mclk);
...
static void rtk_aio_ctrl_remove(struct platform_device *pdev)
{
