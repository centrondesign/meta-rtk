...
#define HIFI_AEC_SET_PRESET 0x10000033
#define HIFI_AEC_DUMP_SETUP 0x10000034
#define HIFI_AEC_DUMP_START 0x10000035
#define HIFI_AEC_DUMP_STOP  0x10000036
...
enum EQ_BAND_TYPE {
	...
	ENUM_AO_INTERFACE_POWER = 86,
	ENUM_AO_MASTER_VOL_MODE = 87,
	ENUM_AO_MASTER_VOL_DB = 88,
	ENUM_AO_I2S0_EN = 89,
	ENUM_AO_SPDIF_EN = 90,
};
...
struct rpc_btpcm_config {
	...
	char drv_offset;   // 0x98006A10[18:11]
	char drv_map;
	char drv_pos;
	unsigned int sample_rate;
	char lat_edgeneg;   // 6a14[0]
	char lat_frame_edgeneg;  // 6a14[1]
	char lat_lsb;    // 6a14[2]
	char lat_offset;    // 6a14[4:3]
	char lat_pos;   // 6a14[7:5]
	char lat_map;   // 6a14[11:8]
	char lat_8bit_loc;  // 6a14[12]
	char drv_edgeneg;   // 6a14[16]
	char drv_frame_edgeneg; // 6a14[17]
	char drv_lsb;       // 6a14[18]
	char drv_8bit_loc;  // 6a14[28]
	char drv_protocol;  // 6a14[29]
	int rvd[11];
};
...
struct audio_ao_param_eq {
	...
	float q[32];
	float gain[32];
	char enable_8ch_param_eq;
	char enable_dmx_param_eq;
	char enable_mixing_mode_param_eq;
};

enum SOUND_EFFECT_COMMAND {
	STEREO_TO_MULTICH_CROSSOVER = 0,
	LOUDNESS_V2,
	PARAM_EQ,
	SUBWOOFER_LEVEL,
	COMMAND_MAX,
...
struct audio_sound_effect_header {
	int command;
	int command_type;
	int size;
	void __user *p;
...
struct rpc_sound_effect {
	u_int instanceID;
	int command_type;
	int addr;
};

struct audio_subwoofer_level {
	bool enable;
	int gain;
	bool mute;
};

struct audio_loudness_config {
	unsigned int instanceID;
	int command_type;
	int n_ch;
	int loudness_level_select; // 可動態更新參數
	float threshold_on_dBFS;  //= -26.0f,
	float threshold_off_dBFS; //= -23.0f,
	float attack_time; //= 0.05f,
	float release_time; // = 0.5f,
	float fade_time; //= 1.0f,
	float fc_low_shelf[2]; //= {120, 120}
	float q_low_shelf[2]; //= {1, 1}
	float gain_low_shelf[2]; //= {9, 6}
	float fc_high_shelf[2]; //= {10000, 10000}
	float q_high_shelf[2]; //= {1, 1}
	float gain_high_shelf[2]; //= {6, 3}
	char enable_8ch_loudness;
	char enable_dmx_loudness;
	char enable_mixing_mode_loudness;
};
...
/* AEC/NR parameters */
struct rtk_aec_param {
	/* duration (in ms) of the initial mute period after startup */
	unsigned int mute_duration;
	/* duration (in ms) of the fade-in period following the mute phase */
...
struct rtk_snd_mixer {
	struct device *dev;
	struct rpc_btpcm_config btpcm_config;
	...
	struct work_struct work_i2s0_mute;
	struct work_struct work_spdif_mute;
	struct work_struct work_aio_ctrl;
...
#define SNDRV_PCM_IOCTL_AO_PARAM_EQ_SET  _IOW('A', 0xEA, struct audio_ao_param_eq)
#define SNDRV_PCM_IOCTL_LOUDNESS_CONFIG  _IOW('A', 0xEC, struct audio_loudness_config)
#define SNDRV_PCM_IOCTL_BTPCM_PARAMETER  _IOW('A', 0xED, struct rpc_btpcm_config)
#define SNDRV_PCM_IOCTL_SUBWOOFER_LEVEL  _IOW('A', 0xEE, struct audio_subwoofer_level)
#define SNDRV_PCM_IOCTL_GENERAL_AUDIO_EFFECT  _IOW('A', 0xEF, struct audio_sound_effect_header)
