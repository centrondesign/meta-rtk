...
static struct snd_kcontrol_new snd_feature_ctrl[] = {
	...
	RTK_FEATURE_CTRL("AO Interface Power", 0, ENUM_AO_INTERFACE_POWER),
	RTK_FEATURE_CTRL("Master Volume Gain Mode", 0, ENUM_AO_MASTER_VOL_MODE),
	RTK_FEATURE_CTRL("Master Volume DB", 0, ENUM_AO_MASTER_VOL_DB),
	RTK_FEATURE_CTRL("AO I2S0 Enable", 0, ENUM_AO_I2S0_EN),
	RTK_FEATURE_CTRL("AO Spdif Enable", 0, ENUM_AO_SPDIF_EN),
};
...
static int write_inband_cmd(struct snd_rtk_pcm *dpcm, void *data, int len)
{
	    ...
	    unsigned int cmd, unsigned long arg)
{
	...
	union {
		struct snd_rtk_pcm dpcm_tmp;
		struct audio_subwoofer_level subwoofer;
		struct audio_ao_param_eq ao_param_eq;
		struct audio_loudness_config loudness_config;
		struct dma_buf_export_info exp_info;
	} param = {0};
	struct audio_sound_effect_header hdr;
	...
	case SNDRV_PCM_IOCTL_MMAP_DATA_FD:
		...
		param.exp_info.exp_name = KBUILD_MODNAME;
		param.exp_info.owner = THIS_MODULE;
		param.exp_info.ops = &rtksnd_dma_buf_ops;
		param.exp_info.size = dmab->bytes;
		param.exp_info.flags = O_RDWR;
		param.exp_info.priv = dmab_kref;

		kref_get(&dmab_kref->ref);
		dmabuf = dma_buf_export(&param.exp_info);
		if (IS_ERR(dmabuf)) {
	...
	case SNDRV_PCM_IOCTL_VOLUME_SET:
		...
		if (!substream || !substream->runtime) {
			...
			param.dpcm_tmp.ao_agent_id = dpcm_c->ao_agent_id;
			param.dpcm_tmp.ao_pin_id = dpcm_c->ao_pin_id;
			param.dpcm_tmp.phy_addr_rpc = dpcm_c->phy_addr_rpc;
			param.dpcm_tmp.vaddr_rpc = dpcm_c->vaddr_rpc;
			dpcm = &param.dpcm_tmp;
		} else
			dpcm = substream->runtime->private_data;
	...
	case SNDRV_PCM_IOCTL_MIX_IDX_SET:
		...
		if (!dpcm_c)
			break;
		if (rpc_ao_config_mix_idx(dpcm, substream->runtime)) {
			ret = -1;
		}
		break;
	...
	case SNDRV_PCM_IOCTL_AI_GAIN_SET:
		...
		break;
	case SNDRV_PCM_IOCTL_AO_PARAM_EQ_SET:
		if (copy_from_user((void *)&param.ao_param_eq,
			(void *)arg, sizeof(struct audio_ao_param_eq))) {
			pr_err("%s %d copy ai_gain_set fail\n",__func__, __LINE__);
			ret = -ENODEV;
			break;
		}
		param.ao_param_eq.instanceID = mixer->ao_agent_id;
		rpc_set_ao_param_eq(dev, &param.ao_param_eq);
		break;
	case SNDRV_PCM_IOCTL_LOUDNESS_CONFIG:
		if (copy_from_user((void *)&param.loudness_config,
			(void *)arg, sizeof(struct audio_loudness_config))) {
			pr_err("%s %d copy ai_gain_set fail\n",__func__, __LINE__);
			ret = -ENODEV;
			break;
		}
		param.loudness_config.instanceID = mixer->ao_agent_id;
		rpc_set_loudness_config(dev, &param.loudness_config);
		break;
	case SNDRV_PCM_IOCTL_BTPCM_PARAMETER:
		if (copy_from_user((void *)&mixer->btpcm_config,
			(void *)arg, sizeof(struct rpc_btpcm_config))) {
			pr_err("%s %d copy ai_gain_set fail\n",__func__, __LINE__);
			ret = -ENODEV;
			break;
		}
		break;
	case SNDRV_PCM_IOCTL_SUBWOOFER_LEVEL:
		if (copy_from_user((void *)&param.subwoofer,
			(void *)arg, sizeof(struct audio_subwoofer_level))) {
			pr_err("%s %d copy subwoofer fail\n",__func__, __LINE__);
			ret = -ENODEV;
			break;
		}
		rpc_config_subwoofer_level(dev, mixer->ao_agent_id, &param.subwoofer);
		break;
	case SNDRV_PCM_IOCTL_GENERAL_AUDIO_EFFECT:
		if (copy_from_user((void *)&hdr, (void *)arg,
			sizeof(struct audio_sound_effect_header))) {
			pr_err("%s %d copy sound effect header fail\n",__func__, __LINE__);
			ret = -ENODEV;
			break;
		}

		rpc_set_sound_effect(dev, &hdr, mixer->ao_agent_id);
		break;
	default:
		ret = -EINVAL;
		break;
...
static void snd_rtk_playback_volume_work(struct work_struct *work)
{
	...
	if (mixer->multi_ao) {
		if (mixer->master_vol_mode == 0)
			rpc_set_volume_new(mixer->dev, mixer->mixer_volume[MIXER_ADDR_MASTER][0], mixer->master_vol_mode, mixer->ao_agent_id);
		else
			rpc_set_volume_new(mixer->dev, mixer->master_vol_db[0], mixer->master_vol_mode, mixer->ao_agent_id);
	} else {
		if (mixer->master_vol_mode == 0)
			rpc_set_volume(mixer->dev, mixer->mixer_volume[MIXER_ADDR_MASTER][0], mixer->master_vol_mode);
		else
			rpc_set_volume(mixer->dev, mixer->master_vol_db[0], mixer->master_vol_mode);
	}
}
...
static void snd_rtk_i2s0_mute_work(struct work_struct *work)
{
	struct rtk_snd_mixer *mixer =
			container_of(work, struct rtk_snd_mixer, work_i2s0_mute);

	rpc_config_i2s0_mute(mixer->dev, mixer->ao_i2s0_en, mixer->ao_agent_id);
}

static void snd_rtk_spdif_mute_work(struct work_struct *work)
{
	struct rtk_snd_mixer *mixer =
			container_of(work, struct rtk_snd_mixer, work_spdif_mute);

	rpc_config_spdif_mute(mixer->dev, mixer->ao_spdif_en, mixer->ao_agent_id);
}

static void snd_rtk_aio_ctrl_work(struct work_struct *work)
{
	struct rtk_snd_mixer *mixer =
			container_of(work, struct rtk_snd_mixer, work_aio_ctrl);

	rpc_config_aio_ctrl(mixer->dev, mixer);
}
				   ...
				   struct rtk_snd_mixer *mixer)
{
	...
	INIT_WORK(&mixer->work_i2s0_mute, snd_rtk_i2s0_mute_work);
	INIT_WORK(&mixer->work_spdif_mute, snd_rtk_spdif_mute_work);
	INIT_WORK(&mixer->work_aio_ctrl, snd_rtk_aio_ctrl_work);
	...
	mixer->master_vol_mode = 0;
	mixer->master_vol_db[0] = mixer->master_vol_db[1] = 0x7FFFFFFF;
				 ...
				 struct snd_ctl_elem_info *uinfo)
{
	...
	} else if (kcontrol->private_value == ENUM_AO_INTERFACE_POWER) {
		uinfo->count = 1;
		uinfo->value.integer.min = 0;
		uinfo->value.integer.max = 0xFFF;
	} else if (kcontrol->private_value == ENUM_AO_MASTER_VOL_MODE) {
		uinfo->count = 1;
		uinfo->value.integer.min = 0;
		uinfo->value.integer.max = 1;
	} else if (kcontrol->private_value == ENUM_AO_MASTER_VOL_DB) {
		uinfo->count = 2;
		uinfo->value.integer.min = 0;
		uinfo->value.integer.max = 0xffffffff;
	} else if (kcontrol->private_value == ENUM_AO_I2S0_EN) {
		uinfo->count = 1;
		uinfo->value.integer.min = 0;
		uinfo->value.integer.max = 1;
	} else if (kcontrol->private_value == ENUM_AO_SPDIF_EN) {
		uinfo->count = 1;
		uinfo->value.integer.min = 0;
		uinfo->value.integer.max = 1;
	} else {
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	} else if (band_num == ENUM_AO_INTERFACE_POWER) {
		ucontrol->value.integer.value[0] = mixer->aio_ctrl;
	} else if (band_num == ENUM_AO_MASTER_VOL_MODE) {
		ucontrol->value.integer.value[0] = mixer->master_vol_mode;
	} else if (band_num == ENUM_AO_MASTER_VOL_DB) {
		ucontrol->value.integer.value[0] = mixer->master_vol_db[0];
		ucontrol->value.integer.value[1] = mixer->master_vol_db[1];
	} else if (band_num == ENUM_AO_I2S0_EN) {
		ucontrol->value.integer.value[0] = mixer->ao_i2s0_en;
	} else if (band_num == ENUM_AO_SPDIF_EN) {
		ucontrol->value.integer.value[0] = mixer->ao_spdif_en;
	} else {
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	if (band_num == ENUM_BALANCE_CONTROL) {
		...
		mixer->aio_ctrl = ucontrol->value.integer.value[0];
	} else if (band_num == ENUM_AO_MASTER_VOL_MODE) {
		mixer->master_vol_mode = ucontrol->value.integer.value[0];
	} else if (band_num == ENUM_AO_MASTER_VOL_DB) {
		mixer->master_vol_db[0] = ucontrol->value.integer.value[0];
		mixer->master_vol_db[1] = ucontrol->value.integer.value[1];
	} else if (band_num == ENUM_AO_I2S0_EN) {
		mixer->ao_i2s0_en = ucontrol->value.integer.value[0];
	} else if (band_num == ENUM_AO_SPDIF_EN) {
		mixer->ao_spdif_en = ucontrol->value.integer.value[0];
	} else {
		pr_err("unknown kcontrol!!\n");
	}
	...
	if (band_num == ENUM_BALANCE_CONTROL)
		...
		schedule_work(&mixer->work_aio_ctrl);
	else if (band_num == ENUM_AO_MASTER_VOL_DB)
		schedule_work(&mixer->work_volume);
	else if (band_num == ENUM_AO_I2S0_EN)
		schedule_work(&mixer->work_i2s0_mute);
	else if (band_num == ENUM_AO_SPDIF_EN)
		schedule_work(&mixer->work_spdif_mute);
...
static int snd_card_probe(struct platform_device *pdev)
{
	...
	err = rtk_create_pcm_instance(card, ENUM_AIN_HDMIRX, 2, 1);
	...
	err |= rtk_create_pcm_instance(card, ENUM_AIN_BTPCM, 0, 1);
	...
	err = of_property_read_u32(np, "aec_mute_duration",
				   &mixer->aec.mute_duration);
	if (err < 0)
		mixer->aec.mute_duration = 0;

	err = of_property_read_u32(np, "aec_fade_duration",
				   &mixer->aec.fade_duration);
	if (err < 0)
		mixer->aec.fade_duration = 0;
	pr_info("aec mute %d fade %d\n",
		mixer->aec.mute_duration, mixer->aec.fade_duration);
	...
	mixer->ao_agent_id = alsa_dev->ao_agent_id;
	mixer->pin_id = pin_id;
...
static void snd_card_remove(struct platform_device *pdev)
{
