...
static void rtk_spi_nor_init_setting(void)
{
	...
	regbase = ioremap(0x9804f20c, 0x4);
	writel((readl(regbase) & 0x00ffffff) | 0x22000000, regbase); //pad mux
	writel((readl(regbase + 0x4) & 0xffffff00) | 0x22, regbase + 0x4);
	iounmap(regbase);
...
#if defined(CONFIG_TARGET_RTD1635)
	regbase = ioremap(0x9804F238, 0x8);
...
#if defined(CONFIG_TARGET_RTD1635)
	rtk_set_reg(regbase, 25, 1, vdd);
	rtk_set_reg(regbase, 19, 3, n_drv);
	rtk_set_reg(regbase, 22, 3, p_drv);

	rtk_set_reg(regbase + 0x4, 12, 1, vdd);
	rtk_set_reg(regbase + 0x4, 6, 3, n_drv);
	rtk_set_reg(regbase + 0x4, 9, 3, p_drv);

	rtk_set_reg(regbase + 0x4, 25, 1, vdd);
	rtk_set_reg(regbase + 0x4, 19, 3, n_drv);
	rtk_set_reg(regbase + 0x4, 22, 3, p_drv);
	rtk_set_reg(regbase + 0x4, 14, 1, 0);

	rtk_set_reg(regbase + 0x8, 12, 1, vdd);
	rtk_set_reg(regbase + 0x8, 6, 3, n_drv);
	rtk_set_reg(regbase + 0x8, 9, 3, p_drv);
	rtk_set_reg(regbase + 0x8, 1, 1, 0);
#elif defined(CONFIG_TARGET_RTD1625)
        /* CLK */
...
#else
static bool rtk_nor_supports_op(struct spi_slave *mem,
				const struct spi_mem_op *op)
#endif /* __UBOOT__ */
{
...
static const struct udevice_id rtk_spi_nor_dt_ids[] = {
	{ .compatible = "realtek,rtd1635-sfc"},
